...
KNOWN GAP (P77 LIVE probe50; do not patch the live job): teacher-forced
...
returns only ``h/z/c`` and drops the encode cache.  CPU @wm_best50:
Markovian vs cached CV |Δ| 0.123; 1step→OL ×0.097 (iter40 |Δ| 0.14 /
×0.077).  Probe H=1 above floor, H=55/conv still dead @10–50 (cached
compounding).  A later attributed A/B would pass ``prev_state`` into
those OL rolls.  Metric: probe H=55 r / conv / val 1step→OL vs P64 ×0.85.
...
class TransformerSSMDynamics(nn.Module):
    ...
    def decode(self, feat: torch.Tensor) -> torch.Tensor:
        ...
        # (re-added by ``apply_dob``).  P74 skip REVERT (matches RSSM).

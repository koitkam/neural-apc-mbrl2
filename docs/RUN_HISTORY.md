...
### p74 (`run_p74_gcvskip`, branch `cursor/p28`) — LAUNCH: decoder gain-c → CV skip (zero-init Linear)
- **One attributed GPU change vs P73:** RSSM/TSSM `decode` adds `gain_cv_skip(c[:G])` onto CV (`index_add_`). Zero-init ⇒ identity to P73 decode at step 0. Persist KEEP. **No new TrainConfig field.** Env-free: **no leftover `DREAMER_*`**. tmux `mbrl2_p74`, out-dir `output/test_sim/run_p74_gcvskip`. Watch freeze vs P64 **0.91@DV** / P73 **0.83**; val **1step→OL / OL-vs-real** vs P73 ×0.761 / ×0.743 and P64 ×0.85 / ×0.84; jsonl `gain_cv_skip_rms` (0 at init, should rise); persist_rel still live; `[resolved-cfg] gcvskip=True`; `actor_experiment_valid`.

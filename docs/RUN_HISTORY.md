...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 13**. Probe10 **H=1 r=+0.128 / H=55 r=+0.122 conv=0.00** gain_fid **0.476** (P76 +0.428/+0.310/0.736; P64 +0.584/+0.309 conv=0.50/0.827). Teacher **×0.99 / ×1.07 @13** (overgain @8 recovered). recon **0.302@13 best** (P76 @13 **0.008**). skip **0**. pid **242511**. | 🔬 LIVE P1 — probe r ~3–4× worse than P76/P64; recon slowly improving; watch freeze vs P64 0.91@DV; later-probe conv>0 |
...
- **LIVE P1 iter 13 (~69 min, nvidia 11.7 GB, `t_wm` ~310 s, sps 19.4):** skip **0**. Teacher jsonl 10 **×1.008 / ×0.951** → 11 **×0.972 / ×1.178** → 12 **×0.994 / ×1.041** → 13 **×0.992 / ×1.072** (DV overgain @7–8 recovered; pinned again). recon **0.305@9 → 0.387@11 → 0.346@12 → 0.302@13 new best** vs P76 **0.008@13** / P64 **0.006@13** (~40×). persist_rel **0.53@12 → 0.65@13**. Rest-IC graph not captured. Probe 10 still the last fidelity (`H=1 r=+0.128 conv=0.00 gain_fid=0.476`); next typically iter 20. Do not patch while live. Watch freeze vs P64 **0.91@DV**; later probes conv>0 (P76 uniquely held 0 through P2@100; P73 also 0 @10); val **1step→OL** vs P64 ×0.85 / P76 ×0.770; `actor_experiment_valid`. Do **not** start a second GPU job.

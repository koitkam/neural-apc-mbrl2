...
- **LIVE (2026-09-01 ~02:55 CDT, still P1):** pid **169231** ALIVE, tmux `mbrl2_p64`, A10 **14260 MiB**. Iter **61**. recon **0.0023** (best **0.0019** @58) skip **0/0** held **6.6e-5** overshoot **0.036** (P62-class). Teacher MV/DV **×0.977 / ×0.977** `clip_frac` **0.083**. `t_wm` **97 s**. last_ok **61 unlocked**. Passed P61 storm window **56–57** and P58 **@61** with skip **0** (P63 stormed 18/25). Fidelity-best still iter **30** (gain-blind WATCH; iter-60 H=55 r=0.253 floor 0.40). Held revert **holding**. P2 amp hypothesis **not yet on** (Stage-1 cap still 0.2). **No second GPU job.** Dummy jsonl `wm_held_ol_ratio` **REMOVED** (HEAD, not this pid — still emits 0). Unwhitelisted `DREAMER_ACT_HIST_REQUIRED` zeros-fallback **REMOVED** (HEAD, not this pid; `imagine_next_z` raises without history). Watch GAIN-READY ~82; P2 `dob_d_absmean` vs P62 ~0.03; val pred_std vs 1.93; det_r vs P62 0.364 / P26 0.68; TM vs P26/P62. Do **not** score actor unless GAIN-READY.
...
| jsonl dummy `wm_held_ol_ratio` (always 0 after P63 REVERT) | **P64 GPU-occupied** (pid **169231** P1) | **REMOVED** | YES — FO magnitude key with no producer. Training graph unchanged. This pid still emits 0. |

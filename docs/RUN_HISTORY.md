...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 43**. Probe40 **H=1 r=+0.554 / H=55 r=+0.267 conv=0.00** gain_fid **0.524**. recon **0.011@43 best** (P64 @43 **0.0022**; ~5×). Teacher **×0.99 / ×1.02 @43**. persist_rel **0.82@41 → 0.24@43** (inject rebound decaying). pid **242511**. | 🔬 LIVE P1 — persist rebound after inject. H=55/conv still dead. Do not patch live. |
...
- **LIVE P1 iter 43 (~225 min, nvidia 11.7 GB, `t_wm` ~309 s, sps 19.5):** skip **0**. Teacher jsonl 41 **×0.996 / ×0.976** → 42 **×0.981 / ×1.084** → 43 **×0.991 / ×1.016**. recon **0.013@41 → 0.012@42 → 0.011@43 new best** vs P64 **0.0022@43** (~5×). last_ok **43 unlocked**. persist_rel **0.181@40 → 0.822@41 → 0.546@42 → 0.239@43** (inject rebound peaking then decaying; not a new knob). Rest-IC graph not captured. Probe 40 still last fidelity. Next typically iter 50.

...
| Held-rollout late−early on GRU `h` (`out='h'`) | **P61 EXIT** (`run_p61_clipdu`; P1 median ~6.4e-4 vs overshoot ~0.041) | SUPERSEDED (P62 `out='obs'` + `cv_index_t`; P63 magnitude not stationarity) | YES — p139: z/c wander while `h` sits; compounding is decoded-CV contraction. |
| Held-rollout late−early on decoded CV | **P62 EXIT** (`run_p62_heldcv`; P1 held median **1.53e-4** vs overshoot **0.040**; val OL-vs-real **×0.785** vs P61 ×0.809) | **FALSIFIED** as compounding (P63 replaces with 1step→K FO magnitude; KEEP `out='obs'`) | YES — silent when prior has settled to a contracted SS. 1step→OL *ratio* ×0.828 vs P61 ×0.772 was 1-step coming down, not OL vs plant. Paired −45 vs −113 loses to P58/P61. |
| Held magnitude std-normalized MSE | **P63 abort** iter 1 (`run_p63_olmag` pid **165318**; loss **1.18e5** skip 5/5) | SUPERSEDED (abs Huber β=1 on ΔK vs FO·sg(Δ1); jsonl scale stays tail std) | YES — imagined-CV std sat 1e-3 floor (P27 1/\|tgt\| class). Smoke init loss ~1.3. |
| `wm_held_rollout_settle_frac` (P62 late−early early-window) | **P63 GPU-occupied** (pid **165969** P1) | **REMOVED** (field + `DREAMER_WM_HELD_ROLLOUT_SETTLE_FRAC`) | YES — unread after P63 1step→K magnitude (K-tail only). Silent A/B. Not in this pid. |

...
- **Liveness:** **GAIN-READY P1→P2 @82** (no extra-P1; `p1_extension=0/175924`). Live TM-style gain-probe: DC **[0.88, 0.94]** @H **[0.93, 0.93]** worst **0.88@DV** (MV 0.94/@H 0.93 DV 0.88/@H 0.93; P52 @82 was last_ok **78** ~0.84@DV). last_ok **82** unlocked. Warm-restore SKIPPED (skip-storm already restored last_ok **72**; `wm_best` gain-blind). STAGE 2 DOB id, g FROZEN. Skip-storm **1/2 @73** recovered (recon 2.5765→0.0022@74). wm-fid @80 @H r=+0.606 gain_fid 0.914 **EMA 6.578**. P2 iter **85** recon **0.0018** dobg **0.0018** bc **0.010** sps ~227. GPU ~11 GB. ~178 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Watch first P3 ent ≈ H(σ_init); unfreeze ~147. Do not KEEP/FALSIFIED as cascade until EXIT.

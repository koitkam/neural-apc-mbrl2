...
### p56 (`run_p56_muslow`, branch `cursor/p28`) — LAUNCH: slow μ-ratio recopy N=50
...
- **Step 4 CONFIRMED:** `[env-override] p3_mu_ratio_refresh_iters=50`. `[resolved-cfg] … p3_muratio=0.2 p3_murefresh=50 es_ent_floor=0.25 compile=eager`. Process environ: `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF` + `DREAMER_P3_MU_RATIO_REFRESH=50`. P1 ~iter **71** recon **0.0017**. Skip-storm 1/2 @68–69 (recon 0.253→0.512, 65 inner skips; last_ok locked **67**, restored, unlocked). Teacher FD MV/DV ~×1.00. `wm_best` **iter 70** EMA 6.224 gain_fid **0.926** (`restore_p2=False` so P2 still uses end-of-P1 g). Rest-IC CUDA-graph **failed this pid** (`ValueError: grad requires non-empty inputs` — nested function `parameters()` empty so `autograd.grad` inputs=(); `t_wm_s` ~124 s of ~128 s/iter). SNR measured CV+DV median **+17.0 dB** min **+14.4 dB**. HEAD after this pid: `_RestICGraphModule` yields RSSM `parameters`/`named_parameters`/`buffers`/`named_buffers`; empty trainable surface / GRU-canary still pin; **VRAM <512 MiB skip does not pin** (warmup retries once after `empty_cache`). Do **not** launch a second GPU job. Do not stack critic knobs. Do not promote N=50 without EXIT+val.
...
- Rest-IC encode length is `lookback` (test_sim **128** sequential GRU steps × 100 inner WM steps → eager `t_wm` ~123 s). A few-τ window would be more sim-adaptive; that is a later A/B (not identity). **HEAD:** CUDA-graph of that T-loop via `_RestICGraphModule` (`gain_match_rest_ic_cuda_graph=True`; `allow_unused_input=True`), warmed **before** the P1 WM autocast loop + parent `enabled=False` then bf16 `cache_enabled=False`; **in-loop never captures** (autocast miss is eager that step, not a fail pin). Nested function is not a graph surface (P56 `parameters()` empty). Overriding only `parameters()` left `named_parameters()`/`buffers()` empty (torch 2.12 capture uses both). **VRAM skip does not pin** `_rest_ic_cg_fail` (warmup retry after `empty_cache`). CPU path stays the Python loop (`CVD=""`). `DREAMER_GAIN_MATCH_REST_IC_CUDA_GRAPH=0` keeps eager.
...
| Rest-IC last_only T-loop = 128 sequential kernel launches × 100 WM steps | P54 GPU-occupied (pid **95956** P1); **P55/P56** first GPU capture failed | SUPERSEDED (`gain_match_rest_ic_cuda_graph=True`; `make_graphed_callables` + GRU-grad canary; **P55 HEAD** capture **before** WM autocast + parent `enabled=False` then bf16 `cache_enabled=False`; **in-loop never captures**; **P56 HEAD** `_RestICGraphModule` **parameters/named_parameters/buffers/named_buffers**; VRAM skip **not** pin) | PARTIAL — P55 pid autocast cache; **P56 pid 110246** nested function `parameters()` empty → `ValueError: grad requires non-empty inputs` → eager `t_wm_s` ~124 s @iter 71. Skip-storm 1/2 @68–69 recovered. Overriding only `parameters()` left `named_parameters()`/`buffers()` empty (torch 2.12 capture uses both). Pinning `_rest_ic_cg_fail` on VRAM skip would freeze eager even after `empty_cache`. Not in this pid. |

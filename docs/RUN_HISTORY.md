...
- **Liveness:** **GAIN-READY P1→P2 @82** (no extra-P1). Live probe worst **0.88@DV** (MV 0.94/@H 0.93 DV 0.88/@H 0.93; P52 @82 last_ok **78** ~0.84@DV). last_ok **82**. Skip-storm **1/2 @73** recovered. STAGE 2 DOB id, g FROZEN. P2 iter **92** recon **0.0019** dobg **0.0025** bc **0.004** last_ok **82**. wm-fid @**90**: H=1 r=+0.799 **@H r=+0.627** conv=1.00 drift=0.009 gain_fid **0.935** best_h=55/55. **New EMA best 6.845** (raw 7.112) @90 → `wm_best.pt` (was 6.578 @80). Still P2 (not P3). sps ~230. GPU ~11 GB. ~182 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Watch first P3 ent ≈ H(σ_init); unfreeze ~147. Do not KEEP/FALSIFIED as cascade until EXIT.

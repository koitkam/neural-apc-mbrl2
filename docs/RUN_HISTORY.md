...
| P48 | 2026-08-29 | P3 collect/val stream measured DV + Kalman | LIVE P1 iter **43** recon 0.002 skip 0 last_ok **locked 24** (iter-25 recon 0.145 / 43× / gnorm 5.26 wrap, skip 0) teacher FD ~×1.01 `t_wm`~126 s `wm_best`@40 | ⏳ env-free; GPU occupied pid **34706**; freeze will restore iter 24 (P41 lock class). No second job |
...
- **P48-live (GPU occupied, no second job):** leftover `DREAMER_TARGET_UTIL` / `DREAMER_MAX_BS` / `DREAMER_BATCH_SIZE` (leftover `OBJ_BATCH_SIZE`) → TrainConfig + `ENV_OVERRIDES` (identity 0.80 / 512; dual-read at GPU probe). P1 iter **43**: recon 0.002 gmatch tiny skip 0 teacher FD MV/DV ~×1.01/×1.02 (not val TM). `t_wm` ~126 s. `wm_best` iter 40 (gain-blind). **last_ok locked iter 24** after iter-25 recon 0.145 (43× best 0.0033, gnorm 5.26, skip 0) then recovered — P41 lock class (42× / gnorm 1.99 / skip 0). Freeze **will restore iter 24** even if recon stays healthy (P40: locked last-ok always restores). Do not retune lock_ratio: wrap can exceed 20× (P48 43×); P41 still needs recon-only lock. Storm 0. Do not launch. Do not promote `p3_reset_log_std`. Do not change last-ok lock this tick.

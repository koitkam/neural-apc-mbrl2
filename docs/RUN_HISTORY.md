...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 21**. Probe20 **H=1 r=+0.325 / H=55 r=+0.145 conv=0.00** gain_fid **0.261** best_h=0/55 (P76 +0.589/+0.543 conv=0 0.644; P64 +0.706/+0.498 conv=0.50 0.734). Probe10 was +0.128/+0.122/0.476. Iter19 wrap-like recon **0.497** teacher **×1.30 / ×−3.35** recovered @20 skip **0**. recon **0.096@21** best (P64 @21 **0.003**). pid **242511**. | 🔬 LIVE P1 — conv=0 @10 **and** @20 (P76-class). H=55 r still dead. OL supervisors start `img_rollout(h,z)` with `kv_cache=None` (Markovian TSSM; probe warms cache). Do not patch live. |
...
- **LIVE P1 iter 21 (~110 min, nvidia 11.7 GB, `t_wm` ~306 s, sps 19.7):** skip **0**. Teacher jsonl 17 **×0.975 / ×1.292** → 18 **×1.008 / ×0.949** → 19 **×1.300 / ×−3.35** (wrap-like; gmatch **1.31** recon **0.497** gnorm **50**) → 20 **×1.003 / ×0.897** recovered → 21 **×0.993 / ×1.012** recon **0.096 new best** (P64 @21 **0.003**). persist_rel **0.327@21**. Rest-IC graph not captured.
...
- **RCA (SysID, not a live patch):** teacher-forced `rollout_observed` and the fidelity probe warm TSSM `kv_cache` over the lookback, but gain-match / overshoot / held / isolation start `img_rollout(h, z)` with `kv_cache=None` (`_rest_ic_last_tensors` returns only `h/z/c`). RSSM `h` *is* history; TSSM then rolls a 1-token Markovian net. Matches 1-step recon falling (cached encode) while probe H=55 r / conv stay dead (cached OL is not the trained path). Do not patch while live. Next-if EXIT FALSIFIED: one attributed A/B pass `prev_state` into those OL rolls. Metric: probe H=55 r / conv / val 1step→OL vs P64 ×0.85. `tssm_d_model=512` / 4 layers is not tied to model_size L `d_model=384` / 8; B=16 vs RSSM 128 is gpu-calib, not a second attributed change. Watch freeze vs P64 **0.91@DV**; later probes conv>0; `actor_experiment_valid`. Do **not** start a second GPU job.

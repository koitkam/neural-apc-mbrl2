...
| P54 | 2026-08-30 | ES entropy thr = σ-band frac (`early_stop_entropy_collapse_floor_frac=0.25`) | LIVE env-free `run_p54_esentband` pid **95956** CUDA ~11 GB. Gate **82 PASS GAIN-READY** 0.91@DV / 0.93@MV. **P3 ~iter 400** (~1.48M/1.63M, ~4.0 h). First P3 H **−0.127** then open-σ **−0.101**. **264/264** P3 iters below old trip **−0.083**; **0** below new **−0.238**. Passed P53 ES death @262. Unfreeze logp_std spike 1.38@150 then held ~0.40–0.55. rscale **2.06**. rtgt 0.06→**0.001**. Do **not** Step-5 PROMOTE until EXIT+val. GPU occupied — no second job. HEAD: `critic_mc_loss` logged (not in this pid); dead `pmpo_loss`/`kl_to` **REMOVED**. | LIVE |
...
### p54 (`run_p54_esentband`, branch `cursor/p28`) — LIVE: ES entropy thr vs open-σ
- **One attributed GPU change vs P53 pid:** `early_stop_entropy_collapse_floor_frac=0.25` (HEAD default `f6739ac`). Trip at `H(σ_min)+0.25·(H(σ_max)−H(σ_min))` instead of `H(σ_min)+0.20 nats`. With `sigma_min_ratio=1.2`, log(1.2)≈0.182 < 0.20 so the old nats margin put **H(σ_max) below the trip** (P53 −0.101 < −0.083). New thr ≈ **−0.238**; open-σ −0.101 does not trip; σ_min −0.283 still does. Tiny σ-band → skip entropy ES. Opt out `DREAMER_ES_ENT_FLOOR_FRAC=0` (legacy `frac·H(σ=1)` only). Also P53 `p3_mu_ratio_clip=0.2`. Env-free. tmux `mbrl2_p54`. `CUDA_VISIBLE_DEVICES=0`.
- **Not:** leftover `DREAMER_*`, tighter `p3_logp_clip`, `p3_reset_log_std`, extra critic knobs, `actor_kl_coef`, raising `min_adv_corr`.
- **Step 4:** env-free. Watch `[resolved-cfg] … p3_muratio=0.2 es_ent_floor=0.25`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`.
- **Watch:** ES reason ≠ `entropy_collapse_window` (expect `p3_plateau` or budget); P3 entropy stays ≈−0.101; unfreeze logp_std stays ~0.47 (μ-ratio still holding); `adv_corr` may stay <0.05 without killing the run; best.pt iter later than 166; val paired vs P53 **−13 vs −98**. Storm / freeze last_ok should match P53 class (GAIN-READY). Do not stack critic knobs if rtgt stays tiny.
- **LIVE P2 (2026-08-30 ~14:03 CDT, pid 95956):** Gate **82 PASS GAIN-READY** `DCgain_ratio[0.91,0.93] @H[0.93,0.96] worst=0.91@DV` (MV 0.93/@H 0.93; DV 0.91/@H 0.96; unbiased, not_noisy). last_ok **82 unlocked** (wrap lock@5 unlock@7; no skip-storm). skip **0**. wm_best iter **90** EMA **6.517** `gain_fid=0.899` best_h=55/55. Freeze = live end-of-P1 g (warm-restore SKIPPED). P2 iter **98**: recon **0.0038**, dobg **0.0022**, bc MSE ~0.003 (P1 was ~0.35), g FROZEN, sps ~233. P2 budget ~1.09M steps (~iter 139). ES A/B (σ-band floor 0.25) not yet scored. `[resolved-cfg] … p3_muratio=0.2 es_ent_floor=0.25 compile=eager`. **Do not launch a second GPU job.**
- **LIVE P3 (2026-08-30 ~14:58 CDT, pid 95956, ~iter 400):** P2→P3 gate **136 PASS**. P3 starts **137**. First P3 H **−0.127** → open-σ **−0.101** (min **−0.136**). **264/264** P3 iters below old trip **−0.083**; **0** below new trip **≈−0.238**. Passed P53 ES death window @262. `[return-scale] FREEZE` @p3_iter=10 scale **2.062**. Unfreeze ~147: logp_std spike **1.38@150** then held **~0.40–0.55**; μ-ratio clip_frac **~0.11–0.42**. rtgt warmup **0.06** then collapse **~0.001** (MC-grounding caveat; `critic_mc_loss` **not logged in this pid**). ema **−217→−114**. `imag_adv_action_corr` often <0.05 (0.00–0.26). mvv spikes (max **90158**) then recover. env_steps **1.48M / 1.63M**. ES A/B is working as a **false-trip fix**; **do not Step-5 PROMOTE/KEEP until EXIT + all-axes val** vs P53 **−13/−98**. Do not stack critic knobs.
- **HEAD (not in pid 95956):** p136 `actor_kl_coef` **REMOVED**. `DREAMER_ACTOR_LOSS=pmpo` **refused at `train()`** (false A/B). Dead `pmpo_loss` / `reinforce_actor_loss` / policy `kl_to` **REMOVED**. `_realsim_actor_critic_step` now logs **`critic_mc_loss`** (sum of detached MC twohot-CE terms; training graph unchanged) + `critic_pred_target_r` / `critic_target_v_r` (jsonl merge already dumps all ac_losses keys; P54 pid launched before this). Rest-IC CUDA-graph canary **saves/restores** in-flight grads. CUDA replay H2D per-slot. CVD="" smoke green. GPU identity deferred.
...
| ES entropy floor margin 0.20 nats (implicit; not TrainConfig) | P52/P53 EXIT; **P54** (`run_p54_esentband`) | SUPERSEDED (`early_stop_entropy_collapse_floor_frac=0.25`) | YES — `log(sigma_min_ratio=1.2)≈0.182 < 0.20` so H(σ_max)≈−0.101 sat below thr −0.083. Frozen-σ P3 died on `adv_corr<0.05`. Band frac: thr=H(σ_min)+0.25·gap ≈ −0.238. Open-σ does not trip; σ_min still does. Opt out `DREAMER_ES_ENT_FLOOR_FRAC=0`. |
...
| Rest-IC last_only T-loop = 128 sequential kernel launches × 100 WM steps | P54 GPU-occupied (pid **95956** P1) | SUPERSEDED (`gain_match_rest_ic_cuda_graph=True`; `make_graphed_callables` + GRU-grad canary) | YES — launch-bound N=6 encode; static cached obs/act; `sample=False`. Raw `CUDAGraph` has no backward (P25). TSSM kv-cache / CPU / capture-fail / free VRAM <512 MiB stay eager. GPU identity deferred (95956 holds A10). `DREAMER_GAIN_MATCH_REST_IC_CUDA_GRAPH=0` keeps the Python loop. Not in P54 pid. |

...
- **P1 through iter 54 (P33 class, not P34–P36).** **Past P32 storm window:** iter 53 recon **0.0047 skip 0** (P32 recon 0.596 skip 71; P33 recon 0.0178 skip 0). Iter 54 recon 0.0038 gmatch 0.0003 iso 0.0012 skip **0**. Iter-1 iso **1.33** (P33 1.69; P36 0.125). gmatch **1.20 → 0.054 by iter 3 → 0.0008 by iter 17**. Iter 19 wrap-class blip recon 0.267 recovered (do **not** lower skip threshold). Iter 50 probe best_h=55/55 gain_fid=0.708. Next gate ~iter 75. ~2.3 min/iter, t_wm ~131 s of 138 s.
- **Do not launch a second GPU job.** If GAIN-READY: first valid actor with `DREAMER_WM_ISOLATION_DCV_MATCH=0` (HEAD default ON would confound). If still GAIN_NOT_READY: `[p3-skip]`; next GPU is env-free P38 `run_p38_isodcv` (isolation |ΔCV| excitation).
- **HEAD (not in this pid):** isolation inv-var A/B **REMOVED**. Abs MSE + **`wm_isolation_dcv_match=True`**: isolation settle `Δu_i ∝ 1/|G_i|` from gain-match WM-norm targets, clipped to ±1 (test_sim scale MV ~0.289 DV ~1.67). Equal-|G| keeps op-band. Not a loss reweight. `[resolved-cfg] iso_dcv=`. Pre-iso resolve feeds **scales only**; post-seed **always re-resolves** Huber targets after isolation+expert (P37 freeze point). Opt out `DREAMER_WM_ISOLATION_DCV_MATCH=0`.
...
- `run_plan.json → config` is rewritten after gain-match resolve. `[resolved-cfg]` prints `iso_dcv=` (inv-var knob removed). Pre-rewrite dump is dataclass defaults (P29 plan still shows `rssm_latent_type=categorical` / `gain_match_coef=0` even though training auto-enabled grounding).
...
| Isolation |ΔCV| excitation `Δu_i ∝ 1/|G_i|` (`wm_isolation_dcv_match`) | P37-live HEAD (not in pid) | PENDING (default ON for next env-free) | PARTIAL — P33 abs isolation FALSIFIED as DV pin (|tgt| 2.82 vs 0.49). Not a loss reweight. Wait P37 EXIT. |

...
| P51 | 2026-08-30 | P3 REINFORCE stop-grad log_std (`p3_stop_grad_log_std=True`) | **LIVE P3** iter **208** (~3h37). pid **54815**, launch `d6a4511`. Gate **GAIN-READY** 0.86@MV / 0.88@DV @82; last_ok **82 unlocked**. P2→P3 PASS 0.922 @136. First P3 ent **−0.125**. Unfreeze **147**: ent **HELD −0.141→−0.101** (not P50 −0.268); `actor_logp_std` **0.54→38** (μ-rail; not policy log_std); actor **−0.01→−28**; rtgt **0.046→0.0004**; rscale freeze **2.17 KEEP**; ret_w **−65→−590**; ret_ema **−865**. Ent later **−0.20** @208 (tanh-rail geometry, not σ_min −0.283). Env-free. Do not launch. | 🔬 stop-grad KEEP as entropy lever; **failing as cascade lever** (μ-rail). Watch EXIT. Do not stack critic knobs. |
...
- **Unfreeze 147 (the A/B canary):** last warmup 146 ent **−0.141** `actor_logp_std` **0.54** actor **−0.010** bc 0.002 rtgt 0.046. Iter 147: ent **−0.101 HELD** (P50 169: **−0.107→−0.268**), `actor_logp_std` **0.54→38** (P50 **0.56→54** — μ-rail logp variance, not σ training), actor **−4.66** then **−28** @191 (P50 first unfreeze **−6.40**), bc 0.002→0.87, logp_mean 0.72→−26. By iter **208**: rtgt **0.0004**, ret_w **−590**, ret_ema **−865**, ent **−0.203** (drifting via tanh-rail geometry, not σ_min pin −0.283). **Stop-grad KEEP as entropy/σ lever** (no −0.268 yank at unfreeze). **Failing as cascade lever** (μ REINFORCE still rails). `actor_logp_std` is not policy log_std.
- **Watch:** EXIT / ES (P50 entropy_collapse @286; this pid may ES on return if entropy stays open). Do not edit val stack while pid 54815 is live. Do **not** launch. Do **not** implement the next μ-rail fix until EXIT.

...
| P65 | 2026-09-01 | P1→P2 `TrajectoryBuffer.clear()` (no new knob) | **LIVE P1** tmux `mbrl2_p65` pid **173526** sha `b4ee586`. Iter **63** recon **0.0028** MV **0.984** DV **1.001** skip **0** last_ok **63** unlocked. Matches P64@63 (0.0030 / 1.009 / 1.011). Flush not fired yet. | ⏳ GPU occupied. Do not score actor. Watch P2 `buf_fill` after banner. |
...
- **LIVE (2026-09-01 ~06:50 CDT, still P1):** iter **63** recon **0.0028** gmatch **0.00027** MV ratio **0.984** DV **1.001** skip **0/0** last_ok **63 unlocked** `buf_fill` 100%. P64@63 recon **0.0030** MV **1.009** DV **1.011**. Held **8.3e-5** vs P64 **7.4e-5**. Fidelity iter60 H=1 r=+0.675 / @H +0.242 (gain-blind; restore OFF). No skip-storm. Flush has **not** fired (`[curriculum] P2 replay flush` absent). P1 quality gate still ~iter 82 (P64 P2 @83, `buf_fill` stayed 1.0). **No second GPU job.**

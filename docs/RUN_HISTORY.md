...
| P50 | 2026-08-29 | P1/P2 BC MSE-on-μ (`bc_mean_only=True`) | **LIVE** P1 iter **66**. Wrap **GPU-confirmed**: lock@56 recon 0.1412 (97×) skip 0; **unlocked wrap-recovery @58**; last_ok **66**. skip **0** (no storm). **bc MSE 65/66 pos** (iter 65 0.000). Do not launch. | 🔬 wrap-unlock KEEP; watch GAIN-READY ~82; P2 bc>0; first P3 ent ≈ **−0.08** |
...
- **LIVE (2026-08-29 18:59 CDT, P1 iter 66):** wrap-recovery unlock **GPU-confirmed** (P49 untested — skip-storm fired first). Iter **57** recon **0.1412** = 97× best 0.0015, gnorm 1.20, skip **0** → `[wm-last-ok] locked iter 56` + wrote `wm_last_ok.pt`. Iter **58** recon 0.0141 < 20× → **`unlocked after wrap recovery`**; last_ok advanced 56→**66**. No skip-storm through 66 (P49 stormed @64–65). recon now 0.0038. **`bc_loss` MSE 65/66 >0** (iter 65 logged 0.000 then 0.230 @66; never NLL). Teacher FD recovered ~×1.0 after wrap (iter 57 MV/DV ×0.90/×0.76). Probe@50 gain_fid 0.749 (wm_best still 30). z_alive 1024→1015 post-wrap. GPU ~11 GB. First P3 ent target ≈ **−0.08**.
...
- **Watch:** GAIN-READY typically ~iter 82; P2 `bc_loss` stays **>0**; first P3 entropy ≈ **−0.08** not −0.283; freeze rscale ~2; do not train P3 on GAIN_NOT_READY. Remaining leftover is env-only paths/device + rest-IC encode=lookback A/B — do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.

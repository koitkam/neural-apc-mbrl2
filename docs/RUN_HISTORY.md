...
- **Liveness:** P1 STAGE 1. Iter **67** (67 jsonl rows, ~145 min). recon **0.128→0.0027** (best **0.0017** @46 held) gmatch **0.0002** (FD MV/DV **×0.99 / ×0.97**). iso 0 ss 0 skip **0**. last_ok **67** unlocked. wm-fidelity probe @**60**: H=1 r=+0.789 @H r=+0.324 conv=1.00 drift=0.011 gain_fid=0.654 best_h=27/55 (not the TM gate). EMA **5.998** held @**30** (raw 6.474, `wm_best.pt` unchanged; live EMA 5.630). Next probe ~70. sps ~48. GPU ~11 GB. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Gate vs P52 last-ok 78 not yet. Do not KEEP/FALSIFIED as cascade until EXIT.

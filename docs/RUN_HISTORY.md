...
- **LIVE (2026-08-26 ~P1 iter 30, GPU still running): CONFOUNDED.** Env-free dropped `DREAMER_RSSM_LATENT_TYPE=deterministic` (P26/P28 env, never a default). P29 is **categorical**: `kl_loss=0.30` pinned, `joint_embed_loss≡0`, recon **0.50→0.28 by iter 30** (P28 recon **0.004** by iter 30; P26/P28 **<0.02** by iter 10). Iter-6 spike (recon 0.89, gain_match 1.21) recovered on the aux losses; recon did not. `z_dim=32` / `zrank/1024` is the same width as deterministic — do not use it. P29 cannot verdict skip-restore. **Also compile-on** (no compile banner on P28; P29 iter-1 sps 128 vs P28 598). **Code fix (prior session, no relaunch):** `rssm_latent_type` default **categorical → deterministic**. **This session (GPU occupied, no relaunch):** rewrite `run_plan.json → config` after promotions (`[resolved-cfg]`); honour explicit `DREAMER_DOB_GROUND_COEF=0`; README restore default was still "1". Next GPU job after P29 EXIT = real skip-restore A/B (follow-ups 1–14 + deterministic default + `DREAMER_COMPILE=0` to match P28 eager). Train-start banner now prints `latent=`.
...
- Isolation/ss-match trajectory MSE is still absolute (same family as abs Huber). Variance-normalized isolation is the next observer A/B **if** a **deterministic** skip-restore run still shows DV ss ~×0.87 after follow-up 10 isomorphic |Δu| + follow-up 13 full-K gain-match + follow-up 14 mean-c0 — do not revive relative Huber.
...
- Next launch after P29: env-free `cursor/p28` + `DREAMER_COMPILE=0` (P28 eager; P29 compiled). Do not add leftover P26 env-vars (`gain_match_coef` / `dob_ground_coef` auto-enable).

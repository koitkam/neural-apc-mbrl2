...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 27**. Probe20 **H=1 r=+0.325 / H=55 r=+0.145 conv=0.00** gain_fid **0.261**. recon **0.044@27 best** (P76 @27 **0.0035**; ~13×). Teacher **×1.03 / ×0.85 @27**. CPU: Markovian vs cached CV gap **0.18** mean, compounds 0.03→0.22. pid **242511**. | 🔬 LIVE P1 — recon still dropping. Cached OL (probe) dead at H=55. Gain-match trains Markovian (do not patch live). |
...
- **LIVE P1 iter 27 (~142 min, nvidia 11.7 GB, `t_wm` ~307 s, sps 19.6):** skip **0**. Teacher jsonl 25 **×0.991 / ×1.040** → 26 **×0.993 / ×0.992** → 27 **×1.026 / ×0.852** (DV slipped). recon **0.052@25 → 0.054@26 → 0.044@27 new best** vs P76 **0.0035@27** / P64 **0.004@25** (~13×). last_ok **27 unlocked**. persist_rel **0.54@26 → 0.40@27**. Held silent **7.7e-4**; overshoot **0.125**. Rest-IC graph not captured. Probe 20 still last fidelity. Next typically iter 30 (~15 min).

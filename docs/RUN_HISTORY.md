...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P1** iter **61** (~138 min). pid **64705**, launch `d910ee2`. Step 4 **CONFIRMED** `p3_sglogstd=True p3_logpclip=8`. recon **0.0022** (best **0.0019@55**) last_ok advancing unlocked skip **0**. lock@38 wrap-unlock@39. Iter 50 probe H=55 r=0.289 gain_fid=0.714 (best still 30 EMA 5.798). GPU ~11 GB. Env-free. Watch gate 82; unfreeze ~147 vs P51 μ-rail. | 🔬 wrap-unlock KEEP. Do not launch. Do not stack critic knobs. |
...
- **P1 iter 61 (~138 min):** recon **0.0022** (best **0.0019@55**, beat 0.0024@51 / 0.0027@37) last_ok advancing **unlocked** skip **0**. Iter 56 bump 0.0042 is 2.2× not lock. lock@38 wrap-unlock@39 last_ok advanced through recovery (P50-class KEEP). Iter 50 wm-fid H=55 r=**0.289** `gain_fid=0.714` best_h=13/55 (did not beat iter 30 EMA **5.798**). Teacher FD mv **1.00** / dv **0.996**. Gate 82 ~45 min; unfreeze ~147 is the A/B.

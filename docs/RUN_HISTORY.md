...
| P67 | 2026-09-01 | teacher `gain_match_len` auto = `wm_tf_horizon` (K=220; encode T lookback) | LIVE P1 pid **187753** sha `9fbede0`. `gmatch_len=220` `wm_train_T=128`. Iter 1 teacher ×0.958/×0.974 skip 0 `t_wm` 134 s ~14 GB. | 🔬 compounding family vs P64 ×0.842 OL-vs-real |
...
- **Step 4 CONFIRMED:** `device=cuda` bs=128 `wm_train_T=128` horizon=55 compile=eager. `[gain-match] len=220 settle=-1 step=0.4`. `[resolved-cfg] … isolation=0 ss_match=0 gmatch_len=220 gmatch_step=0.4 gmatch_clip=True gmatch_rest=True gmatch_rest_L=128 held_cv=True p1amp=0.2 p2amp=1 p3amp=1 compile=eager`. Rest-IC graph captured `N=6 T=128`. Iter 1 teacher MV/DV **×0.958 / ×0.974** skip **0** `t_wm` **134 s** (P58 ~102 s) gpu_peak **11.3 GB** nvidia **~14.3 GB**. **No OOM** at K=220. **One GPU job.**
- **Watch:** jsonl `*_ratio` ~×1 at K=220; P1 skip-storm; freeze vs last_ok 82 / 0.91@DV; val OL-vs-real vs P64 **×0.842**; MV/DV vs P26/P64; paired vs P64 **−4.54**; det_r/pred_std (not a DOB experiment). Do not score actor if GAIN_NOT_READY.
...
| Gain-match FD K = control H (last-only Huber) | P26–P66; val TM / decomp at `wm_tf_horizon=4H` | P67 auto K = `wm_tf_horizon` (LIVE pid **187753**) | PARTIAL as protocol gap — 1-step faithful / OL short of plant (P64 ×0.842; P66 ×0.718). Not S=H. Not a Huber reweight. |

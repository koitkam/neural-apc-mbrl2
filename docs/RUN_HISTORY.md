...
### p60 (`run_p60_tmstep`, branch `cursor/p28`) — LIVE P1: teacher Δu auto = val TM `wm_tf_step_frac`
...
- **LIVE P1 (~iter 65, 2026-08-31 15:06):** jsonl teacher `gain_match_mv_ratio` **×0.990** / `gain_match_dv_ratio` **×0.996** at Δu=0.4 (iters 10/20/30/40/50/60/65 all sit **×0.95–1.02**). recon **0.0023**, gmatch **0.0002**, jemb 0.006, iso/ss 0, kl 0. `t_wm` **97 s** / `t_collect` **3.3 s**. Graph captured. skip **0/0**. last_ok **65 unlocked**. `wm_best` iter **30** (gain-blind; probe@30 gain_fid 0.936 vs @20 0.664 vs @40 0.552 vs @60 0.625). **Teacher is pinning G(0.4).** P58 stormed 2/2 @61; this pid still healthy past that iter. Val TM still pending freeze. **No second GPU job.**
- **HEAD (not in pid 144896):** overshoot/isolation gather idx cache + one `obs_win`; PRBS gain-match start arange cached (`_gmatch_starts`; env-free rest-IC skips that path). **This session:** skip discarded prior-c `randn` when det-roll replaces the whole sample with the mean (`_prior_c_from_net`; RSSM+TSSM); cache GRU/token/`img_rollout` `(B,D)` zeros; dict-cache Huber/`*_ratio` teacher G. Identity speed. CVD="" `_smoke_rssm` + `_smoke_tssm` green.
- **Watch:** jsonl `wm_gain_match_{mv,dv}_ratio` stay ~×1 at Δu=0.4; live GAIN-READY MV closer to val TM; val MV ss toward ×1.0 / P26 ×0.97; compounding open-loop; paired econ vs P58 −6.24.

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 55**. Probe50 still last **H=1 r=+0.509 / H=55 r=+0.273 conv=0.00** gain_fid **0.389**. recon **0.0057@53 best** (P64 @50 **0.0026**; ~2.2×) then **0.0067@55**. Teacher **×1.01 / ×0.98 @55** (DV **×1.09@54**). persist_rel **0.73@51 → 0.11@55**. pid **242511**. | 🔬 LIVE P1 — persist decaying after inject (not held ~0.5). Probe60 pending. Do not patch live. |
...
- **LIVE P1 iter 55 (~288 min, nvidia 11.7 GB, `t_wm` ~307 s, sps 19.7):** skip **0**. Teacher jsonl 53 **×1.001 / ×0.988** → 54 **×1.000 / ×1.087** → 55 **×1.005 / ×0.980**. recon **0.0060@52 → 0.0057@53 new best → 0.0061@54 → 0.0067@55** vs P64 **0.0026@50** (~2.2× best; P76 @50 0.0028). last_ok **55 unlocked**. persist_rel **0.73@51 inject → 0.49@52 → 0.37@53 → 0.29@54 → 0.11@55** (faster decay than @41→@43 plateau; do **not** persist N+1). Rest-IC graph not captured. Next probe typically iter 60 (~00:42 CDT).

...
- **LIVE (2026-08-27 04:50, tmux `mbrl2_p33`, GPU ~19.1 GB, pid 4146603):** env-free det+eager CONFIRMED. `[resolved-cfg] latent=deterministic restore_p2=False gain_match=1 dob_ground=2 isolation=1 ss_match=3 n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 compile=eager`. Train-start `device=cuda bs=128`. `[gate-budget]` p1_ext_cap **175924**. Stage 1 @iter0. P1 iter **25** recon **0.0053** `kl=0` `jemb` 0.051 gnorm 0.84 skip **0** (P26/P28 class). `wm_best` iter **20** `best_h=55/55` gain_fid=0.676. Mean `t_wm` ~131 s / `t_collect` ~5.7 s. No skip-storm yet (P32 stormed @53). **No second GPU job.** GPU-occupied: vectorized `TrajectoryBuffer.sample` (identity); architecture.md DOB/curriculum banners match TrainConfig defaults ON; dropped leftover imagination gain-rand from the unused DOB-free curriculum log.

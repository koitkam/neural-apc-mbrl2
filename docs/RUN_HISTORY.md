...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 3**. Teacher jsonl **×0.91 / ×1.14** (pinned @3). recon **0.81→0.52→0.42** (P76 @3 **0.074**; P64 **0.020**). `t_wm` **~307 s** steady (P76 P1 ~96 s). skip **0**. Rest-IC graph not captured. pid **242511**. | 🔬 LIVE P1 — recon slower than RSSM; watch freeze vs P64 0.91@DV; conv>0 |
...
- **LIVE P1 iter 3 (~18 min, nvidia 11.7 GB, `t_wm` ~307 s steady, sps 19.7):** skip **0**. Teacher jsonl 1 **×0.643 / ×−1.07** → 2 **×0.355 / ×−0.14** → 3 **×0.908 / ×1.14** (P76 pinned @3 ×0.99/×1.04; P64 ~×1 from iter 1). recon **0.806 → 0.524 → 0.416** vs P76 **0.370 → 0.135 → 0.074** / P64 **0.076 → 0.031 → 0.020**. persist_rel **0.45 → 0.63** (not yet pinned). No fidelity probe yet (first at 10). `t_wm` is not first-iter warmup — ~5 min/iter (eager rest-IC + TSSM; P76 P1 ~96 s). Do not patch while live. Watch freeze vs P64 **0.91@DV**; conv>0 (P76 uniquely 0); val **1step→OL** vs P64 ×0.85 / P76 ×0.770; `actor_experiment_valid`. Do **not** start a second GPU job.

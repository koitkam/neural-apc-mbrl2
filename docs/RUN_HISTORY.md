...
| P53 | 2026-08-30 | P3 PPO ratio clip vs frozen unfreeze-μ (`p3_mu_ratio_clip=0.2`) | LIVE `126f011` pid **75966** tmux `mbrl2_p53`. Step 4 `p3_sglogstd=True p3_logpclip=8 p3_muratio=0.2`. P1 iter **48** recon **0.0021** (best **0.0017** @46) gmatch **0.0004** FD MV/DV ×0.98/×0.97 skip **0**. wm-fid @40 @H r=+0.453 gain_fid 0.544; EMA **5.998** @30. Watch unfreeze 147. | 🔬 LIVE |
...
- **Liveness:** P1 STAGE 1. Iter **48** (48 jsonl rows, ~104 min). recon **0.128→0.0021** (best **0.0017** @46 held) gmatch **0.0004** (FD MV/DV **×0.98 / ×0.97**). iso 0 ss 0 skip **0**. last_ok **48** unlocked. wm-fidelity probe @**40**: H=1 r=+0.707 @H r=+0.453 conv=1.00 drift=0.011 gain_fid=0.544 (not the TM gate). EMA **5.998** held @**30** (raw 6.474, `wm_best.pt` unchanged). Next probe ~50. sps ~48. GPU ~11 GB. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Gate vs P52 last-ok 78 not yet. Do not KEEP/FALSIFIED as cascade until EXIT.

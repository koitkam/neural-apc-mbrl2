...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 65**. Probe60 still last **H=1 r=+0.682 / H=55 r=+0.293 conv=0.00** gain_fid **0.882** best_h=1. recon **0.0056@64 best** (P64 @60 **0.0022** ~2.5×) then **0.008@65**. Teacher **×1.02 / ×0.83 @65**. persist_rel **0.59@62 → 0.055@64 → 0.52@65**. pid **242511**. | 🔬 LIVE P1 — new recon best; persist collapse+re-spike (not persist N+1). Probe70 pending. Do not patch live. |
...
- **LIVE P1 iter 65 (~339 min, nvidia 11.7 GB, `t_wm` ~306 s, sps 19.7):** skip **0**. recon **0.0081@62 → 0.0070@63 → 0.0056@64 new best → 0.0084@65** vs P64 **0.0022@60** (~2.5× best). Teacher jsonl 63 **×0.992 / ×1.045** → 64 **×0.997 / ×1.007** → 65 **×1.017 / ×0.829**. last_ok **65 unlocked**. persist_rel **0.59@62 inject peak → 0.32@63 → 0.055@64 collapse → 0.52@65 re-spike** (same cycle as 0.73@51→0.11@55 then wrap; do **not** persist N+1). Rest-IC graph not captured. Next probe typically iter 70 (~01:34 CDT).

...
- **Liveness:** P1 STAGE 1. Iter **76** (76 jsonl rows, ~164 min). **Skip-storm 1/2 @73** recovered; no storm 2. recon **0.0016** (best **0.0015** @75) last_ok **76** unlocked gmatch **0.0001** FD **×1.01 / ×1.01**. gnorm **0.14** (was 35 @75, 4.7e10 @73). Banner skip still 42 (cumulative). wm-fid @**70** @H r=+0.593 gain_fid 0.909 EMA **6.243**. Next probe ~80. Storm 2/2 would cap — watch. sps ~48. GPU ~11 GB. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Gate vs P52 last-ok 78 not yet (~2 iters). Do not KEEP/FALSIFIED as cascade until EXIT.

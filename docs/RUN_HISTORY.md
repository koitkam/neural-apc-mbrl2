...
### p37 GPU LIVE (`run_p37_isoabs`, tmux `mbrl2_p37`, launch `4d349fb`) — abs isolation confirmation
- **Launch KEEP:** env-free det+eager `storm_cap=2`. `[resolved-cfg] iso_varnorm=False` (pid still has the knob; HEAD removed it) gain_match=1 isolation=1 ss_match=3. Gain-match tgt MV −2.81 DV 0.487. device=cuda bs=128. No leftover `DREAMER_*`. GPU ~17.7 GB. pid **4162106**.
- **P1 through iter 77 (P33 class, not P34–P36).** **Past P32 storm window:** iter 53 recon **0.0047 skip 0** (P32 recon 0.596 skip 71). Iter-1 iso **1.33** (P33 1.69; P36 0.125). gmatch **1.20 → 0.0008 by iter 17**. Iter 19 wrap-class blip recon 0.267 recovered (do **not** lower skip threshold). Iter 70 gain_fid=**0.732** best_h=55/55 (P33 same iter 0.462). Iter 75 recon 0.0040 skip **0**.
- **Iter 75 gate = P33:** `DCgain_ratio[0.68,0.99]` worst=**0.68@DV** (MV 0.99 in `[0.8,1.3]`). **EXTENDED** +75396 to 829356 (cap 929884). Keep-ext used. Abs isolation **CONFIRMED** as P33 DV pin (P33 was `[0.68,1.05]`). Next gate ~iter 85. ~2.3 min/iter.
- **Do not launch a second GPU job.** Expect `[p3-skip]` if still GAIN_NOT_READY at cap. Next GPU after EXIT: env-free P38 `run_p38_isodcv` (HEAD `wm_isolation_dcv_match`). Actor only if GAIN-READY with `DREAMER_WM_ISOLATION_DCV_MATCH=0`.
- **HEAD (not in this pid):** isolation inv-var A/B **REMOVED**. Abs MSE + **`wm_isolation_dcv_match=True`**: isolation settle `Δu_i ∝ 1/|G_i|` from gain-match WM-norm targets, clipped to ±1 (test_sim scale MV ~0.289 DV ~1.67). Equal-|G| keeps op-band. Not a loss reweight. `[resolved-cfg] iso_dcv= mv= dv=`; `run_plan.isolation_dcv_scales`. Pre-iso resolve feeds **scales only**; post-seed **always re-resolves** Huber targets after isolation+expert (P37 freeze point). Opt out `DREAMER_WM_ISOLATION_DCV_MATCH=0`.
...
- P36 EXIT (`run_p36_isoinpscale`): storm 2/2 CAPPED GAIN_NOT_READY 0.00@DV. Val MV ×0.91/@H ×0.95, DV ×0.004, det_r 0.076, `[p3-skip]`. Per-input `|G|²` fired (ratio 33) but inv-var DISCARDED then **REMOVED**. P37 GPU: abs isolation, env-free, skip 0. **Iter 75 EXTEND 0.68@DV** (MV 0.99; P33 `[0.68,1.05]`). Abs isolation CONFIRMED as DV pin. HEAD isolation |ΔCV| excitation default ON (not in pid); `run_plan.isolation_dcv_scales` audit trail. Post-seed gain-match re-resolve KEEP. Do not pass leftover compile/latent/DOB env-vars. Do not launch a second GPU job.
...
| Isolation |ΔCV| excitation `Δu_i ∝ 1/|G_i|` (`wm_isolation_dcv_match`) | P37-live HEAD (not in pid) | PENDING (default ON for next env-free) | PARTIAL — P37 iter 75 EXTEND **0.68@DV** (MV 0.99) = P33 pin. Not a loss reweight. Wait P37 EXIT then P38. |

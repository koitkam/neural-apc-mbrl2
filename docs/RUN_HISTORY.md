...
| P57 | 2026-08-31 | rest-IC encode L=last `max(K,2τ/sr)` (`gain_match_rest_ic_len=-1`) | **LIVE P1** iter **64**. Graph **captured** N=6 T=55. `t_wm` median **99 s** (P56 124 s). Teacher FD MV/DV ~×1.00. skip-storm **1/2 @41** recovered (recon 0.274, lock 40 unlock). last_ok **64** unlocked. `wm_best` iter **30**. | ⏳ LIVE GPU. Encode L PARTIAL until EXIT+val TM. Capture KEEP this pid. Do not launch a second job. |
...
- **Next GPU: P57** env-free `gain_match_rest_ic_len=-1` → encode last `max(K, 2τ/sr)` of settle (test_sim L=55, settle=128). Actor recipe freeze-forever (refresh=0). Judge rest-IC CUDA graph captured + `t_wm` ≪ 124 s + val TM vs P53 ×0.90 / paired −13/−98.

### p57 (`run_p57_resticlen`, branch `cursor/p28`) — LIVE P1: rest-IC encode L=last max(K, 2τ/sr)
...
- **Liveness:** **LIVE P1** iter **64**. GPU occupied (~12320 MiB). skip-storm **1/2 @41** (recon **0.2738**, MV ratio **−0.014**, DV **0.081**, lock last_ok **40**, restored `wm_last_ok.pt`, unlocked, extension kept). Iter 42 recovered recon **0.0037**. last_ok **64 unlocked**. recon **~0.002**. Teacher FD MV/DV ~**×1.00**. `t_wm` median **99 s** (P56/P53 ~124 s). `wm_best` iter **30** (gain-blind; do not freeze on that). jsonl `gpu_mem_mb` ~307 allocated after backward (nvidia-smi ~12320 reserved).
- **HEAD (not this pid):** capture hygiene (zero_grad + `cuda.synchronize` + suppress AccumulateGrad warn); skip encoder-var / Huber MV–DV jsonl except last logged inner; P1 H2D `obs`/`act` (and `dist` only if `_wm_need_dist_target`) except MTP last; cache rest-IC FD `a_seq`; RSSM/TSSM T/K `_time_unbind`; Stage-1 `cached_zeros_btd`. Identity. CVD="" `_smoke_rssm.py` + `_smoke_tssm.py` green. **Do not launch a second GPU job.**
- **Watch at EXIT:** GAIN-READY vs P56 0.83@DV / P53 0.88@DV; graph **released** at P1→P2; val TM vs P53 ×0.90; paired vs **−13 vs −98**. Encode L still PARTIAL until val TM.
...
- Rest-IC encode length is last ``max(K, round(2·τ/sr))`` of a ``max(H, lookback)`` collect (`gain_match_rest_ic_len=-1`; test_sim **L=55**, settle **128**). P45–P56 used ``L=lookback=128`` (reset→SS transient in the GRU IC). ``0`` = lookback. Missing τ keeps lookback (no 50 s fake). Do **not** set settle=`wm_tf_horizon`. **HEAD:** CUDA-graph of that T-loop via `_RestICGraphModule` (`gain_match_rest_ic_cuda_graph=True`; `allow_unused_input=True`; capture `wrapper.train(rssm.training)` so graphed fwd matches RSSM train/eval), warmed **before** the P1 WM autocast loop + parent `enabled=False` then bf16 `cache_enabled=False`; **in-loop never captures**. Nested function is not a graph surface (P56 `parameters()` empty). Overriding only `parameters()` left `named_parameters()`/`buffers()` empty (torch 2.12 capture uses both). **VRAM skip does not pin** `_rest_ic_cg_fail` (warmup retry after `empty_cache`). **P57 pid 116788 captured** `N=6 T=55` (`t_wm` median **99 s** vs P56 124 s). HEAD also zeros+syncs and suppresses the one-shot AccumulateGrad stream-mismatch warn around capture; caches rest-IC FD `a_seq`; skips encoder-var / Huber MV–DV jsonl except last logged inner; P1 H2D `obs`/`act` (and `dist` only if `_wm_need_dist_target`) except MTP last; RSSM/TSSM T/K `_time_unbind`; Stage-1 `cached_zeros_btd`. Skip `rssm_cont_kl_loss` when `cont_kl_scale<=0` (env-free 1.0 identity). CPU path stays the Python loop (`CVD=""`). `DREAMER_GAIN_MATCH_REST_IC_CUDA_GRAPH=0` keeps eager. `DREAMER_GAIN_MATCH_REST_IC_LEN`.
...
| Rest-IC encode L=lookback (reset transient in GRU IC) | P45–P56; **P57** | SUPERSEDED (`gain_match_rest_ic_len=-1` → last `max(K, 2τ/sr)` of settle; test_sim L=55 settle=128; `0` = lookback) | PARTIAL — teacher FD ×1 vs gate 0.83 vs val TM ×0.76 (P56). Mechanism: L=settle=128 encodes reset→SS as rest. Do not settle=`wm_tf_horizon`. Missing τ keeps lookback. |
| Rest-IC last_only T-loop sequential kernel launches × 100 WM steps | P54 GPU-occupied (pid **95956** P1); **P55/P56** first GPU capture failed; **P57 LIVE** pid **116788** | SUPERSEDED (`gain_match_rest_ic_cuda_graph=True`; `make_graphed_callables` + GRU-grad canary; **P55 HEAD** capture **before** WM autocast; **in-loop never captures**; **P56 HEAD** `_RestICGraphModule` **parameters/named_parameters/buffers/named_buffers** + `wrapper.train(rssm.training)`; VRAM skip **not** pin; **P57** T=L not lookback) | YES (this pid) — **P57 captured** `N=6 T=55`. `t_wm` median **99 s** (P56 eager 124 s). Encode L still PARTIAL until EXIT+val TM. |
| Rest-IC capture leftover AccumulateGrad stream-mismatch warn | P57 GPU-occupied (pid **116788** P1) | SUPERSEDED (zero_grad + `cuda.synchronize` + suppress warn around `make_graphed_callables`; restore after) | YES — one-shot warning immediately before capture. Identity for training. Not in this pid. CVD="" smoke. |
...
| P1 H2D `rew`/`expert` when MTP not in the graph | P57 GPU-occupied (pid **116788** P1) | SUPERSEDED (`_batch_np_to_device(..., keys=...)` except last logged inner) | YES — paper `reward_scale_loss_p1=0`. P2 still full batch. Identity values. Not in this pid. |
| P1 H2D still copies `dist` when WM does not read it | P57 GPU-occupied (pid **116788** P1) | SUPERSEDED (`_wm_need_dist_target` / `_p1_wm_h2d_keys`) | YES — env-free Stage-1 `dob_active=False`, `dist_match=0`, `head_dim=0`. P2 full batch still copies dist (dob_ground). P25: do not key off `disturbance_head_dim` alone. Not in this pid. |

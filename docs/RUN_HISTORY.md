...
- **RCA (SysID, not a live patch; CPU-confirmed @wm_best iter40, CVD=""):** rest-IC `rollout_observed` **does** keep KV (`pos=128`, 4 layers). Gain-match / overshoot / held start `img_rollout(h,z)` with `kv_cache=None`. Markovian vs `prev_state` held-K CV **mean |Δ|=0.142** (iter20 **0.184**; iter30 **0.157**). Shape flipped vs iter20: step1 **0.195** (was 0.030) → stepK **0.056** (was 0.218); Markovian ΔCV(K) **0.407** vs cached **0.161** (~2.5×). Fidelity probe **warms cache** (so probe H=55 r dead is **not** the Markovian hole — it is cached compounding + remaining 1-step underfit). CPU posterior-prior @H=55 settle=128: real→post **×0.810** (iter20 **×0.547** — autoencoder moved with recon) / post→1step **×0.538** / 1step→OL **×0.077** (iter20 **×−0.306**). Verdict still `autoencoder` (0.81 < 0.9) but the bigger multiplicative hole is compounding (P64 1step→OL **×0.85**). Do not patch while live. Recon still dropping @50 (~2.5× P64) — not an EXIT. Probe50 **conv still 0**; H=1 dipped, H=55 dead (cached compounding). persist_rel held **~0.51**. Next-if EXIT FALSIFIED: one attributed A/B pass `prev_state` into gain-match / overshoot / held. Metric: probe H=55 r / conv / val 1step→OL vs P64 ×0.85. Watch freeze vs P64 **0.91@DV**; probe60 conv>0 / H=55 vs floor 0.40; `actor_experiment_valid`. Do **not** start a second GPU job.

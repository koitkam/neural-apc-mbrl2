...
- **Liveness (11:18 CDT):** LIVE **P1** iter **82**. tmux `mbrl2_p42`, pid **4192010**, HEAD `72f7b48`. GPU ~15.7 GB. **Lock GPU-confirmed KEEP as mechanism.** `[wm-last-ok] locked iter 66` on spike iter **67** recon **0.3066** (170× best 0.0018) skip **1**. last_ok **stays 66** through 82. Iter 81–82 recon **0.89–0.92** (second spike, gmatch ~1.11 stuck). `wm_last_ok.pt` **absent on disk** (HEAD persist-on-lock not in pid). **First gate iter 82 GPU-confirmed gain-probe** (recent-floor KEEP) on **LIVE detonated** weights: **FAIL GAIN_NOT_READY worst=0.01@DV** (MV 2.01; noise spread_x9.3 signflips=9 unbiased=False). That is **not** last_ok 66 — freeze restore + re-probe at cap is the lock test. EXTEND to 829356. **No second GPU job.** HEAD (not in pid): persist last-ok at lock; jsonl `p1_recon_best` + `wm_gain_match_{mv,dv}_loss`.

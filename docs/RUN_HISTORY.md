...
- **LIVE 2026-09-02T13:53Z P1 iter 25** (pid **221013**, ~15.3 GB, skip **0**, last_ok **25** unlocked). Teacher pinned by iter **5** (MV **×0.981** DV **×0.957**; P73 iter 5 ×0.996/×0.980). `gain_cv_skip` **stalled**: jsonl rms **0.0006→0.0046 @5 then 0.0042 @25**. `wm_last_ok.pt` iter 19 weights **[+0.0040, −0.0051]** vs |G_mv| **2.62** — skip ΔCV ~0.2% of DC if `c~O(1)`. persist_rel still **0.1–0.6** (P73 only hit 0.037 @ last_ok 81). Wrap @20 recon **0.1926** locked 19 / unlocked 21 (hygiene KEEP). Fidelity iter 10 gain_fid **0.922** / iter 20 wrap **0.711**. jsonl ×1 ≠ val TM. **Do not** score actor. **Do not** start a second GPU job. **Do not** change training while this pid is live.
- Watch freeze vs P64 **0.91@DV** / P73 **0.83**; val **1step→OL / OL-vs-real** vs P73 ×0.761 / ×0.743 and P64 ×0.85 / ×0.84; whether `gain_cv_skip_rms` ever leaves ~0.004; persist_rel at last_ok vs P73 0.037; `actor_experiment_valid`. If skip stays dead through freeze, P74 is a teacher-pin no-op (decoder MLP already saturates Huber) — compounding attack not yet tested.

...
- **LIVE (2026-08-27 01:02, tmux `mbrl2_p32`, GPU ~19.5 GB reserved, pid 4139864):** env-free det+eager CONFIRMED (`[resolved-cfg] latent=deterministic compile=eager storm_cap=2`; no leftover `[env-override]`; `[dob-ground] replay n_dist=1`). P1 iter **28** recon **0.0041** `kl=0` `jemb` 0.019 gmatch 0.0003 iso 0.0006 `ss_wmean=1.00` skip **0** alive 1024. `wm_best` iter **20** `best_h=55/55` gain_fid=0.747 raw 5.926. t_wm ~131 s (P26/P28 eager class). gnorm blips iter 14/28 (~21–23) recovered with skip 0 — not a skip-storm. Hypothesis (detonated-freeze last-ok) fires only at P1→P2. **No second GPU job.** HEAD isolation TBPTT is chunked `img_rollout` (same P25 cuts; identity `max|Δ|=0`; not in this process).

### Sim-adaptive leftovers (env-free multi-sim; do not promote plants yet)

Already unitless / derived: isolation TBPTT `max(8, round(K/3.5))` (chunked `img_rollout`, same `h`-only `keep_c` cuts; HEAD not P32 process); `gain_match_len` / isolation len = H; isolation sample window `max(seq_len, K+1)` (test_sim seq_len ≥ H unchanged); P1/P2 main WM sample `max(seq_len, K+1)` (overshoot/gain-match; P3 stays seq_len); gain-match open-loop `K` not truncated to `T-1`; `skip_storm_last_ok_recon_ratio=5` (recon/recon); `skip_storm_p1_cap_after=2` (storm count, not plant units); `p1_gate_wm_ema_min=1.5` (fidelity mix); inject EVERY = f(buffer lap) (test_sim 20/10); inject N = f(n_mv, n_dv) (test_sim 5/2/2/3); isolation settle = 24 **per input** (test_sim 24+24 / cap 48 settle-only; distillation 96+24 / cap 120). Isolation_buf holds only those settle episodes (not MIMO PRBS). Isolation settle is a whole-episode hold at a stratified level (`action_std=0`); DV step is MV-action-isomorphic (`isolated_level × span/2`). Opt-in: `gain_match_huber_beta<=0` → median |tgt| (P28 still uses β=1). Gain-ready band `[0.80, 1.30]` and wm-fidelity mix weights are unitless TrainConfig (were os.environ). `rssm_latent_type` default **deterministic** (P26 proven; P29 env-free drop of the leftover env-var). `compile_mode` default **eager**; `DREAMER_COMPILE` / `DREAMER_COMPILE_MODE` are in `ENV_OVERRIDES` (MODE was CLI-only). Intra-op threads and inductor workers scale with `sched_getaffinity` (host-adaptive). `skip_invalid_p3` default ON (unitless validity flag, not a plant unit).
...
- P32 launch: env-free `cursor/p28` (deterministic + eager + `storm_cap=2` + detonated-freeze last-ok restore). Do not pass leftover P26 env-vars (`DREAMER_COMPILE=0`, `DREAMER_RSSM_LATENT_TYPE`, `DREAMER_DOB_ENABLED`, `DREAMER_ACT_HIST_REQUIRED`). Opt-in compile is `DREAMER_COMPILE=1`. Live iter banner prints `kl`/`jemb`/`iso`/`ss` + `storm_cap=`. jsonl: `wm_ss_match_loss`, `wm_ss_match_wmean`, `wm_isolation_traj_loss`. `skip_invalid_p3` default-on. Isolation K-step decode is batched; overshoot MSE is vectorized; **gain-match FD** stacks baseline+per-input into one `img_rollout`; **P1/P2 random collect is numpy-only**; **P2 DOB Kalman scan is a closed-form mix**. Isolation TBPTT is chunked `img_rollout` (HEAD, not P32 process). Variance-normalized isolation waits until a **completed** det+eager P1 still shows DV ss ~×0.87 on a **healthy** freeze.

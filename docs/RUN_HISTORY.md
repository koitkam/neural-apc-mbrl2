...
- **Liveness:** **P2.** GPU occupied (~15.7 GB). tmux `mbrl2_p43`, pid **4465**, launch `51b0f45`. Iter **104 CAPPED GAIN_NOT_READY 0.75@DV** (MV ×1.06); freeze last_ok **94** probe **0.76@DV / MV ×1.02**. P2 through iter **120**: recon **0.0018**, dobg **0.0045** (rising), g frozen, skip 60. This pid's warm-restore skip line still cites storm **iter 73** (HEAD `1ade665` prints last_ok 94). `huber_per_in=True`. **Do not launch a second GPU job.** `[p3-skip]` at P2-end.
...
| Per-input Huber β = `|tgt_ij|` (L1 sat ±1) | P43 cap 104 | KEEP as P1 form (not P27); **FALSIFIED as DV pin** | YES — gates 82/94/104 all 0.75@DV. Freeze last_ok 94 → 0.76@DV / MV ×1.02. Storm 1/2 @74 recovered. |

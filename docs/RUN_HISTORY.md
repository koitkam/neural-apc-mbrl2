...
| P67 | 2026-09-01 | teacher `gain_match_len` auto = `wm_tf_horizon` (K=220; encode T lookback) | LIVE P1 iter **45** pid **187753** sha `9fbede0`. Teacher med ×0.99/×1.01 at K=220 skip 0 last_ok **45 unlocked** recon **0.0022** `t_wm` 134 s ~14.3 GB. Fidelity@40 gain_fid **0.557** (P64 0.559). | 🔬 compounding family vs P64 ×0.842 OL-vs-real |
...
- **LIVE P1 iter 45** (~1.75 h wall, 2026-09-01 14:57). Recon **0.063→0.0022** (last-10 median **0.0029**; P64 same window **0.0024**). `kl=0` `jemb` 0.062→0.006 alive **1024**. skip **0/0**. last_ok **45 unlocked** (advances every iter; recon_best **0.00218**). Teacher last-10 median **×0.995 / ×1.006** at **K=220** (P64 at K=H **×0.998 / ×0.996**). Huber gml **~3e-4**. Overshoot **0.029**. held **1.0e-4**. `t_wm` **134 s** vs P64 **97 s** (+38%; 4H `img_rollout`). nvidia **14359 MiB / 100%** (`gpu_mem_reserved_mb` **13804**; jsonl `gpu_mem_mb` is allocated **325**). gnorm spike **23.7@45** with recon at **best** (P64 max **26@7** same class; skip 0). Fidelity: iter 10 `wm_best` gain_fid **0.834** (restore OFF; gain-blind WATCH); iter 40 H=55 r **+0.403** conv **0.00** gain_fid **0.557** (P64 same iter r **+0.456** conv **0.25** gain_fid **0.559**). Isolation=0. **jsonl ×1 at K=220 is protocol, not val TM.** Do **not** FALSIFY/PROMOTE until freeze @~82 and val OL-vs-real vs P64 **×0.842**.
- **Watch:** first gate ~iter 82 (~1.4 h); skip-storm; freeze vs last_ok 82 / 0.91@DV; val OL-vs-real vs P64 **×0.842** / P66 ×0.718; MV/DV vs P26/P64; paired vs P64 **−4.54**; det_r/pred_std (not a DOB experiment). Do not score actor if GAIN_NOT_READY. **No second GPU job.** No training-behavior patch this session (live logs readable; next lever waits on EXIT).
...
| Gain-match FD K = control H (last-only Huber) | P26–P66; val TM / decomp at `wm_tf_horizon=4H` | P67 auto K = `wm_tf_horizon` (LIVE pid **187753** P1 iter **45**) | PARTIAL as protocol gap — jsonl teacher last-10 **×0.995/×1.006 at K=220** (P64 at K=H ×0.998/×0.996). last_ok **45 unlocked** recon **0.0022** skip 0. Fidelity@40 gain_fid **0.557** (P64 0.559). Val OL-vs-real pending (P64 ×0.842). Not S=H. Not a Huber reweight. |

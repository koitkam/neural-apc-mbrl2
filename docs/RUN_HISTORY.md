...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 28**. Probe20 **H=1 r=+0.325 / H=55 r=+0.145 conv=0.00** gain_fid **0.261**. recon **0.034@28 best** (P76 @27 **0.0035**; ~10×). Teacher **×0.97 / ×1.15 @28** (DV slipped @27 ×0.85). CPU: Markovian vs cached CV gap **0.18**. pid **242511**. | 🔬 LIVE P1 — recon still dropping. Cached OL dead at H=55. Probe 30 imminent. Do not patch live. |
...
- **LIVE P1 iter 28 (~147 min, nvidia 11.7 GB, `t_wm` ~309 s, sps 19.5):** skip **0**. Teacher jsonl 26 **×0.993 / ×0.992** → 27 **×1.026 / ×0.852** → 28 **×0.972 / ×1.154** (DV oscillating). recon **0.044@27 → 0.034@28 new best** vs P76 **0.0035@27** (~10×). last_ok **28 unlocked**. persist_rel **0.40@27 → 0.50@28**. Rest-IC graph not captured. Probe 20 still last fidelity. Next typically iter 30 (~10 min).

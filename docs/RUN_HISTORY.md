...
| P78 | 2026-09-03 | TSSM gain-match rest-IC continues KV via `prev_state=` (P77 Markovian FALSIFIED) | LIVE P1 iter **8**. Teacher **×0.994 / ×0.938** persist **0.182** recon **0.374** skip **0** gnorm **1.55**. persist rebound @6 collapsed. pid **261143**. | 🔬 LIVE — jsonl ×1 ≠ freeze. probe@10 next. Do not score actor. |
...
- **LIVE P1 iter 8 (~42 min, nvidia 12.3 GB, `t_wm` 295 s):** skip **0**. last_ok **8 unlocked**. recon **0.423@6→0.353@7→0.374@8** (~P77@8 **0.386**). Teacher jsonl **×0.994 / ×0.938** (iter 7 **×0.979 / ×1.197**; iter 6 **×1.000 / ×0.980**). persist_rel **0.783@6→0.147@7→0.182@8** (rebound collapsed; ~P77@8 **0.175**). gnorm **26@6→5.9@7→1.55@8** (calmed; P77@8 still **12.9**). DV @8 **short of ×1** vs P77@8 **×1.376** overgain. jsonl still last-step pinned ~×1 — KV-continue did **not** unpin teacher DC. persist@6 rebound was transient. First probe @10 (~10 min; P77 probe10 H=1 **+0.128** H=55 **+0.122 conv=0**). Do **not** score actor. Do **not** persist N+1. Do **not** launch a second GPU job.

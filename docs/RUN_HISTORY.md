...
### p47 (`run_p47_p3sigadam`, branch `cursor/p28`) — LAUNCH: complete P46 σ-reset (Adam log_std-row zero)
...
- **Liveness:** P1 live. GPU python pid **29434**. tmux `mbrl2_p47`. Launch `0acde1d`. Iter **27** (~05:14 CDT): recon **0.0047**, jemb **0.011**, gmatch teacher FD mv/dv **~×0.99/×1.00**, skip **0**, last_ok **unlocked**, `t_wm` **~124–140 s** (P46-class). device=cuda. `[env-override] p3_reset_log_std=True` only. **Do not launch a second GPU job.** GPU-occupied this tick: leftover auto-tune formula inputs + inner-step counts → TrainConfig / `ENV_OVERRIDES` (identity). CPU smoke PASS (5 sims). Do not patch collect-DV / P3 Kalman.
...
- Reward clip/cal + `DREAMER_DIAG_*` / `DREAMER_RUN_WM_DIAGNOSTIC` / `DREAMER_WM_DIAG_{N_STARTS,HORIZON}` are TrainConfig + `ENV_OVERRIDES` (identity defaults). `DREAMER_WM_DIAG_DEVICE` stays env-only (device picker). Auto-tune formula inputs `seed_target_cv_frac` / `seed_sigma_cap` / `pmpo_entropy_eta_v3` / `pmpo_entropy_sigma_ref` / `prbs_seg_min` / `prbs_seg_min_floor` are the same class (P47 live; leftover `SEED_TARGET_CV_FRAC` / `PMPO_ENTROPY_COEF_BASELINE` / `PRBS_SEG_MIN` still win when the DREAMER_* field is not explicit). Inner-step counts `train_steps_per_iter` / `phase3_train_steps_per_iter` were already TrainConfig; now in `ENV_OVERRIDES` (a `DREAMER_*` A/B used to silently lose).

...
### p39 GPU LIVE P2 (`run_p39_isodcvfloor`, tmux `mbrl2_p39`, launch `92c7662`, pid 4170377) — dcv_match scale floor 1.0
- **Launch KEEP:** env-free det+eager `storm_cap=2`. `[resolved-cfg] iso_dcv=True min_scale=1.0 mv=['1'] dv=['1.67'] edge_du_mv=['0.6'] edge_du_dv=['1']` isolation=1 ss_match=3 gain_match=1. device=cuda bs=128 GPU **17765 MiB**. pid **4170377**.
...
- **P1:** gmatch pinned **0.0002–0.001** (P37-class). Storm **1/2 @iter 53** recovered last-ok 51, **extension kept**. Iter 75 EXTEND **0.71@DV** (MV 1.04); iter 85 EXTEND **0.69@DV** (MV 0.99). Extra-P1 **did not detonate** (P37 iter 88 gnorm 62; P31 iter 95 recon 0.71) — iter 88/95 recon 0.002–0.003 gnorm O(1).
- **P1→P2 CAPPED iter 97** `GAIN_NOT_READY` DC `[0.70,1.00]` worst=**0.70@DV** (MV 1.00 in band), cap 175924 reached. Freeze recon **0.0031** (detonated-restore silent). Warm-restore SKIPPED. **Cube-boost FALSIFIED as DV pin** (same 0.68–0.71 plateau as P32/P33/P37). Floor KEEP as P1 form vs P38 (gmatch pinned; extra-P1 stable).
- **ROOT (SysID, structural):** `|G_mv|/|G_dv|≈5.8 > 1/op_band≈1.67`. Equal |ΔCV| needs either shrinking MV below op-band (P38 FALSIFIED) or DV |Δu|>1 (cube clip). Floor 1.0 + cube ⇒ residual |ΔCV|² still ~12× MV-dominated. Not a loss reweight.
- **P2 LIVE (~iter 117):** recon 0.003, `dobg` 0.004 (ground live), iso/gmatch 0 (`g` frozen), gnorm 0.15. Fidelity iter 110 `best_h=55/55` gain_fid=0.931. Expect `[p3-skip]` after remaining ~34 P2 iters + val. Do **not** launch a second GPU job.
- **HEAD (not in this pid):** `wm_isolation_dcv_min_scale=1.0`; isolation mv/dv traj weighted-mean (no CUDA `.any()`); `rssm_imag_latent_mode` **REMOVED**; SNR / MIMO holds. **This session:** May-2026 `DREAMER_DIAG_PERHEAD_GRADS_EVERY` / `LATENT_STABILITY` default **10 → 0** (extra `retain_graph` backward every 10 iters; not `ENV_OVERRIDES`). Isolation span audit `g_ratio`/`smax`/`equalize_possible` (test_sim False; no scale change).
- **Next GPU after EXIT+val:** not another isolation reweight; not extra-P1; not no-floor; not leftover `DREAMER_*`. If val DV ~×0.70, candidate is P26 **gain-match-only** (`DREAMER_WM_INPUT_ISOLATION_COEF=0` `DREAMER_WM_SS_MATCH_COEF=0`; P26 isolation/ss-match were 0; DV ×0.87). Wait for TM+det_r.
...
- Isolation/ss-match is **abs MSE only**. Inv-var A/B **REMOVED**. Default **`wm_isolation_dcv_match=True`** with **`wm_isolation_dcv_min_scale=1.0`** (named TrainConfig; was a magic 1.0). Isolation settle `Δu_i ∝ 1/|G_i|` then cube-clip. **Structural:** when `|G_max|/|G_min| > 1/op_band`, |ΔCV| cannot equalize without shrinking the strong teacher (P38 FALSIFIED) or exceeding the cube. test_sim ~5.8 > 1.67 → residual |ΔCV|² ~12× MV-dominated. P39 CAPPED **0.70@DV** (MV 1.00) — cube-boost FALSIFIED as DV pin; floor KEEP as P1 form vs P38. Audit `run_plan.isolation_dcv_scales.min_scale` + `edge_du_*` + **`g_ratio`/`smax`/`equalize_possible`**. Do **not** re-add `wm_isolation_var_norm`. Do not try another isolation reweight. May-2026 per-head/latent diag probes default **OFF**.
...
- `run_plan.json → config` is rewritten after gain-match resolve. `[resolved-cfg]` prints `iso_dcv=` **and** `min_scale=` **and** `mv=`/`dv=` multipliers **and** `edge_du_mv=`/`edge_du_dv=` **and** `g_ratio=`/`smax=`/`equalize=`; `run_plan.isolation_dcv_scales` is the P38/P39 audit trail (inv-var knob removed). Pre-rewrite dump is dataclass defaults (P29 plan still shows `rssm_latent_type=categorical` / `gain_match_coef=0` even though training auto-enabled grounding).
...
| Isolation |ΔCV| excitation with **scale floor 1.0** | P39 CAPPED (`run_p39_isodcvfloor`, `92c7662`, pid 4170377) | KEEP as P1 form; FALSIFIED as DV pin (val pending) | YES — `|G_max|/|G_min|>1/op_band` cannot equalize \|ΔCV\| without P38 shrink or cube overflow. Cap-time 0.70@DV (MV 1.00). Extra-P1 did not detonate. Floor is now `wm_isolation_dcv_min_scale`. |

...
| P56 | 2026-08-30 | Slow μ-ratio recopy `DREAMER_P3_MU_RATIO_REFRESH=50` (~patience/4) | LAUNCH `run_p56_muslow` pid **110246** sha `9d05866`. Default 0 + env 50. **GAIN-READY still pending.** P1 ~iter **39** recon **0.0023** skip **0** teacher FD ~×1.02. Rest-IC CUDA-graph warmup **failed** (`ValueError: grad requires non-empty inputs` on nested function; P1 `t_wm_s` ~124 s). HEAD: `_RestICGraphModule` yields RSSM `named_parameters`/`named_buffers` (not in this pid). Watch logp_std ~0.5 not 18; clip_frac not 0.11 and not 0.02–0.80; val vs P53 −13/−98. | LIVE |
...
- **Step 4 CONFIRMED:** `[env-override] p3_mu_ratio_refresh_iters=50`. `[resolved-cfg] … p3_muratio=0.2 p3_murefresh=50 es_ent_floor=0.25 compile=eager`. Process environ: `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF` + `DREAMER_P3_MU_RATIO_REFRESH=50`. P1 ~iter **39** recon **0.0023** skip 0 teacher FD ~×1.02. Rest-IC CUDA-graph **failed this pid** (`ValueError: grad requires non-empty inputs` — nested function `parameters()` empty so `autograd.grad` inputs=(); `t_wm_s` ~124 s). HEAD after this pid: `_RestICGraphModule` yields RSSM `named_parameters`/`named_buffers` so capture `parameters()` **and** `buffers()` match the RSSM (`allow_unused_input=True`). Do **not** launch a second GPU job. Do not stack critic knobs.
...
| Rest-IC last_only T-loop = 128 sequential kernel launches × 100 WM steps | P54 GPU-occupied (pid **95956** P1); **P55/P56** first GPU capture failed | SUPERSEDED (`gain_match_rest_ic_cuda_graph=True`; `make_graphed_callables` + GRU-grad canary; **P55 HEAD** capture **before** WM autocast + parent `enabled=False` then bf16 `cache_enabled=False`; **in-loop never captures**; **P56 HEAD** `_RestICGraphModule` **named_parameters/named_buffers**) | PARTIAL — P55 pid autocast cache; **P56 pid 110246** nested function `parameters()` empty → `ValueError: grad requires non-empty inputs` → eager `t_wm_s` ~124 s. Overriding only `parameters()` left `named_parameters()`/`buffers()` empty (torch 2.12 capture uses both). Not in this pid. |

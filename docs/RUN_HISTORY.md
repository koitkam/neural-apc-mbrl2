...
- **LIVE (2026-08-26 P1 iter 72, GPU still running): CONFOUNDED.** Env-free dropped `DREAMER_RSSM_LATENT_TYPE=deterministic` (P26/P28 env, never a default). P29 is **categorical**: `kl_loss=0.30` pinned, `joint_embed_loss≡0`, recon **0.50→0.16 by iter 46 then 0.65 at iter 49** (buffer wrap; skip=0) recovering to **0.228 at iter 72** vs P28 recon **0.003** at iter 50. Isolation extra unroll **is live** (`wm_input_isolation_loss≈0.003`) — follow-ups 1–14 are in this GPU job, still inseparable from categorical+compile. `z_dim=32` / `zrank/1024` is the same width as deterministic — do not use it. P29 cannot verdict skip-restore. **Also compile-on** (`[dreamer_v4] torch.compile`; iter-1 t_wm **1681 s** inductor warmup vs P28 eager ~163 s; 20 inductor workers on 20 cores; collect 22.6 s vs P28 17.6 s). P1 remaining ~3 iters then P2/P3. **Code (no relaunch):** live iter banner now prints `kl`/`jemb`/`gmatch`/`iso` (P2+ `dobg`, P3 `rscale`/`rtgt`) instead of RSSM `sf≡0` + deleted `img_ret`; jsonl `gpu_mem_reserved_mb` (allocated-at-log ~0.8 GB hid the 16 GB cache). Next GPU job after P29 EXIT = real skip-restore A/B (follow-ups 1–14 + deterministic + eager **env-free** — do not pass leftover `DREAMER_COMPILE=0`). Train-start / `[resolved-cfg]` print `latent=` and `compile=`.
...
- Next launch after P29: env-free `cursor/p28` (deterministic + eager). Do not pass leftover P26 env-vars (`DREAMER_COMPILE=0`, `DREAMER_RSSM_LATENT_TYPE`, `gain_match_coef` / `dob_ground_coef` auto-enable). Opt-in compile is `DREAMER_COMPILE=1`. Live iter banner (HEAD) prints `kl`/`jemb` so a leftover latent type is visible in tmux.
...
| P29 live iter banner `sf≡0` / `img_ret` | P29 GPU-occupied | KEPT (print `kl`/`jemb`/`gmatch`/`iso`) | YES — leftover latent type was only in jsonl |

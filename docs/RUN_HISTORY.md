...
| P55 | 2026-08-30 | PPO μ-ratio recopy every P3 iter (`p3_mu_ratio_refresh_iters=1`) | LAUNCH env-free `run_p55_murefresh`. P54 extra P3 was a freeze-forever ceiling. Watch clip_frac not die to ~0.11; val vs P53 −13/−98 | LAUNCH |
...
- **One attributed GPU change vs P54 pid:** `p3_mu_ratio_refresh_iters=1` (HEAD default). Recopy live μ at the first AC call of every P3 collect iter (window = `phase3_train_steps_per_iter` inner steps). 0 = P53 freeze-forever. First unfreeze epoch still clips vs unfreeze-μ (warmup does not step the actor). Opt out `DREAMER_P3_MU_RATIO_REFRESH=0`. Also P54 ES floor 0.25, P53 `p3_mu_ratio_clip=0.2`. Env-free. tmux `mbrl2_p55`. `CUDA_VISIBLE_DEVICES=0`.
- **Not:** leftover `DREAMER_*`, tighter `p3_logp_clip`, `p3_reset_log_std`, extra critic knobs, `actor_kl_coef`, loosening ε.
- **Step 4:** env-free. Watch `[resolved-cfg] … p3_muratio=0.2 p3_murefresh=1 es_ent_floor=0.25`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`.
- **Watch:** unfreeze 147 logp_std ~0.5 not 80; late-P3 `actor_ratio_clip_frac` not stuck ~0.11; ES ≠ entropy_collapse (floor KEEP); val paired vs P53 **−13 vs −98**; no mvv 813k. Storm / freeze last_ok should match P53/P54 GAIN-READY class.

...
- **Liveness:** P1 live. GPU python pid **29434**. tmux `mbrl2_p47`. Launch `0acde1d`. Iter **54** (~06:13 CDT): recon **0.0019**, jemb **0.005**, gmatch Huber ~9e-5, teacher FD mv/dv **×0.995 / ×0.996**, skip **0**, last_ok **unlocked** 54, gpu **11730 MiB**, `t_wm` **~124 s**. wm_best freeze iter **30** (gain_fid 0.916); iter-50 probe gain_fid **0.724** (gain-blind `wm_best` — do not freeze on that). Tracking P45/P46 P1 (jsonl ratios ~×1 are teacher FD, not TM). First TM gate still ~iter 82. device=cuda. `[env-override] p3_reset_log_std=True` only. **Do not launch a second GPU job.** GPU-occupied this tick: leftover SIGMA dual-read + `max(1.3)` floor SUPERSEDED (`_resolve_policy_sigma_bounds`; TrainConfig **1.2** now actually resolves — P47 auto-tune still `log(sigma_max/1.3)=-1.782`). `OBJ_REWARD_SCALE` / `DREAMER_ATTN_IMPL` / leftover `DREAMER_FAST_ATTN` → TrainConfig + whitelist. CausalAttention `auto` no longer re-reads FAST_ATTN (would beat explicit `auto`). CPU smoke. Do not patch collect-DV / P3 Kalman. Do not promote `p3_reset_log_std`.

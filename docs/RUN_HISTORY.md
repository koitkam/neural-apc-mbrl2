...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 38**. Probe30 **H=1 r=+0.403 / H=55 r=+0.172 conv=0.00** gain_fid **0.387**. recon **0.014@38 best** (P64 @38 **0.0024**; ~6×; P76 0.005). Teacher **×1.01 / ×0.97 @38**. persist_rel **0.041** held. Iter37 rebound 0.023 recovered skip 0. pid **242511**. | 🔬 LIVE P1 — recon gap ~6×. H=55/conv still dead. Probe 40 imminent. Do not patch live. |
...
- **LIVE P1 iter 38 (~199 min, nvidia 11.7 GB, `t_wm` ~306 s, sps 19.7):** skip **0**. Teacher jsonl 36 **×0.995 / ×1.010** → 37 **×0.995 / ×1.088** → 38 **×1.012 / ×0.975**. recon **0.020@35 → 0.016@36 → 0.023@37 rebound → 0.014@38 new best** vs P64 **0.0024@38** (~6×; P76 @38 0.005). last_ok **38 unlocked**. persist_rel **held 0.037–0.041**. Rest-IC graph not captured. Probe 30 still last fidelity. Next typically iter 40 (~10 min).
...
- **RCA (SysID, not a live patch; CPU-confirmed @wm_best iter20):** rest-IC `rollout_observed` **does** keep KV (`pos=128`, 4 layers). Gain-match / overshoot / held start `img_rollout(h,z)` with `kv_cache=None`. CVD="" on `wm_best.pt`: Markovian vs `prev_state` held-K CV **mean |Δ|=0.184**, step1 **0.030 → stepK 0.218**; Markovian ΔCV(K) **0.339** vs cached **0.123**. Fidelity probe **warms cache** (so probe H=55 r dead is **not** the Markovian hole — it is underfit 1-step + cached compounding). CPU posterior-prior @H=55: real→post **×0.547** (autoencoder; recon still 0.14 at that ckpt) / post→1step **×0.535** / 1step→OL **×−0.306**. Do not patch while live. Recon still dropping @38 — not an EXIT. Probe30 **conv still 0**; H=1 at floor, H=55 dead (cached compounding + underfit). persist_rel held **~0.04** after 0.40→0.055 collapse (P73-class; not a new knob). Iter37 wrap-like recon 0.023 recovered skip 0. Next-if EXIT FALSIFIED: one attributed A/B pass `prev_state` into gain-match / overshoot / held. Metric: probe H=55 r / conv / val 1step→OL vs P64 ×0.85. `tssm_d_model=512` / 4 layers is not tied to model_size L `d_model=384` / 8; B=16 vs RSSM 128 is gpu-calib, not a second attributed change. Watch freeze vs P64 **0.91@DV**; probe40 conv>0; `actor_experiment_valid`. Do **not** start a second GPU job.

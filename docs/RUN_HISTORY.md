...
| P54 | 2026-08-30 | ES entropy thr = σ-band frac (`early_stop_entropy_collapse_floor_frac=0.25`) | LIVE env-free `run_p54_esentband` pid **95956** CUDA ~11 GB. P1 ~iter 23: recon 0.0052, gmatch 0.0006, skip 0, wrap lock@5 unlock@7, wm_best@20 gain_fid=0.677. vs P53 cfg: **only** `es_ent_floor=0.25`. GPU occupied — no second job. HEAD: actor-KL A/B **REMOVED**; pinned replay H2D reused. | LIVE |
...
- **LIVE P1 (2026-08-30 ~12:55 CDT, pid 95956):** device=cuda bs=128. Still P1 **iter 54**. recon **0.0025**, gmatch **0.0004**, skip **0**, jemb **0.011**, last_ok **54 unlocked** after wrap lock@5 unlock@7. Teacher FD MV/DV ~×0.977 (not TM). wm_best iter 50 EMA **5.011** `gain_fid=0.678` best_h=27/55 (H=55 r=+0.395 vs floor 0.40). sps ~48, **t_wm ~123 s / ~127 s iter**. gpu peak ~10.7 GB. Gate historically ~82. `[resolved-cfg] … p3_muratio=0.2 es_ent_floor=0.25 compile=eager`. **Do not launch a second GPU job.**
- **HEAD (not in pid 95956):** p136 `actor_kl_coef` / `DREAMER_ACTOR_KL_COEF` **REMOVED** (FALSIFIED; never wired in `_realsim_actor_critic_step` — false A/B). π_prior snapshot skipped unless `actor_loss_type=pmpo`. CUDA replay H2D reuses pinned host + GPU dest **per slot** (`replay` / `iso` / `critic`). Rest-IC numpy→device pins once. **Rest-IC CUDA-graph** (`gain_match_rest_ic_cuda_graph=True`; `make_graphed_callables` + GRU-grad canary; RSSM only; CPU/TSSM/capture-fail → eager). Banner `gmatch_rest_cg=`. CVD="" smoke green. GPU identity deferred (95956 holds the A10).
...
| p136 `actor_kl_coef` / `DREAMER_ACTOR_KL_COEF` (default 0; not wired in `_realsim_actor_critic_step`) | P54 GPU-occupied (pid **95956** P1) | **REMOVED** | YES — FALSIFIED as σ-collapse TR; whitelist was a false A/B (`run_plan` would change, actor would not). π_prior snapshot now only when `actor_loss_type=pmpo`. `p3_prior_refresh_iters` kept for PMPO. |

...
- **Liveness (06:05 CDT):** LIVE P1 iter **34**. tmux `mbrl2_p41`, pid **4185725**, launch `acb8a7b`. GPU ~15.7 GB. `CUDA_VISIBLE_DEVICES=0`. **No leftover `DREAMER_*`.** `[resolved-cfg] isolation=0 ss_match=0 iso_dcv=off` gain_match=1 dob_ground=2 restore_p2=False skip_invalid_p3=True storm_cap=2 compile=eager. No `[seed] isolated-settle`. No `[isolation-buf]`. Iter 1 recon 0.0755 gmatch 0.0020 **iso 0 ss 0 skip 0**. Iter 10 probe H=55 r=+0.590 gain_fid=0.897 `wm_best` 6.378. Iter 20 probe H=55 r=+0.525 gain_fid=0.550 (warmup drop; recent-floor uses this, not 6.378). Iter **29** recon 0.0031 gnorm **25.2** applied (skip 0; recovered). Iter **34** recon 0.0026 gmatch 0.0002 skip 0. Tracks P40 P1. **Do not launch a second GPU job.** First gate ~iter 82 (~07:20 CDT).
- **HEAD (not in pid):** lock last-ok after a silent recon spike (`skip_storm_last_ok_lock_ratio=20`). P40 recovery overwrote 83→104; freeze recon was healthy so detonated-freeze missed 83. Skip-storm restore still unlocks. **Not stacked onto live P41.**
...
- P1/P2 random collect is **numpy-only** (no `torch.inference_mode` / SF `a_history`). P3 on-policy still streams. P2 DOB Kalman scan is a closed-form mix (same recurrence; host-adaptive T×T cap `clamp(GPU_mem/1500, 4MiB, 64MiB)` — A10 24GB still **16 MiB**). Stage-1 skips `apply_dob` clone + ground/reg. CV-std tensor cached. P1 skips unused `agent_finetune_loss` when `reward_scale_loss_p1=0`. Last-ok snapshot `copy_` reuse. Isolation is abs MSE (inv-var helpers deleted). Gain-match FD uses `img_rollout(..., last_only=True, out='obs')` (last-step Huber ≡ `decode(stack[:, -1])`; no unused K-stack; sequential `img_step` fallback **REMOVED**). Overshoot uses `out='obs'` and held-rollout `out='h'` (no unused F-stack; last_only materializes `out` once after the K-loop). P1 gate floor is **recent EMA max** (not return to warmup `wm_score_ema_best`; P40 RCA). jsonl emits `wm_score_ema*`. jsonl always emits `wm_gain_match_loss` / isolation/ss keys (0 when teacher off). Banner `n_grad_skip` is cumulative inner-step skips; skip-storm window is `grad_skip_history` (cleared on restore). HEAD last-ok **locks** after recon > `skip_storm_last_ok_lock_ratio` × best (default 20, unitless; P40 RCA). Skip-storm restore unlocks. P41 pid does **not** have the lock.
...
| Extra-P1 last-ok overwrite after silent recon spike | P40 EXIT iter 98 (83→104) | HEAD lock (`skip_storm_last_ok_lock_ratio=20`; not in P41 pid) | YES — recovered recon 0.0068 < 5× best overwrote pre-blip 83; freeze recon 0.0045 healthy so detonated-freeze missed. Skip-storm restore still unlocks. Wrap ~5–10× must not lock. |

...
- **Liveness:** live. GPU occupied. tmux `mbrl2_p43`, pid **4465**, launch `51b0f45`. Env-free. `device=cuda` bs=128 `latent=deterministic compile=eager`. `[gain-match] huber_per_input=True` β_mv=**2.63** β_dv=**0.512**. `[resolved-cfg]` isolation=0 ss_match=0 lock=20 **huber_per_in=True**. No leftover `DREAMER_*`. P1 iter 1–2: gmatch **0.0040→0.0017**, skip **0**, recon 0.112→0.031, kl=0 jemb>0, gnorm 6.8→19 (not P27). jsonl mv/dv Huber 0.007/0.001 → 0.0028/0.0006. **Do not launch a second GPU job.**

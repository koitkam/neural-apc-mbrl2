...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 35**. Probe30 **H=1 r=+0.403 / H=55 r=+0.172 conv=0.00** gain_fid **0.387**. recon **0.020@35 best** (P64 @35 **0.0033**; ~6×; P76 0.007). Teacher **×1.01 / ×0.99 @35**. persist_rel **0.044** (held after 0.40@31 collapse). pid **242511**. | 🔬 LIVE P1 — recon gap closing. H=55/conv still dead. Do not patch live. |
...
- **LIVE P1 iter 35 (~183 min, nvidia 11.7 GB, `t_wm` ~305 s, sps 19.8):** skip **0**. Teacher jsonl 33 **×1.004 / ×1.060** → 34 **×0.997 / ×1.031** → 35 **×1.011 / ×0.985** (pinned). recon **0.027@33 → 0.021@34 → 0.020@35 new best** vs P64 **0.0033@35** (~6×; P76 @35 0.007). last_ok **35 unlocked**. persist_rel **0.055@33 → 0.043@34 → 0.044@35** (held low; not rebound). Rest-IC graph not captured. Probe 30 still last fidelity. Next typically iter 40.

...
| P76 | 2026-09-02 | RSSM GRU update-gate bias `log(H/16)` (~1.23; no new knob) | LIVE P1 **iter 15**. Teacher pinned @3 ×0.988/×1.04. recon **0.0070** skip **0**. Probe10 gain_fid **0.736** H=1 r **0.428** (P64 0.584 / P73 0.549) H=55 r **0.310**. pid **237453** sha `0ddc372`. | 🔬 LIVE — keep-h stall watch (H=1 lag). No second GPU job. |
...
- **LIVE P1 iter 15 (~27 min, nvidia 14.2 GB, `t_wm` ~100 s, sps 61):** skip **0**, no wrap, last_ok **15 unlocked**. Teacher pinned @3 (MV **×0.988** DV **×1.04**); @15 MV **×1.010** DV **×0.930**. recon **0.3699→0.0070** (P73@15 **0.0072**, P64 **0.0049**, P75 **0.0120**). persist_rel **0.087 @15** (P73 **1.06** noisy). jsonl has **no** FO mid/wfrac keys.
- **Probe iter 10 (WATCH-only; restore OFF):** H=1 r **+0.428** / H=13 **+0.527** / H=27 **+0.472** / H=55 **+0.310** conv **0.00** drift **0.223** best_h **27/55** gain_fid **0.736**. P64@10: H=1 **0.584** H=55 **0.309** conv **0.50** drift **0.185** gain_fid **0.827**. P73@10: H=1 **0.549** H=55 **0.368** drift **0.128** gain_fid **0.772**. Keep-h **has not detonated**; H=1 lag + higher drift is the stall-dynamics watch (P76 risk). wm_best@10 gain-blind — ignore for freeze.

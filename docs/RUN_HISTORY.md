...
- **LIVE (2026-08-26 P1 iter 59, GPU still running): CONFOUNDED.** Env-free dropped `DREAMER_RSSM_LATENT_TYPE=deterministic` (P26/P28 env, never a default). P29 is **categorical**: `kl_loss=0.30` pinned, `joint_embed_loss≡0`, recon **0.50→0.17 by iter 46 then 0.65 at iter 49** (buffer wrap; skip=0) recovering to **0.30 at iter 59** vs P28 recon **0.003** at iter 50. `z_dim=32` / `zrank/1024` is the same width as deterministic — do not use it. P29 cannot verdict skip-restore. **Also compile-on** (`[dreamer_v4] torch.compile`; iter-1 t_wm **1681 s** inductor warmup vs P28 eager ~163 s; 20 inductor workers on 20 cores; collect 22.6 s vs P28 17.6 s). P1 remaining ~20 iters (~1 h) then P2/P3. **Code (no relaunch):** `rssm_latent_type` default **deterministic**; rewrite `run_plan.json → config` after promotions (`[resolved-cfg]`); honour explicit `DREAMER_DOB_GROUND_COEF=0`; **compile default eager**; **`DREAMER_COMPILE` / `DREAMER_COMPILE_MODE` now in `ENV_OVERRIDES`** (MODE was CLI-only → `single_run` silent-drop); host-adaptive PyTorch intra-op threads + inductor worker cap; skip-storm last-ok snapshot stays on GPU (no per-iter H2D); isolation H2D uses pin+non_blocking like main replay. Next GPU job after P29 EXIT = real skip-restore A/B (follow-ups 1–14 + deterministic + eager env-free). Train-start / `[resolved-cfg]` print `latent=` and `compile=`.
...
- Next launch after P29: env-free `cursor/p28` (deterministic + eager). Do not pass leftover P26 env-vars (`DREAMER_COMPILE=0`, `DREAMER_RSSM_LATENT_TYPE`, `gain_match_coef` / `dob_ground_coef` auto-enable). Opt-in compile is `DREAMER_COMPILE=1`.

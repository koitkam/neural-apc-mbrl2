...
- Gain-probe log prints **per-input** `ss_pairs` + `unbiased`/`not_noisy` (HEAD). P31 live gates only had min/max+worst.
- P1→P2 **detonated-freeze last-ok restore** (HEAD, P31 RCA): if freeze-iter recon > `skip_storm_last_ok_recon_ratio` × last-ok best, restore `wm_last_ok`, reset AdamW, re-probe gain. Unitless (recon/recon). P31 GPU process does **not** have this — it froze exploded g. **P31 GPU process includes neither this restore nor the collect/Kalman/FD-batch opts.**

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 80**. Probe80 **H=1 r=+0.583 / H=55 r=+0.423 conv=0.00** gain_fid **0.520** best_h=55. H=55 recovered above floor (was **0.160@70**); H=1 slipped 0.746→0.583 (P64 @80 **+0.763 / +0.594 conv=0.25 0.936**). recon **0.0032@78 best** then **0.0042@80**. Teacher **×1.00 / ×0.98**. persist_rel **0.034**. CPU @wm_best70 1step→OL **×0.272**. pid **242511**. | 🔬 LIVE P1 — H=55 above floor; **conv still 0 @10–80**. Gate ~82. Do not patch live. |
...
- **LIVE P1 iter 80 (~421 min, nvidia 11.7 GB, `t_wm` ~309 s, sps 19.6):** skip **0**. recon **0.0032@78 best → 0.0033@79 → 0.0042@80**. Teacher jsonl 78 **×0.996 / ×1.065** → 79 **×0.997 / ×1.041** → 80 **×1.001 / ×0.983**. last_ok **80 unlocked**. persist_rel **0.036@78 → 0.038@79 → 0.034@80** (still collapsed; full inject const+step+dv-prbs+expert @80 — watch next-iter spike, do **not** persist N+1). Rest-IC graph not captured. Original-P1 gate typically ~iter 82. Next probe typically iter 90 (~03:18 CDT).

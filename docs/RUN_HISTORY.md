...
- **Launch KEEP:** env-free det+eager `storm_cap=2`. `[resolved-cfg] iso_varnorm=False` (pid still has the knob; HEAD removed it) gain_match=1 isolation=1 ss_match=3. Gain-match tgt MV −2.81 DV 0.487. device=cuda bs=128. No leftover `DREAMER_*`. GPU ~17.7 GB.
- **P1 through iter 49 (P33 class, not P34–P36):** iter-1 iso **1.33** (P33 1.69; P36 0.125). gmatch **1.20 → 0.054 by iter 3 → 0.0008 by iter 17**. skip **0** through 49. Iter 19 wrap-class blip recon 0.267 / gmatch 1.07 recovered (do **not** lower skip threshold). jsonl `wm_input_isolation_loss`. Iter 30 probe best_h=55/55 gain_fid=0.441. Approaching P32 storm window (iter 53) — continue-after-storm KEEP if it fires.
...
- **HEAD (not in this pid):** isolation inv-var A/B **REMOVED**. Abs MSE + **`wm_isolation_dcv_match=True`**: isolation settle `Δu_i ∝ 1/|G_i|` from gain-match WM-norm targets, clipped to ±1 (test_sim scale MV ~0.289 DV ~1.67). Equal-|G| keeps op-band. Not a loss reweight. `[resolved-cfg] iso_dcv=`. Opt out `DREAMER_WM_ISOLATION_DCV_MATCH=0`.
...
- Isolation/ss-match is **abs MSE only**. Inv-var A/B **REMOVED**. Default **`wm_isolation_dcv_match=True`**: isolation settle `Δu_i ∝ 1/|G_i|` (WM-norm |G|, clip ±1, equal-|G| no-op). Sim-adaptive (derived from identified gains + op_band). P37 pid is unscaled abs. Do **not** re-add `wm_isolation_var_norm`. Do not launch P38 while P37 is live.
...
- P36 EXIT (`run_p36_isoinpscale`): storm 2/2 CAPPED GAIN_NOT_READY 0.00@DV. Val MV ×0.91/@H ×0.95, DV ×0.004, det_r 0.076, `[p3-skip]`. Per-input `|G|²` fired (ratio 33) but inv-var DISCARDED then **REMOVED**. P37 GPU: abs isolation, env-free, P1 iter 49 skip 0 (P33 class). HEAD isolation |ΔCV| excitation default ON (not in pid). Do not pass leftover compile/latent/DOB env-vars. Do not launch a second GPU job.

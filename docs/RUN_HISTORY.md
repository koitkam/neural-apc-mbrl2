...
| P50 | 2026-08-29 | P1/P2 BC MSE-on-μ (`bc_mean_only=True`) | **LIVE** P1 iter 1. Env-free tmux `mbrl2_p50` pid **49389**. `[resolved-cfg] bc_mean=True` det+eager isolation=0 rest-IC. P1 bc **0.277** (MSE; P49 P2 NLL was **−0.87…−1.13**). No leftover `DREAMER_*`. Do not launch a second GPU job. | 🔬 live; watch P2 `bc_loss`>0; first P3 ent ≈ H(σ_init)≈**−0.08** not −0.283 |
...
- **LIVE (2026-08-29 16:38 CDT, P1 iter 1):** recon **0.0772** kl 0 jemb 0.0716 gmatch **0.0007** iso 0 skip **0** encvar 0.02 alive 1023. **`bc_loss=0.277` (positive MSE)** vs P49 P2 NLL **−0.87…−1.13**. Teacher FD ratios MV **×1.003** / DV **×0.976**. t_wm **125 s** (eager T-GRU). GPU ~11 GB, util ~62%. `p1_last_ok_iter=1` unlocked. Auto-tune `policy_init_log_std=-1.5`, `log_std_max=-1.520`, `log_std_min=-1.702` (σ_min_ratio **1.2**). First P3 ent target ≈ H(σ_init)=0.5+0.5·ln(2π)+log_std ≈ **−0.08**, not P45–P49 floor **−0.283**.
- **Watch:** P1 recon→~0.003 / skip-storm; P2 `bc_loss` stays **>0**; first P3 entropy ≈ **−0.08** not −0.283; freeze rscale ~2; do not train P3 on GAIN_NOT_READY. Remaining leftover is env-only paths/device (`AGENT_*_JSON`, `SIM_NOISE_CONFIG_JSON`, `DREAMER_WM_DIAG_DEVICE`) — do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.

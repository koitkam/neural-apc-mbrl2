...
| P53 | 2026-08-30 | P3 PPO ratio clip vs frozen unfreeze-μ (`p3_mu_ratio_clip=0.2`) | LIVE `126f011` pid **75966** tmux `mbrl2_p53`. Step 4 `p3_sglogstd=True p3_logpclip=8 p3_muratio=0.2`. P1 iter **17** recon **0.0042** gmatch **0.0003** FD MV/DV ×0.99/×1.01 skip **0**. First wm-fid probe @10 EMA **5.287** gain_fid 0.831. Watch unfreeze 147. | 🔬 LIVE |
...
- **Liveness:** P1 STAGE 1. Iter **17** (17 jsonl rows, ~38 min). recon **0.128→0.0042** (best held) gmatch **0.0003** (FD MV/DV **×0.99 / ×1.01**). iso 0 ss 0 skip **0**. last_ok **17** unlocked. First wm-fidelity probe @**10**: EMA **5.287** (saved `wm_best.pt`) H=1 r=+0.609 @H r=+0.285 conv=0.75 gain_fid=0.831 (not the TM gate). sps ~48. GPU ~11 GB. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Gate vs P52 last-ok 78 not yet. Do not KEEP/FALSIFIED as cascade until EXIT.

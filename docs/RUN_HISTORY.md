...
- **Attributed change vs P37:** isolation settle `Δu_i ∝ 1/|G_i|` (not a loss reweight). Seed log `dcv_match MV=['0.317'] DV=['1.67']`. `run_plan.isolation_dcv_scales` on.
- **Iter 1:** recon 0.095 kl 0 jemb 0.081 **gmatch 1.30 iso 0.140 ss 0.106 skip 0**. Iso smaller than P37 1.33 because MV |Δu| shrank (matched |ΔCV|); not P35 starve (0.0008) and not P34 explode (7088). Watch gmatch fall (P37 →0.0008 by iter 17) and skip 0 through P32 window.
...
| Isolation |ΔCV| excitation `Δu_i ∝ 1/|G_i|` (`wm_isolation_dcv_match`) | P38 LIVE (`run_p38_isodcv`, `42bc7c2`) | IN FLIGHT (default ON) | PARTIAL — P37 EXIT unscaled abs val DV **×0.69**. P38 seed scales MV 0.317 DV 1.67; iter-1 iso 0.14 skip 0. |

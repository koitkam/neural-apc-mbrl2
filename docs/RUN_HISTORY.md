...
| P73 | 2026-09-02 | OL gain-c persist at teacher K (`||c_K[:G]−sg(c0[:G])||²` × existing `cont_gain_persist_coef`) | LIVE P1. Step 4 CONFIRMED. Iter 1 teacher ×0.671/×−1.21 persist_rel **1.46** (not held-K ×1). | 🔬 LIVE |
...
- **Step 4 CONFIRMED:** `device=cuda` bs=128 `wm_train_T=128` horizon=55 compile=eager. `[cont-latent] GAIN-ONLY` gain_dim=2. `[resolved-cfg] isolation=0 gmatch_len=55 gmatch_ol_tail=0 gmatch_step=0.4 gmatch_clip=True gmatch_rest=True dob_ground=2 dob_reg=0 held_cv=True p1amp=0.2 p2amp=1 p3amp=1`. Rest-IC graph captured `N=6 T=128`. Huber baseline = rest last-obs CV. Process environ: `CUDA_VISIBLE_DEVICES=0` only (no `DREAMER_*`). nvidia **~15.5 GB** one job.
- **LIVE (2026-09-02 04:23, P1 iter 1):** teacher **×0.671 / ×−1.21** (P68-class, not held-K tautology). jsonl `gain_match_ol_persist_rel=1.46` (term is live). recon **0.071** gmatch **1.53** iso **0** skip **0**. Watch freeze vs P64 **0.91@DV**; persist_rel falling; val 1step→OL / OL-vs-real vs P64 ×0.85/×0.84. **No second GPU job.**

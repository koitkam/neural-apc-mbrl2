...
- **Liveness:** live. GPU occupied (~15.7 GB). tmux `mbrl2_p43`, pid **4465**, launch `51b0f45`. Env-free. `device=cuda` bs=128 `latent=deterministic compile=eager`. `[gain-match] huber_per_input=True` β_mv=**2.63** β_dv=**0.512**. `[resolved-cfg]` isolation=0 ss_match=0 lock=20 **huber_per_in=True**. No leftover `DREAMER_*`. P1 through iter **25** (13:57): skip **0**, recon **0.0042**, gmatch **0.0008** (mv **0.00120** / dv **0.00038**), gnorm **0.62** (iter 1 was 6.8 then settled; **not P27-class**). Iter-10 wm_best gain_fid=0.949 then iter-20 0.600 (gain-blind spike; typical). iso=0. last_ok unlocked. Tracking P40/P41 recon; gnorm quieter than P42's 20–26 blips. **Do not launch a second GPU job.** Watch skip through ~53; first gate ~82 gain-probe DV toward [0.8,1.3]. Storm 2/2 early / P27 gnorm → REVERT.
- **Judge by:** `[gain-match] huber_per_input=True` β_mv≈2.62 β_dv≈0.51. gmatch falling like P40 (not stuck 1.30). skip 0 through ~53. jsonl mv vs dv Huber closer. First gate ~82 **gain-probe** DV toward [0.8,1.3]. If storm 2/2 early / gnorm P27-class → **REVERT** per-input (relative-Huber cousin). Val MV ss/@H ~×1.0, DV toward P26 ×0.87 not P42 ×0.74. If GAIN-READY: first valid actor (min-of-2 + freeze return_scale). Else `[p3-skip]`.
...
- Abs Huber gain-match is count-equal per input but **scale-unequal** when `|G_mv|≫|G_dv|`. P42 `[gain-match] targets` |MV|=2.62 |DV|=0.51 (~5.1×). Isolation-off moved that drowning into the sole DC supervisor. **P43 default** `gain_match_huber_per_input=True`: β_ij = `|tgt_ij|` so L1 sat is still ±1 (not P27 relative Huber). Scalar `gain_match_huber_beta=1` stays the A/B (`DREAMER_GAIN_MATCH_HUBER_PER_INPUT=0`). Median `|tgt|` (`beta<=0`) only when per-input is off. jsonl `wm_gain_match_mv_loss` / `wm_gain_match_dv_loss` still splits Huber. Do **not** revive relative Huber.
...
- `[resolved-cfg]` prints `iso_dcv=off` when isolation/ss-match are off (P40); when the teacher is on it still prints `iso_dcv=` **and** `min_scale=` **and** `mv=`/`dv=` multipliers **and** `edge_du_mv=`/`edge_du_dv=` **and** `g_ratio=`/`smax=`/`equalize=`. Also prints `lock=` (`skip_storm_last_ok_lock_ratio`; P42) and `huber_per_in=` (P43 per-input Huber β). `run_plan.isolation_dcv_scales` is the P38/P39 audit trail (inv-var knob removed). Pre-rewrite dump is dataclass defaults (P29 plan still shows `rssm_latent_type=categorical` / `gain_match_coef=0` even though training auto-enabled grounding).
...
| Per-input Huber β = `|tgt_ij|` (L1 sat ±1) | P43 launch | pending GPU | PARTIAL — equalizes L2 region without P27 1/|tgt| L1; REVERT if storm 2/2 / P27 gnorm |
...
| `DREAMER_CRITIC_IMAG_LOSS_COEF` whitelist (imagination CE) | mbrl2 real-sim | **REMOVED** from `ENV_OVERRIDES` (field kept inert) | YES — `_realsim_actor_critic_step` never reads it; override was a false A/B |

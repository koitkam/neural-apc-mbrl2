...
| P58 | 2026-08-31 | env-free rest-IC encode L=lookback (`gain_match_rest_ic_len=0`) + HEAD graph T=128 | LIVE ~iter **59**. Graph **captured** T=128. Wrap lock@43 unlock@47; lock@50 unlock@51; **skip-storm 1/2 @58** restored last_ok **57** (recon 0.518 skip 31). Iter 59 recon **0.007** skip 0/31. `t_wm` median **102 s**. Probe@40 `gain_fid=0.537` did **not** update wm_best (gain-blind). Watch val TM vs P53 ×0.90 / P56 ×0.76. | LIVE |
...
- **Liveness (P1 ~iter 59, ~07:11 CDT):** pid **126332** alive, tmux `mbrl2_p58`, GPU **13632 MiB**. Graph **captured** `N=6 T=128`. `t_wm` median **102 s** (P57 T=55 **99 s**; P56 eager T=128 **124 s**). skip-storm **1/2 @58** (recon 0.518 skip 31) restored last_ok **57**; iter 59 recon **0.007** skip 0/31 last_ok **59 unlocked**. Wrap lock@43 unlock@47; lock@50 unlock@51. Probe 10 `gain_fid=0.853` wm_best; 20 `0.740`; 30 **`0.899` wm_best**; **40 `0.537` no wm_best** (gain-blind); 50 `0.764` best_h=1/55. Teacher FD after restore ×1.00/×0.98. Do not launch a second GPU job.
- **Watch:** val TM vs P53 ×0.90 / P56 ×0.76; paired −13/−98. Do not attack compounding on this pid. Do not lower skip threshold. Storm 2 still caps.

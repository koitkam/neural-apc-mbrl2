...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P1** iter **65** (~147 min). pid **64705**, launch `d910ee2`. Step 4 **CONFIRMED** `p3_sglogstd=True p3_logpclip=8`. recon **0.0031** (best **0.0017@60**) last_ok **65 unlocked** skip **0**. lock@38 wrap-unlock@39. Iter 60 probe H=55 r=0.216 gain_fid=0.693 (best still 30 EMA 5.798). Iter 65 bc=0 one-iter (not skip-storm). GPU ~11 GB. Env-free. Watch gate 82 (~37 min); unfreeze ~147 vs P51 μ-rail. | 🔬 wrap-unlock KEEP. Do not launch. Do not stack critic knobs. |
...
- **P1 iter 65 (~147 min):** recon **0.0031** (best **0.0017@60**) last_ok **65 unlocked** skip **0**. lock@38 wrap-unlock@39 last_ok advanced (P50-class KEEP). Iter 60 wm-fid H=55 r=**0.216** `gain_fid=0.693` best_h=27/55 (did not beat iter 30 EMA **5.798**). Iter 65 `bc_loss=0` one-iter (skip 0, last_ok advanced). Teacher FD mv **1.007** / dv **0.992**. Gate 82 ~37 min; unfreeze ~147 is the A/B.

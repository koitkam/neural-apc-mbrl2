...
| P63 | 2026-09-01 | held 1-step→K decoded-CV magnitude (FO τ-scale; stop-grad 1-step teacher; no new knob) | **LAUNCH** `run_p63_olmag`. Same knobs; late−early **replaced**. Watch jsonl `wm_held_ol_ratio` (~1=match) + val 1step→OL / OL-vs-real vs P62 ×0.828 / ×0.785. | 🔬 Step 4 pending launch confirm |
...
- **Step 4 (first launch aborted):** pid **165318** sha `f67ecfa` `device=cuda` `held_mag=True` `gmatch_clip=True` `gmatch_step=0.4`. Iter 1 held loss **1.18e5** skip **5/5** (`ol_ratio` 0.078, `cv_drift` 10.8) — std-normalized MSE sat the 1e-3 floor (P27-class). **Killed.** Huber β=1 on (ΔK, FO·sg(Δ1)); jsonl scale stays tail std. Relaunch same tag after abort dir `run_p63_olmag_abort_iter1`.

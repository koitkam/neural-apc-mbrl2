...
### p37 GPU P1→P2 (`run_p37_isoabs`, tmux `mbrl2_p37`, launch `4d349fb`) — abs isolation = P33 DV pin; silent detonation; last-ok KEEP
- **Launch KEEP:** env-free det+eager `storm_cap=2`. `[resolved-cfg] iso_varnorm=False` (pid still has the knob; HEAD removed it) gain_match=1 isolation=1 ss_match=3. Gain-match tgt MV −2.81 DV 0.487. device=cuda bs=128. No leftover `DREAMER_*`. GPU ~17.7 GB. pid **4162106**. Still in **P2** (g frozen). No second GPU job.
- **P1 healthy through iter 87 (P33 class, not P34–P36).** **Past P32 storm window:** iter 53 recon **0.0047 skip 0**. Iter-1 iso **1.33**. gmatch **1.20 → 0.0008 by iter 17**. Iter 19 wrap-class blip recon 0.267 / gnorm ~9.5 recovered (do **not** lower skip threshold). Iter 70 gain_fid=**0.732** best_h=55/55. Iter 80 `wm_best` (gain-blind; restore OFF).
- **Iter 75 gate:** `DCgain_ratio[0.68,0.99]` worst=**0.68@DV** (MV 0.99). **EXTENDED**. **Iter 85 gate:** `DCgain_ratio[0.72,0.98]` worst=**0.72@DV** (MV 0.98). **EXTENDED** +75396 to 904752. Keep-ext used. Extra P1 moved DV 0.68→0.72 — still GAIN_NOT_READY. Abs isolation **CONFIRMED** as the DV pin.
- **Silent detonation (P31 class):** iter 88 recon **0.848** gnorm **62.4** skip **0** (62 ≪ skip 1e4). Iters 88–97 recon ~0.7, gmatch stuck 1.21, skip 2 at iter 94. Iter 90 probe gain_fid=0.003. **Iter 97 CAPPED** cap-time probe **0.00@DV / 0.59@MV** (exploded g).
- **Detonated-freeze last-ok KEEP (GPU-confirmed again):** restored `wm_last_ok` iter **87**; recon 0.750 > 5× 0.0023. Restored probe `DCgain_ratio[0.71,1.00]` worst=**0.71@DV** (MV 1.00). `[p1→p2] WM warm-restore SKIPPED`. P2 iter 98–112 recon **0.003**, `dobg` 0.003–0.007 live, g frozen. Expect `[p3-skip]`. Actor INVALID.
- **Do not launch a second GPU job.** Full val axes at EXIT. Next GPU after EXIT: env-free P38 `run_p38_isodcv` (HEAD `wm_isolation_dcv_match`). Not more extra-P1. Not a lower skip threshold. Not another isolation reweight.
- **HEAD (not in this pid):** isolation inv-var A/B **REMOVED**. Abs MSE + **`wm_isolation_dcv_match=True`**: isolation settle `Δu_i ∝ 1/|G_i|` (test_sim scale MV ~0.289 DV ~1.67). jsonl now aliases `wm_isolation_loss` and logs `p1_last_ok_iter`. Pre-iso resolve feeds **scales only**; post-seed **always re-resolves** Huber targets. Opt out `DREAMER_WM_ISOLATION_DCV_MATCH=0`.
...
- Isolation/ss-match is **abs MSE only**. Inv-var A/B **REMOVED**. Default **`wm_isolation_dcv_match=True`**: isolation settle `Δu_i ∝ 1/|G_i|` (WM-norm |G|, clip ±1, equal-|G| no-op). Sim-adaptive (derived from identified gains + op_band). Pre-iso resolve is **scales only**; post-seed always re-resolves Huber targets after isolation+expert (P37 freeze point). P37 pid is unscaled abs; P1→P2 last-ok **0.71@DV**. Do **not** re-add `wm_isolation_var_norm`. Do not launch P38 while P37 is live.
...
- `run_plan.json → config` is rewritten after gain-match resolve. `[resolved-cfg]` prints `iso_dcv=` **and** `mv=`/`dv=` Δu scales; `run_plan.isolation_dcv_scales` is the P38 audit trail (inv-var knob removed). Pre-rewrite dump is dataclass defaults (P29 plan still shows `rssm_latent_type=categorical` / `gain_match_coef=0` even though training auto-enabled grounding).
...
- P36 EXIT (`run_p36_isoinpscale`): storm 2/2 CAPPED GAIN_NOT_READY 0.00@DV. Val MV ×0.91/@H ×0.95, DV ×0.004, det_r 0.076, `[p3-skip]`. Per-input `|G|²` fired (ratio 33) but inv-var DISCARDED then **REMOVED**. P37 GPU P1→P2: abs isolation, env-free. Iter 75 EXTEND 0.68@DV, iter 85 0.72@DV, silent detonation iter 88, last-ok restore iter 87 **0.71@DV / MV 1.00**. Abs isolation CONFIRMED as DV pin. Expect `[p3-skip]`. HEAD isolation |ΔCV| excitation default ON (not in pid). Do not pass leftover compile/latent/DOB env-vars. Do not launch a second GPU job.
...
| Extra P1 via keep-ext to pin DV DC-gain | P33 P1→P2 (full 175924 ext, no storm); P37 extra-P1 | FALSIFIED as DV lever (keep-ext KEEP as mechanism) | YES — P33 0.68@DV; P37 0.68→0.72 then detonated; last-ok 0.71@DV |
| Abs isolation/ss-match MSE on mixed MV/DV | P33 EXIT; P37 P1→P2 | KEPT as default (P36 REVERT); FALSIFIED as DV pin | YES — |tgt| MV 2.82 vs DV 0.49; P37 freeze 0.71@DV after last-ok. Inv-var (P34–P36) skip-stormed worse. |
...
| Isolation |ΔCV| excitation `Δu_i ∝ 1/|G_i|` (`wm_isolation_dcv_match`) | P37-live HEAD (not in pid) | PENDING (default ON for next env-free P38) | PARTIAL — P37 abs freeze **0.71@DV** (MV 1.00) after silent detonation + last-ok. Not a loss reweight. Wait P37 EXIT then P38. |

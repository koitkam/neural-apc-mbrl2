...
| P76 | 2026-09-02 | RSSM GRU update-gate bias `log(H/16)` (~1.23; no new knob) | LIVE **P2 iter 124**. P1→P2 @94: live 5-level **0.84@DV PASS** then **detonated-freeze last_ok 84** 5-level **0.80@MV ready=False** (`p1_gain_not_ready_capped`). orig-P1 @82 **FAIL 0.79@DV**. conv=**0** through P2@100; P2@110 and @120 **conv=0.25** (frozen g). CPU n=1 last_ok MV **×0.907** DV **×0.928**. z-bias still **1.23**. pid **237453**. | 🔬 LIVE P2 — freeze GAIN_NOT_READY; expect `[p3-skip]`. Do not GRU-bias N+1. |
...
### p76 (`run_p76_grubias`, branch `cursor/p28`) — LIVE P2: GRU update-gate bias
...
- **LIVE P2 iter 124 (~3.0 h, nvidia 4.0 GB after graph release, `t_wm` ~20.3 s, sps 255):** skip-storm **1/2 @77** restored last_ok **76** (recon 0.267, 8 skips); wrap@29 recovered. last_ok **locked 84** (recon 0.083 @85, skip 0). persist_rel pinned **0.021 @82** then extra-P1 oscillated 0.05–0.96. Teacher jsonl ×1 ≠ TM. P2 remaining ~24 iters to natural P2→P3 (`p2_end≈p1+ext+p2`; mtp med **0.97 ≪ 3.0** so the P2 gate should PASS then `[p3-skip]`).
- **Original-P1 gate @82 FAIL 0.79@DV** (MV 0.79/@H 0.87, DV 0.79/@H 0.85, `unbiased=False`). P64 @82 **PASS 0.91@DV**; P75 **0.84@MV**; P73 **0.83@DV**. First of this identity to miss original-P1 GAIN-READY. Extra-P1 +75396 (cap 175924).
- **Live extra-P1 gate @94 PASS 0.84@DV** (MV 0.84/@H 0.96, DV 0.84/@H 0.91, `unbiased=True`). Then `[p1→p2] recon 0.3699 → 0.0027` — extra-P1 silent detonation at freeze (P40 class). **Detonated-freeze restored last_ok 84**; 5-level re-probe **ready=False 0.80@MV / 0.81@DV @H 0.87–0.89 `unbiased=False`**. Sets `p1_gain_not_ready_capped`. wm_best restore SKIPPED. Frozen observer = last_ok **84**, not live 0.84@DV. Expect `[p3-skip]` / `actor_experiment_valid=false`. Extra P1 FALSIFIED as freeze lever again (P40 lock).
...
- **CPU CVD="" (not a training change):** freeze last_ok **z-bias still 1.23 / z_mean 0.89** (P64 ckpt60 bias ~0.005 / z 0.73). n=1 rest-step K=55 TM: last_ok MV **×0.907** DV **×0.928**; ckpt80 **×0.826 / ×0.810**; ckpt60 **×0.821 / ×0.842**. P64 ckpt80 **×0.971 / ×0.915**; P64 ckpt60 **×0.923 / ×0.888**. n=1 last_ok closed toward P64; **5-level gate is the freeze** (0.80@MV). Do not treat n=1 ×0.91 as GAIN-READY.
- **P2 Kalman:** `dobg` P2 med **0.065** last **0.048** peak **0.175** @116 (`dob_d` last **0.037**; P64 first-20 med **0.032**, full-P2 med **0.121** last **0.210** / `dob_d` **0.063**). recon **0.0026–0.0035**. reward_mtp med **0.97**. Do **not** score actor if `[p3-skip]`.
- If EXIT FALSIFIES as compounding (5-level freeze 0.80 + P1 conv=0 vs P64 0.91), **change family** (do not GRU-bias N+1). Keep-h preserved `||h||` and **prevented settling**, which is the opposite of a DC fixed point. Next family after EXIT is observer `h`/decoder (TSSM successor or a non-z-bias `h` attack), not critic knobs. Do **not** start a second GPU job. Step 5 deferred to EXIT.

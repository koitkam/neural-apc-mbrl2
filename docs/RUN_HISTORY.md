...
| P51 | 2026-08-30 | P3 REINFORCE stop-grad log_std (`p3_stop_grad_log_std=True`) | **LIVE P3** iter **249** (~3h43). pid **54815**, launch `d6a4511`. Gate **GAIN-READY** 0.86@MV / 0.88@DV @82; last_ok **82 unlocked**. P2→P3 PASS 0.922 @136. First P3 ent **−0.125**. Unfreeze **147**: ent **HELD −0.141→−0.101** (not P50 −0.268); `actor_logp_std` **0.54→38** (μ-rail). By **249**: actor **−43**, mvv **749k**, rtgt **0.0002**, ret_w **−1470**, ret_ema **−1658**; ent min **−0.226** @210 then **−0.15** (not σ_min −0.283). rscale freeze **2.17 KEEP**. Env-free. Do not launch. | 🔬 stop-grad KEEP as entropy lever; **failing as cascade lever** (μ-rail). Watch EXIT. Do not stack critic knobs. |
...
- **Unfreeze 147 (the A/B canary):** last warmup 146 ent **−0.141** `actor_logp_std` **0.54** actor **−0.010** bc 0.002 rtgt 0.046. Iter 147: ent **−0.101 HELD** (P50 169: **−0.107→−0.268**), `actor_logp_std` **0.54→38** (P50 **0.56→54** — μ-rail logp variance, not σ training). **Stop-grad KEEP as entropy/σ lever** (no −0.268 yank at unfreeze). **Failing as cascade lever** (μ REINFORCE still rails). `actor_logp_std` is not policy log_std.
- **Cascade @249 (~3h43, still LIVE):** actor **−43** @240 / gnorm **568** @247; `actor_logp_std` 44–96; bc ~1.1–1.5; mvv **369k** @236 → **749k** @246; rtgt **0.0002**; ret_w **−1470**; ret_ema **−1658**. Ent since unfreeze min **−0.226** @210, last **−0.15** — still not σ_min **−0.283**. rscale freeze **2.17 KEEP**. last_ok **82 unlocked**.
- **Watch:** EXIT / ES (P50 entropy_collapse @286; this pid may ES on return while entropy stays open). Do not edit val stack while pid 54815 is live. Do **not** launch. Do **not** implement the next μ-rail fix until EXIT.
...
| P3 REINFORCE stop-grad log_std (μ-only) | P51 LIVE P3 (`run_p51_sglogstd`, `d6a4511`) | KEEP as σ-yank block (unfreeze ent −0.141→−0.101); **not yet EXIT** as cascade lever | PARTIAL — unfreeze 147: no P50 −0.107→−0.268; `actor_logp_std` still 0.54→38 (μ rails). By 249: rtgt 0.0002, ret_w −1470, mvv 749k, ent min −0.226 then −0.15 (not σ_min). rscale 2.17 KEEP. Do not stack critic knobs. Do not promote `p3_reset_log_std`. |

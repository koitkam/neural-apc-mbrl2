...
| P78 | 2026-09-03 | TSSM gain-match rest-IC continues KV via `prev_state=` (P77 Markovian FALSIFIED) | LIVE P1 iter **2**. Teacher **×0.686 / ×−0.176** persist **0.811** recon **0.545** skip **0**. KV print `pos=128+K=55`. pid **261143**. | 🔬 LIVE — not ×1 pin yet (P77 pinned @4–5). persist high vs P77@2 0.52. Do not score actor. |
...
- **LIVE P1 iter 2 (~11 min, nvidia 12.3 GB, `t_wm` 295 s):** skip **0**. last_ok **2 unlocked**. recon **0.553→0.545**. Teacher jsonl **×0.686 / ×−0.176** (iter 1 **×0.146 / ×−0.028**). persist_rel **0.837→0.811** (held high vs P77@2 **0.522** / P76@2 **0.338**). gmatch **0.663→0.308**. DV still inverted (P77@2 also **×−0.139**). **Not** a Markovian ×1 pin yet — P77 pinned MV×0.97/DV×1.07 by iter 4–5; P76 MV×0.99 by iter 2. Do **not** treat as health. First probe @10 (~40 min). Do **not** score actor. Do **not** persist N+1. Hourly cron must skip a second GPU job.

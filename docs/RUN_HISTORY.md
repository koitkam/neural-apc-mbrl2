...
| P67 | 2026-09-01 | teacher `gain_match_len` auto = `wm_tf_horizon` (K=220; encode T lookback) | LIVE P1 iter **69** pid **187753** sha `9fbede0`. Storm **1/2 @67** recovered last_ok **69 unlocked**. Teacher ×0.995/×0.983 at K=220 recon **0.0056** `t_wm` 134 s ~14.3 GB. Fidelity@60 H=55 r **+0.270** gain_fid **0.670** (P64 +0.253 / 0.649). | 🔬 compounding family vs P64 ×0.842 OL-vs-real |
...
- **LIVE P1 iter 69** (~2.7 h wall, 2026-09-01 15:53). Pre-storm iter 65 recon **0.0024** last_ok **65 unlocked** teacher **×0.993 / ×0.984**. **Storm 1/2:** iter 66 silent recon **0.5736** skip **0** gnorm **0.59** teacher **×0.005 / ×0.022** locked last_ok **65**; iter 67 skip **16/16** gnorm **1.46e4** recon **0.314** gmatch **4.59** MV **×−0.058**. `[skip-storm] P1 recovered` restored last_ok 65, unlocked, extension **kept**. Iter 68 recon **0.0159** teacher **×0.994 / ×1.004**; iter 69 recon **0.0056** last_ok **69 unlocked** teacher **×0.995 / ×0.983** gnorm **0.154** skip **0/16**. Same class as P64 storm 1/2 @78 (last_ok 77→82). K=220 did **not** prevent the wrap and did **not** block restore. `t_wm` **134 s** vs P64 **97 s**. nvidia **14359 MiB** (`gpu_mem_reserved_mb` **13804**; jsonl `gpu_mem_mb` allocated **~305**). Fidelity@60 H=55 r **+0.270** conv 0.00 gain_fid **0.670** (P64 same iter r **+0.253** gain_fid **0.649**). Isolation=0. **jsonl ×1 at K=220 is protocol, not val TM.**
- **Watch:** first gate ~iter 82 (~30 min); last_ok advancing vs freeze-65 confound; val OL-vs-real vs P64 **×0.842**; MV/DV vs P26/P64; paired vs P64 **−4.54**; det_r/pred_std (not a DOB experiment). Do not score actor if GAIN_NOT_READY. **No second GPU job.** No training-behavior patch this session.
...
| Gain-match FD K = control H (last-only Huber) | P26–P66; val TM / decomp at `wm_tf_horizon=4H` | P67 auto K = `wm_tf_horizon` (LIVE pid **187753** P1 iter **69**) | PARTIAL as protocol gap — teacher returned **×0.995/×0.983 at K=220** after storm 1/2 @67 (P64 storm 1/2 @78; P64 teacher at K=H ×0.998/×0.996). last_ok **69 unlocked** recon **0.0056** skip 0/16. Fidelity@60 H=55 r **+0.270** gain_fid **0.670** (P64 +0.253 / 0.649). Val OL-vs-real pending (P64 ×0.842). Not S=H. Not a Huber reweight. |

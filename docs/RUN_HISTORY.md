...
- **LIVE (2026-08-26 P1 iter 92, GPU still running): CONFOUNDED.** Env-free dropped `DREAMER_RSSM_LATENT_TYPE=deterministic` (P26/P28 env, never a default). P29 is **categorical**: `kl_loss` 0.30–0.81, `joint_embed_loss≡0`, recon **0.50→0.16 by iter 46 then 0.65 at iter 49** (buffer wrap; skip=0) recovering to **0.323 at iter 92** vs P28 recon **0.003** at iter 50. Overshoot stuck **~0.10** (P28 0.22→0.05). `z_alive` 1024→**97**. Isolation extra unroll **is live** (`wm_input_isolation_loss≈0.002`) — follow-ups 1–14 are in this GPU job, still inseparable from categorical+compile. `z_dim=32` / `zrank/1024` is the same width as deterministic — do not use it. P29 cannot verdict skip-restore. **Also compile-on** (`[dreamer_v4] torch.compile`; iter-1 t_wm **1681 s** inductor warmup vs P28 eager ~163 s; 20 inductor workers on 20 cores; collect 22.6 s vs P28 17.6 s; steady t_wm ~147 s). Gates: iter 75 `gain_not_ready` DC[0.58, 3.05] worst=3.05@MV; iter 85 `not_plateaued` wm_ema_best=1.889. P1 extending toward cap 929884 then P2/P3. **Code (GPU occupied, no relaunch):** live iter banner prints `kl`/`jemb`/`gmatch`/`iso`/`ss`; jsonl splits `wm_ss_match_loss` / `wm_ss_match_wmean` / `wm_isolation_traj_loss` (P28/P29 never logged SS — folded into iso); host-adaptive OpenBLAS thread cap. Next GPU job after P29 EXIT = real skip-restore A/B (follow-ups 1–14 + deterministic + eager **env-free** — do not pass leftover `DREAMER_COMPILE=0` / `DREAMER_DOB_ENABLED` / `DREAMER_ACT_HIST_REQUIRED`). Train-start / `[resolved-cfg]` print `latent=` and `compile=`.

### Sim-adaptive leftovers (env-free multi-sim; do not promote plants yet)

Already unitless / derived: isolation TBPTT `max(8, round(K/3.5))`; `gain_match_len` / isolation len = H; isolation sample window `max(seq_len, K+1)` (test_sim seq_len ≥ H unchanged); P1/P2 main WM sample `max(seq_len, K+1)` (overshoot/gain-match; P3 stays seq_len); gain-match open-loop `K` not truncated to `T-1`; `skip_storm_last_ok_recon_ratio=5` (recon/recon); `p1_gate_wm_ema_min=1.5` (fidelity mix); inject EVERY = f(buffer lap) (test_sim 20/10); inject N = f(n_mv, n_dv) (test_sim 5/2/2/3); isolation settle = 24 **per input** (test_sim 24+24 / cap 48 settle-only; distillation 96+24 / cap 120). Isolation_buf holds only those settle episodes (not MIMO PRBS). Isolation settle is a whole-episode hold at a stratified level (`action_std=0`); DV step is MV-action-isomorphic (`isolated_level × span/2`). Opt-in: `gain_match_huber_beta<=0` → median |tgt| (P28 still uses β=1). Gain-ready band `[0.80, 1.30]` and wm-fidelity mix weights are unitless TrainConfig (were os.environ). `rssm_latent_type` default **deterministic** (P26 proven; P29 env-free drop of the leftover env-var). `compile_mode` default **eager**; `DREAMER_COMPILE` / `DREAMER_COMPILE_MODE` are in `ENV_OVERRIDES` (MODE was CLI-only). Intra-op threads and inductor workers scale with `sched_getaffinity` (host-adaptive).
...
- Next launch after P29: env-free `cursor/p28` (deterministic + eager). Do not pass leftover P26 env-vars (`DREAMER_COMPILE=0`, `DREAMER_RSSM_LATENT_TYPE`, `DREAMER_DOB_ENABLED`, `DREAMER_ACT_HIST_REQUIRED`, `gain_match_coef` / `dob_ground_coef` auto-enable). Opt-in compile is `DREAMER_COMPILE=1`. Live iter banner (HEAD) prints `kl`/`jemb`/`iso`/`ss` so a leftover latent type **and** a starved SS-match are visible in tmux. jsonl: `wm_ss_match_loss`, `wm_ss_match_wmean`, `wm_isolation_traj_loss`.

...
| P56 | 2026-08-30 | Slow μ-ratio recopy `DREAMER_P3_MU_RATIO_REFRESH=50` (~patience/4) | LIVE P1 `run_p56_muslow` pid **110246** sha `9d05866`. **GAIN-READY still pending.** P1 ~iter **71** recon **0.0017**. Skip-storm 1/2 @68–69 (recon 0.253→0.512, 65 inner skips) restored last_ok **67** then unlocked; teacher FD MV/DV ~×1.00. `wm_best` **iter 70** EMA 6.224 gain_fid **0.926** (`restore_p2=False`). Rest-IC CUDA-graph warmup **failed this pid** (`ValueError: grad requires non-empty inputs`; `t_wm_s` ~124 s). HEAD: `_RestICGraphModule` yields RSSM `parameters`/`named_parameters`/`buffers`/`named_buffers`; VRAM skip does **not** pin eager-fail (warmup retry after `empty_cache`). Watch logp_std ~0.5 not 18; clip_frac not 0.11 and not 0.02–0.80; val vs P53 −13/−98. | LIVE |
...
### p56 (`run_p56_muslow`, branch `cursor/p28`) — LIVE P3: slow μ-ratio recopy N=50 (first recopy not yet)
...
- **Liveness:** GPU occupied. tmux `mbrl2_p56` LIVE, pid **110246**. sha **`9d05866`**. **Do not launch a second GPU job.**
  ...
  - **Unfreeze 147:** first P3 ent warmup **−0.18** then **−0.101 HELD**. logp_std **1.67@147** then **~0.50@176** (P53 held ~0.47; P55 **18.8@175**). `clip_frac` **0.70@147 → 0.16@176** (freeze-forever drop, not P55 0.02–0.80). rtgt **0.058→0.004** (P54-class collapse). adv_corr 0.23→~0.05. mvv 4.2k@166 not 813k. ema **−218→−119**. **N=50 first recopy ~iter 197** (50× inner after unfreeze; not fired by 176).
- HEAD after this pid: `_RestICGraphModule` yields RSSM `parameters`/`named_parameters`/`buffers`/`named_buffers`; capture `wrapper.train(rssm.training)` (graphed fwd is training-state gated); empty trainable / GRU-canary still pin; VRAM skip does not pin. Skip `rssm_cont_kl_loss` when `cont_kl_scale<=0` (env-free 1.0 identity). Do not stack critic knobs. Do not promote N=50 without EXIT+val. Watch logp_std ~0.5 not 18 after ~197; clip not stuck at 0.11 *and* not 0.02–0.80; val vs P53 −13/−98.

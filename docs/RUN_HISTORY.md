...
| P73 | 2026-09-02 | OL gain-c persist at teacher K (`||c_K[:G]−sg(c0[:G])||²` × existing `cont_gain_persist_coef`) | LIVE P1 iter **53**. skip **0**. last_ok **53 unlocked** (P72 froze 15 after storm 2@16). Teacher pinned @5. persist_rel **0.05–1.46**. | 🔬 LIVE |
...
- **LIVE (2026-09-02 10:52Z, P1 iter 53):** skip **0/0** last_ok **53 unlocked**. P72 storm **1@6** (gnorm **1.73e15** skip 67) then **2@16** (gnorm **3.96e9** skip 135) froze last_ok **15** and CAPPED extra-P1 (`gmatch=0` after). P73 teacher pinned @5 **×0.996 / ×0.980** (P72 @5 still ×0.64/×−1.08). Wrap-class recon spikes @17 **0.113** and @44 **0.033** recovered without skip (last_ok held then advanced). Iter **52** vs P64/P68: recon **0.0034 / 0.0021 / 0.0033**; last_ok **52/52/52**; jsonl MV **×0.997** DV **×0.978** (P64 ×0.992/×0.998). persist_rel **0.05–1.46** (not collapsed to 0 — not P70 hold). Dummy `gmatch_ol_tail=0` banner **REMOVED** on HEAD (not this pid). Watch last_ok toward P64 **82**; freeze vs **0.91@DV**; val 1step→OL / OL-vs-real vs ×0.85/×0.84. jsonl ×1 ≠ val TM. **No second GPU job.**

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 57**. Wrap **recon 0.147@56** (25.8× best 0.0057) last_ok locked 55 then **unlocked @57 skip 0**. recon **0.022@57**. Probe50 still last **H=1 r=+0.509 / H=55 r=+0.273 conv=0.00** gain_fid **0.389**. Teacher **×0.99 / ×1.04 @57**. persist_rel **0.50@57**. pid **242511**. | 🔬 LIVE P1 — wrap recovered skip 0 (same as iter 19). Probe60 pending. Do not patch live. |
...
- **LIVE P1 iter 57 (~298 min, nvidia 11.7 GB, `t_wm` ~307 s, sps 19.6):** skip **0**. Wrap-like spike iter **56** recon **0.1475** (25.8× best **0.0057**; gmatch 0.134; teacher **×0.86 / ×1.42**); last_ok **locked 55** wrote `wm_last_ok.pt`. Iter **57** recon **0.0220** (<20×) **unlocked**; last_ok **57**. Teacher jsonl 55 **×1.005 / ×0.980** → 56 **×0.860 / ×1.419** → 57 **×0.991 / ×1.037**. persist_rel **0.11@55 → 0.44@56 → 0.50@57**. Best recon still **0.0057@53** (~2.2× P64 0.0026). Same class as iter-19 wrap (0.497→0.143@20 skip 0). Rest-IC graph not captured. Next probe typically iter 60 (~00:42 CDT).

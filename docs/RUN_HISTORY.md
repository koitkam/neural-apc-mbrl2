...
- **Liveness:** live. GPU occupied (~15.7 GB). tmux `mbrl2_p43`, pid **4465**, launch `51b0f45`. Env-free. `device=cuda` bs=128 `latent=deterministic compile=eager`. `[gain-match] huber_per_input=True` β_mv=**2.63** β_dv=**0.512**. `[resolved-cfg]` isolation=0 ss_match=0 lock=20 **huber_per_in=True**. No leftover `DREAMER_*`. P1 through iter **69** (15:15): skip **0** (past P32 storm window @53), recon **0.0030**, gmatch **0.0004**, bc 2.55. Iter 54 recon blip **0.022** (~10×, not 20× lock) recovered; last_ok advancing unlocked. Iter-10 wm_best gain_fid=0.949; iter-60 H=55 r=0.253 best_h=27/55 gain_fid=0.582 (gain-blind spike; typical). iso=0. **Not P27-class.** **Do not launch a second GPU job.** First gate ~82 gain-probe DV toward [0.8,1.3]. Storm 2/2 / P27 gnorm → REVERT.

...
| P62 | 2026-08-31 | held-rollout decode-CV stationarity (`out='obs'` + CV late−early; no new knob) | **LIVE** tmux `mbrl2_p62` pid **160203** sha `6841f5c`. P1 iter **28**. held **1.5e-4** (quieter than P61 matched ~7e-4; P61 later silent ~3e-4). Teacher ×1.02/×1.01 `clip_frac` **0.083** skip 0. recon 0.0032. HEAD: jsonl `wm_held_cv_drift`/`scale` + `img_rollout out='h'` **REMOVED**. | 🔬 Do not FALSIFY from P1. Watch val 1step→openloop vs ×0.772; paired vs P58 |
...
- **LIVE (2026-08-31 ~20:56 CDT, P1 iter 28, GPU occupied):** recon **0.0032** (best 0.0028) kl **0** skip **0/0** last_ok **28 unlocked**. held **1.5e-4** (iters 10–28 median **2.0e-4**) vs P61 matched **~7e-4** and P61 later-P1 silent **~3e-4**. overshoot **0.035** (P61 0.038). teacher MV/DV **×1.016 / ×1.013** `clip_frac` **0.083**. `t_wm` **97 s**. Decode-CV is **not** a louder supervisor than h at matched iters — either CV-std vs h-std (same formula `(Δ/std)²`) or held-stationarity is the wrong lever for 1step→openloop compounding. HEAD (not this pid): jsonl `wm_held_rollout_scale` + `wm_held_cv_drift`; `img_rollout out='h'` **REMOVED** (isolation TBPTT still default-feat + `keep_c` on h). **No second GPU job.** Do not FALSIFY until val TM.
- **Watch:** jsonl held vs P61 silent ~6e-4 (already below); live gate TM vs P60/P61; val 1step→openloop vs **×0.772**; MV/DV ss/@H vs P26 ×0.97 / P61 ×0.91/×0.77; det_r vs P61 0.153 / P59 0.577; paired vs P58 **−6.24**. **One GPU job at a time.**

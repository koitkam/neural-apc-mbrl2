...
| P53 | 2026-08-30 | P3 PPO ratio clip vs frozen unfreeze-μ (`p3_mu_ratio_clip=0.2`) | LIVE `126f011` pid **75966** tmux `mbrl2_p53`. Step 4 `p3_sglogstd=True p3_logpclip=8 p3_muratio=0.2`. P1 iter **32** recon **0.0030** (best **0.0026** @24) gmatch **0.0005** FD MV/DV ×1.03/×1.02 skip **0**. wm-fid @30 EMA **5.998** @H r=+0.502 conv=1.00 gain_fid 0.915. Watch unfreeze 147. | 🔬 LIVE |
...
- **Liveness:** P1 STAGE 1. Iter **32** (32 jsonl rows, ~71 min). recon **0.128→0.0030** (best **0.0026** @24) gmatch **0.0005** (FD MV/DV **×1.03 / ×1.02**). iso 0 ss 0 skip **0**. last_ok **32** unlocked. wm-fidelity probe @**30**: EMA **5.998** (raw 6.474, saved `wm_best.pt`) H=1 r=+0.729 @H r=+0.502 conv=1.00 gain_fid=0.915 (not the TM gate). sps ~48. GPU ~11 GB. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Gate vs P52 last-ok 78 not yet. Do not KEEP/FALSIFIED as cascade until EXIT.

...
- **Liveness:** **GAIN-READY P1→P2 @82** (no extra-P1). Live probe worst **0.88@DV**. last_ok **82**. Skip-storm **1/2 @73** recovered. STAGE 2 DOB id, g FROZEN. P2 iter **131** recon **0.0017** dobg **0.014** last_ok **82**. wm-fid @**130**: H=1 r=+0.757 **H=55 r=+0.430** conv=1.00 drift=0.009 gain_fid **0.539** best_h=55/55 (not the TM gate; @H down vs @120 +0.596 / 0.634). **wm-fidelity-ES suppressed** @130: P2 remaining **36600** env_steps < 75396 — defer to natural P2→P3 (~5–6 iters). Live EMA mixed to **5.784**; best **6.845** @90 held. Still P2. sps ~197 post-probe. GPU ~11 GB. ~199 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Watch first P3 ent ≈ H(σ_init); unfreeze ~147. Do not KEEP/FALSIFIED as cascade until EXIT.

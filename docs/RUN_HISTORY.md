...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8`) | **LIVE P1** iter **21** (~48 min). pid **64705**, launch `d910ee2`. Step 4 **CONFIRMED** `p3_sglogstd=True p3_logpclip=8`. recon **0.101→0.0031** gmatch **0.172→5e-5** skip **0** last_ok **21 unlocked**. Iter 20 wm-fid H=55 r=0.484 (was 0.311@10) gain_fid=0.709 `wm_best` EMA 5.082. GPU ~11 GB. Env-free. Watch gate 82 then unfreeze ~147 vs P51 μ-rail. | 🔬 μ-side clamp after P51 cascade FALSIFIED. Do not launch. Do not stack critic knobs. |
...
- **P1 iter 21 (~48 min):** recon **0.101→0.0031** gmatch **0.172→5e-5** skip **0**. last_ok **21 unlocked**. Iter 20 wm-fidelity H=55 r=**0.484** (iter 10 was 0.311) `gain_fid=0.709` saved `wm_best.pt` EMA **5.082**. FD ratio mv **1.004** / dv **1.016** (teacher, not TM). ~2.2 min/iter after iter 1. No skip-storm/lock/wrap. Gate 82 ~2.3 h away; unfreeze ~147 is the A/B.

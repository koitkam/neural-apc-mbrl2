...
| P67 | 2026-09-01 | teacher `gain_match_len` auto = `wm_tf_horizon` (K=220; encode T lookback) | LIVE P1 iter **17** pid **187753** sha `9fbede0`. Teacher med ×1.01/×1.01 at K=220 skip 0 `t_wm` 134 s ~14.3 GB. | 🔬 compounding family vs P64 ×0.842 OL-vs-real |
...
- **LIVE P1 iter 17** (~41 min wall, 2026-09-01 13:53). Recon **0.063→0.0049** (median iters 5–17 **0.0061**; P64 **0.0067**). `kl=0` `jemb` 0.062→0.016 alive **1024**. skip **0/0**. last_ok **17 unlocked**. Teacher MV/DV median **×1.014 / ×1.010** at **K=220** (P64 at K=H **×0.997 / ×1.012**). Huber gml **3e-4**. Overshoot 0.46→0.047. held **1.6e-4** drift **0.011**. `t_wm` **133.5 s** vs P64 **96.7 s** (+38%; 4H `img_rollout`). nvidia **14359 MiB / 100%** during WM. Fidelity probe iter 10 `best_h=55/55` gain_fid **0.834** → `wm_best` (restore OFF; gain-blind WATCH). Isolation=0. **jsonl ×1 at K=220 is protocol, not val TM** — last-only Huber can match G once the prior has settled by H; compounding (OL-vs-real) is a val-only score. Do **not** FALSIFY/PROMOTE until freeze @~82 and val OL-vs-real vs P64 **×0.842**.
- **Watch:** P1 skip-storm; freeze vs last_ok 82 / 0.91@DV; val OL-vs-real vs P64 **×0.842** / P66 ×0.718; MV/DV vs P26/P64; paired vs P64 **−4.54**; det_r/pred_std (not a DOB experiment). Do not score actor if GAIN_NOT_READY. **No second GPU job.** No training-behavior patch this session (live logs readable; next lever waits on EXIT).
...
| Gain-match FD K = control H (last-only Huber) | P26–P66; val TM / decomp at `wm_tf_horizon=4H` | P67 auto K = `wm_tf_horizon` (LIVE pid **187753** P1 iter **17**) | PARTIAL as protocol gap — jsonl teacher **×1.01/×1.01 at K=220** (same as P64 at K=H). Val OL-vs-real pending (P64 ×0.842). Not S=H. Not a Huber reweight. |

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 74**. Probe70 still last **H=1 r=+0.746 / H=55 r=+0.160 conv=0.00** gain_fid **0.537** best_h=27. recon **0.0033@73 best** (P64 @70 **0.0035**). Teacher **×1.00 / ×0.98 @74**. persist_rel **0.57@70 → 0.049@74**. CPU @wm_best70 1step→OL **×0.272** (iter50 ×0.097) Markovian absdiff **0.256** (iter50 0.123). pid **242511**. | 🔬 LIVE P1 — recon at P64; cached H=55/conv still dead **@10–70**. Teacher/Markovian vs probe/cache diverging. Do not patch live. |
...
- **LIVE P1 iter 74 (~389 min, nvidia 11.7 GB, `t_wm` ~306 s, sps 19.7):** skip **0**. recon **0.0041@72 → 0.0033@73 new best → 0.0038@74** vs P64 **0.0035@70** (P64-class). Teacher jsonl 72 **×1.002 / ×0.969** → 73 **×0.984 / ×1.100** → 74 **×1.001 / ×0.978**. last_ok **74 unlocked**. persist_rel **0.57@70 → 0.077@72 → 0.058@73 → 0.049@74** (G pinning after dv-prbs inject; do **not** persist N+1). Rest-IC graph not captured. Next probe typically iter 80 (~02:26 CDT).

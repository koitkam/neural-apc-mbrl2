...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 75**. Probe70 still last **H=1 r=+0.746 / H=55 r=+0.160 conv=0.00** gain_fid **0.537** best_h=27. recon **0.0033@73 best** (P64 @70 **0.0035**) then **0.0041@75**. Teacher **×1.00 / ×1.02 @75**. persist_rel **0.57@70 → 0.040@75**. CPU @wm_best70 1step→OL **×0.272** Markovian |Δ| **0.256**. pid **242511**. | 🔬 LIVE P1 — recon at P64; persist collapsed; H=55/conv still dead **@10–70**. Do not patch live. |
...
- **LIVE P1 iter 75 (~392 min, nvidia 11.7 GB, `t_wm` ~308 s, sps 19.6):** skip **0**. recon **0.0033@73 new best → 0.0038@74 → 0.0041@75** vs P64 **0.0035@70** (P64-class best). Teacher jsonl 73 **×0.984 / ×1.100** → 74 **×1.001 / ×0.978** → 75 **×0.996 / ×1.020**. last_ok **75 unlocked**. persist_rel **0.058@73 → 0.049@74 → 0.040@75** (P73-class collapse after dv-prbs inject; do **not** persist N+1). Rest-IC graph not captured. Next probe typically iter 80 (~02:26 CDT).

...
| P66 | 2026-09-01 | per-seq `dob_ground` var-skip (`var>1e-3`; mixed ring KEEP) | **LIVE P1** tmux `mbrl2_p66` pid **181468** sha `38880f9`. Step 4 `device=cuda` `dob_ground=2` p2amp=1. **No** flush banner. | ⏳ GPU occupied. Watch P2 buf 327/327; val pred_std vs 1.93 vs P64. |
...
- **Step 4 CONFIRMED:** `device=cuda` bs=128. `[resolved-cfg] … isolation=0 ss_match=0 held_cv=True p1amp=0.2 p2amp=1 p3amp=1 compile=eager`. `dob_ground=2`. Rest-IC graph captured `N=6 T=128`. Isolation=0. **No** `[curriculum] P2 replay flush` (STAGE 1 @iter0 only). P1 iter 1–2 recon 0.075→0.031 skip 0. GPU ~14.2 GB.

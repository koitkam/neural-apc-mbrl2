...
| P46 | 2026-08-29 | P3 log_std reset (`DREAMER_P3_RESET_LOG_STD=1`) | P1 live iter 21 recon 0.0028 skip 0; GPU occupied | ⏳ one attributed change vs P45 identity |
...
- **P1 live (2026-08-29 ~01:06, iter 21, pid 24426, ~51 min):** recon 0.0968→**0.0028**, kl=0, jemb>0, gmatch ~1e-4, skip 0, zrank ~820/1024. Probe iter 10 gain_fid 0.788; iter 20 H=55 r=+0.462 conv=1.00 gain_fid 0.676, wm_best EMA 5.457. Injects firing. Matches P45 early-P1 (no observer regression from rest-IC default). **Do not launch a second GPU job.**
- **Watch:** `[p3] reset policy log_std residual`; first P3 entropy **not** −0.363 (toward init clamp ≈−1.52 → H≈−0.10). Warmup rtgt stays >0.015. After unfreeze: no actor_loss −300; no entropy-collapse; econ vs baseline **and** vs P45 −216 / P27 BC −59; MV not railed. Storm 2/2 or GAIN_NOT_READY → observer regression (rest_ic default). If entropy still at floor → **REVERT** reset (residual not in last Linear). If σ opens at P3 entry then re-collapses at unfreeze → P46 pid is **weights-only**; HEAD zeros Adam log_std-row moments (same lever). If σ opens but still limit-rides → do **not** stack critic knobs; next is DR-timing or DOB amp. **Do not** also change collect-DV / P3 `dob_active` until this A/B verdicts.
- **GPU-occupied (no second job):** complete `p3_reset_log_std` with Adam log_std-row moment zero; P3 collect streams `_posterior_step` (skip unused prior; identity vs `obs_step(obs=None)`). jsonl `jemb_loss` alias. Stale “warmup was removed” TrainConfig note fixed (warmup default 10 is live). Reviewed, not patched (would confound P46): collect still `dv=None` while training slices DV from obs; P3 `set_dob_active(False)` + `obs=None` leaves `d_t≡0` in the actor belief; P1/P2 BC is still NLL (pins σ).

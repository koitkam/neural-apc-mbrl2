...
- P32 launch: env-free `cursor/p28` (deterministic + eager + `storm_cap=2` + detonated-freeze last-ok restore). Do not pass leftover P26 env-vars (`DREAMER_COMPILE=0`, `DREAMER_RSSM_LATENT_TYPE`, `DREAMER_DOB_ENABLED`, `DREAMER_ACT_HIST_REQUIRED`). Opt-in compile is `DREAMER_COMPILE=1`. Live iter banner prints `kl`/`jemb`/`iso`/`ss` + `storm_cap=`. jsonl: `wm_ss_match_loss`, `wm_ss_match_wmean`, `wm_isolation_traj_loss`. `skip_invalid_p3` default-on. Isolation K-step decode is batched; overshoot MSE is vectorized; **gain-match FD** stacks baseline+per-input into one `img_rollout`; **P1/P2 random collect is numpy-only**; **P2 DOB Kalman scan is a closed-form mix**. Variance-normalized isolation waits until a **completed** det+eager P1 still shows DV ss ~×0.87 on a **healthy** freeze.

...
| Rest-IC last_only T-loop sequential kernel launches × 100 WM steps | P54 GPU-occupied (pid **95956** P1); **P55/P56** first GPU capture failed | SUPERSEDED (`gain_match_rest_ic_cuda_graph=True`; `make_graphed_callables` + GRU-grad canary; **P55 HEAD** capture **before** WM autocast; **in-loop never captures**; **P56 HEAD** `_RestICGraphModule` **parameters/named_parameters/buffers/named_buffers** + `wrapper.train(rssm.training)`; VRAM skip **not** pin; **P57** T=L not lookback) | PARTIAL — P55 pid autocast cache; **P56 pid 110246** nested function `parameters()` empty → `ValueError: grad requires non-empty inputs` → eager `t_wm_s` ~124 s. Next launch must judge `[gain-match] rest-ic CUDA graph captured`. |

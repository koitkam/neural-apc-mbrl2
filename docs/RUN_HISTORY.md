...
- **Liveness:** P1 STAGE 1. Iter **72** (72 jsonl rows, ~155 min). recon **0.128→0.0024** (best **0.0017** @46 held; mild bump from 0.0018 @71, skip 0). gmatch **0.0001** (FD MV/DV **×0.99 / ×0.99**). iso 0 ss 0 skip **0**. last_ok **72** unlocked. wm-fidelity probe @**70**: H=1 r=+0.734 **@H r=+0.593** conv=1.00 drift=0.010 gain_fid **0.909** best_h=55/55 (not the TM gate). EMA **6.243** (raw 6.856) @**70** → `wm_best.pt` (was 5.998 @30). dv-prbs inject @70 only. Next probe ~80. sps ~48. GPU ~11 GB. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Gate vs P52 last-ok 78 not yet (~6 iters). Do not KEEP/FALSIFIED as cascade until EXIT.

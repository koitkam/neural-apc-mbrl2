...
- **P1 (iters 1–3, still running):** Iter 1 teacher MV/DV **×0.671 / ×−1.30** (P68-class, not tautological held-K ×1); tail **×0.669 / ×−1.29** `ol_tail_loss=1.555` `t_wm` **133 s** (P68 ~98 s — extra 165-step tail) gpu_peak **11.3 GB**. Iter 2 recon **0.072** gmatch 2.74 tail_loss **1.37** DV **×−0.43**. Iter 3 recon 0.072 teacher **×0.664 / ×−1.25** tail **×0.664 / ×−1.24** skip **0**. Tail loss not exploding. Watch freeze vs P64 **0.91@DV**; jsonl `*_ol_tail_*` pin toward ×1 with K-teacher (not stuck/explode); val 1step→OL / OL-vs-real toward P64 ×0.85 / ×0.84; ss toward ×0.93; `actor_experiment_valid`. Do **not** score actor if GAIN_NOT_READY.

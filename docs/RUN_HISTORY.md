...
- **Step 4:** `[resolved-cfg] latent=deterministic restore_p2=False gain_match=1 dob_ground=2 dob_reg=0 isolation=0 ss_match=0 iso_dcv=off n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 lock=20 huber_per_in=True gmatch_settle=-1 gmatch_rest=True p3_sigreset=False bc_mean=True p3_sglogstd=True compile=eager`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. `[run] pinned eval modules at launch`. STAGE 1 @iter0. P1 iter **35** recon **0.0025** (min **0.0025** @35; was 0.070 @1) bc **0.300 MSE** (range 0.238–0.380, never NLL) skip **0** last_ok **35 unlocked** teacher FD MV/DV ~×1.005/×0.987. No wrap/lock/storm (P50 wrap/lock @56). `t_wm` ~125 s. GPU ~11 GB.

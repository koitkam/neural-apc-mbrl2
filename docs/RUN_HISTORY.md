...
| P66 | 2026-09-01 | per-seq `dob_ground` var-skip (`var>1e-3`; mixed ring KEEP) | **LIVE P1** tmux `mbrl2_p66` pid **181468** sha `38880f9`. Iter **31** recon **0.0041** skip **0** last_ok **31**. Teacher MV/DV ×1.01/×1.01. Fidelity iter 30 H=55 r=0.492 gain_fid=0.964. Step 4 `device=cuda` `dob_ground=2` p2amp=1. **No** flush banner. | ⏳ GPU occupied (~14.3 GB). Watch P2 buf 327/327; val pred_std vs 1.93 vs P64. HEAD cleanup (not this pid): dead `expert_bc_p3_adaptive_scale` / `early_stop_p1_min_sf_drop_frac` REMOVED. |
...
- **Step 4 CONFIRMED:** `device=cuda` bs=128. `[resolved-cfg] … isolation=0 ss_match=0 held_cv=True p1amp=0.2 p2amp=1 p3amp=1 compile=eager`. `dob_ground=2`. Rest-IC graph captured `N=6 T=128`. Isolation=0. **No** `[curriculum] P2 replay flush` (STAGE 1 @iter0 only). P1 iter **31** recon **0.0041** skip **0** last_ok **31** (P64/P65-class). Teacher jsonl MV/DV ×1.01/×1.01. Fidelity iter 30 `best_h=55` r=0.492 `gain_fid=0.964` (gain-blind score; restore OFF). GPU ~14.3 GB. Hypothesis is P2-only (Stage-1 DOB suppressed).
...
- **HEAD cleanup (not in pid 181468):** `expert_bc_p3_adaptive_scale` / `DREAMER_EXPERT_BC_P3_ADAPTIVE_SCALE` **REMOVED** (never read by `_realsim_actor_critic_step`; smoke tested a formula that was not in the P3 branch). `early_stop_p1_min_sf_drop_frac` / `DREAMER_ES_P1_MIN_SF_DROP` **REMOVED** (SF-era mid-check never applied; RSSM `sf_loss≡0`). dist_match + dob_ground share `_DIST_TARGET_VAR_GATE=1e-3` (identity). Training graph of P66 unchanged.

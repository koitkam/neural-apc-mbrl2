...
| P78 | 2026-09-03 | TSSM gain-match rest-IC continues KV via `prev_state=` (P77 Markovian FALSIFIED) | LIVE `run_p78_prevkv` tmux `mbrl2_p78` pid **261143** sha `9c8727a` (code `b9089c4`). Env-free `device=cuda` bs=16. `[gain-match] TSSM OL continues rest-IC KV-cache (pos=128+K=55)`. Overshoot/held still Markovian. | 🔬 LIVE — freeze vs P64 **0.91@DV**; jsonl teacher vs 5-level TM |

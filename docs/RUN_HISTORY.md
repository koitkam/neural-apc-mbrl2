...
| P60 | 2026-08-31 | env-free `gain_match_step<=0` auto = `wm_tf_step_frac` (0.4) | LIVE **P1** ~iter 65. Teacher jsonl MV/DV **×0.990 / ×0.996** at Δu=0.4 (iters 1–65 ~×0.95–1.02; pins G(0.4)). recon 0.0023 gmatch 0.0002. skip **0/0**. last_ok **65 unlocked**. `wm_best` iter **30** (gain-blind). Rest-IC graph **captured N=6 T=128**. `t_wm` ~97 s. pid **144896**, sha `bdb2e49`. **No second GPU job.** | LIVE |
...
### p60 (`run_p60_tmstep`, branch `cursor/p28`) — LIVE P3: teacher Δu auto = val TM `wm_tf_step_frac`
- **One attributed GPU change vs P59:** `_resolve_gain_match_step`: dataclass sentinel **0.0** → `wm_tf_step_frac` (default 0.4). Explicit `DREAMER_GAIN_MATCH_STEP=1.0` keeps the P59 teacher. Isolation still off. Rest-IC lookback. μ-ratio 0.2 freeze-forever. Serve graph KEEP. Eager. Det latent. DOB on. tmux `mbrl2_p60`, pid **144896**, sha `bdb2e49`.
- **Not:** clip+realized-Δu (p136-class follow-up if 0.4 still leaves a TM gap; rest 0.6+0.4 sits on the cube edge). Overshoot `sample=True→False` (already identity: `latent_noise=0` + `cont_gain_deterministic_roll`). Critic knobs. μ-refresh. S=H. Leftover `DREAMER_*`.
- **Step 4 CONFIRMED:** `[gain-match] targets … step=0.4`. `[gain-match] rest-ic N=6 L=128 settle=128 rest_ic_len=0`. **`[gain-match] rest-ic CUDA graph captured N=6 T=128`**. `[resolved-cfg] … gmatch_settle=-1 gmatch_step=0.4 gmatch_rest=True gmatch_rest_L=128 gmatch_rest_cg=True p3_muratio=0.2 p3_murefresh=0 compile=eager`. `run_plan.json` `gain_match_step=0.4`. `device=cuda` bs=128. Proc environ: CVD=0 + PYTORCH_CUDA_ALLOC_CONF only. P1 iter 1 recon 0.075 gmatch 0.0018.
- **P1→P2 GATE PASS @iter 82 GAIN-READY.** last_ok **77** (wrap lock @77 recon 0.196; recovered; freeze restored 77 because live recon 0.011 > 5× best 0.0015). Live probe **MV ×1.10/@H ×1.11, DV ×0.86/@H ×0.92** (`worst=0.86@DV`, unbiased, not_noisy). Teacher jsonl at last-ok 77 **×0.986 / ×0.977** and at gate live 82 **×1.021 / ×1.014** — Huber FD pins G(0.4); the plant TM on the same freeze is still 0.86@DV. skip **0/0**. `wm_best` iter **90** (P2, g frozen; gain-blind). Rest-IC graph **released**.
- **P2→P3 GATE PASS @iter 136** (reward_mtp median 0.896, p2_ext 0). Serve CUDA graph captured B=1. Critic warmup 10; **unfreeze 147**. rscale **FREEZE 2.731 KEEP**.
- **LIVE P3 (~iter 200, 2026-08-31 16:00):** ent **−0.164** (first P3 **−0.190**; not P50/P53 open-σ −0.107; HELD vs yank). clip **0.60@147 → 0.12@200** (P53-class). logp_std **0.50–0.59** (not P55 walk). μ-ratio mean **~0.98**. rtgt **0.067→0.0014** (P58 also 0.065→0.006 and still won econ — do **not** stack critic knobs). critic_pred_target_r **~0.98** (not inverted). `t_collect` **1.86 s** / `t_ac` **1.79 s** (graph speed KEEP vs P58 4.18/3.52). ret_w noisy (−96→+226). Val TM / paired econ pending EXIT. **No second GPU job.**
- **HEAD (not in pid 144896):** overshoot/isolation gather idx cache + one `obs_win`; PRBS gain-match start arange cached; `_prior_c_from_net` skip discarded `randn`; `cached_zeros_bd`; Huber teacher-G dict-cache. **This session:** `initial_state` reuses zero/`one-hot` ICs (`cached_onehot_z`; dedicated `_init_*` attrs so they do not alias `img_rollout` fills); KL free-bits `clamp_min(float)` (no 0-d tensor); TD-λ `α^{0..T-1}` cache (`_lambda_discount_weights`; P3 2–4×/inner). Collect CUDA-graph still `copy_`s a clone. Identity speed. CVD="" `_smoke_rssm` + `_smoke_tssm` green.
- **Watch:** val TM vs P26 ×0.97 / vs freeze 0.86@DV; compounding open-loop; paired econ vs P58 −6.24; rscale KEEP; μ-ratio freeze; rtgt collapse is the known P58-class canary not a new critic-knob trigger. If val TM still short of P26: clip+realized-Δu (one attributed change). Do not stack critic knobs.

### Sim-adaptive leftovers (env-free multi-sim; do not promote plants yet)

Already unitless / derived: held-rollout `win` clamped to `(K-1)//4` (identity 8 at K=55; K=15 was dead); isolation TBPTT `max(8, round(K/3.5))` (chunked `img_rollout`, same `h`-only `keep_c` cuts); `gain_match_len` / isolation len = H; **`gain_match_settle_len` default `-1` (P44 storm 2/2 REVERT; `0` still auto-H A/B)**; TM probe settle is `wm_tf_horizon(H)=max(80,4H)`; **rest-IC encode L = lookback** (`gain_match_rest_ic_len=0`; P57 EXIT **REVERT**; `-1` A/B last `max(K, 2τ/sr)`); **`gain_match_step<=0` auto = `wm_tf_step_frac`** (P60; test_sim 0.4; explicit `1.0` A/B's the P59 teacher); isolation sample window `max(seq_len, K+1)` (test_sim seq_len ≥ H unchanged); P1/P2 main WM sample `max(seq_len, K+1)` (overshoot/gain-match; P3 stays seq_len); gain-match open-loop `K` not truncated to `T-1`; `skip_storm_last_ok_recon_ratio=5` (recon/recon); `skip_storm_p1_cap_after=2` (storm count; **P32 GPU-confirmed** continue 1/2 @iter 53). Storm-1 continue now **keeps** `p1_gate_max_ext_steps` (P32 CAPPED 0.71@DV when it was closed); storm 2 still `_force_p1_cap_at`. Isolation / overshoot / held are **RSSM-interface** (rssm + tssm; SF no-op). HEAD not in P33 process; `p1_gate_wm_ema_min=1.5` (fidelity mix); inject EVERY = f(buffer lap) (test_sim 20/10); inject N = f(n_mv, n_dv) (test_sim 5/2/2/3); isolation settle = 24 **per input** (test_sim 24+24 / cap 48 settle-only; distillation 96+24 / cap 120). Isolation_buf holds only those settle episodes (not MIMO PRBS). Isolation settle is a whole-episode hold at a stratified level (`action_std=0`); DV step is MV-action-isomorphic (`isolated_level × span/2`). Opt-in: `gain_match_huber_beta<=0` → median |tgt| only when `gain_match_huber_per_input` is False (P43 default is per-input `|tgt_ij|`). Gain-ready band `[0.80, 1.30]` and wm-fidelity mix weights are unitless TrainConfig (were os.environ). `rssm_latent_type` default **deterministic** (P26 proven; P29 env-free drop of the leftover env-var). `compile_mode` default **eager**; `DREAMER_COMPILE` / `DREAMER_COMPILE_MODE` are in `ENV_OVERRIDES` (MODE was CLI-only). Intra-op threads and inductor workers scale with `sched_getaffinity` (host-adaptive). `skip_invalid_p3` default ON (unitless validity flag, not a plant unit). Stage-1 skips unused DOB prior-core when `dob_active=False` **and** skips `apply_dob` clone + ground/reg (`d_t≡0`; HEAD, not P33 process). SNR summary/WARN is **measured CV+DV** (`summary_scope`; constant aug channels excluded). Isolation jsonl splits detached `wm_isolation_mv_traj` / `wm_isolation_dv_traj`. jsonl `wm_gain_match_{mv,dv}_ratio` = mean `G_pred/G_tgt` (HEAD; P44 pid Huber-only; teacher FD, not TM). TM / postprior / smoke share `wm_tf_horizon` (was inlined `max(80,4H)`). Rest-IC encode `rollout_observed last_only return_feats=False` + Stage-1 `_posterior_step` (P46 pid; P45 pid 16548 still stacked). `gain_match_rest_ic` default **True** (P45 EXIT PROMOTE).
...
| Teacher FD `gain_match_step=1.0` vs val TM `wm_tf_step_frac=0.4` | P59 EXIT (`run_p59_headserve`) | SUPERSEDED (P60 `_resolve_gain_match_step`: sentinel `<=0` auto = `wm_tf_step_frac`) | YES — jsonl Huber `*_ratio` ~×1.01 at Δu=1; live gate 0.88@DV / val TM MV ×0.876 DV ×0.840 at Δu=0.4. GRU nonlinear ⇒ G(1)≠G(0.4). Rest 0.6+1.0 outside TM cube. Held-rollout ~9e-5 / SS converges=1.0 so compounding is a wrong DC attractor, not h-drift. Explicit `DREAMER_GAIN_MATCH_STEP=1.0` A/B. |

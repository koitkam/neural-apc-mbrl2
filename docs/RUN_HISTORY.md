...
- **Liveness:** still **LIVE** pid **75966**, tmux `mbrl2_p53`, GPU ~11 GB. ~212 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. jsonl through **P3 iter 207**. last_ok **82 unlocked**. Approaching P52 ES iter **209** (P52 ema **−2231**; P53 **−119**). Do not KEEP/FALSIFIED as cascade until EXIT.
  ...
  - **Unfreeze 147 (A/B canary — no cascade verdict yet):** ent **−0.101 HELD** through **207**. rscale **2.302 FREEZE**. actor_loss bounded (|aloss|≲0.20). Unclipped logp_std **held ~0.51–0.58** (not P52 **80+**). clip_frac **~0.20**. mvv **56.8k@191** then **11.1k@196 / 267@201 / 0@206** (spike was a collect window, not a walk to 813k). ret_w **−85@194 → −91@207**. ema **−139@194 → −119@207** (recovering; not P52 **−2231@209**).
  - **Passed P52 delayed-rail window @165–169:** P52 std **80.8 / 100.7**; P53 **0.61 / 0.55**, still **~0.54@207**.
  - **Watch (not a verdict):** rtgt **0.0016@194 → 0.0017@207** (plateau ~0.002, **not** collapsed to **1e-4**). mvv spike **recovered**. logp_std held. Next: whether rtgt stays ~0.002 vs 1e-4; whether ema keeps recovering vs P50 **−56**; P52-class ES window ~209.
- **Hypothesis:** in-support `(u−μ)/σ²` SGD cannot walk μ onto the tanh rail once `π_new(a)/π_snap(a)` leaves `[0.8, 1.2]`. **So far:** unclipped std held; 56.8k mvv was transient; rtgt low but plateaued; ema recovering. Do not call KEEP/FALSIFIED as cascade until EXIT.
- **Watch:** late-P3 rtgt / mvv / ema vs P52 ES @209. Do not call KEEP/FALSIFIED as cascade until EXIT.

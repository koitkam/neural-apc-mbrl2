...
| P50 | 2026-08-29 | P1/P2 BC MSE-on-μ (`bc_mean_only=True`) | **LIVE** extra-P1 iter **103**. Gate 82 FAIL 1.37@MV; extra-P1 detonate @89; last_ok **88 locked**; gate 94 FAIL 0.69@DV noisy. recon **0.629→0.011** recovering. skip 1. **bc MSE never NLL.** Approaching gate ~104 / cap. Do not launch. | 🔬 extra-P1 FALSIFIED as DV lever; do not score actor until GAIN-READY |
...
- **LIVE (2026-08-29 20:20 CDT, extra-P1 iter 103):** wrap-unlock ×2 GPU-confirmed (56→58, 70→73). **bc MSE never NLL** (2× logged 0.000). skip **1** (one huge-grad, not a storm). last_ok **88 locked** (extra-P1 recovered basin stays locked — wrap-unlock did not fire). recon **0.629→0.0109** (still 7.5× best). z_alive 941→1004. Probe@100 best_h=0/55 gain_fid 0.573 (detonated basin). Steps 899140 / ext target 904752 / cap 929884 → **gate ~iter 104 then likely CAPPED**. GPU ~11 GB. Do **not** score `bc_mean_only`. Extra P1 FALSIFIED as DV lever (P41).
  - **Gate 82 FAIL** `worst=1.37@MV` (MV ×1.37/@H ×1.33; DV ×0.82/@H ×0.86; spread×2.4). Probe recon **0.0283** = 19× best (wrap-adjacent). Teacher FD ~×1. EXTEND to 829356.
  - Extra-P1 **detonated iter 89** recon **0.6286** (430×) gmatch 0.068 alive 941; iter 90 skip 1 zrank 326/1024.
  - **Gate 94 FAIL** `worst=0.69@DV` (MV ×1.02/@H ×1.10; DV ×0.69/@H ×0.79; spread×6.1 signflips=3 `not_noisy=False`) on detonated weights. EXTEND to 904752 (cap 929884 → next gate ~104). `wm_best` @80 gain_fid 0.913.
- **Code follow-up (HEAD, not P50 pid):** leftover expert move-law + CLI extras → TrainConfig + `ENV_OVERRIDES` (`2232112`). **Gain-probe last-ok when live recon >5× best** (wrap-adjacent MV ×1.37 RCA); restores live after probe; identity when recon healthy. Smoke helpers. **Do not launch.**
- **Watch:** next gate ~iter 104 / CAPPED. If CAPPED GAIN_NOT_READY then `[p3-skip]` KEEP unless detonated-freeze last_ok **88** re-probes READY. Remaining leftover is env-only paths — do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.

...
- **LIVE (2026-08-28 03:58, iter 91, pid 4176670, `73a5116`):** device=cuda bs=128 det+eager. Isolation/ss jsonl keys **absent** (HEAD emits 0). Survived P30 (iter 19) and P32 (iter 53) windows. **Skip-storm 1/2 @iter 65** (same full-BPTT gain-match class as P32). Recovered through iter 83 (recon 0.0022). Iter 75 **not** the P1→P2 gate (env_steps 696620 < p1 753960; isolation-off skipped settle seed). **Iter 82 first gate: `FAIL (not_plateaued) wm_ema_best=6.541`** — that score is the **iter-10 warmup spike** (`[wm-best]` only ever saved at iter 10). Gain probe **did not run**. Extra-P1 **silent detonation iter 84**: recon **0.482**, gmatch 0.085, zrank 407, gnorm **2.51**, skip still 7 (skip-storm 2 silent). last_ok **83**. Recovering: iter 91 recon **0.013** gnorm 0.97 last_ok still 83. Iter-80 (healthy) r@H55 **+0.495** `gain_fid=0.932`; iter-90 (post-detonation) r@H55 **+0.419** `gain_fid=0.626`. Next gate ~iter 94 (829356). t_wm **~99 s**. GPU ~17 GB. **No second GPU job.** Isolation-off **did not** eliminate storms **or** extra-P1 detonation.
- **HEAD (not in pid):** P1 gate floor is **recent EMA max** (`_p1_fidelity_local_plateau`), not return-to-`wm_score_ema_best`. Extra P1 waiting on a warmup spike is how a healthy original-budget observer detonated. Gain-match `last_only=True, out='obs'`. jsonl `wm_score_ema*`. Overshoot `out='obs'` / held `out='h'`. jsonl isolation/ss keys emit **0** when teacher off. `[resolved-cfg] iso_dcv=off` when teacher off (live pid still prints `iso_dcv=True`, inert).
...
- P1/P2 random collect is **numpy-only** (no `torch.inference_mode` / SF `a_history`). P3 on-policy still streams. P2 DOB Kalman scan is a closed-form mix (same recurrence; host-adaptive T×T cap `clamp(GPU_mem/1500, 4MiB, 64MiB)` — A10 24GB still **16 MiB**). Stage-1 skips `apply_dob` clone + ground/reg. CV-std tensor cached. P1 skips unused `agent_finetune_loss` when `reward_scale_loss_p1=0`. Last-ok snapshot `copy_` reuse. Isolation is abs MSE (inv-var helpers deleted). Gain-match FD uses `img_rollout(..., last_only=True, out='obs')` (last-step Huber ≡ `decode(stack[:, -1])`; no unused K-stack; sequential `img_step` fallback **REMOVED**). Overshoot uses `out='obs'` and held-rollout `out='h'` (no unused F-stack; last_only materializes `out` once after the K-loop). P1 gate floor is **recent EMA max** (not return to warmup `wm_score_ema_best`; P40 RCA). jsonl emits `wm_score_ema*`. jsonl always emits `wm_gain_match_loss` / isolation/ss keys (0 when teacher off). Banner `n_grad_skip` is cumulative inner-step skips; skip-storm window is `grad_skip_history` (cleared on restore).
...
| P1 gate return-to-global `wm_ema_best` (`not_plateaued` EXTEND) | P40 LIVE iter 82 | SUPERSEDED (recent-floor `_p1_fidelity_local_plateau`) | YES — warmup spike 6.541 blocked the gain probe at healthy iter 82; extra-P1 silent-detonated iter 84 (gnorm 2.51, skip-storm silent). Extra P1 already FALSIFIED as DV lever |

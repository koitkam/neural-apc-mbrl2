...
- **LIVE (2026-08-26 18:50, tmux `mbrl2_p31`, GPU ~19.6 GB, HEAD launch `4ffe11f`):** env-free confirmed. P1 iter **16** / ~344k of 754k. `kl=0` `jemb=0.040` recon **0.0415** (recovered after iter-11 dv-prbs blip recon 0.71 / 2 skips — **not** a skip-storm; window needs >5). gmatch 0.015 iso 0.005 alive 1022. t_wm ~152 s eager. `wm_best` still iter 10 (restore OFF). Watch iter 18–25 (P30 detonated @19). Remaining P1 ~69 iters ≈ 3.2 h. **No second GPU job.** HEAD now batches gain-match FD through `img_rollout` (same last-step Huber; MIMO-width no longer multiplies sequential K-loops) for the *next* launch.
...
- P31 launch: env-free `cursor/p28` (deterministic + eager + `storm_cap=2`). Do not pass leftover P26 env-vars (`DREAMER_COMPILE=0`, `DREAMER_RSSM_LATENT_TYPE`, `DREAMER_DOB_ENABLED`, `DREAMER_ACT_HIST_REQUIRED`). Opt-in compile is `DREAMER_COMPILE=1`. Live iter banner prints `kl`/`jemb`/`iso`/`ss` + `storm_cap=`. jsonl: `wm_ss_match_loss`, `wm_ss_match_wmean`, `wm_isolation_traj_loss`. `skip_invalid_p3` default-on. Isolation K-step decode is batched; overshoot MSE is vectorized; **gain-match FD** stacks baseline+per-input into one `img_rollout` (same Huber mean; RSSM eager). P31 GPU process does **not** include the FD batch (`4ffe11f`). Variance-normalized isolation waits until a **completed** det+eager P1 (not a cap-at-iter-18 freeze) still shows DV ss ~×0.87.

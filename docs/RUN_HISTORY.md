...
- **Liveness:** **ES @262** `entropy_collapse_window` 30/30 below thr=**−0.083** (latest **−0.101**, adv_corr=**0.047**). jsonl **262** rows. `best.pt` / `final.pt` iter **166** det **−46.2**. pid **75966** still alive — **held-out val IN PROGRESS** (wm-ss-diag done; seed 10000+). GPU dropped ~11 GB → **~0.8 GB** (val). Do **not** launch P54 until val EXIT and GPU free. Do not KEEP/FALSIFIED as cascade until Step 5 val axes.
  - **P1:** skip-storm **1/2 @73** recovered; **GAIN-READY P1→P2 @82** live worst **0.88@DV**. last_ok **82**.
  - **P2:** STAGE 2 @82. wm-fid @130 H=55 r=+0.430; **fidelity-ES suppressed**. best EMA **6.845 @90**.
  - **P2→P3 PASS @136** mtp **0.909**. rscale **FREEZE 2.302**. First P3 warmup ent **−0.163**. Unfreeze 147: ent **−0.101 HELD** through ES (stop-grad yank-block). Unclipped logp_std spiked **6.71@149** then **held ~0.47@262** (not P52 **80+** / **813k** mvv). PPO clip fired then eased. mvv transient **56.8k@191**. rtgt plateau **~0.0016** (not 1e-4). ema **−145@262** vs P52 **−2231@209**.
  - **Passed P52 delayed-rail @165–169 and P52 ES @209** still training until **262**. Same ES *reason* as P52 (H(σ_init)≈−0.101 already below thr −0.083 + adv_corr <0.05 for 30 iters) — **not** a μ-rail std explosion.
- **Hypothesis:** PPO ratio clip vs frozen unfreeze-μ stops in-support μ-walk. **Train canary:** logp_std held. **Cascade / val:** pending Step 5 (best.pt **166** is early-unfreeze class like P50/P51).
- **Watch:** val TM MV+DV ss/@H, det_r, critic, paired econ vs P50 **−56/−104**, P51 **−72/−111**, P52 **−377/−87**. Do not launch until val done.
...
| P3 in-support μ-walk (\|logp\|<8 SGD) | P52 EXIT; **P53 LIVE** (`run_p53_muratio`) | LIVE (`p3_mu_ratio_clip=0.2` vs frozen unfreeze-μ) | YES — passed P52 rail @165–169 **and ES @209** still LIVE. logp_std **~0.51@209**. best.pt **166 det −46.2**. mvv transient. rtgt ~0.0015. No cascade verdict until EXIT. Opt out `DREAMER_P3_MU_RATIO_CLIP=0`. |

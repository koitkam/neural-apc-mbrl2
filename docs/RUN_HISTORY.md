...
| P49 | 2026-08-29 | original-P1 wrap last-ok unlock | LIVE **P3** @253. Gate **GAIN-READY** @82 worst **0.82@MV** (DV 0.88). last_ok **82** (not freeze-24). First P3 ent **−0.283**; rscale **2.10 KEEP**; unfreeze 147 bc 0.002→1.24 mvv 667k; rtgt 0.048→0.0001; ema **−2064** vs expert **−64** | ❌ freeze-24 **FALSIFIED as cascade lever** (val pending). Wrap-unlock untested. Do not launch |
...
### p49 (`run_p49_wrapunlock`, branch `cursor/p28`) — LIVE P3: wrap-unlock FALSIFIED as cascade lever
- **One attributed GPU change vs P48 pid:** original-P1 wrap recovery unlocks last-ok (`extra_p1=False` and recon back below 20×) so snapshots resume. Extra-P1 recovered basin stays locked (P40). Not a knob. Env-free. `sim_param_randomization_pct=-1` (auto; identity bake).
...
- **Live (2026-08-29 ~16:10 CDT, pid 41994, launch `4ef9bcb`):** **P3** iter **253** (P2→P3 @136 rmtp 0.913 PASS; DR ON). Gate **GAIN-READY** @82 worst **0.82@MV** (MV 0.82/@H 0.84, DV 0.88/@H 0.94). last_ok **82** unlocked — **not** P48 freeze-24. No detonated-freeze. Skip-storm 1/2 @64–65 unlocked (wrap-unlock **untested**). `wm_best` 90. **Actor VALID and FAIL (live).** First P3 ent **−0.283** (σ_min_ratio 1.2; same as P48, not P46 −0.101). Critic warmup 137–146: mvv **0**, raw −74/−188, cvv 5k–18k, bc 0.001, actor_loss ~0. rscale **FREEZE 2.10 KEEP** @146. Unfreeze **147**: bc **0.002→1.24**, actor_loss **−4.9**, logp **+0.7→−59**; **148** actor_loss **−12.9** ent −0.238; **151** mvv **667k** raw **−4475** rtgt 0.013. rtgt **0.048→0.0001**. Ent wandered −0.28→−0.11@200 then re-pinned **−0.283** @221. ema **−2064**, expert_det **−64**, agent−expert **−2573**. Same cascade class as P45–P48.
- **ROOT (Control / ML, live):** freeze-24 is **not** the actor confound. Late-P1 freeze (82 ≈ live gate 0.88@DV) still cascades on first unfreeze REINFORCE. P2 `bc_loss` **−0.87…−1.13** = peaked Gaussian NLL pinning σ_min. Freeze rscale KEEP. Do not stack critic knobs. Do not promote `p3_reset_log_std`. Wrap-unlock KEEP as lock hygiene (untested on P48-class recovered wrap). **HEAD (not this pid):** `bc_mean_only=True` — P1/P2 MSE-on-μ like P3 (`DREAMER_BC_MEAN_ONLY=0` restores NLL). **Next GPU after EXIT: env-free P50.** Watch P2 `bc_loss`>0; first P3 ent ≈ H(σ_init) not −0.283. Do not launch while pid 41994 holds the GPU.
...
| Last-ok original-P1 wrap never unlocks | P48 freeze (`run_p48_collectdv` pid 34706); **P49 LIVE P3** wrap@64 then skip-storm | SUPERSEDED (original-P1 wrap recovery unlock; extra-P1 stay locked); skip-storm unlock **GPU-confirmed**; wrap-recovery path still untested; **FALSIFIED as cascade lever** | YES — P48 live 0.89@DV @82; freeze restored 24 → 0.81@DV. P49 last_ok **82** (skip-storm unlock). First P3 ent **−0.283** = P48 σ_min (P2 NLL `bc_loss` ≈ −1). Freeze-24 was not the actor confound. Next: P1/P2 `bc_mean_only`. Do not raise lock_ratio. |
...
| Freeze last_ok 24 (wrap-era observer) as actor cascade confound | P48 EXIT; **P49 P3 live** (`run_p49_wrapunlock`) | **FALSIFIED as cascade lever** (val pending) | YES — P49 last_ok **82** ≈ live gate 0.88@DV (no detonated-freeze). Warmup mvv **0** then unfreeze 147 still bc 0.002→1.24, mvv 667k, rtgt 0.048→0.0001, ent −0.283. Same class as P45–P48. Freeze rscale **2.10 KEEP**. Do not stack critic knobs. Next: P1/P2 BC pins σ_min (P46/P47 reset-at-P3-start too late). |

...
| P42 | 2026-08-28 | last-ok lock after silent recon spike (20×) | LIVE extra-P1 iter 85; lock @67 last_ok 66; gate 82 gain-probe 0.01@DV (live detonated) | ⏳ GPU |
...
- **Liveness (11:22 CDT):** LIVE **P1 extra-P1** iter **85**. tmux `mbrl2_p42`, pid **4192010**, launch HEAD `72f7b48` (workspace HEAD `130a2f7` not in pid). GPU ~15.7 GB. Isolation=0 / ss=0; **no leftover `DREAMER_*`**; CVD=0. **Lock GPU-confirmed KEEP as mechanism.** `[wm-last-ok] locked iter 66` on spike iter **67** recon **0.3066** (170× best 0.0018) skip **1**. last_ok **stays 66 through 85** (P41-class overwrite did not happen). Iter 81–82 recon **0.89–0.92** gmatch ~1.11; extra-P1 iter 83–85 gmatch **collapsed 1.11→0.0015** with recon still detonated (0.30→0.16) skip **1** locked=true. `wm_last_ok.pt` **absent on disk** — freeze uses in-memory `p1_last_ok_sd` (`last_ok_locked=True` restores even if freeze-iter recon looks recovered). **First gate iter 82 GPU-confirmed `gain-probe`** (recent-floor KEEP; not `not_plateaued`) on **LIVE detonated** weights: **FAIL GAIN_NOT_READY worst=0.01@DV** (MV 2.01; noise spread_x9.3 signflips=9 unbiased=False). That is **not** last_ok 66 quality. EXTEND +75396 → 829356 (cap 929884). Extra-P1 on detonated live g (already FALSIFIED as DV lever). Second gate ~iter 94; **CAPPED ~iter 104 is the lock test** (restore 66, reset AdamW, `[p1→p2] detonated-freeze gain-probe`). Wrap 5–10× still untested (jump was 170×). **No second GPU job.** HEAD (not in pid): persist last-ok at lock; jsonl `p1_recon_best` + `wm_gain_match_{mv,dv}_loss`.
...
- Abs Huber gain-match is count-equal per input but **scale-unequal** when `|G_mv|≫|G_dv|`. P42 `[gain-match] targets` |MV|=2.62 |DV|=0.51 (~5.1×, same class as abs isolation drowning). Isolation-off moved that drowning into the sole DC supervisor. jsonl `wm_gain_match_mv_loss` / `wm_gain_match_dv_loss` (HEAD, not P42 pid) makes it visible. Do **not** revive relative Huber. After P42 EXIT (freeze must restore 66), this is the next attributed SysID lever — not a P42 mid-run stack.
...
| Extra-P1 last-ok overwrite after silent recon spike | P40 EXIT iter 98 (83→104); P41 EXIT original-P1 iter 64 (56→64 then 104); **P42 GPU-confirmed lock @67** | KEEP lock (`skip_storm_last_ok_lock_ratio=20`; recon-only) | YES — P41 recovered <5× overwrote pre-spike last-ok. P42: spike iter 67 recon 0.3066 skip 1 locked last_ok **66**; held through extra-P1 iter **85** (gate 82 live-detonated 0.01@DV; freeze+re-probe at cap still pending). Skip-storm unlock untested. Wrap 5–10× untested (jump was 170×). |
| Last-ok RAM-only until storm/freeze | P42 live (`wm_last_ok.pt` absent after lock) | SUPERSEDED (`_persist_last_ok_ckpt` on lock; HEAD, not pid 4192010) | YES — crash after lock would lose the freeze snapshot. Same blob storm/freeze already saved. Objective unchanged. |

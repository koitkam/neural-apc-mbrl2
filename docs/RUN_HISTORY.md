...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 11**. Probe10 **H=1 r=+0.128 / H=55 r=+0.122 conv=0.00** gain_fid **0.476** (P76 +0.428/+0.310 conv=0 gain_fid 0.736; P64 +0.584/+0.309 conv=0.50 gain_fid 0.827). Teacher jsonl **×1.01 / ×0.95 @10** (DV overgain @8 recovered). recon **0.305@9 best → 0.386@11** (P76 @10 **0.010**). skip **0**. pid **242511**. | 🔬 LIVE P1 — probe r ~3–4× worse than P76/P64; conv=0 at first probe (P73 also 0 @10; P76 uniquely held 0 through P2). Watch freeze vs P64 0.91@DV |
...
- **LIVE P1 iter 11 (~59 min, nvidia 11.7 GB, `t_wm` ~306 s steady, sps 19.7):** skip **0**. Teacher jsonl 8 **×0.945 / ×1.376** → 9 **×0.997 / ×1.029** → 10 **×1.008 / ×0.951** → 11 **×0.972 / ×1.178** (DV overgain @7–8 recovered by 9–10). recon **0.305@9 best → 0.384@10 → 0.387@11** vs P76 **0.010@10** / P64 **0.005@10** (~40×). persist_rel **0.175@8 → 0.377@9 → 0.553@10 → 0.754@11**. Rest-IC graph not captured.

...
| P59 | 2026-08-31 | env-free HEAD collect/val CUDA-graph + P3 warmup (knobs = P58) | LIVE P1 ~iter 57 identity vs P58. Wrap@55 recon 0.461 storm **1/2** recovered last_ok **54** (P58 wrap@50 then storm 2/2 @61). recon ~0.002; gmatch ×1; `t_wm` **99 s** vs 102; `t_collect` 3.28 s. `wm_best` iter **30**. Graph rest-ic **captured N=6 T=128**. `device=cuda`. `[resolved-cfg] … gmatch_rest_L=128 p3_muratio=0.2 p3_murefresh=0 compile=eager`. AccumulateGrad warn **in this pid**; HEAD flatten/warn-arm **not in pid**. No leftover `DREAMER_*`. | LIVE |
...
- **HEAD (not in pid 126332):** P3 fused encode + collect/val CUDA-graph serve + P3 warmup under bf16 autocast. **P59 LIVE** (`run_p59_headserve`).

### p59 (`run_p59_headserve`, branch `cursor/p28`) — LIVE: HEAD collect/val CUDA-graph identity vs P58
- **One attributed GPU change vs P58 pid:** HEAD fused P3 encode + frozen-RSSM collect CUDA-graph + val reuse + P3 warmup under bf16 (`7685484`). Knobs identical to P58 (env-free lookback, freeze-forever μ-ratio). tmux `mbrl2_p59`, pid **134383**, `CUDA_VISIBLE_DEVICES=0`. Started ~09:16 CDT 2026-08-31.
- **Not:** leftover `DREAMER_*` (proc environ: CVD=0 + PYTORCH_CUDA_ALLOC_CONF only), μ-refresh N, critic knobs, compounding attack, S=H.
- **Step 4 CONFIRMED:** `[gain-match] rest-ic N=6 L=128 settle=128 rest_ic_len=0`. **`[gain-match] rest-ic CUDA graph captured N=6 T=128`**. `[resolved-cfg] … gmatch_rest_L=128 gmatch_rest_cg=True p3_muratio=0.2 p3_murefresh=0 compile=eager`. `device=cuda` bs=128. Pinned eval at launch. GPU identity max\|Δfeat\|=0 (scratch; reset under inference_mode).
- **Watch:** `t_collect`/`t_ac` vs P58 4.2/3.5 s once P3; val TM/econ ≈ P58 ×0.806 / −6.24 vs −102. Do not launch a second GPU job.
- **P1 ~iter 25 (2026-08-31 10:01 CDT):** identity vs P58 P1 iter 25 (recon **0.0032** vs 0.0039; gmatch MV/DV **×1.00/×1.00** vs ×0.99/×1.00; `t_wm` **99.9 s** vs 102 s; `t_collect` 3.28 vs 3.30 s — P3 graph not in play). `wm_best` iter **20** (P58 also refreshed at 20). Rest-IC graph captured. One-shot AccumulateGrad UserWarning at first WM backward (HEAD now arms warn-off until g freeze; not in this pid). No leftover `DREAMER_*`.

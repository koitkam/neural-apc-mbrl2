...
| **Disturbance (Kalman, dynamic)** | `r_detrended` (→1) | **P26** (mbrl2) | **det_r 0.68** | P64 det_r **0.352** (P62 0.364). P73 det_r **0.630** pred_std **0.533 vs 1.93** (amp still dead; weaker TM ×0.72 — not a champ swap). P64 amp closer (pred_std **0.608 vs 1.93**) but not P26. |
| **Critic** | `critic_rew_to_tgt_var` (>0.015) | none healthy | — | P73 val `critic_r` **+0.713** PASS (P72 +0.694; P68 +0.699; P64 +0.705); P3 rtgt still collapses (P73 med **0.0035**). Freeze rscale KEEP. |
| **Actor** | econ vs baseline + vs expert BC | **P64** (mbrl2) | paired **−4.54 vs −94** | Env-free P2 amp=P3. 9/9 `all_pass`. **P73 −4.91 vs −125 VALID 9/9** (mv_viol **2.75**, TM ×0.72) — closest econ since P68, not a champ swap. **P68 −3.90 vs −85 VALID 9/9** (mv_viol **1.92 vs 0.33**, TM **×0.69**). P72 **−22.71 vs −92.72 VALID 9/9** (mv_viol **9.70**, last_ok **15**). P53 stays the μ-ratio-clip RCA. |
...
### p74 (`run_p74_gcvskip`, branch `cursor/p28`) — LIVE: decoder gain-c → CV skip (zero-init Linear)
- **One attributed GPU change vs P73:** RSSM/TSSM `decode` adds `gain_cv_skip(c[:G])` onto CV (`index_add_`). Zero-init ⇒ identity to P73 decode at step 0. Persist KEEP. **No new TrainConfig field.** Env-free: **no leftover `DREAMER_*`**. tmux `mbrl2_p74`, pid **221013**, sha `4489c43`, out-dir `output/test_sim/run_p74_gcvskip`.
- **Step 4 CONFIRMED:** `device=cuda` bs=128 `wm_train_T=128` horizon=55 compile=eager. `[cont-latent] GAIN-ONLY` gain_dim=2. `[resolved-cfg] isolation=0 gmatch_len=55 gmatch_step=0.4 gmatch_clip=True gmatch_rest=True held_cv=True gcvskip=True p1amp=0.2 p2amp=1 p3amp=1`. Rest-IC graph captured `N=6 T=128`. Huber baseline = rest last-obs CV. Process environ: `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF` only (no `DREAMER_*`). Iter 1 teacher **×0.636 / ×−1.04** skip **0** persist_rel **0.441** gskip **0.0006** recon **0.433** nvidia **~15.3 GB**. **One GPU job.**
- **LIVE 2026-09-02T15:54Z P2 iter ~131** (pid **221013**, nvidia **~4.0 GB** after rest-IC graph **released** at freeze; skip **0**). **P1→P2 @82 GAIN-READY** worst **0.81@MV** / DV **0.87** (band [0.8, 1.3]; P64 **0.91@DV** / P73 **0.83@DV**). last_ok **82 unlocked**. Teacher freeze MV **×1.001** DV **×0.971**. `gain_cv_skip` **dead through freeze**: rms max **0.0046 @5**, **0.00387 @82**, P2 locked **0.003867** (g frozen). Freeze W **[+0.00254, −0.00484]** vs last_ok-19 **[+0.00397, −0.00512]** — shrinking, ~0.2% of |G_mv| **2.62**. persist_rel **0.103 @82** (P73 **0.037 @81**; still bounced 0.05–0.87 in P1, not pinned). Wrap @20 recovered. Fidelity @80 gain_fid **0.996** `wm_best` (gain-blind mix; restore SKIPPED). **P2** buf **327/327** recon **0.004** dobg **0.19–0.27** `dob_d` **0.066** (P64 P2 last **0.210 / 0.063**; P73 last **0.180 / 0.086**). P2 remaining **36.6k** env-steps (defer ES; natural P2→P3). Skip is a teacher-pin **no-op** — Huber ~0 after iter 5, skip never left ~0.004. Compounding not tested until val **1step→OL**. jsonl ×1 ≠ val TM. Actor not yet in P3 — do not score. **Do not** start a second GPU job. **Do not** change training while this pid is live.
- Watch P2→P3 unfreeze vs P64 **147**; val **1step→OL / OL-vs-real** vs P73 ×0.761 / ×0.743 and P64 ×0.85 / ×0.84; freeze **0.81@MV** is the weak edge of the band (P64 0.91@DV); `actor_experiment_valid`. If val TM ≈ P73, skip is FALSIFIED as compounding (identity-plus-tiny-W).
...
| jsonl `gain_cv_skip_rms` every WM inner | **P74 GPU-occupied** (pid **221013** P1) | SUPERSEDED (`_wm_need_logged_aux`; default True) | YES — 1×2 weight; not in `wm_total`. Same last-logged-inner gate as Huber MV/DV. This pid still computes every inner. Training graph unchanged. |

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P2** after **CAPPED GAIN_NOT_READY −0.46@MV / 0.74@DV** last_ok **104**. Orig-P1 @82 **FAIL −1.06@MV**; extra-P1 @94 **FAIL −0.29@MV / 0.80@DV** (2nd extra-P1 **worsened** freeze). Probe100 **H=1 +0.567 / H=55 +0.299 conv=0**; P2@110 H=1 **+0.810** H=55 **+0.262 conv=0**. recon freeze **0.0026**. jsonl @104 **×1.00 / ×1.02** ≠ 5-level. pid **242511**. | ❌ CAPPED — vs P64 **0.91@DV**. Do **not** score actor (`skip_invalid_p3`). |
...
- **LIVE P2 iter 111+ (~549 min, nvidia 11.7 GB, P2 `t_wm` ~41 s, sps ~138):** P1 **CAPPED** @104. g FROZEN, DOB training (`dobg` 0.0007–0.010). recon **0.0024–0.0041**. skip **0**. last_ok **104 unlocked**. No detonated-freeze (freeze recon **0.0026** vs best 0.0021 = 1.2×, not 5×). `wm_best_restore_at_p2=False`. Do **not** score actor. HEAD P78 `prev_state=` is **not** this pid — do **not** launch P78 while GPU occupied. Next: P2→P3 likely `[p3-skip]` (`skip_invalid_p3`).

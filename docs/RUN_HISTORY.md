...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 23**. Probe20 **H=1 r=+0.325 / H=55 r=+0.145 conv=0.00** gain_fid **0.261** (P76 +0.589/+0.543 conv=0 0.644; P64 +0.706 conv=0.50 0.734). Iter19 wrap recovered skip **0**. recon **0.073@22 best → 0.109@23**. Teacher **×1.00 / ×0.98 @23**. pid **242511**. | 🔬 LIVE P1 — conv=0 @10 and @20. H=55 r dead. OL `img_rollout` drops TSSM kv_cache (do not patch live). |
...
- **LIVE P1 iter 23 (~121 min, nvidia 11.7 GB, `t_wm` ~307 s, sps 19.6):** skip **0**. Teacher jsonl 21 **×0.993 / ×1.012** → 22 **×1.008 / ×0.903** → 23 **×1.001 / ×0.975** (pinned after iter19 wrap). recon **0.096@21 → 0.073@22 new best → 0.109@23** rebound. last_ok **23 unlocked** recon_best **0.073**. persist_rel **0.33@21 → 0.50@22 → 0.40@23**. Rest-IC graph not captured. Probe 20 still last fidelity. Next typically iter 30.

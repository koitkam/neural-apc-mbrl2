...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 60**. Probe60 **H=1 r=+0.682 / H=55 r=+0.293 conv=0.00** gain_fid **0.882** best_h=1/55 drift **0.341**. recon **0.011@60** (best **0.0057@53**; P64 @60 **0.0022** ~4.8×). Teacher **×1.00 / ×1.01 @60**. persist_rel **0.34**. Wrap 0.147@56 recovered skip 0. pid **242511**. | 🔬 LIVE P1 — H=1 beat P64; H=55/conv still dead **@10–60**. Do not treat gain_fid 0.882 as health (best_h=1). Do not patch live. |
...
- **LIVE P1 iter 60 (~314 min, nvidia 11.7 GB, `t_wm` ~307 s, sps 19.6):** skip **0**. Wrap recovered: recon **0.147@56 → 0.022@57 → 0.014@58 → 0.010@59 → 0.011@60**. Teacher jsonl 58 **×0.994 / ×1.034** → 59 **×0.992 / ×1.023** → 60 **×1.002 / ×1.005**. last_ok **60 unlocked**. persist_rel **0.50@57 → 0.36@58 → 0.35@59 → 0.34@60**. Best recon still **0.0057@53** vs P64 **0.0022@60** (~4.8× live; ~2.6× vs best). Rest-IC graph not captured. Next probe typically iter 70.

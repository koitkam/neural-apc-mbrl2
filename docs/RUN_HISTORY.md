...
- Abs Huber gain-match is count-equal per input but **scale-unequal** when `|G_mv|≫|G_dv|`. P42 `[gain-match] targets` |MV|=2.62 |DV|=0.51 (~5.1×, same class as abs isolation drowning). Isolation-off moved that drowning into the sole DC supervisor. jsonl `wm_gain_match_mv_loss` / `wm_gain_match_dv_loss` (HEAD, not P42 pid) makes it visible. Do **not** revive relative Huber. P42 EXIT freeze-66 was GAIN_NOT_READY 0.75@DV (val DV ×0.737, det_r 0.124). This |tgt| drowning is the next attributed SysID lever.

...
- Next launch after P29: env-free `cursor/p28` (deterministic + eager). Do not pass leftover P26 env-vars (`DREAMER_COMPILE=0`, `DREAMER_RSSM_LATENT_TYPE`, `DREAMER_DOB_ENABLED`, `DREAMER_ACT_HIST_REQUIRED`, `gain_match_coef` / `dob_ground_coef` auto-enable). Opt-in compile is `DREAMER_COMPILE=1`. Live iter banner (HEAD) prints `kl`/`jemb`/`iso`/`ss` so a leftover latent type **and** a starved SS-match are visible in tmux. jsonl: `wm_ss_match_loss`, `wm_ss_match_wmean`, `wm_isolation_traj_loss`. `skip_invalid_p3` default-on (cap-time gain probe so GAIN_NOT_READY cannot hide behind `not_plateaued`).

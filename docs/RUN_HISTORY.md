...
| P50 | 2026-08-29 | P1/P2 BC MSE-on-μ (`bc_mean_only=True`) | **LIVE** P1 iter **29**. Env-free pid **49389**. recon **0.077→0.0028** (now 0.0045) skip **0** last_ok **29** unlocked. **bc +0.27…+0.41 MSE every iter.** Probe@20 `best_h=55`. No leftover `DREAMER_*`. Do not launch. | 🔬 live; watch P2 `bc_loss`>0; first P3 ent ≈ **−0.08** not −0.283 |
...
### p50 (`run_p50_bcmean`, branch `cursor/p28`) — LIVE: P1/P2 BC MSE-on-μ
...
- **LIVE (2026-08-29 18:13 CDT, P1 iter 45):** recon **0.077→0.0015** (iter 42 best; iter 45 **0.0023**) kl 0 jemb 0.006 gmatch ~0 skip **0** alive 1024. **`bc_loss` +0.27…+0.41 MSE every iter (never negative; 45/45 pos)** vs P49 P2 NLL ≈ −1. Teacher FD MV/DV **×0.99 / ×0.98** (not TM). last_ok **45** unlocked; no wrap / no storm. Fidelity: @20 gain_fid 0.692 → `wm_best`; @30 **0.928** EMA 6.084 (still `wm_best`); @40 H=55 r=+0.419 gain_fid **0.546** (no new best — gain-blind dip). GPU ~11 GB. First P3 ent target ≈ **−0.08**, not floor **−0.283**.
- **Code follow-up (no GPU this session):** leftover expert move-law (`DREAMER_EXPERT_MOVE_FRAC` / backoff / econ / loop / ridge / feas / opt) → TrainConfig + `ENV_OVERRIDES` (identity 0.30 / 0.12 / 0.02 / 0.6 / 0.05 / 0.02 / 1.0 / 40 / 0.1; dual-read leftover; explicit cfg wins). CLI-only extras `single_run` dropped (`DREAMER_PMPO_{ALPHA,BETA}` / `POLICY_*` / `GRAD_CLIP` / seed counts / plant-derived arch) now in `ENV_OVERRIDES` (identity; plant size still set first). Smoke PASSED. **Do not launch.**
- **Watch:** skip-storm ~iter 50–65 if it fires; P2 `bc_loss` stays **>0**; first P3 entropy ≈ **−0.08** not −0.283; freeze rscale ~2; GAIN-READY typically ~iter 82; do not train P3 on GAIN_NOT_READY. Remaining leftover is env-only paths/device + rest-IC encode=lookback A/B — do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.
...
| Last-ok original-P1 wrap never unlocks | P48 freeze (`run_p48_collectdv`); **P49 EXIT** wrap@64 then skip-storm | SUPERSEDED (original-P1 wrap recovery unlock; extra-P1 stay locked); skip-storm unlock **GPU-confirmed**; wrap-recovery path still untested; **FALSIFIED as cascade lever** | YES — P48 live 0.89@DV @82; freeze restored 24 → 0.81@DV. P49 last_ok **82**. Val MV ×0.816/×0.830 DV ×0.867/×0.924 det_r 0.632. First P3 ent **−0.283**. Seed kpi **−714**. Next: P1/P2 `bc_mean_only`. Do not raise lock_ratio. |
| P1/P2 expert-BC Gaussian NLL | P45–P49 valid P3 | SUPERSEDED (`bc_mean_only=True` default; MSE-on-μ like P3) | YES — NLL pins last-Linear log_std to σ_min (P49 first P3 ent −0.283; P2 bc_loss −0.87…−1.13). P3 `expert_bc_p3_loss` already MSE-on-μ. P46/P47 reset undoes the pin after the fact (FALSIFIED as sufficient). Opt out `DREAMER_BC_MEAN_ONLY=0`. HEAD, not P49 pid. |
...
| Original-P1 wrap last-ok unlock | P49 EXIT (`run_p49_wrapunlock`) | KEEP as lock hygiene; **FALSIFIED as cascade lever**; wrap-recovery unlock **untested** (skip-storm fired first) | YES — skip-storm 1/2 @64–65 unlocked; last_ok **82**. Same P3 cascade as P45–P48. |

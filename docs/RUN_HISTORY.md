...
- **LIVE (2026-08-27 11:56 UTC, tmux `mbrl2_p33`, GPU ~19.4 GB, pid 4146603, launch `1cfb530` / keep-ext `8205f4b`):** env-free det+eager CONFIRMED. `[resolved-cfg] latent=deterministic restore_p2=False gain_match=1 dob_ground=2 isolation=1 ss_match=3 n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 compile=eager`. Train-start `device=cuda bs=128`. `[gate-budget]` p1_ext_cap **175924**. Through **iter 77**: recon **0.0041**, skip **0**, alive 1020. Wrap blip 46–48 recovered without skip-storm. Iter 50 probe `best_h=0/55` did **not** overwrite `wm_best` (iter **40** `best_h=55/55`; iter 60/70 `best_h=55/55`). **Past P32 storm window** without skip-storm. **Iter 75 quality gate EXTENDED** (`FAIL not_plateaued`, `wm_ema_best=5.206`): +75396 steps to **829356** (cap 929884) — keep-ext path is live even without a storm (P32 CAPPED here because continue had closed ext). Next gate ~iter 87. **Do not lower skip threshold.** **No second GPU job.** GPU-occupied HEAD (not in this pid): Stage-1 skip `apply_dob` clone + ground/reg (`d_t≡0`); isolation/overshoot/held on RSSM-interface (TSSM too); CV-std tensor cache. Do not retune `wm_overshoot_max_starts` / `gain_match_max_starts`.

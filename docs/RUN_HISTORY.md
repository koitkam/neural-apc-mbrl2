...
| **Actor** | econ vs baseline + vs expert BC | **P50** (mbrl2) | paired **−56 vs −104** | First GAIN-READY val beat (9 pairs; seed kpi −74). Unfreeze still yanked σ. P45–P49 valid P3 all lost to baseline. Do not score GAIN_NOT_READY freezes. |
...
| P51 | 2026-08-30 | P3 REINFORCE stop-grad log_std (`p3_stop_grad_log_std=True`) | **LIVE P3** iter **348** (~3h58). pid **54815**, launch `d6a4511`. Gate **GAIN-READY** 0.86@MV / 0.88@DV @82; last_ok **82 unlocked**. P2→P3 PASS 0.922 @136. First P3 ent **−0.125**. Unfreeze **147**: ent **HELD −0.141→−0.101**. Delayed dip **−0.282** @315 (σ_min ≈−0.283) then **recovered −0.106** @348 (back to H(σ_init); 23-iter <−0.25 streak 296–318, ES did **not** fire). Cascade continues: actor **−259**, gnorm **833**, ret_w **−2065**, rtgt **0.0001**. rscale freeze **2.17 KEEP**. Past P50 ES iter 286. Env-free. Do not launch. | 🔬 stop-grad KEEP as unfreeze-yank + non-sticky-floor; **failing as cascade lever**. Watch EXIT. Do not stack critic knobs. |
...
### p51 (`run_p51_sglogstd`, branch `cursor/p28`) — LIVE P3: P3 stop-grad log_std
- **One attributed GPU change vs P50 pid:** `p3_stop_grad_log_std=True` (HEAD default `d6a4511`). P3 REINFORCE `log_prob_of` / entropy detach log_std so σ stays at `policy_init_log_std`; μ still trains. Opt out `DREAMER_P3_STOP_GRAD_LOG_STD=0`. Also carries P50 `bc_mean_only=True` and HEAD gain-probe last-ok (`f2c9092`, not in P50 pid). Env-free. tmux `mbrl2_p51`, pid **54815**, `CUDA_VISIBLE_DEVICES=0`. Started ~21:47 CDT.
- **Not:** leftover `DREAMER_*`, `p3_reset_log_std`, extra critic knobs, rest-IC settle=`wm_tf_horizon`. Do **not** launch a second GPU job.
- **Step 4:** `[resolved-cfg] … isolation=0 … gmatch_settle=-1 gmatch_rest=True p3_sigreset=False bc_mean=True p3_sglogstd=True compile=eager`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. GPU ~11 GB.
- **P1 / wrap / skip-storm:** (1) lock@**47** recon 0.4725; skip-storm **1/2** @52 restore 47. (2) wrap lock@**66** recon 0.318; wrap-unlock @**69** recon 0.013; last_ok advanced 66→74. Storm still **1/2**. Gate **GAIN-READY** 0.86@MV / 0.88@DV @**82**; last_ok **82 unlocked**. P1→P2 wm_best SKIPPED (skip-storm already restored last-ok 47). P2 bc **MSE** never NLL; recon ~0.0027 dobg live. Wrap-recovery unlock **GPU-confirmed P51** (also P50). Skip-storm unlock **GPU-confirmed P51**.
- **P2→P3:** PASS @**136** reward_mtp median=0.922 (max=3.00, n=5); p2_extension=0. STAGE 3 @136. Critic warmup 10 iters; rscale **FREEZE 2.170 KEEP**.
- **First P3 entropy (warmup):** iter 137 **−0.125** ≈ H(σ_init) (P50 **−0.107**; P45–P49 pinned **−0.283**). μ-only KEEP as σ-open.
- **Unfreeze 147 (the A/B canary):** last warmup 146 ent **−0.141** `actor_logp_std` **0.54** actor **−0.010** bc 0.002 rtgt 0.046. Iter 147: ent **−0.101 HELD** (P50 169: **−0.107→−0.268**), `actor_logp_std` **0.54→38** (μ-rail logp variance, not σ training). **Stop-grad KEEP as unfreeze-yank block.** **Failing as cascade / durable-σ lever** (μ REINFORCE still rails). `actor_logp_std` is not policy log_std.
- **Cascade @348 (~3h58, still LIVE):** entropy dipped **−0.282** @315 then **recovered −0.106** @348 (≈ H(σ_init)). 23-iter <−0.25 streak **296–318** — ES did **not** fire (floor is σ_min-relative, not −0.25). Actor **−259** @345 / **+61** @348, gnorm **833**; `actor_logp_std` ~150; bc ~1.9; rtgt **0.0001**; ret_w **−2065**; ret_ema **−1943**. Past P50 ES iter 286 because σ is not stuck at floor. rscale freeze **2.17 KEEP**. last_ok **82 unlocked**.
- **Watch:** EXIT / ES / return-cascade. Do not edit val stack while pid 54815 is live. Do **not** launch. Do **not** implement the next μ-rail fix until EXIT.
- **GPU-occupied leftover (not a P51 launch):** `derive_horizon` / sim `reset()` now `horizon_formula_knobs()` / `ic_randomization_knobs()` (TrainConfig 4.0/120 / ON/0.6). `derive_episode_length` now `episode_formula_knobs()` (HEAD `a555dfc`; TrainConfig 20 / 500 / 4000 then leftover `DREAMER_EPISODE_*`; test_sim L=1220; `SIM_EPISODE_LENGTH` still wins). Same class as `gpu_probe_knobs()`. Env-free numbers unchanged. Did **not** switch APCEnv to `RuntimeSetpointConfig.auto_derive`. Smoke `tools/_smoke_rssm.py` `CVD=""` `PYTHONPATH` **PASSED**. Not in P51 pid.
...
- Reward-engine leftovers `objective_integral_{coef,windup,leak}` / `obj_auto_*` / `objective_{penalty,reward}_clip` (sentinel `<0` = adaptive) / `objective_violation_rate_coef='auto'` / `objective_penalty_sat_mode='tanh'` / `objective_feasibility_{cap,scale}` **and remaining `derive_auto_weights` formula knobs** (`obj_auto_{mv,cv}_violation_base` / rank_decay / mv_over_cv / typical_cv 0.10 / move_* / reversal / rate_coef_{div,min,max} / differentiable_depth / reward_clip_floor) **and `objective_use_normalized=True`** are TrainConfig + `ENV_OVERRIDES` (identity; dual-read leftover `OBJECTIVE_*` / `OBJ_AUTO_*` / `OBJ_USE_NORMALIZED` when DREAMER unset; `_explicit_fields` beats leftover). `obj_auto_cv_over_econ_ratio=0` follows margin. APCEnv jitter is dataclass **0.15 / 0.20** (`runtime_setpoint_{bounds,target}_jitter_frac`; leftover `RUNTIME_SETPOINT_*_JITTER_FRACTION`; do **not** `auto_derive` 0.25). `resolve_integral_config` / `compute_objective_components` take `cfg`. Auto-weights cache on quiet steps. Dead unused `estimate_reward_scale` **REMOVED**.
...
| Original-P1 wrap last-ok unlock | P49 EXIT; **P50 EXIT**; **P51 LIVE** wrap@66–69 | KEEP as lock hygiene; **FALSIFIED as cascade lever** (P49); wrap-recovery unlock **GPU-confirmed P50** @57–58/@70–73 and **P51** @66–69; skip-storm unlock **GPU-confirmed P51** @52 | YES — P51 lock@66 recon 0.318 skip banner 98 (not a new storm); unlock @69 recon 0.013. Storm still 1/2. Extra-P1 stay locked (P40). |
...
| P3 REINFORCE stop-grad log_std (μ-only) | P51 LIVE P3 (`run_p51_sglogstd`, `d6a4511`) | KEEP as unfreeze-yank block + non-sticky floor (147 held; 315 −0.282 then 348 −0.106); **not yet EXIT** as cascade lever | PARTIAL — delayed rail-geometry dip, ES did not fire (23-iter <−0.25 streak). Cascade continues: actor −259, ret_w −2065, rtgt 0.0001. rscale 2.17 KEEP. Do not stack critic knobs. Do not promote `p3_reset_log_std`. |

...
| P37 | 2026-08-27 | abs isolation (P33 recipe) | LIVE P1 iter 54 skip 0 (past P32 storm @53); recon 0.0038 | ⏳ GPU; no second job |
...
### p38 GPU CAPPED (`run_p38_isodcv`, tmux `mbrl2_p38`, launch `42bc7c2`, pid 4167186) — match-at-`g_min` starved MV; still in P2
- **Launch KEEP:** env-free det+eager `storm_cap=2`. `[resolved-cfg] iso_dcv=True mv=['0.317'] dv=['1.67']` isolation=1 ss_match=3 gain_match=1. Pre-iso tgt MV −2.48 DV 0.47; **post-seed re-resolve** MV −2.98 DV 0.52. device=cuda bs=128 GPU ~17.7 GB. pid **4167186**. No leftover `DREAMER_*`. Live process has **no** scale floor (HEAD floor is for P39).
- **Attributed change vs P37:** isolation settle `Δu_i ∝ 1/|G_i|` (not a loss reweight). Applied edge |Δu| MV **0.190** (P37 0.60), DV **1.0** (P37 0.60). Logged DV 1.67 is the multiplier, not |Δu|.
- **P1 FAIL (P36 class).** gmatch **stuck 1.30** (P37 →0.0008 by iter 17). iso/ss ~0.14/0.11 (~8× weaker than P37 iter-1 ss 1.00). Skip 4 from iter 15. Storm **1/2 @iter 45** restored last-ok **13**. Storm **2/2 @iter 48** CAPPED P1 at 570960. Cap-time GAIN_NOT_READY **0.01@DV** (MV 1.12 in `[0.8,1.3]`). Detonated-freeze restored last-ok iter **47** (recon 0.14 vs last-ok best 0.027; gmatch still 1.30). `[p1→p2] WM warm-restore SKIPPED`. P2 iter 55 recon 0.11 `g` frozen `dobg` 0.28. `[p3-skip]` expected. Val not yet.
- **ROOT (SysID / signal):** match-at-`g_min` equalized |ΔCV| by **shrinking** the strong-|G| isolation step (MV |Δu| 0.19 vs P37 0.60). Isolation teacher too weak to pin G → gain-match Huber stuck at the untrained |tgt| (~1.30 = abs Huber of MV ~3 and DV ~0.5). Same class as P36 weak teacher (iso reweight), different lever (data amplitude). Storm-1 last-ok 13 never learned gain; post-restore iter 47 became last-ok and was the freeze.
- **HEAD (not in this pid):** `dcv_match` **scale floored at 1.0** — never shrink below op-band; still boost weak-|G| to the cube. test_sim expected edge |Δu| MV **0.60** / DV **1.0**. `run_plan.isolation_dcv_scales.min_scale=1`. Opt out `DREAMER_WM_ISOLATION_DCV_MATCH=0`.
- Do **not** launch a second GPU job (P38 P2 still on the A10). Do not train P3 on this freeze. Do not revive relative Huber. Do not re-add inv-var. Do not lower skip threshold.

### Sim-adaptive leftovers (env-free multi-sim; do not promote plants yet)

Already unitless / derived: isolation TBPTT `max(8, round(K/3.5))` (chunked `img_rollout`, same `h`-only `keep_c` cuts); `gain_match_len` / isolation len = H; isolation sample window `max(seq_len, K+1)` (test_sim seq_len ≥ H unchanged); P1/P2 main WM sample `max(seq_len, K+1)` (overshoot/gain-match; P3 stays seq_len); gain-match open-loop `K` not truncated to `T-1`; `skip_storm_last_ok_recon_ratio=5` (recon/recon); `skip_storm_p1_cap_after=2` (storm count; **P32 GPU-confirmed** continue 1/2 @iter 53). Storm-1 continue now **keeps** `p1_gate_max_ext_steps` (P32 CAPPED 0.71@DV when it was closed); storm 2 still `_force_p1_cap_at`. Isolation / overshoot / held are **RSSM-interface** (rssm + tssm; SF no-op). HEAD not in P33 process; `p1_gate_wm_ema_min=1.5` (fidelity mix); inject EVERY = f(buffer lap) (test_sim 20/10); inject N = f(n_mv, n_dv) (test_sim 5/2/2/3); isolation settle = 24 **per input** (test_sim 24+24 / cap 48 settle-only; distillation 96+24 / cap 120). Isolation_buf holds only those settle episodes (not MIMO PRBS). Isolation settle is a whole-episode hold at a stratified level (`action_std=0`); DV step is MV-action-isomorphic (`isolated_level × span/2`). Opt-in: `gain_match_huber_beta<=0` → median |tgt| (P28 still uses β=1). Gain-ready band `[0.80, 1.30]` and wm-fidelity mix weights are unitless TrainConfig (were os.environ). `rssm_latent_type` default **deterministic** (P26 proven; P29 env-free drop of the leftover env-var). `compile_mode` default **eager**; `DREAMER_COMPILE` / `DREAMER_COMPILE_MODE` are in `ENV_OVERRIDES` (MODE was CLI-only). Intra-op threads and inductor workers scale with `sched_getaffinity` (host-adaptive). `skip_invalid_p3` default ON (unitless validity flag, not a plant unit). Stage-1 skips unused DOB prior-core when `dob_active=False` **and** skips `apply_dob` clone + ground/reg (`d_t≡0`; HEAD, not P33 process).
...
- Isolation/ss-match is **abs MSE only**. Inv-var A/B **REMOVED**. Default **`wm_isolation_dcv_match=True`** with **scale floor 1.0** (P38 RCA): isolation settle `Δu_i ∝ 1/|G_i|` (WM-norm |G|, `level*scale` clip ±1, never shrink below op-band; equal-|G| no-op). P38 CAPPED (no floor, pid 4167186, still P2): applied edge |Δu| MV **0.19** / DV **1.0**; storm 2/2 @iter 48 CAPPED GAIN_NOT_READY **0.01@DV**; gmatch stuck 1.30. HEAD expected test_sim edge |Δu| MV **0.60** / DV **1.0**. Audit `run_plan.isolation_dcv_scales.min_scale` + `edge_du_*`. Pre-iso resolve is **scales only**; post-seed always re-resolves Huber. P37 EXIT unscaled abs: val MV ×0.98 DV **×0.69** det_r 0.37 `[p3-skip]`. Do **not** re-add `wm_isolation_var_norm`. Do not launch a second GPU job.
...
- `run_plan.json → config` is rewritten after gain-match resolve. `[resolved-cfg]` prints `iso_dcv=` **and** `min_scale=` **and** `mv=`/`dv=` multipliers **and** `edge_du_mv=`/`edge_du_dv=`; `run_plan.isolation_dcv_scales` is the P38/P39 audit trail (inv-var knob removed). Pre-rewrite dump is dataclass defaults (P29 plan still shows `rssm_latent_type=categorical` / `gain_match_coef=0` even though training auto-enabled grounding).
...
| Extra P1 via keep-ext to pin DV DC-gain | P33 EXIT; P37 EXIT | FALSIFIED as DV lever (keep-ext KEEP as mechanism) | YES — P33 val DV ×0.66; P37 0.68→0.72 then detonated; val DV ×0.69 |
...
| Isolation |ΔCV| excitation `Δu_i ∝ 1/|G_i|` match-at-`g_min` (no floor) | P38 CAPPED (`run_p38_isodcv`, `42bc7c2`, still P2) | FALSIFIED as P1 form (HEAD floor 1.0) | YES — MV edge \|Δu\| 0.19 vs P37 0.60; gmatch stuck 1.30; storm 2/2 @iter 48; freeze 0.01@DV (MV 1.12). Same weak-teacher class as P36. |
| Isolation |ΔCV| excitation with **scale floor 1.0** | HEAD (not in P38 pid) | IN FLIGHT (default ON) | PARTIAL — P38 falsified shrink-the-strong-channel. Floor keeps P37 MV SNR and still boosts DV to cube. Next GPU after P38 EXIT. |

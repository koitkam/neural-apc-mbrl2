...
| P62 | 2026-08-31 | held-rollout decode-CV stationarity (`out='obs'` + CV late−early; no new knob) | **LIVE** tmux `mbrl2_p62` pid **160203** sha `6841f5c`. P1 iter **60**. skip **0** through P61's storm window 56–57. held **~7e-5** vs overshoot **0.032**. Teacher ×1.03/×1.01 `clip_frac` **0.083**. recon 0.0026. HEAD: jsonl `wm_held_cv_drift`/`scale` + `img_rollout out='h'` **REMOVED**. | 🔬 Do not FALSIFY from P1. Do not call skip-0 a storm-preventer (P60 also 0 storms). Watch val 1step→openloop vs ×0.772; paired vs P58 |
...
### p62 (`run_p62_heldcv`, branch `cursor/p28`) — LIVE: held decode-CV stationarity
...
- **Step 4 CONFIRMED:** `device=cuda` bs=128. `[held-rollout] decode-CV stationarity (P62; not GRU h)`. `[gain-match] … step=0.4 clip_realized=True`. `[resolved-cfg] … gmatch_clip=True gmatch_step=0.4 held_cv=True compile=eager`. Rest-IC graph captured `N=6 T=128`. Isolation=0. Process environ: `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF` only.
- **P1 iter 1:** recon **0.101** kl **0** skip **0/0**. held **0.00166** (decode-CV; P61 later P1 held-on-h ~6e-4). overshoot **0.506**. teacher MV/DV **×0.988 / ×0.967**. `du_frac=1.0` `clip_frac=0.083` (HEAD jsonl; P61 pid had no clip_frac). GPU ~14.2 GB.
- **LIVE (2026-08-31 ~21:50 CDT, P1 iter 60, GPU occupied):** recon **0.0026** (best **0.0020**) kl **0** skip **0/0** last_ok **60 unlocked**. Held iters 50–60 median **~7e-5** vs P61 matched **~2.2e-4** (P61 later-P1 silent ~3e-4 then **storm 2/2 @56–57** gnorm **5e9**). P62 **passed that window** (iter 56–60 recon 0.0022–0.0031, gnorm ≤26 wrap-class at inject, skip 0). overshoot **0.032** (P61 0.034) — decode-CV is **still ~400× quieter than overshoot**, so it is not a louder compounding supervisor. teacher MV/DV **×1.026 / ×1.012** `clip_frac` **0.083** `du_frac=1.0`. `t_wm` **97 s**. Rest-IC graph captured N=6 T=128. Do **not** call skip-0 a storm-preventer (P60 also had 0 storms; P58/P59 stormed later at 61/76). HEAD (not this pid): jsonl `wm_held_rollout_scale` + `wm_held_cv_drift`; `img_rollout out='h'` **REMOVED**. **No second GPU job.** Do not FALSIFY until val TM.
- **Watch:** quality-gate typically ~82; val 1step→openloop vs **×0.772**; MV/DV ss/@H vs P26 ×0.97 / P61 ×0.91/×0.77; det_r vs P61 0.153 / P59 0.577; paired vs P58 **−6.24**. If compounding is unchanged at EXIT, next family is **open-loop magnitude** (1step vs K), not held-stationarity N+1 and not P59–P61 teacher-protocol. **One GPU job at a time.**
...
| Held-rollout late−early on GRU `h` (`out='h'`) | **P61 EXIT** (`run_p61_clipdu`; P1 median ~6.4e-4 vs overshoot ~0.041) | SUPERSEDED (P62 `out='obs'` + `cv_index_t` late−early; still gain-neutral; no new knob) | YES — p139: z/c wander while `h` sits; compounding is decoded-CV contraction (1step-faithful / open-loop ×0.77). Isolation TBPTT is default-feat + `keep_c` on `h` (never called `out='h'`). |

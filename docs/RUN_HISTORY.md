...
| P66 | 2026-09-01 | per-seq `dob_ground` var-skip (`var>1e-3`; mixed ring KEEP) | **LIVE P1** tmux `mbrl2_p66` pid **181468** sha `38880f9`. Storm **1/2 @60** (recon **0.0718**, skip **77**, gmatch **1.52**, teacher MV **×0.119** / DV **×1.31**); restored last-ok **59**, unlocked. Iter **71** recon **0.0022** skip **0** last_ok **71 unlocked**. Teacher MV/DV ×1.021/×1.045. skip0 recon median **0.0034**. P1 `dob_ground≡0`. Fidelity iter 50 H=55 r=0.436; iter 60 gain-blind. | ⏳ GPU occupied (~14.3 GB). Hypothesis is **P2-only**. Watch P2 buf 327/327; val pred_std vs 1.93 vs P64. HEAD (not this pid): shared `dreamer_v4_config_from_train` (rssm fallback). |
...
### p66 (`run_p66_dobvar`, branch `cursor/p28`) — LIVE P1: per-seq `dob_ground` var-skip

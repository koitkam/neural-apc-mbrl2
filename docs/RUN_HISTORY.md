...
| P51 | 2026-08-30 | P3 REINFORCE stop-grad log_std (`p3_stop_grad_log_std=True`) | **LIVE P1** iter **69** (~2h33). pid **54815**, launch `d6a4511`. Skip-storm **1/2** @52 (lock@47). Wrap lock@**66** recon 0.318 unlock@**69** recon 0.013. last_ok **66 unlocked**. recon recovering 0.013. Storm still 1/2. bc **MSE**. Env-free. Do not launch. | 🔬 wrap-unlock GPU-confirmed; watch last_ok advance, gate 82, storm 2/2 |
...
- **Step 4:** `[resolved-cfg] … bc_mean=True p3_sglogstd=True compile=eager`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. STAGE 1. P1 iter **69** (~2h33) recon **0.0133** recovering (min 0.0017 @65) bc **0.312 MSE** (never NLL; one 0.0 @65) last_ok **66 unlocked** after wrap. GPU ~11 GB.
- **Wrap / skip-storm (LIVE):** (1) lock@**47** recon 0.4725; skip-storm **1/2** @52 restore 47. Iter 56 healthy. (2) **Wrap lock@66** recon **0.3178** (P50 was lock@70 recon 0.180); iter 67 detonated; 68 recon 0.040; **wrap-unlock @69** recon **0.0133** < 20× best 0.0017. last_ok snapshot **66** (healthy recon 0.0020), now unlocked so it can advance. Storm still **1/2** (wrap path, not skip-storm 2). Banner skip 98 is the first detonation window. Wrap-recovery unlock **GPU-confirmed P51** (also P50).
- **Watch:** last_ok should advance on healthy recon; GAIN-READY gate 82; extra-P1 if EXTEND; storm 2/2 still caps; P2 bc MSE ≥0; first P3 ent ≈ H(σ_init)≈−0.08 not −0.283; **unfreeze must not yank** (P50 169: −0.107→−0.268). Do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.

...
- **Liveness:** P1 STAGE 1. Iter **70** (70 jsonl rows, ~151 min). recon **0.128→0.0020** (best **0.0017** @46 held). Iter-68 bump **0.0038** recovered **0.0021 @69 → 0.0020 @70**. gmatch **0.0001** (FD MV/DV **×1.00 / ×0.99**). iso 0 ss 0 skip **0**. last_ok **70** unlocked. wm-fidelity probe @**70**: H=1 r=+0.734 H=13 r=+0.687 H=27 r=+0.622 **@H r=+0.593** conv=1.00 drift=0.010 gain_fid **0.909** best_h=55/55 (not the TM gate; @H up vs @60 +0.324 / gain_fid 0.654 / best_h 27/55). **New EMA best 6.243** (raw 6.856) @**70** → saved `wm_best.pt` (was 5.998 @30). Jsonl still 5.630 until next mix-in write. dv-prbs inject only. Next probe ~80. sps ~48. GPU ~11 GB. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Gate vs P52 last-ok 78 not yet (~8 iters). Do not KEEP/FALSIFIED as cascade until EXIT.

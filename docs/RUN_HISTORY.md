...
- **Liveness:** **GAIN-READY P1→P2 @82** (no extra-P1). Live probe worst **0.88@DV**. last_ok **82**. Skip-storm **1/2 @73** recovered. STAGE 2 DOB id, g FROZEN. P2 iter **111** recon **0.0017** dobg **0.007** last_ok **82**. wm-fid @**110**: H=1 r=+0.811 H=13 r=+0.740 H=27 r=+0.655 **H=55 r=+0.564** conv=1.00 drift=0.009 gain_fid **0.579** best_h=55/55 (not the TM gate; @H up vs @100 +0.357, still below @90 +0.627; gain_fid down vs @100 0.752 / @90 0.935). Live EMA **6.019**; best **6.845** @90 held. Next probe ~120. Still P2 (not P3). sps ~197 post-probe. GPU ~11 GB. ~190 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Watch first P3 ent ≈ H(σ_init); unfreeze ~147. Do not KEEP/FALSIFIED as cascade until EXIT.

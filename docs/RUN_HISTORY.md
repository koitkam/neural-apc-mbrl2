...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P1** iter **79** (~179 min). pid **64705**, launch `d910ee2`. Step 4 **CONFIRMED** `p3_sglogstd=True p3_logpclip=8`. **lock@79** last_ok **78** (recon **0.331** = 196× best **0.0017@71**, skip **0**; 2nd silent spike after lock@38 wrap@39). `wm_last_ok.pt` iter 78. wm-fid 80 / wrap / gate 82 next (~6 min). GPU ~11 GB. Env-free. | 🔬 wrap-unlock KEEP. Watch wrap@80 then gate 82. Do not launch. |
...
- **P1 iter 79 (~179 min):** **lock@79** last_ok **78 locked** (recon **0.331** = **196×** best **0.0017@71**, skip **0**, gnorm 0.90). Same class as lock@38 (recon 0.412 = 151×, skip 0) → wrap-unlock@39. Iter 78 was healthy (recon 0.00193, FD mv 1.009 / dv 1.012); wrote `wm_last_ok.pt` iter 78. Spike: jemb 0.005→0.153, encvar 0.03→0.13, zrank 833→646, teacher FD mv **0.960** / dv **0.920**. `wm_best` still EMA **5.863@70**. Wrap-unlock needs recon <20× (**0.0337**). wm-fid @80 then gate 82 (~6 min). Unfreeze ~147 is the A/B.
- **Watch:** wrap-unlock @80 (P50-class) vs stay-locked into gate 82. wm-fid 80 may probe **live detonated** weights (HEAD last-ok probe-when-recon>5× is **not** in pid `d910ee2`). If still locked at freeze, last_ok 78 is the restore (healthy). Then unfreeze ~147: ent stays ≈ H(σ_init); unclipped `actor_logp_std` may jump; actor_loss / rtgt / mvv should **not** P51-rail (147 logp_std 38 / actor −4.66 / later mvv 19k→681k). Do not launch a second GPU job. Do not edit val stack while pid 64705 is live.

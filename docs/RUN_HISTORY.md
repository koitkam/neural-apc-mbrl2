...
| P60 | 2026-08-31 | env-free `gain_match_step<=0` auto = `wm_tf_step_frac` (0.4) | LIVE **P1**. Step 4: `[gain-match] … step=0.4`. `[resolved-cfg] … gmatch_step=0.4`. Rest-IC graph **captured N=6 T=128**. `device=cuda` bs=128 eager. No leftover `DREAMER_*`. pid **144896**, sha `bdb2e49`. **No second GPU job.** | LIVE |
...
- **Watch:** jsonl `wm_gain_match_{mv,dv}_ratio` ~×1 at Δu=0.4; live GAIN-READY MV closer to val TM; val MV ss toward ×1.0 / P26 ×0.97; compounding open-loop; paired econ vs P58 −6.24. **No second GPU job.**

...
| P42 | 2026-08-28 | last-ok lock after silent recon spike (20×) | LIVE P1 iter 43 healthy; lock untested; P41 storm window passed | ⏳ GPU |
...
- **Liveness (10:07 CDT):** LIVE **P1** iter **42**. tmux `mbrl2_p42`, pid **4192010**, HEAD `72f7b48`. GPU ~15.7 GB. recon **0.0022** gmatch **0.0003** iso **0** gnorm **0.36** skip **0**. last_ok **42** (monotonic). **locked=false**. Max recon/best **1.56×** (iter 22) — wrap/lock not triggered. **P41 storm window (iter 39–41) passed healthy** (P41 recon 0.40→0.59 then skip 93; P42 0.0033/0.0021/0.0028 skip 0). Probe iter 40: H=1 r=+0.672 H=55 +0.476 conv=1.00 gain_fid=0.572. wm_best still **6.337@iter10**. **Lock not yet tested** (needs ≥20×; P41 iter-57 class). **No second GPU job.** Watch `[wm-last-ok] locked`; wrap 5–10× must not lock; skip-storm still unlocks; first gate still **gain-probe**.

...
| P51 | 2026-08-29 | P3 REINFORCE stop-grad log_std (`p3_stop_grad_log_std=True`) | **LIVE P1** iter **17**. pid **54815**, tmux `mbrl2_p51`, launch `d6a4511`. `[resolved-cfg] … bc_mean=True p3_sglogstd=True`. recon **0.0034** skip 0 last_ok **17 unlocked** teacher FD ~×1.00/×1.01 bc **MSE 0.26–0.31**. Env-free. Do not launch. Leftover `OBJ_AUTO_*`/jitter HEAD `95f64c6` (not in pid). | 🔬 watch wrap ~56; unfreeze: ent stays ≈−0.10, not −0.268 yank |
...
### p51 (`run_p51_sglogstd`, branch `cursor/p28`) — LIVE P1: P3 stop-grad log_std
...
- **Step 4:** `[resolved-cfg] latent=deterministic restore_p2=False gain_match=1 dob_ground=2 dob_reg=0 isolation=0 ss_match=0 iso_dcv=off n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 lock=20 huber_per_in=True gmatch_settle=-1 gmatch_rest=True p3_sigreset=False bc_mean=True p3_sglogstd=True compile=eager`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. `[run] pinned eval modules at launch`. STAGE 1 @iter0. P1 iter **17** (~40 min) recon **0.0034** (min so far; was 0.070 @1) bc **0.308 MSE** (range 0.259–0.314, never NLL) skip **0** last_ok **17 unlocked** teacher FD MV/DV ~×0.997/×1.013. No wrap/lock/storm (P50 wrap ~iter 56). GPU ~11 GB.
- **Watch:** P2 bc stays MSE ≥0; first P3 ent ≈ H(σ_init)≈−0.08 not −0.283; **unfreeze must not yank** (P50 169: −0.107→−0.268, logp_std 0.56→54). Remaining leftover is env-only paths — do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.
- **GPU-occupied leftover (not a P51 launch):** remaining `derive_auto_weights` `OBJ_AUTO_*` / `OBJ_USE_NORMALIZED` / APCEnv `RUNTIME_SETPOINT_*_JITTER` → TrainConfig identity + `ENV_OVERRIDES` (`95f64c6`; not in this pid). Dual-read leftover when the field is not explicit. Env-free numbers unchanged. Did **not** switch APCEnv to `RuntimeSetpointConfig.auto_derive`. Smoke `tools/_smoke_rssm.py` `CVD=""` **PASSED** (incl. `_test_auto_weights_cfg`).
...
| Original-P1 wrap last-ok unlock | P49 EXIT (`run_p49_wrapunlock`); **P50 EXIT** (`run_p50_bcmean`) | KEEP as lock hygiene; **FALSIFIED as cascade lever** (P49); wrap-recovery unlock **GPU-confirmed P50** @57–58 and @70–73 | YES — P49 skip-storm 1/2 @64–65 unlocked (wrap path untested). P50 lock@56 recon 0.1412 skip 0; unlock @58; lock@70 unlock@73. Extra-P1 lock@88 stays locked (P40). |

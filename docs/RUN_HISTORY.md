...
| **Actor** | econ vs baseline + vs expert BC | **P50** (mbrl2) | paired **−56 vs −104** | First GAIN-READY val beat (9 pairs; seed kpi −74). Unfreeze still yanked σ. P51 also beats (−72 vs −111) but worse; stop-grad FALSIFIED as cascade. P45–P49 valid P3 all lost to baseline. Do not score GAIN_NOT_READY freezes. |
...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P1** iter **34** (~77 min). pid **64705**, launch `d910ee2`. Step 4 **CONFIRMED** `p3_sglogstd=True p3_logpclip=8`. recon **0.101→0.0029** (best **0.0029@34**) gmatch **0.172→0.00014** skip **0** last_ok **34 unlocked**. Iter 30 wm-fid H=55 r=0.490 gain_fid=**0.944** `wm_best` EMA 5.798. GPU ~11 GB. Env-free. HEAD: clamp `8×n_mv` (1-MV identity). Watch gate 82 then unfreeze ~147 vs P51 μ-rail. | 🔬 μ-side clamp after P51 cascade FALSIFIED. Do not launch. Do not stack critic knobs. |
...
### p52 (`run_p52_logpclip`, branch `cursor/p28`) — LIVE P1: clamp P3 REINFORCE logp
- **One attributed GPU change vs P51 pid:** `p3_logp_clip=8.0` (HEAD default `d910ee2`). Clamp `logp` in `-(adv * logp)` so frozen-σ `(u−μ)/σ²` cannot explode μ. Jsonl `actor_logp_*` stays **unclipped** (rail still visible). Opt out `DREAMER_P3_LOGP_CLIP=0`. Also carries P51 `p3_stop_grad_log_std=True` (KEEP as yank block) and P50 `bc_mean_only=True`. Env-free. tmux `mbrl2_p52`, pid **64705**, `CUDA_VISIBLE_DEVICES=0`. Started 02:01:47 CDT.
- **Not:** leftover `DREAMER_*`, `p3_reset_log_std`, extra critic knobs, `actor_kl_coef`.
- **Step 4 CONFIRMED:** `[resolved-cfg] latent=deterministic restore_p2=False gain_match=1 dob_ground=2 dob_reg=0 isolation=0 ss_match=0 iso_dcv=off n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 lock=20 huber_per_in=True gmatch_settle=-1 gmatch_rest=True p3_sigreset=False bc_mean=True p3_sglogstd=True p3_logpclip=8 compile=eager`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. GPU ~11 GB. STAGE 1 @iter0.
- **P1 iter 34 (~77 min):** recon **0.101→0.0029** (best **0.0029@34**) gmatch **0.172→0.00014** skip **0**. last_ok **34 unlocked**. Latest wm-fidelity still iter 30: H=55 r=**0.490** `gain_fid=0.944` `wm_best.pt` EMA **5.798**. FD ratio mv **1.005** / dv **1.020** (teacher, not TM). ~2.2 min/iter after iter 1. No skip-storm/lock/wrap. Gate 82 ~1.8 h away; unfreeze ~147 is the A/B.
- **Watch:** gate 82 GAIN-READY; unfreeze ~147: ent stays ≈ H(σ_init); unclipped `actor_logp_std` may jump; actor_loss / rtgt / mvv should **not** P51-rail (147 logp_std 38 / actor −4.66 / later mvv 19k→681k). Do not launch a second GPU job. Do not edit val stack while pid 64705 is live.
- **HEAD (not in pid 64705):** `p3_logp_clip` is nats **per action dim**; summed logp clamp = `8×n_mv` (`_p3_logp_clip_bound`; 1-MV test_sim identity). SNR window no longer invents τ=50 s when the identifier is missing (lookback/4 frames).
...
| P3 REINFORCE unclipped logp (μ-rail with frozen σ) | P51 EXIT; **P52** (`run_p52_logpclip`) | SUPERSEDED (`p3_logp_clip=8` per action dim) | YES — advantage_clip 8 + grad_clip 10 already saturate (147 gnorm 9.65) but each step still shoves μ via (u−μ)/σ² (logp mean −26 @147 / −227 @315). Clamp summed logp at `8×n_mv`; jsonl diag stays unclipped. 1-MV identity. Opt out `DREAMER_P3_LOGP_CLIP=0`. |

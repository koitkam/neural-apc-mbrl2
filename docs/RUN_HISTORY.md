...
| P73 | 2026-09-02 | OL gain-c persist at teacher K (`||c_K[:G]−sg(c0[:G])||²` × existing `cont_gain_persist_coef`) | LIVE P1 iter **16**. skip **0**. last_ok **16 unlocked** (P72 froze 15 after storm 2@16). persist_rel **0.26–1.46**. Teacher pinned @5. | 🔬 LIVE |
...
- **LIVE (2026-09-02 04:49, P1 iter 16):** skip **0/0** last_ok **16 unlocked** (P72 storm 1@6 then **2@16** froze last_ok **15** and CAPPED extra-P1). Teacher pinned @5 **×0.996 / ×0.980** (P72 still ×0.64/×−1.08 @5). Iter 16 recon **0.0061** gmatch **0.0016** MV **×1.00** DV **×0.89–1.28** (wanders). persist_rel **0.26–1.46** (live; not collapsing to 0). recon vs P64 @15 **0.0049** / P72 **0.010**. Dummy `gmatch_ol_tail=0` banner **REMOVED** on HEAD (not this pid). Watch last_ok past 15 toward P64 **82**; freeze vs **0.91@DV**; val 1step→OL / OL-vs-real vs ×0.85/×0.84. jsonl ×1 ≠ val TM. **No second GPU job.**

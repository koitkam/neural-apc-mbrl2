...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P2** iter **122** (~204 min). pid **64705**, launch `d910ee2`. Gate **GAIN-READY** @82 last-ok **78**: MV **0.95/@H 0.95** DV **0.84/@H 0.89**. lock@79 wrap@81; freeze last_ok **78**. P2 recon **0.0021** bc MSE **0.003** skip **0**. Iter 120 `wm_best` EMA **6.293** (beat 6.184@90) H=55 r=0.526 gain_fid=0.973. Unfreeze ~147 (~11 min). | 🔬 last-ok probe GPU-confirmed. Do not launch. |
...
- **P2 iter 122 (~204 min):** recon **0.0021** bc MSE **0.003** skip **0** last_ok **78 unlocked** dobg ~0.009. Iter 120 wm-fid H=55 r=**0.526** **gain_fid=0.973** new `wm_best` EMA **6.293** (beat 6.184@90). Iter 110 was 0.495/0.623. t_iter ~27 s. Unfreeze ~147 (~11 min, 25 iters).

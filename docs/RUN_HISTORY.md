...
| P42 | 2026-08-28 | last-ok lock after silent recon spike (20×) | LIVE P2 iter 132; freeze restored last_ok 66; freeze probe 0.75@DV | ⏳ GPU |
...
- Abs Huber gain-match is count-equal per input but **scale-unequal** when `|G_mv|≫|G_dv|`. P42 `[gain-match] targets` |MV|=2.62 |DV|=0.51 (~5.1×, same class as abs isolation drowning). Isolation-off moved that drowning into the sole DC supervisor. jsonl `wm_gain_match_mv_loss` / `wm_gain_match_dv_loss` (HEAD, not P42 pid) makes it visible. Do **not** revive relative Huber. P42 freeze **did** restore 66 (GAIN_NOT_READY 0.75@DV). After P42 EXIT+val, this |tgt| drowning is the next attributed SysID lever — not a P42 mid-run stack.
- Isolation/ss-match is **off env-free** (P40 EXIT KEEP as default; **FALSIFIED as DV pin** — val DV ×0.723 not P26 ×0.87; P41 EXIT ×0.700). P08 auto-enable isolation=1 / ss_match=3 ran on P26 (jsonl iso 1.44; `run_plan` 0.0 was the pre-rewrite dump) and P32–P39. Abs MSE + whole-episode holds drowned DV (×0.87 → ×0.66–0.70) but turning it off did not recover P26 DV. Gain-match is the DC supervisor. Opt in `DREAMER_WM_INPUT_ISOLATION_COEF=1` (len/settle auto). When on: abs MSE only, inv-var **REMOVED**, `wm_isolation_dcv_match=True` with `wm_isolation_dcv_min_scale=1.0`. Structural `|G_max|/|G_min| > 1/op_band` cannot equalize |ΔCV| (P38 shrink FALSIFIED; P39 cube-boost FALSIFIED; P40 isolation-off FALSIFIED as DV pin; P41 recent-floor FALSIFIED as DV pin). Do **not** re-add `wm_isolation_var_norm`. Do not try another isolation reweight. May-2026 per-head/latent diag probes default **OFF**.
...
| Extra-P1 last-ok overwrite after silent recon spike | P40 EXIT iter 98 (83→104); P41 EXIT original-P1 iter 64 (56→64 then 104); **P42 GPU-confirmed lock @67 + freeze 66** | KEEP lock as 20× fire; **<5× overwrite-prevention untested this run** | YES — P41 recovered <5× overwrote pre-spike last-ok (val det_r 0.079). P42: spike iter 67 recon 0.3066 skip 1 locked last_ok **66**; extra-P1 recon stayed ≥14× (min 0.0255) so overwrite never had a <5× opening. Freeze restored **66**; first P2 recon **0.0025**. Freeze probe of 66 **0.75@DV** (MV 1.19) ≈ P40/P41; live extra-P1 at 104 was GAIN-READY **0.81@DV** (declined). Skip-storm unlock untested. Wrap 5–10× untested. Val det_r pending. |
...
| jsonl gain-match scalar only | P42 GPU-occupied | SUPERSEDED (`wm_gain_match_mv_loss` / `wm_gain_match_dv_loss` + `p1_recon_best`; HEAD) | PARTIAL — abs Huber mean drowns DV when \|tgt\| 2.62 vs 0.51; split is observability (loss unchanged). Do not revive relative Huber. |

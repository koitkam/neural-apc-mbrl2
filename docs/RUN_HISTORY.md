...
- **HEAD (not in pid 95956):** p136 `actor_kl_coef` **REMOVED**. `DREAMER_ACTOR_LOSS=pmpo` **refused at `train()`** (false A/B: P3 always inlines REINFORCE + μ-ratio; `pmpo_loss` is never called). Rest-IC CUDA-graph canary **saves/restores** in-flight grads (identity when None). CUDA replay H2D per-slot; rest-IC CUDA-graph still not in this pid. CVD="" smoke green. GPU identity deferred.
...
| `DREAMER_ACTOR_LOSS=pmpo` / `actor_loss_type` (P3 always inlines REINFORCE + μ-ratio) | P54 GPU-occupied (pid **95956** P2) | **REFUSED at `train()`** (false A/B) | YES — `pmpo_loss` / `reinforce_actor_loss` have no P3 call sites. Whitelist kept so an explicit override fails loud. `_actor_uses_prior_policy` still gates π_prior snapshot (unreachable in env-free). |

...
| P51 | 2026-08-30 | P3 REINFORCE stop-grad log_std (`p3_stop_grad_log_std=True`) | **LIVE P3** iter **161+**. pid **54815**, launch `d6a4511`. Gate **GAIN-READY** 0.86@MV / 0.88@DV @82; last_ok **82 unlocked**. Skip-storm 1/2 @52; wrap lock@66 unlock@69. First P3 ent **−0.125**; rscale **2.17 KEEP**. Unfreeze 147: ent stayed **~−0.10** (no P50 −0.268 σ-yank); μ railed (logp_std 0.54→38, mvv 18→19k, rtgt 0.058→0.006). Env-free. Do not launch. | 🔬 σ-yank blocked; μ cascade still on. Watch ES / val. Do not stack critic knobs. |
...
- **Step 4:** `[resolved-cfg] … bc_mean=True p3_sglogstd=True compile=eager`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. STAGE 1. P1 iter **74** (~2h43) recon **0.0068** (recovered from wrap 0.013 @69; min 0.0017 @65) bc **MSE** last_ok **74 unlocked** (advanced 66→70→74 after wrap-unlock). GPU ~11 GB.
- **Wrap / skip-storm (LIVE):** (1) lock@**47** recon 0.4725; skip-storm **1/2** @52 restore 47. (2) wrap lock@**66** recon 0.318; wrap-unlock @**69** recon 0.013. last_ok **advanced** after unlock (iter 70+). Storm still **1/2**. No new lock through 74. FD ~×1.03/×1.01. Wrap-recovery unlock **GPU-confirmed P51** (also P50).
- **Watch:** GAIN-READY gate 82 (P50 wrap-adjacent 1.37@MV FAIL — HEAD probes last-ok when recon detonated); extra-P1 if EXTEND; storm 2/2 still caps; P2 bc MSE ≥0; first P3 ent ≈ H(σ_init)≈−0.08; **unfreeze must not yank**. Do not edit val stack while pid 54815 is live.
- **GPU-occupied leftover (not a P51 launch):** `derive_horizon` / sim `reset()` now `horizon_formula_knobs()` / `ic_randomization_knobs()` (TrainConfig 4.0/120 / ON/0.6 + leftover env; changing the dataclass sizes H / IC span). Same class as `gpu_probe_knobs()`. Env-free numbers unchanged. Did **not** switch APCEnv to `RuntimeSetpointConfig.auto_derive`. Smoke `tools/_smoke_rssm.py` `CVD=""` **PASSED** (incl. `_test_horizon_ic_overhead_cfg_or_env`). Not in P51 pid.
...
| P3 REINFORCE stop-grad log_std (μ-only) | P51 LIVE P3 (`run_p51_sglogstd`, `d6a4511`) | KEEP as σ-yank block (entropy ~−0.10); **not yet EXIT** as cascade lever | PARTIAL — unfreeze 147: no P50 −0.107→−0.268; logp_std still 0.54→38 (μ rails, same class as P50). rtgt 0.058→0.006; mvv 18→19k; rscale 2.17 KEEP. Do not stack critic knobs. Do not promote `p3_reset_log_std`. |

...
| P48 | 2026-08-29 | P3 collect/val stream measured DV + Kalman | LIVE P1 iter **23** recon 0.006 skip 0 teacher FD ~×0.99/×1.02 `t_wm`~126 s `wm_best`@10 then 20 | ⏳ env-free; GPU occupied pid **34706**; no second job |
...
- **P48-live (GPU occupied, no second job):** leftover `DREAMER_PROCESS_NOISE_AMP_RAMP` / `DREAMER_HIDDEN_DIST_*` / `DREAMER_HIDDEN_OU_*` / `DREAMER_DISTURBANCE_PROB_*` → TrainConfig + `ENV_OVERRIDES` (identity). Dual-read at `noise_curriculum_scale` / `HiddenDisturbance` / `get_phase_disturbance_prob`. Stage-1 skip unused `sigmoid·d` decay; rest-IC last_only slices not `unbind`; collect/val pinned H2D obs copy. P1 iter **23**: recon 0.006 gmatch tiny skip 0 teacher FD MV/DV ~×0.99/×1.02 (not val TM). `t_wm` ~126 s. `wm_best` iter 10 then 20 (gain-blind). Storm 0. Do not launch. Do not promote `p3_reset_log_std`.

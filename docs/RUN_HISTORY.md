...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P3** iter **201** (~221 min). pid **64705**, launch `d910ee2`. Gate **GAIN-READY** @82 last-ok **78** MV **0.95** DV **0.84**. Unfreeze 147 delayed bound then **@165** std **80.8** / actor **−5.80** (peak 100.7@169). **@196** actor **−9.81** mvv **813k** (std back **0.88**). Ent **−0.105 HELD**. rtgt **0.00012**. best.pt **176**. rscale **2.83 KEEP**. | 🔬 clip delayed first-unfreeze; collect still P51-class. Do not call EXIT yet. Do not launch. |
...
- **P3 unfreeze @147 then delayed μ-rail @165:** STAGE 3 @136; first P3 @137 ent **−0.143** logp_std **0.53** actor **+0.65** bc MSE **0.002**. Critic warmup 137–146 rscale 1.20→**2.83 FREEZE KEEP**. Unfreeze 147: ent **−0.101 HELD** (opened from −0.137, not yanked to σ_min); unclipped logp_std **0.53→6.11** (peak **10.4@152** — **not** P51 **38** on the first unfreeze step); actor **−0.77** (**not** P51 **−4.66** at 147). **Delayed rail @165–169:** unclipped logp_std **80.8@165 / peak 100.7@169**; actor **−5.80@165 / −4.55@166**.
- **P3 collect iter 201 (~221 min):** after the delayed rail, unclipped logp_std eased **46@182 → 0.88@196** (back near warmup 0.53) while actor_loss went **more** negative: **−7.83@191 / −8.37@192 / −8.15@195 / −9.81@196** (worse than P51 **−4.66**). mvv **841k@191 / 813k@196 / 332k@201** (already past P51 **681k@361**). rtgt **0.00012**. ema **−1951**. bc ~0.7–1.3. skip 0. last_ok **78 unlocked**. `best.pt` still **176** (`best_det_return` **−634**). Entropy **−0.105 HELD** (dip −0.115@195, not σ_min −0.36). logp_mean flipped **−72@181 → +4.3@196**.
- **Watch:** clip **delayed** the first-unfreeze μ-rail (~18 iters) but collect is now P51-class on actor_loss / mvv / rtgt even with unclipped std ≈0.9. Do **not** call KEEP/FALSIFIED as cascade until EXIT + val vs P50 **−56/−104** and P51 **−72/−111**. Do not launch a second GPU job. Do not edit val stack while pid 64705 is live.
...
| P3 REINFORCE unclipped logp (μ-rail with frozen σ) | P51 EXIT; **P52 LIVE** (`run_p52_logpclip`, iter 201) | SUPERSEDED (`p3_logp_clip=8` per action dim); EXIT pending | YES — clamp delayed first-unfreeze (147 std **6.11** / actor **−0.77**) then unclipped std **80.8@165 / 100.7@169**. Later std collapsed to **0.88@196** while actor **−9.81** mvv **813k** (past P51 681k@361). rtgt 0.00012. best.pt 176. Ent HELD. Do not call cascade until EXIT. Opt out `DREAMER_P3_LOGP_CLIP=0`. |

...
| P56 | 2026-08-30 | Slow μ-ratio recopy `DREAMER_P3_MU_RATIO_REFRESH=50` (~patience/4) | LIVE P3 `run_p56_muslow` pid **110246** sha `9d05866` ~iter **236**. **GAIN-READY @82** 0.83@DV last_ok **82**. Unfreeze **147** ent **−0.101 HELD**. logp_std **~0.50–0.59 through 236** (not P55 18). `clip_frac` 0.70@147 → **~0.10 after recopy ~197** (P54 ceiling, not P55 0.02–0.80). rtgt 0.058→**0.002**. rscale **2.26 KEEP**. ema **~−106**. Rest-IC graph **failed this pid**. Do not promote N=50 without EXIT+val. | LIVE |
...
- **ROOT (Control / ML):** N=1 recopies the ε=0.2 ball onto **last-iter μ** each collect → 20%/iter compounding walk, not a BC ball. Freeze-forever (P53) stopped the P52 rail; every-iter recopy **re-opens** it. Do not loosen ε. Do not stack critic knobs. Do not promote `p3_reset_log_std`. **REVERT** TrainConfig default **1→0**. **Next GPU: P56** `DREAMER_P3_MU_RATIO_REFRESH=50` (~patience/4; 1–4 recopies in a 200-iter plateau) vs freeze-forever ceiling without P55's every-iter walk. Watch logp_std ~0.5 not 18; clip_frac not stuck at 0.11 *and* not 0.02–0.80; val paired vs P53 −13/−98; rest-IC graph captured + `t_wm` ≪ 127 s.
- **HEAD:** default `p3_mu_ratio_refresh_iters=0`. In-place `load_state_dict`; rest-IC graph released at g freeze; never capture under parent autocast. jsonl `actor_pos_adv_frac` / `adv_action_corr`. `training_diagnostics` 3×3. PMPO prior-refresh **REMOVED**.

### p56 (`run_p56_muslow`, branch `cursor/p28`) — LIVE P3: slow μ-ratio recopy N=50 (first recopy ~197, no P55 walk)
- **One attributed GPU change vs P55 pid:** `DREAMER_P3_MU_RATIO_REFRESH=50` (HEAD default **0** after P55 REVERT `9d05866`). Window = 50 × `phase3_train_steps_per_iter` inner AC steps (~1–4 recopies in a 200-iter plateau). Not a default. Keep ε=0.2. tmux `mbrl2_p56`, pid **110246**, `CUDA_VISIBLE_DEVICES=0`. Started 20:25 CDT.
...
- **Liveness:** GPU occupied. tmux `mbrl2_p56` LIVE, pid **110246**. sha **`9d05866`**. **Do not launch a second GPU job.**
  - **P1→P2 @82 GAIN-READY** worst **0.83@DV** (MV 0.92/@H 0.91, DV 0.83/@H 0.88). last_ok **82 unlocked**. Skip-storm **1/2 @68–69** (recon 0.253→0.512, 65 inner skips; lock 67, restore, unlock). Teacher FD MV/DV ~×1.00. `wm_best` **iter 70** EMA 6.224 gain_fid **0.926** (`restore_p2=False`). Rest-IC CUDA-graph **failed this pid** (`ValueError: grad requires non-empty inputs` — nested function; `t_wm` median **124 s** P1 / **23 s** P2). SNR measured CV+DV median **+17.0 dB** min **+14.4 dB**.
  - **P2→P3 PASS @136** mtp median **0.909**. `[return-scale] FREEZE` **2.260 KEEP**.
  - **Unfreeze 147:** first P3 ent warmup **−0.18** then **−0.101 HELD**. logp_std **1.67@147** then **~0.50@176** (P53 held ~0.47; P55 **18.8@175**). `clip_frac` **0.70@147 → 0.16@176** (freeze-forever drop, not P55 0.02–0.80). rtgt **0.058→0.004** (P54-class collapse). adv_corr 0.23→~0.05. mvv 4.2k@166 not 813k. ema **−218→−119**.
  - **N=50 first recopy ~197 through ~236:** logp_std **0.50–0.59** (not 18). `clip_frac` **0.03–0.20** (P54 ceiling, not P55 0.02–0.80). rtgt **~0.002**. ema **~−106**. ratio_mean ~0.95–1.02. Freeze-forever μ-ball recopied onto nearly the same μ is a near-no-op vs N=0. Do **not** promote N=50 without EXIT+val. Do not loosen ε.
- HEAD after this pid: `_RestICGraphModule` yields RSSM `parameters`/`named_parameters`/`buffers`/`named_buffers`; capture `wrapper.train(rssm.training)` (graphed fwd is training-state gated); empty trainable / GRU-canary still pin; VRAM skip does not pin. Skip `rssm_cont_kl_loss` when `cont_kl_scale<=0` (env-free 1.0 identity). P3 banner prints `logp`/`clip` and `skip this/cum` (`n_grad_skip_iter`; P56 pid still prints cumulative 65). Do not stack critic knobs. Do not promote N=50 without EXIT+val. Watch until EXIT: logp_std ~0.5 not 18; clip not 0.02–0.80; val vs P53 −13/−98.
...
- Rest-IC encode length is `lookback` (test_sim **128** sequential GRU steps × 100 inner WM steps → eager `t_wm` ~123 s). A few-τ window would be more sim-adaptive; that is a later A/B (not identity). **HEAD:** CUDA-graph of that T-loop via `_RestICGraphModule` (`gain_match_rest_ic_cuda_graph=True`; `allow_unused_input=True`; capture `wrapper.train(rssm.training)` so graphed fwd matches RSSM train/eval), warmed **before** the P1 WM autocast loop + parent `enabled=False` then bf16 `cache_enabled=False`; **in-loop never captures** (autocast miss is eager that step, not a fail pin). Nested function is not a graph surface (P56 `parameters()` empty). Overriding only `parameters()` left `named_parameters()`/`buffers()` empty (torch 2.12 capture uses both). **VRAM skip does not pin** `_rest_ic_cg_fail` (warmup retry after `empty_cache`). Skip `rssm_cont_kl_loss` when `cont_kl_scale<=0` (env-free 1.0 identity). CPU path stays the Python loop (`CVD=""`). `DREAMER_GAIN_MATCH_REST_IC_CUDA_GRAPH=0` keeps eager.
...
| P3 freeze-forever μ-ratio snapshot (P53 KEEP as walk limiter) | P53 EXIT; **P54 EXIT** (`run_p54_esentband`); **P55 EXIT** (`run_p55_murefresh`) | P55 N=1 **FALSIFIED**; ε=0.2 KEEP; **REVERT** default 1→0 | YES — P53 KEEP as cascade (val −13/−98). P54 extra P3 FALSIFIED as econ lever: `clip_frac` 0.42→0.11. **P55 EXIT:** N=1 recopies the ε-ball onto last-iter μ → compounding walk (logp_std 0.67@147 → 18.8@175, still 19.4@320 / 64@366; ema −170→−2163; rtgt 0.0001). Val paired **−87 vs −78 FAIL 5/9**. best.pt **166** det **−28.8** then detonates @169 — val of that ckpt still FAIL (do not KEEP N=1). 0 = freeze-forever. Slow recopy is A/B (`DREAMER_P3_MU_RATIO_REFRESH`). |
...
| Leftover jsonl `pmpo_pos_frac` name (REINFORCE pos-advantage fraction) | P55 GPU-occupied (pid **103504** P3) | SUPERSEDED (`actor_pos_adv_frac`; leftover key kept as alias) | YES — PMPO actor loss REMOVED. Parsers still read `pmpo_pos_frac`. |
| Leftover jsonl `imag_adv_action_corr` name + 2×3 plot missing cascade axes | P55 GPU-occupied (pid **103504** P3 ~347) | SUPERSEDED (`adv_action_corr` canonical; leftover imag_ alias; ES reads both; `training_diagnostics` 3×3 + csv logp_std/clip/rtgt/vs_expert; diagnostics.py prefers `actor_pos_adv_frac`) | YES — imagination actor deleted; P55 compounding walk (logp_std 18.8, clip 0.02–0.80) was invisible on the 2×3 plot. Identity for training. Not in P55 pid. |
...
| Rest-IC last_only T-loop = 128 sequential kernel launches × 100 WM steps | P54 GPU-occupied (pid **95956** P1); **P55/P56** first GPU capture failed | SUPERSEDED (`gain_match_rest_ic_cuda_graph=True`; `make_graphed_callables` + GRU-grad canary; **P55 HEAD** capture **before** WM autocast + parent `enabled=False` then bf16 `cache_enabled=False`; **in-loop never captures**; **P56 HEAD** `_RestICGraphModule` **parameters/named_parameters/buffers/named_buffers** + `wrapper.train(rssm.training)`; VRAM skip **not** pin) | PARTIAL — P55 pid autocast cache; **P56 pid 110246** nested function `parameters()` empty → `ValueError: grad requires non-empty inputs` → eager `t_wm_s` ~124 s. P3 unfreeze 147 ent −0.101 / logp_std ~0.50 (N=50 recopy not before ~197). Skip-storm 1/2 @68–69 recovered. Overriding only `parameters()` left `named_parameters()`/`buffers()` empty (torch 2.12 capture uses both). Graphed fwd is training-state gated; RSSM is not a child. Pinning `_rest_ic_cg_fail` on VRAM skip would freeze eager even after `empty_cache`. Not in this pid. |

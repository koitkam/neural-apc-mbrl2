...
- **Liveness (10:05 CDT):** LIVE **P1** iter **41**. tmux `mbrl2_p42`, pid **4192010**, HEAD `72f7b48` (lock `a80bad6` ancestor). GPU ~15.7 GB. `CUDA_VISIBLE_DEVICES=0`. **No leftover `DREAMER_*`.** `[resolved-cfg] isolation=0 ss_match=0 iso_dcv=off`. jsonl `p1_last_ok_locked=false`. **Past P41 storm window without skip-storm:** iter 39–41 recon **0.0033 / 0.0021 / 0.0028**, skip **0**, gnorm **0.3–0.4** (P41 detonated 0.40→0.59 then skip-storm @41). last_ok advancing (41). Probe iter 40: H=1 r=+0.672 H=13 +0.705 H=55 +0.476 conv=1.00 gain_fid=0.572. wm_best still **6.337@iter10**. **Lock not yet tested** (needs >20× recon). **No second GPU job.** Watch `[wm-last-ok] locked` on P41 iter-57 class (42×); wrap 5–10× must not lock; skip-storm still unlocks; first gate still **gain-probe**. HEAD after this session prints `lock=` on `[resolved-cfg]` (P42 pid already running).
...
| Extra-P1 last-ok overwrite after silent recon spike | P40 EXIT iter 98 (83→104); **P41 EXIT original-P1** iter 64 (56→64 then 104) | P42 GPU (`skip_storm_last_ok_lock_ratio=20`; recon-only; not in P41 pid `acb8a7b`) | YES — recovered recon < 5× best overwrites pre-spike last-ok; freeze recon healthy so detonated-freeze misses. P41: 0.0887/0.0021=42× then 0.0098 overwrote; val det_r collapsed 0.490→0.079. Skip-storm restore still unlocks. Wrap ~5–10× must not lock. |

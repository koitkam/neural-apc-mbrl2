...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 62**. Probe60 still last **H=1 r=+0.682 / H=55 r=+0.293 conv=0.00** gain_fid **0.882** best_h=1. recon **0.008@62** (best **0.0057@53**; P64 @60 **0.0022** ~3.6×). Teacher **×1.00 / ×1.01 @62**. persist_rel **0.34@60 → 0.59@62 inject**. Wrap 0.147@56 recovered skip 0. pid **242511**. | 🔬 LIVE P1 — H=1 beat P64; H=55/conv still dead **@10–60**. persist inject milder than @41/@51. Do not patch live. |
...
- **LIVE P1 iter 62 (~324 min, nvidia 11.7 GB, `t_wm` ~306 s, sps 19.7):** skip **0**. Wrap recovered; recon **0.011@60 → 0.0084@61 → 0.0081@62** (best still **0.0057@53**; vs P64 **0.0022@60** ~3.6× live). Teacher jsonl 60 **×1.002 / ×1.005** → 61 **×1.002 / ×0.993** → 62 **×0.996 / ×1.012**. last_ok **62 unlocked**. persist_rel **0.34@60 → 0.49@61 inject → 0.59@62** (milder than **0.82@41 / 0.73@51**; still climbing at +2; do **not** persist N+1). Rest-IC graph not captured. Next probe typically iter 70 (~01:34 CDT).

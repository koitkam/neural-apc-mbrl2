...
| P76 | 2026-09-02 | RSSM GRU update-gate bias `log(H/16)` (~1.23; no new knob) | LIVE **P2 iter 109**. P1→P2 @94: live 5-level **0.84@DV PASS** then **detonated-freeze last_ok 84** 5-level **0.80@MV ready=False** (`p1_gain_not_ready_capped`). orig-P1 @82 **FAIL 0.79@DV**. conv=**0** through probe100. CPU n=1 last_ok MV **×0.907** DV **×0.928** vs P64@80 ×0.971/×0.915; ckpt80 ×0.826/×0.810. z-bias still **1.23**. pid **237453**. | 🔬 LIVE P2 — freeze is last_ok 84 GAIN_NOT_READY; expect `[p3-skip]`. Do not GRU-bias N+1. |
...
- **LIVE P2 iter 109 (~2.8 h, nvidia 4.0 GB after graph release, `t_wm` ~20.5 s, sps 253):** `dobg` now **0.11**. skip-storm **1/2 @77** restored last_ok **76** (recon 0.267, 8 skips); wrap@29 recovered. last_ok **locked 84** (recon 0.083 @85, skip 0). persist_rel pinned **0.021 @82** then extra-P1 oscillated 0.05–0.96. Teacher jsonl ×1 ≠ TM.
...
- **Probes (conv=0 unique vs P64 0.25 / P75 0.50 / P73 0.75):** 10 H=1 **0.428** H=55 **0.310**; 20 **0.589 / 0.543**; 40 **0.561 / 0.546**; 50 **0.502 / 0.401**; 60 **0.555 / 0.291**; 70 **0.539 / 0.571**; 80 **0.679 / 0.532**; 90 **0.681 / 0.573**; **P2@100 0.534 / 0.279** (g frozen; best_h=1/55 gain-blind). Keep-h stalls held-SS (`eps_std` tail never drops).
...
- **P2 Kalman:** `dobg` **0.11 @109** (early-P2 med **0.021**, P64 same class). recon **0.0024–0.0030**. Do **not** score actor if `[p3-skip]`.
- If EXIT FALSIFIES as compounding (5-level freeze 0.80 + conv=0 vs P64 0.91), **change family** (do not GRU-bias N+1). Keep-h preserved `||h||` and **prevented settling**, which is the opposite of a DC fixed point. Do **not** start a second GPU job. Step 5 deferred to EXIT.

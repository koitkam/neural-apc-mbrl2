...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P2** iter **111** (~199 min). pid **64705**, launch `d910ee2`. Gate **GAIN-READY** @82 last-ok **78**: MV **0.95/@H 0.95** DV **0.84/@H 0.89**. lock@79 wrap@81; freeze last_ok **78**. P2 recon **0.0021** bc MSE **0.002** skip **0**. `wm_best` EMA **6.184@90**; wm-fid 110 H=55 r=0.495 gain_fid=0.623 (recovered vs 0.377@100, no new best). Unfreeze ~147 (~16 min). | 🔬 last-ok probe GPU-confirmed. Do not launch. |
...
### p52 (`run_p52_logpclip`, branch `cursor/p28`) — LIVE P2: clamp P3 REINFORCE logp
...
- **P2 iter 111 (~199 min):** recon **0.0021** bc MSE **0.002** skip **0** last_ok **78 unlocked** dobg ~0.007. Iter 90 `wm_best` EMA **6.184** gain_fid=0.989. Iter 100 wm-fid H=55 r=0.377 gain_fid=0.584. Iter 110 wm-fid H=55 r=**0.495** best_h=55 **gain_fid=0.623** (recovered vs 100, no new best). t_iter ~27 s. Unfreeze ~147 (~16 min, 36 iters).
- **Watch:** unfreeze ~147 vs P51: ent stays ≈ H(σ_init); unclipped `actor_logp_std` may jump; actor_loss / rtgt / mvv should **not** P51-rail (147 logp_std 38 / actor −4.66 / later mvv 19k→681k). Do not launch a second GPU job. Do not edit val stack while pid 64705 is live.

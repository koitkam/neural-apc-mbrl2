...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 17**. Probe10 **H=1 r=+0.128 / H=55 r=+0.122 conv=0.00** gain_fid **0.476** (P76 +0.428/0.736; P64 +0.584/0.827). Teacher **×0.98 / ×1.29 @17** (DV overgain again; was ×1.00/×0.96 @15). recon **0.150@17 best** (P76 @17 **0.008**; ~19×). skip **0**. pid **242511**. | 🔬 LIVE P1 — recon 0.21→0.15; DV overgain recurred; watch freeze vs P64 0.91@DV; probe20 conv>0 |
...
- **LIVE P1 iter 17 (~90 min, nvidia 11.7 GB, `t_wm` ~306 s, sps 19.7):** skip **0**. Teacher jsonl 15 **×0.995 / ×0.961** → 16 **×1.011 / ×0.961** → 17 **×0.975 / ×1.292** (DV overgain recurred; gmatch **0.015**). recon **0.212@15 → 0.176@16 → 0.150@17 new best** vs P76 **0.008@17** / P64 **0.005@17** (~19×). persist_rel **0.35@15 → 0.61@17**. Rest-IC graph not captured. Probe 10 still the last fidelity (`H=1 r=+0.128 conv=0.00 gain_fid=0.476`); next typically iter 20 (~15 min). Do not patch while live. Watch freeze vs P64 **0.91@DV**; later probes conv>0 (P76 uniquely held 0 through P2@100; P73 also 0 @10); val **1step→OL** vs P64 ×0.85 / P76 ×0.770; `actor_experiment_valid`. Do **not** start a second GPU job.

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 25**. Probe20 **H=1 r=+0.325 / H=55 r=+0.145 conv=0.00** gain_fid **0.261** (P76 +0.589/+0.543 conv=0 0.644). recon **0.052@25 best** (P76 @25 **0.004**; ~13×). Teacher **×0.99 / ×1.04 @25**. Iter19 wrap recovered skip **0**. pid **242511**. | 🔬 LIVE P1 — conv=0 @10 and @20. H=55 r dead. recon 0.07→0.05. OL drops kv_cache (do not patch live). |
...
- **LIVE P1 iter 25 (~132 min, nvidia 11.7 GB, `t_wm` ~307 s, sps 19.6):** skip **0**. Teacher jsonl 23 **×1.001 / ×0.975** → 24 **×1.011 / ×0.989** → 25 **×0.991 / ×1.040** (pinned). recon **0.073@22 → 0.109@23 → 0.080@24 → 0.052@25 new best** vs P76 **0.004@25** / P64 **0.004@25** (~13×, was ~40× @15). last_ok **25 unlocked**. persist_rel **0.22@24 → 0.25@25**. Rest-IC graph not captured. Probe 20 still last fidelity. Next typically iter 30 (~25 min).

...
### p49 (`run_p49_wrapunlock`, branch `cursor/p28`) — LAUNCH: original-P1 wrap last-ok unlock
...
- **ROOT (Control / ML, live):** freeze-24 is **not** the actor confound. Late-P1 freeze (82 ≈ live gate 0.88@DV) still cascades on first unfreeze REINFORCE. Freeze rscale KEEP. Do not stack critic knobs. Do not promote `p3_reset_log_std`. Wrap-unlock KEEP as lock hygiene (untested on P48-class recovered wrap). **Next GPU after EXIT:** stop P1/P2 BC from pinning σ_min (BC **μ-only** / stop-grad log_std, or skip P1/P2 actor BC) — P46/P47 reset-at-P3-start was too late (μ already railed). Do not launch while pid 41994 holds the GPU.
...
| Last-ok original-P1 wrap never unlocks | P48 freeze (`run_p48_collectdv` pid 34706); **P49 LIVE** wrap@64 | SUPERSEDED (original-P1 wrap recovery unlock; extra-P1 stay locked); **wrap-recovery path still untested** | YES — P48 live 0.89@DV @82; freeze restored 24 → 0.81@DV. P49 wrap locked 64 (recon 0.3668) then **skip-storm unlock** (same as P45); last_ok advanced to **82**. `unlocked after wrap recovery` never printed. Freeze-24 confound is in test via late-P1 freeze. Do not raise lock_ratio. |

...
| P78 | 2026-09-03 | TSSM gain-match rest-IC continues KV via `prev_state=` (P77 Markovian FALSIFIED) | LIVE P1 iter **4**. Teacher **×1.014 / ×0.787** persist **0.315** recon **0.375** skip **0**. MV jsonl pinned ×1 at same iter as P77; DV short (P77@4 ×1.07). pid **261143**. | 🔬 LIVE — jsonl MV pin ≠ freeze health. First probe @10. Do not score actor. |
...
- **LIVE P1 iter 4 (~21 min, nvidia 12.3 GB, `t_wm` 295 s):** skip **0**. last_ok **4 unlocked**. recon **0.545→0.417@3→0.375@4** (~P77@4 0.385). Teacher jsonl **×1.014 / ×0.787** (iter 3 **×0.979 / ×0.579**; iter 2 **×0.686 / ×−0.176**). persist_rel **0.811→0.601@3→0.315@4** (dropped; still above P77@4 **0.168**). gmatch **0.308→0.057@3→0.018@4**. **MV jsonl pinned ×1 at the same iter P77 pinned** (P77@4 **×0.970 / ×1.074**). DV recovered from invert but **short of ×1** (not P77 overgain). KV-continue did **not** prevent last-step teacher MV from pinning ×1 — freeze/5-level TM is still the test (P77 jsonl ×1 ≠ 5-level **−0.46@MV**). Do **not** treat jsonl ×1 as GAIN-READY. First probe @10 (~30 min). Do **not** score actor. Do **not** persist N+1. Do **not** launch a second GPU job.

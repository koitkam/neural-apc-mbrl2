...
### p38 GPU LIVE (`run_p38_isodcv`, tmux `mbrl2_p38`, launch `42bc7c2`) — isolation |ΔCV| excitation
- **Launch KEEP:** env-free det+eager `storm_cap=2`. `[resolved-cfg] iso_dcv=True mv=['0.317'] dv=['1.67']` isolation=1 ss_match=3 gain_match=1. Pre-iso tgt MV −2.48 DV 0.47; **post-seed re-resolve** MV −2.98 DV 0.52. device=cuda bs=128 GPU ~17.7 GB. pid **4167186**. No leftover `DREAMER_*`.
- **Attributed change vs P37:** isolation settle `Δu_i ∝ 1/|G_i|` (not a loss reweight). Seed log `dcv_match MV=['0.317'] DV=['1.67']`. `run_plan.isolation_dcv_scales` on. **Applied edge |Δu|** (linspace ±op_band=0.6, then cube clip): MV **0.190** (P37 0.60), DV **1.0** (P37 0.60). Logged DV 1.67 is the multiplier, not |Δu|.
- **Mid-run (iter 27, ~1.1 h, GPU occupied — no second job):** recon **0.86** (P37 ~0.005), **gmatch stuck 1.308** (P37 →0.0008 by iter 17), iso **0.22** / ss **0.12** (P37 iter-1 ss 1.00 — ~8× weaker teacher), skip **4** @iter 15, last_ok **13**. Iter 4/10/14 recon blips; iter 15 skip-4 detonation (encvar 0.004, jemb 0.006) then jemb/encvar recovering then slipping (iter 27 jemb 0.04). `wm_best` saved at detonated iters 10/20 (gain_fid 0.007/0.002, restore_p2 OFF). Watch: gmatch fall, skip 0 through iter 53, recon →0.005. If gmatch stays ~1.3 through the P32 window this is P36-class weak isolation teacher (match-at-`g_min` starved MV |ΔCV|). Do **not** revert HEAD while the A/B is live. Do not stack a second observer change.
- Do **not** launch a second GPU job. Watch iter 75 DV ratio. `[p3-skip]` unless GAIN-READY.
...
- Isolation/ss-match is **abs MSE only**. Inv-var A/B **REMOVED**. Default **`wm_isolation_dcv_match=True`**: isolation settle `Δu_i ∝ 1/|G_i|` (WM-norm |G|, `level*scale` clip ±1, equal-|G| no-op). Audit **`edge_du_* = min(1, op_band·scale)`** — P38 LIVE logged DV 1.67 is edge |Δu|=1.0; MV 0.317 is edge |Δu|=0.19 (P37 both 0.60). Matching at `g_min` is sim-adaptive but shrinks strong-|G| isolation SNR as `|G|_max/|G|_min` grows. Pre-iso resolve is **scales only**; post-seed always re-resolves Huber targets after isolation+expert (P37 freeze point). P37 EXIT unscaled abs: val MV ×0.98 DV **×0.69** det_r 0.37 `[p3-skip]`. Do **not** re-add `wm_isolation_var_norm`. Do not change the P38 formula while the GPU A/B is live.
...
- `run_plan.json → config` is rewritten after gain-match resolve. `[resolved-cfg]` prints `iso_dcv=` **and** `mv=`/`dv=` multipliers **and** `edge_du_mv=`/`edge_du_dv=` (applied |Δu| after cube clip); `run_plan.isolation_dcv_scales` is the P38 audit trail (inv-var knob removed). Pre-rewrite dump is dataclass defaults (P29 plan still shows `rssm_latent_type=categorical` / `gain_match_coef=0` even though training auto-enabled grounding).
...
| Isolation |ΔCV| excitation `Δu_i ∝ 1/|G_i|` (`wm_isolation_dcv_match`) | P38 LIVE (`run_p38_isodcv`, `42bc7c2`) | IN FLIGHT (default ON) | PARTIAL — P37 EXIT unscaled abs val DV **×0.69**. P38 multipliers MV 0.317 DV 1.67 = applied edge |Δu| **0.19 / 1.0** (P37 0.60/0.60). Iter 27: gmatch stuck 1.31, recon 0.86, iso 0.22, skip 4, last_ok 13. Watch vs P36 weak-teacher class. |

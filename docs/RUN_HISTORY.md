...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 50**. Probe50 **H=1 r=+0.509 / H=55 r=+0.273 conv=0.00** gain_fid **0.389** best_h=1/55 drift **0.321**. recon **0.007@50 best** (P64 @50 **0.0026**; ~2.5×). Teacher **×1.00 / ×0.92 @50**. persist_rel **0.51**. pid **242511**. | 🔬 LIVE P1 — H=1 dipped vs @40; H=55/conv still dead **@10–50**. Do not patch live. |
...
- **LIVE P1 iter 50 (~261 min, nvidia 11.7 GB, `t_wm` ~306 s, sps 19.7):** skip **0**. Teacher jsonl 48 **×1.029 / ×0.835** → 49 **×1.000 / ×0.944** → 50 **×0.996 / ×0.925**. recon **0.007@47–49 → 0.007@50 new best** vs P64 **0.0026@50** (~2.5×; P76 @50 0.0028). last_ok **50 unlocked**. persist_rel **0.50@47 → 0.49@48 → 0.35@49 → 0.51@50** (held high after re-spike). Rest-IC graph not captured. Next probe typically iter 60.

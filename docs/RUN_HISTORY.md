...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P2** iter **153** after **CAPPED GAIN_NOT_READY −0.46@MV / 0.74@DV** last_ok **104**. Orig-P1 @82 **FAIL −1.06@MV**; extra-P1 @94 **FAIL −0.29@MV / 0.80@DV**. Probe100 **H=1 +0.567 / H=55 +0.299 conv=0**; P2@130 H=55 **+0.475**; P2@150 H=1 **+0.585** H=55 **+0.152 conv=0** (H=55 collapsed; not freeze health). recon freeze **0.0026**. jsonl @104 **×1.00 / ×1.02** ≠ 5-level. pid **242511**. | ❌ CAPPED — vs P64 **0.91@DV**. Do **not** score actor (`skip_invalid_p3`). |
...
### p77 (`run_p77_tssm`, branch `cursor/p28`) — LIVE P2: env-free TSSM observer (GAIN_NOT_READY CAPPED)
...
- **LIVE P2 iter 153 (~581 min, nvidia 11.7 GB, P2 `t_wm` ~42 s, sps ~133):** P1 **CAPPED** @104. g FROZEN, DOB training (`dobg` 0.0007→**0.13** @153 peak **0.244** @152; P64 P2 med **0.121** — inverted-g, do not score DOB). recon **0.003–0.009** (0.0091@152, not 20×). skip **0**. last_ok **104 unlocked**. Disk `wm_last_ok.pt` is lock@55 (mtime 00:22), not freeze 104. No detonated-freeze (freeze recon **0.0026** vs best 0.0021 = 1.2×). `wm_best_restore_at_p2=False`. `[wm-fidelity-ES]` **suppressed** @150 (`P2 remaining 48312 < 75396`; defer to natural P2→P3). Do **not** score actor. HEAD P78 `prev_state=` is **not** this pid — do **not** launch P78 while GPU occupied. P2 budget left ~**4 min** at this snapshot; expect `[p3-skip]` then val.

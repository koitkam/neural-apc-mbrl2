...
| P39 | 2026-08-27 | dcv_match scale floor 1.0 | CAPPED 0.70@DV P2 live recon 0.003 dobg>0; extra-P1 no detonation | ⏳ GPU P2; `[p3-skip]` expected |
...
- **P2 LIVE:** recon 0.003, `dobg` 0.002–0.004 (ground live), iso/gmatch 0 (`g` frozen). Fidelity iter 100 `best_h=55/55`. Expect `[p3-skip]`. Do **not** launch a second GPU job.
- **HEAD (not in this pid):** `wm_isolation_dcv_min_scale=1.0` named TrainConfig + `DREAMER_WM_ISOLATION_DCV_MIN_SCALE`; isolation mv/dv traj weighted-mean (no CUDA `.any()` sync); `rssm_imag_latent_mode` **REMOVED** (imagination leftover, never wired). SNR / MIMO holds already HEAD.
- **Next GPU after EXIT+val:** not another isolation reweight; not extra-P1; not no-floor. If val DV ~×0.70, candidate is P26 **gain-match-only** (isolation/ss-match were 0; DV ×0.87). Wait for TM+det_r.

...
| P41 | 2026-08-28 | P1 gate recent-floor (not warmup ema_best) | LIVE extra-P1; gates 82/94 gain-probe 0.76@DV EXTEND | ⏳ GPU |
...
- **Liveness (07:55 CDT):** LIVE extra-P1 through **iter 96**. tmux `mbrl2_p41`, pid **4185725**, launch `acb8a7b`. GPU ~15.7 GB. **No leftover `DREAMER_*`.** `[resolved-cfg] isolation=0 ss_match=0 iso_dcv=off`. `wm_best` **iter 10 / 6.378**. last_ok **96**. **No second GPU job.**
...
- **P40 last-ok overwrite already happened in original P1:** iter 57 recon **0.0887** / gnorm **1.99** / skip 0 (42× best ~0.0021); last_ok **overwrote 56→64** at recon **0.0098 < 5×**. Iter 66 wrap 0.096; iter 74 silent 0.281 skip 94. last_ok then advanced to **82** at the gate, then **86** in extra-P1.
- **First gate iter 82 (original budget 753960) GPU-confirmed:** recent-floor **KEEP as mechanism** (Step 5; already TrainConfig default). Printed **`[gate p1->p2] gain-probe`** (P40 printed `not_plateaued` / **no probe** on ema_best 6.541). `recent_ok` (ema 4.391 ≥ 1.5) so the probe ran. **FAIL GAIN_NOT_READY worst=0.76@DV** (MV **1.01** in [0.8,1.3]; DV 0.76). recon **0.0058** skip 94 iso/ss 0. **EXTEND** +75396 to 829356 (cap 929884). Did **not** freeze pre-extra-P1. Original-budget DV 0.76 ≈ P40 CAPPED 0.75 — extra-P1 is not the DV gap. Expect `[p3-skip]` unless a later probe hits 0.80.
- **Extra-P1 through 96:** **did not detonate** at P40 extra-P1 iter 84 (recon **0.0062** / gnorm **1.04**) nor P37 iter 88 (recon **0.013** / gnorm **0.57**, last_ok held 87 then 89). Iter 94 gnorm **34.8** with recon **0.0036** (applied; below skip 1e4); recovered iter 95 gnorm 0.20. last_ok advancing (**96**). Overwrite hole remains; lock is HEAD not in pid.
- **Second gate iter 94 (budget 829356) GPU-confirmed:** **gain-probe** (not `not_plateaued`). **FAIL GAIN_NOT_READY worst=0.76@DV** (MV **1.03**; DV **0.76** same as gate 1). recon **0.0036** skip 94. **EXTEND** +75396 to 904752 (cap 929884). Did **not** freeze. Extra P1 did not move DV (0.76=0.76). ema 4.391→**5.457**. Expect `[p3-skip]`.
- **HEAD (not in pid):** last-ok lock (`skip_storm_last_ok_lock_ratio=20`) recon-only. **Not stacked onto live P41.** Next GPU after EXIT: last-ok lock.
- **Judge remaining:** CAPPED ~iter 104 (P40 same budget). Extra-P1 can still silent-detonate. If CAPPED GAIN_NOT_READY: `[p3-skip]`. Val MV ~×1.0 ±0.1; DV vs P40 ×0.72 / P26 ×0.87. Actor INVALID if GAIN_NOT_READY.
...
- Isolation/ss-match is **off env-free** (P40 EXIT KEEP as default; **FALSIFIED as DV pin** — val DV ×0.723 not P26 ×0.87). P08 auto-enable isolation=1 / ss_match=3 ran on P26 (jsonl iso 1.44; `run_plan` 0.0 was the pre-rewrite dump) and P32–P39. Abs MSE + whole-episode holds drowned DV (×0.87 → ×0.66–0.70) but turning it off did not recover P26 DV. Gain-match is the DC supervisor. Opt in `DREAMER_WM_INPUT_ISOLATION_COEF=1` (len/settle auto). When on: abs MSE only, inv-var **REMOVED**, `wm_isolation_dcv_match=True` with `wm_isolation_dcv_min_scale=1.0`. Structural `|G_max|/|G_min| > 1/op_band` cannot equalize |ΔCV| (P38 shrink FALSIFIED; P39 cube-boost FALSIFIED; P40 isolation-off FALSIFIED as DV pin). Do **not** re-add `wm_isolation_var_norm`. Do not try another isolation reweight. May-2026 per-head/latent diag probes default **OFF**.
...
- P1/P2 random collect is **numpy-only** (no `torch.inference_mode` / SF `a_history`). P3 on-policy still streams. P2 DOB Kalman scan is a closed-form mix (same recurrence; host-adaptive T×T cap `clamp(GPU_mem/1500, 4MiB, 64MiB)` — A10 24GB still **16 MiB**). Stage-1 skips `apply_dob` clone + ground/reg. CV-std tensor cached. P1 skips unused `agent_finetune_loss` when `reward_scale_loss_p1=0`. Last-ok snapshot `copy_` reuse. Isolation is abs MSE (inv-var helpers deleted). Gain-match FD uses `img_rollout(..., last_only=True, out='obs')` (last-step Huber ≡ `decode(stack[:, -1])`; no unused K-stack; sequential `img_step` fallback **REMOVED**). Overshoot uses `out='obs'` and held-rollout `out='h'` (no unused F-stack; last_only materializes `out` once after the K-loop). P1 gate floor is **recent EMA max** (not return to warmup `wm_score_ema_best`; P40 RCA). jsonl emits `wm_score_ema*`. jsonl always emits `wm_gain_match_loss` / isolation/ss keys (0 when teacher off). Banner `n_grad_skip` is cumulative inner-step skips; skip-storm window is `grad_skip_history` (cleared on restore). HEAD last-ok **locks** after recon > `skip_storm_last_ok_lock_ratio` × best (default 20, unitless; P40 RCA). Call site is recon-only (P41 live: spike iter 58 skip 1 still locks). Skip-storm restore unlocks. P41 pid does **not** have the lock.
- `run_plan.json → config` is rewritten after gain-match resolve. `[resolved-cfg]` prints `iso_dcv=off` when isolation/ss-match are off (P40); when the teacher is on it still prints `iso_dcv=` **and** `min_scale=` **and** `mv=`/`dv=` multipliers **and** `edge_du_mv=`/`edge_du_dv=` **and** `g_ratio=`/`smax=`/`equalize=`. `run_plan.isolation_dcv_scales` is the P38/P39 audit trail (inv-var knob removed). Pre-rewrite dump is dataclass defaults (P29 plan still shows `rssm_latent_type=categorical` / `gain_match_coef=0` even though training auto-enabled grounding).
...
| Extra P1 via keep-ext to pin DV DC-gain | P33 EXIT; P37 EXIT; P39 CAPPED | FALSIFIED as DV lever (keep-ext KEEP as mechanism) | YES — P33 val DV ×0.66; P37 0.68→0.72 then detonated; P39 extra-P1 stable then CAPPED 0.70@DV |
...
| P1 gate recent-floor (`_p1_fidelity_local_plateau`) | P41 LIVE iters 82+94 | KEEP as mechanism; did **not** freeze (GAIN_NOT_READY) | YES — both gates printed gain-probe (ema 4.391 then 5.457 ≥ 1.5 vs global 6.378@10). FAIL 0.76@DV both (MV 1.01 then 1.03). Extra P1 did not move DV. Extra-P1 healthy through 96 (P40 detonated @84; P37 @88). |
| Extra-P1 last-ok overwrite after silent recon spike | P40 EXIT iter 98 (83→104); **P41 live original-P1** iter 64 (56→64) | HEAD lock (`skip_storm_last_ok_lock_ratio=20`; recon-only call site; not in P41 pid) | YES — recovered recon < 5× best overwrites pre-spike last-ok; freeze recon healthy so detonated-freeze misses. P41: 0.0887/0.0021=42× then 0.0098 overwrote. Skip-storm restore still unlocks. Wrap ~5–10× must not lock. |

...
| **Critic** | `critic_rew_to_tgt_var` (>0.015) | none healthy | — | P74 val `critic_r` **+0.738** PASS (P73 +0.713; P72 +0.694; P68 +0.699; P64 +0.705); P3 rtgt still collapses (P74 med **0.0009**, P73 med **0.0035**). Freeze rscale KEEP. |
| **Actor** | econ vs baseline + vs expert BC | **P64** (mbrl2) | paired **−4.54 vs −94** | Env-free P2 amp=P3. 9/9 `all_pass`. **P73 −4.91 vs −125 VALID 9/9** (mv_viol **2.75**, TM ×0.72) — closest econ since P68, not a champ swap. **P74 −48.48 vs −105 VALID 9/9** (mv_viol **20.05**, det_r collapse — skip steal). **P68 −3.90 vs −85 VALID 9/9** (mv_viol **1.92 vs 0.33**, TM **×0.69**). P72 **−22.71 vs −92.72 VALID 9/9** (mv_viol **9.70**, last_ok **15**). P53 stays the μ-ratio-clip RCA. |
...
| P75 | 2026-09-02 | FOPDT rise teacher `G_tgt·FO(k)/FO(K)` (last step = DC; no new knob) | LIVE P3 ~260. GAIN-READY **0.84@MV** @82 last_ok **82** skip **0**. CPU TM@K=55 freeze ckpt80 MV **×0.83** mid/FO/ss **1.24** (P64@80 ×0.97 / 1.15). P2→P3 @136 mtp 0.950. Unfreeze 147 ent HELD; logp spike recovered; rscale 1.85; rtgt med 0.002. | 🔬 LIVE — FO at K=H did not pin freeze rise vs P64. Valid actor experiment. Do not FO N+1 if FALSIFIED. No second GPU job. |
...
### p75 (`run_p75_gmatchfo`, branch `cursor/p28`) — LIVE P3: FOPDT rise teacher; GAIN-READY 0.84@MV
...
| jsonl gain-match `*_ratio` last-step only (FO rise weight invisible) | **P75 GPU-occupied** (pid **230450** P3 ~260) | SUPERSEDED (`gain_match_{mv,dv}_ratio_mid` / `*_mid_fo` at `k=(K-1)//4`; then `gain_match_{rise,last}_wfrac`) | YES — last-step FO(K)/FO(K)=1 cannot show a contracted rise. `(k/K)^2` rise mass **0.014** at K=55. CPU TM@K=55 freeze ckpt80: P75 MV **×0.83 / 1.24** vs P64@80 **×0.97 / 1.15** vs P73@80 **×0.57 / 1.43**. Gate **GAIN-READY 0.84@MV**. Not in this pid. CVD="" smoke. |

...
| P68 | 2026-09-01 | rest-IC Huber baseline = last rest-obs CV (TM `pre`; not held-K) | LIVE P1 iter **1** pid **193014** sha `9c18daa`. `gmatch_len=55`. Iter 1 teacher **×0.655 / ×−1.12** gmatch **1.53** (not jsonl ×1). skip 0. | 🔬 vs P64 0.91@DV / MV ×0.927; no MV overgain |
...
- **Step 4 CONFIRMED:** `device=cuda` bs=128 `wm_train_T=128` horizon=55 compile=eager. `[gain-match] len=55 settle=-1 step=0.4`. Huber baseline print **ON**. `[resolved-cfg] … isolation=0 gmatch_len=55 gmatch_step=0.4 gmatch_clip=True gmatch_rest=True gmatch_rest_L=128 held_cv=True p1amp=0.2 p2amp=1 p3amp=1 compile=eager`. Rest-IC graph captured `N=6 T=128`. Iter 1 teacher **×0.655 / ×−1.12** gmatch **1.53** skip **0** (P67 iter 1 was ×0.96/×0.97 gmatch ~0.002 — protocol is live). nvidia **~14 GB**. **One GPU job.**
- **Watch:** jsonl `*_ratio` vs tautological ×1; first gate vs P64 **0.91@DV**; no MV overgain (P67 val ×1.35). Do not teacher-K N+1. Do not score actor if GAIN_NOT_READY. **No second GPU job.**

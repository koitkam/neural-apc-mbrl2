...
- **Liveness:** **P1 live.** GPU ~11.3 GB. tmux `mbrl2_p45`, pid **16548**, launch `cf25923`. `[env-override] gain_match_rest_ic=True`. `[gain-match] rest-ic N=6 L=128 settle=128` (not `wm_tf_horizon=220`). `[resolved-cfg] gmatch_rest=True gmatch_settle=-1`. Iter 29 recon **0.0036** gmatch **0.0002** skip **0** last_ok **29**. jsonl ratio MV/DV ~×1.00 (teacher FD, not TM). `t_wm` **134 s** vs P43 **98 s** (100 inner × extra L-step GRU encode; last_only HEAD `deb0236` not in this pid). gnorm spikes iter 2/13/23 recovered. **Do not launch a second GPU job.**
- **HEAD (not this pid):** `rollout_observed(..., last_only=True)` for rest-IC encode (`deb0236`). Same last `h/z/c_mean` as the full T-stack (smoke identity + GRU grad). Drops unused `(N,L,F)` + Stage-1 `(N,L,n_cv)` zeros every inner step. Does **not** skip the GRU loop (that *is* the rest IC). P45 pid still stacks. `actor_train_source` other than `realsim` now raises at `train()` start (leftover imagination override used to skip `onpol_buf`).

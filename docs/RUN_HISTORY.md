...
### p69 (`run_p69_oltail`, branch `cursor/p28`) — LIVE: stop-grad OL tail at val TM window
- **One attributed GPU change vs P68:** rest-IC last-only Huber at K=H KEEP, then stop-grad the prior state at K and continue the held step for `wm_tf_horizon−K` (test_sim **165**) with the same identifier tgt / rest-pre `cv_base`. Pins DC at the val TM window without P67 full-BPTT 4H and without P63 FO 1step→K. **No new TrainConfig field.** Isolation off. Clip KEEP. P2 amp = P3 cap KEEP. Mixed ring KEEP. Env-free: **no leftover `DREAMER_*`**. tmux `mbrl2_p69`, pid **198448**, sha `256c889`, out-dir `output/test_sim/run_p69_oltail`. Process environ: `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF` only.
- **Step 4 CONFIRMED:** `device=cuda` bs=128 `wm_train_T=128` horizon=55 compile=eager. `[gain-match] Huber baseline = rest last-obs CV (TM pre; not held-K)`. `[gain-match] rest-IC OL tail Huber K_tail=165 (stop-grad @K=55; val TM window; not teacher-K)`. `[resolved-cfg] isolation=0 gmatch_len=55 gmatch_ol_tail=165 gmatch_step=0.4 gmatch_clip=True gmatch_rest=True gmatch_rest_L=128 held_cv=True p1amp=0.2 p2amp=1 p3amp=1 compile=eager`. Rest-IC graph captured `N=6 T=128`. **No leftover DREAMER_***. **One GPU job** (~14.2 GB).
- **P1 (iters 1–17, still running):** Iter 1 teacher MV/DV **×0.671 / ×−1.30** (P68-class, not tautological held-K ×1); tail **×0.669 / ×−1.29** `ol_tail_loss=1.555` `t_wm` **133 s** (P68 **96 s** — extra 165-step tail) gpu_peak **11.5 GB** nvidia **~14.3 GB**. Iter 5 recon **0.277** teacher DV **×−4.55** (P68-class early basin; P68 recon **0.216** @iter 6) then recovered. Teacher **and** tail pin ~×1 by iter **9**. Iter **15** recon **0.0106** teacher **×0.990 / ×1.014** tail **×1.000 / ×1.023** tail_loss **0.0006**. Iter **17** recon **0.0076** (P68 **0.010** @15) gmatch **0.0485** teacher **×0.952 / ×1.304** tail **×0.983 / ×1.300** (tail tracks teacher; not exploding) skip **0**. `wm_best` @iter 10 (gain-blind H=1; restore OFF). jsonl tail ≈ teacher is **not** val TM (P67 identifier ×1 at K=220 still missed plant rest). Watch wrap ~32; freeze vs P64 **0.91@DV** (no P67 MV overgain); val 1step→OL / OL-vs-real toward P64 ×0.85 / ×0.84; ss toward ×0.93; `actor_experiment_valid`. Do **not** score actor if GAIN_NOT_READY. **No second GPU job.**
...
| Rest-IC Huber baseline = last rest-obs CV (TM `pre`; not held-K) | **P68 EXIT** (`run_p68_tmpre`, pid **193014**, sha `9c18daa`, 541 iters) | KEEP as protocol / **FALSIFIED as TM-closer-to-P64 / compounding / actor-champ** | YES as tautology-fix (iter 1 ×0.66/×−1.12; no P67 overgain) / **NO as TM pin**. Val MV **×0.692 / ×0.725** vs P64 ×0.927/×0.932. 1step→OL **×0.787** OL-vs-real **×0.716** (P64 ×0.850/×0.842). Paired **−3.90 vs −85** 9/9 VALID but mv_viol 1.92 vs P64 0.33. Freeze last_ok 73 vs P64 82. Do not revert held-K `cv_base`. Do not teacher-K N+1. Next: stop-grad OL tail (P69). |
| Stop-grad OL tail after teacher K (`wm_tf_horizon−K`; test_sim 165) | **P69 LIVE** (`run_p69_oltail`, pid **198448**, sha `256c889`) | LIVE (jsonl teacher+tail pin ×1 by iter 9; @17 DV ×1.30 together; skip 0; `t_wm` 133 s) | — pending freeze vs P64 0.91@DV and val 1step→OL / OL-vs-real vs P64 ×0.85/×0.84. jsonl tail ≈ teacher ≠ val TM (P67 class). Not P24 TBPTT-on-K (teacher K still full-BPTT). Not P67 full-BPTT 4H. GRU weights still see 165-step tail grads. CVD="" smoke: `prev_state.detach()` leak=0 (control 5e-2). |

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **extra-P1 iter 83**. Orig-P1 @82 **FAIL GAIN_NOT_READY −1.06@MV** DC **[−1.06, 0.75] @H [−0.20, 0.70]** noisy (spread×5.2 signflips=6 `unbiased=False`). EXTEND +75396 to 829356. Probe80 **H=1 r=+0.583 / H=55 r=+0.423 conv=0**. recon **0.0032@78 best**. persist **0.030@83** (full inject @80 **did not spike**). Teacher jsonl @82 **×1.01 / ×0.97** ≠ 5-level. pid **242511**. | 🔬 LIVE extra-P1 — orig-P1 miss vs P64 **0.91@DV** (sign-flip, worse than P76 0.79@DV). Do not score actor. Do not patch live. |
...
- **LIVE extra-P1 iter 83 (~434 min, nvidia 11.7 GB, `t_wm` ~307 s, sps 18.6):** skip **0**. recon **0.0032@78 best → 0.0032@81 → 0.0033@82 → 0.0034@83**. Teacher jsonl 81 **×1.000 / ×0.961** → 82 **×1.005 / ×0.974** → 83 **×1.002 / ×0.976**. last_ok **83 unlocked**. persist_rel **0.034@80 → 0.031@81 → 0.036@82 → 0.030@83** — full inject @80 **did not spike** (unlike 0.82@41 / 0.73@51 / 0.59@62; do **not** persist N+1). Rest-IC graph not captured. Extra-P1 +75396 steps (to 829356, cap 929884). Next extra-P1 gate typically ~92–94 (P76 live @94). Next probe typically iter 90 (~03:18 CDT).

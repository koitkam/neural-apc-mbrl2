...
| P53 | 2026-08-30 | P3 PPO ratio clip vs frozen unfreeze-μ (`p3_mu_ratio_clip=0.2`) | LIVE. One attributed μ-walk limiter vs P52. Watch unfreeze 147: unclipped logp_std ~0.5 and mvv warmup-class (~16k), not 813k. | 🔬 LIVE |
...
- **One attributed GPU change vs P52 pid:** `p3_mu_ratio_clip=0.2` (HEAD default). PPO surrogate `min(ratio·adv, clip(ratio, 1−ε, 1+ε)·adv)` with `logp_old` from a **frozen deepcopy of the policy at the first `_realsim_actor_critic_step`** (critic warmup does not `opt_actor.step`, so this is the unfreeze μ). Jsonl `actor_logp_*` stays unclipped; adds `actor_ratio_mean` / `actor_ratio_clip_frac`. Opt out `DREAMER_P3_MU_RATIO_CLIP=0` (identity REINFORCE + P52 logp clip). Also carries P52 `p3_logp_clip=8`, P51 `p3_stop_grad_log_std=True`, P50 `bc_mean_only=True`. Env-free. tmux `mbrl2_p53`. `CUDA_VISIBLE_DEVICES=0`.
...
- **Watch:** Gate GAIN-READY vs P52 last-ok 78. First P3 ent ≈ H(σ_init). Unfreeze 147: ent HELD, ratio clip_frac, actor_loss / mvv. Do not call KEEP/FALSIFIED as cascade until EXIT.

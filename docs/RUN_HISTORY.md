...
| P53 | 2026-08-30 | P3 PPO ratio clip vs frozen unfreeze-μ (`p3_mu_ratio_clip=0.2`) | LIVE `126f011` pid **75966** tmux `mbrl2_p53`. Step 4 `p3_sglogstd=True p3_logpclip=8 p3_muratio=0.2`. P1 iter **57** recon **0.0024** (best **0.0017** @46) gmatch **0.0001** FD MV/DV ×1.01/×0.99 skip **0**. wm-fid @50 @H r=+0.420 conv=1.00 gain_fid 0.780; EMA **5.998** held @30. Next probe ~60. Watch unfreeze 147. | 🔬 LIVE |
...
- **Liveness:** P1 STAGE 1. Iter **57** (57 jsonl rows, ~123 min). recon **0.128→0.0024** (best **0.0017** @46 held; bump 0.0037 @56 recovered **0.0024** @57) gmatch **0.0001** (FD MV/DV **×1.01 / ×0.99**). iso 0 ss 0 skip **0**. last_ok **57** unlocked. wm-fidelity probe @**50**: H=1 r=+0.721 @H r=+0.420 conv=1.00 drift=0.012 gain_fid=0.780 (not the TM gate). EMA **5.998** held @**30** (raw 6.474, `wm_best.pt` unchanged; live EMA 5.765). Next probe ~60. sps ~48. GPU ~11 GB. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Gate vs P52 last-ok 78 not yet. Do not KEEP/FALSIFIED as cascade until EXIT.

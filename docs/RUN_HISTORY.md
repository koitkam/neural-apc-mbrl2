...
- **LIVE (2026-08-29 17:38 CDT, P1 iter 29):** recon **0.077→0.0028** (iter 26 best; iter 29 **0.0045**) kl 0 jemb 0.011 gmatch ~0 skip **0** alive 1024. **`bc_loss` +0.27…+0.41 every iter (MSE, never negative)** vs P49 P2 NLL. Teacher FD ~×1.0. gnorm blips iter 5 **20.9** and iter 23 **23.6** recovered next iter (no skip-storm; last_ok **29** unlocked; no 20× wrap). Fidelity probe iter 20 H=55 r=+0.495 `best_h=55/55` gain_fid=0.692 → `wm_best`. GPU ~11 GB. First P3 ent target ≈ **−0.08**, not floor **−0.283**.
- **Watch:** skip-storm ~iter 50–65 if it fires; P2 `bc_loss` stays **>0**; first P3 entropy ≈ **−0.08** not −0.283; freeze rscale ~2; do not train P3 on GAIN_NOT_READY. Remaining leftover is env-only paths/device — do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.

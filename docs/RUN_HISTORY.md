...
| **Actor** | econ vs baseline + vs expert BC | **P50** (mbrl2) | paired **−56 vs −104** | First GAIN-READY val beat (9 pairs; seed kpi −74). Unfreeze still yanked σ. P51 also beats (−72 vs −111) but worse. P52 logp-clip **FAIL** (−377 vs −87, 0/9). Do not score GAIN_NOT_READY freezes. |
...
| P53 | 2026-08-30 | P3 PPO ratio clip vs frozen unfreeze-μ (`p3_mu_ratio_clip=0.2`) | LIVE `126f011` pid **75966** tmux `mbrl2_p53`. Step 4 `p3_sglogstd=True p3_logpclip=8 p3_muratio=0.2`. P1 iter **67** recon **0.0027** (best **0.0017** @46) gmatch **0.0002** FD MV/DV ×0.99/×0.97 skip **0**. wm-fid @60 @H r=+0.324 gain_fid 0.654; EMA **5.998** held @30 (live 5.630). Next probe ~70. Watch unfreeze 147. | 🔬 LIVE |
...
### p53 (`run_p53_muratio`, branch `cursor/p28`) — LIVE: PPO ratio clip vs frozen unfreeze-μ
- **One attributed GPU change vs P52 pid:** `p3_mu_ratio_clip=0.2` (HEAD default `126f011`). PPO surrogate `min(ratio·adv, clip(ratio, 1−ε, 1+ε)·adv)` with `logp_old` from a **frozen deepcopy of the policy at the first `_realsim_actor_critic_step`** (critic warmup does not `opt_actor.step`, so this is the unfreeze μ). Jsonl `actor_logp_*` stays unclipped; adds `actor_ratio_mean` / `actor_ratio_clip_frac`. Opt out `DREAMER_P3_MU_RATIO_CLIP=0` (identity REINFORCE + P52 logp clip). Also carries P52 `p3_logp_clip=8`, P51 `p3_stop_grad_log_std=True`, P50 `bc_mean_only=True`. Env-free. tmux `mbrl2_p53`, pid **75966**, `CUDA_VISIBLE_DEVICES=0`. Started 06:09 CDT.
- **Not:** leftover `DREAMER_*`, tighter `p3_logp_clip`, `p3_reset_log_std`, extra critic knobs, `actor_kl_coef`, same-forward `logp.detach()` as old, lag-copy of the collector (1 AC step/batch ⇒ ratio≡1).
- **Step 4 CONFIRMED:** `[resolved-cfg] latent=deterministic restore_p2=False gain_match=1 dob_ground=2 dob_reg=0 isolation=0 ss_match=0 iso_dcv=off n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 lock=20 huber_per_in=True gmatch_settle=-1 gmatch_rest=True p3_sigreset=False bc_mean=True p3_sglogstd=True p3_logpclip=8 p3_muratio=0.2 compile=eager`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`.
- **Liveness:** still **LIVE** pid **75966**, tmux `mbrl2_p53`, GPU ~11 GB. ~208 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. jsonl through **P3 iter 178**. last_ok **82 unlocked**. sps ~156. Do not KEEP/FALSIFIED as cascade until EXIT.
  - **P1:** skip-storm **1/2 @73** recovered; **GAIN-READY P1→P2 @82** (no extra-P1) live worst **0.88@DV** (MV 0.94/@H 0.93, DV 0.88/@H 0.93). vs P52 last-ok **78** ~0.84@DV. Warm-restore SKIPPED (skip-storm already restored last_ok 72).
  - **P2:** STAGE 2 @82, g FROZEN, DOB id. wm-fid @**130** H=55 r=+0.430 gain_fid **0.539**; **wm-fidelity-ES suppressed** (P2 remaining 36600 < 75396). Live EMA mixed to **5.784**; best **6.845 @90** held (`wm_best.pt`).
  - **P2→P3 PASS @136:** `[gate p2->p3] PASS` reward_mtp_loss median=**0.909** (max=3.00, n=5); `p2_extension=0/39284`. STAGE 3 @iter **136** env_steps **1090680**. `[p3-start] return_scale=1.00`. `[return-scale] FREEZE after critic warmup (p3_iter=10, scale=2.302)`.
  - **First P3 (warmup 137, actor frozen):** ent **−0.163** (not σ_min **−0.283**; slightly below H(σ_init)≈**−0.101**; P50/P51 first-open **−0.107 / −0.101**). logp_std **0.48**, ratio≡1 clip_frac **0**. rtgt **0.062**. mvv **1203@141 / 76@146**.
  - **Unfreeze 147 (A/B canary — no cascade verdict yet):** ent **−0.163→−0.101 HELD** through **178** (stop-grad yank-block holding). rscale **2.302 FREEZE** KEEP-class. actor_loss **−0.013** @147 (not P51 **−4.66** / P52 delayed **−0.77**); |aloss|≤0.16 through 178. Unclipped logp_std **0.52@146 → 1.58@147 → 6.71@149** (P52 first-unfreeze class **6.11**, not P51 **38**) then **returned ~0.51@178**. `actor_ratio_clip_frac` **0.61@147 / 0.97@149** then **0.23@178**; `actor_ratio_mean` **0.82→0.46@149** then recovered **~0.99**. PPO clip **fired then eased** (ratio≈1). mvv **0@151 / 807@161 / 751@166 / 5831@171 / 186@176** (still ≪ **813k**; 5.8k bump @171 watch). bc spike **0.55@149** then **0.004@178**. ret_w **−48@151 / −54@178** (not P51 **−1822**). ema **−219@137 → −110@178** (not P52 **−2231@209**).
  - **Passed P52 delayed-rail window @165–169:** P52 unclipped std **80.8@165 / 100.7@169**. P53 same iters **0.61 / 0.55**. Not a cascade verdict (rtgt still low; run not EXIT).
  - **Watch (not a verdict):** rtgt **0.054@147 → 0.0037@168 → 0.0059@178** (below critic healthy **0.015** after ~156; P51 best.pt **161** had **0.0057** then collapsed to **1e-4**). logp_std recovered and **held ~0.51 through the P52 walk window**. Next ticks: rtgt stay ~0.005 vs walk to **1e-4**; mvv stay <~10k vs jump 100k+; unclipped logp_std stay ~0.51 vs re-rail.
- **Hypothesis:** in-support `(u−μ)/σ²` SGD cannot walk μ onto the tanh rail once `π_new(a)/π_snap(a)` leaves `[0.8, 1.2]`. Unfreeze 147 unclipped logp_std stays ~0.5 and mvv stays warmup-class (~16k), not 813k. **So far:** first-unfreeze std spiked then **came back and held** through P52's delayed-rail window; mvv still ≪813k; clip_frac high then easing as ratio_mean ~1. Do not call KEEP/FALSIFIED as cascade until EXIT.
- **Watch:** late-P3 rtgt / mvv / logp_std. Do not call KEEP/FALSIFIED as cascade until EXIT.
...
| P3 in-support μ-walk (\|logp\|<8 SGD) | P52 EXIT; **P53 LIVE** (`run_p53_muratio`) | LIVE (`p3_mu_ratio_clip=0.2` vs frozen unfreeze-μ) | YES — first AC call snapshots policy (warmup does not step actor). Unfreeze 147: ent HELD −0.101; clip_frac 0.61→0.97@149 then 0.23@178; logp_std spiked 6.71@149 then **held ~0.51 through P52 walk window 165–169** (P52 80.8/100.7); mvv 5.8k@171 not 813k; actor_loss bounded. rtgt 0.054→0.006 **watch**. No cascade verdict until EXIT. Opt out `DREAMER_P3_MU_RATIO_CLIP=0`. Not `actor_kl_coef`. Not lag-copy. |

...
| jsonl gain-match `*_ratio` last-step only (FO rise invisible) | **P75 GPU-occupied** (pid **230450** P1) | SUPERSEDED (`gain_match_{mv,dv}_ratio_mid` / `*_mid_fo` at `k=(K-1)//4`; aliases `wm_*`) | YES — last-step FO(K)/FO(K)=1 cannot show a contracted rise. Same FD K-stack; no extra roll. CPU TM@K=55 iter20: jsonl ×1 vs MV ss ×0.80 / mid/FO/ss 0.70. Not in this pid. CVD="" smoke. |

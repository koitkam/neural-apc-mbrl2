...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 70**. Probe70 **H=1 r=+0.746 / H=55 r=+0.160 conv=0.00** gain_fid **0.537** best_h=27/55 drift **0.389**. recon **0.0047@68 best** (P64 @70 **0.0035** ~1.3×) then **0.0055@70**. Teacher **×1.01 / ×0.87 @70**. persist_rel **0.57**. pid **242511**. | 🔬 LIVE P1 — H=1 still beat P64; H=55 **collapsed** 0.293→0.160; conv still 0 **@10–70**. Do not patch live. |
...
- **LIVE P1 iter 70 (~366 min, nvidia 11.7 GB, `t_wm` ~306 s, sps 19.7):** skip **0**. recon **0.0052@67 → 0.0047@68 new best → 0.0051@69 → 0.0055@70** vs P64 **0.0035@70** (~1.6× live; ~1.3× vs best). Teacher jsonl 68 **×0.998 / ×1.025** → 69 **×1.008 / ×0.942** → 70 **×1.011 / ×0.868**. last_ok **70 unlocked**. persist_rel **0.39@68 → 0.40@69 → 0.57@70**. Rest-IC graph not captured. Next probe typically iter 80.

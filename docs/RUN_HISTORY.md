...
| P61 | 2026-08-31 | env-free cube-clip + realized Δu for gain-match FD | LIVE `run_p61_clipdu` pid **155682** sha `c4ed6f3`. P1 ~iter **50** (~88 min): recon **0.002**, teacher ×0.977/×0.979, `du_frac=1.0`, skip **0/0**, last_ok **50**. Tracks P60. Gate ETA ~19:43. | LIVE |
...
- **LIVE ~iter 50 (2026-08-31 18:45 CDT, pid 155682, ~88 min):** P1 still healthy; **no second GPU job.** recon last-10 **0.0020–0.0034** (P60 matched **0.0020–0.0034**). jemb **0.005–0.007**. gmatch **~3e-4**. Teacher last-10 MV/DV **×0.972–×1.025 / ×0.977–×1.009**; iter 50 **×0.977 / ×0.979**. `du_frac=1.0` every iter (this pid has **no** `clip_frac` key). skip **0/0**. last_ok **50** unlocked. `t_wm` **~98 s**. Rest-IC graph still captured. `wm_best` iter **30** (gain-blind; P60 also froze 30 until later 70). Fidelity: @30 gain_fid **0.974** (P60 0.936); @40 **0.555** (P60 0.552); @50 H1/13/27/55 **0.676 / 0.488 / 0.392 / 0.388** conv=1.00 drift=0.009 `best_h=13` gain_fid **0.794** (P60 @50 **0.767**, `best_h=55`). gnorm spikes ~23 @19/25/28/48 (P60-class; skip never fired). Gate ~82 ETA **~19:43**. Matched-iter P1 is **P60-class** — on test_sim `op_band+step=1.0` reverse may be near-identity; **do not FALSIFY clip until val TM / det_r / paired**. Watch live gate vs P60 0.86@DV.
...
| jsonl `du_frac` as “did the cube fire?” | P61 LIVE (`run_p61_clipdu` pid **155682**) | SUPERSEDED (`gain_match_clip_frac` = mean `+step`-would-clip; HEAD, not this pid) | YES — reverse keeps `|Δu|` so du_frac stayed **1.0** through iter **50** even if outer rest+jitter hits the rail. Matched-iter P1 ≈ P60. Loss identity. CVD="" smoke interior 0 / rail 1. |

...
| P41 | 2026-08-28 | P1 gate recent-floor (not warmup ema_best) | LIVE P2 after CAPPED 0.74@DV; `[p3-skip]` | ⏳ GPU |
...
- **Liveness (08:16 CDT):** LIVE **P2** iter **119**. tmux `mbrl2_p41`, pid **4185725**, launch `acb8a7b`. GPU ~15.7 GB. recon **0.0033** gmatch **0** (g frozen) **dobg 0.005 live**. last_ok **104**. **No second GPU job.** Expect `[p3-skip]`.
...
- **CAPPED iter 104 (cap 175924) GPU-confirmed:** cap-time **gain-probe** (P40 cap-time still said `not_plateaued` + 0.75@DV). **GAIN_NOT_READY worst=0.74@DV** (MV **1.02**; *slightly worse* than 0.76). Freeze recon **0.0028**. Warm-restore **SKIPPED** (storm used last-ok iter 38; wm_best gain-blind). STAGE 2 g FROZEN. P2 through **119** recon **0.0031–0.0037** **gmatch 0** **dobg 0.003–0.011 live**. Extra P1 FALSIFIED as DV lever (0.76→0.76→**0.74**). Expect `[p3-skip]`. Actor INVALID.
...
- **Judge remaining:** Val MV ~×1.0 ±0.1; DV vs P40 ×0.72 / P26 ×0.87. Actor INVALID (GAIN_NOT_READY). Do not train P3.
...
| Extra P1 via keep-ext to pin DV DC-gain | P33 EXIT; P37 EXIT; P39 CAPPED; **P41 CAPPED 0.74@DV** | FALSIFIED as DV lever (keep-ext KEEP as mechanism) | YES — P33 val DV ×0.66; P37 0.68→0.72 then detonated; P39 extra-P1 stable then CAPPED 0.70@DV; P41 original-budget 0.76 then cap **0.74** (healthy extra-P1, slightly worse) |
...
| P1 gate recent-floor (`_p1_fidelity_local_plateau`) | P41 LIVE CAPPED iter 104 | KEEP as mechanism; did **not** freeze (GAIN_NOT_READY) | YES — gates 82/94/104 all printed gain-probe (P40 `not_plateaued`). FAIL 0.76@DV then CAPPED **0.74@DV** (MV 1.02). Extra P1 FALSIFIED as DV lever (0.76→0.74). Extra-P1 healthy (no P40-84 / P37-88 detonation). |

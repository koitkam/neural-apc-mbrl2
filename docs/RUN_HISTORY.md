...
| P39 | 2026-08-27 | dcv_match scale floor 1.0 | CAPPED 0.70@DV P2 ~iter 117 recon 0.003 dobg>0; extra-P1 no detonation | ⏳ GPU P2; `[p3-skip]` expected |
...
- **LIVE (2026-08-28 03:16, iter 70, pid 4176670, `73a5116`):** device=cuda bs=128 det+eager. Isolation/ss jsonl keys **absent**. Survived P30 (iter 19) and P32 (iter 53) windows. **Skip-storm 1/2 @iter 65** (same full-BPTT gain-match class as P32, later): iter 63 recon 0.0021 gmatch 1e-4 skip 0; iter 64 recon **0.246** gmatch **1.07** gnorm 1.03 skip 0 (applied, below skip 1e4); iter 65 recon 0.466 gmatch 0.66 gnorm 9.4 **skip 7** last_ok=63; `[skip-storm] restored wm_last_ok iter 63; continuing P1 (storm 1/2, original P1 753960, extension kept 175924)`. **Recovered and held:** iter 66 recon 0.0041; **iter 70 recon 0.0021** gmatch 1.7e-4 gnorm 0.24 last_ok=70. Banner `skip 7` is **cumulative** inner-step skips (`n_grad_skip`); window is `grad_skip_history.clear()` on restore — not a storm-2 hair-trigger. Isolation-off **did not** eliminate storms. Do not lower skip threshold. Storm 2 still `_force_p1_cap_at`. Iter-60 probe H=1/13/27/55 r=+0.664/+0.472/+0.388/**+0.219** `best_h=13/55` `gain_fid=0.718` vs P39 iter-60 r@H55 **+0.521** / P28 **+0.668** (WATCH: isolation-off removed the CV-trajectory teacher; gain-match is last-step FD; overshoot already uses recon `cv_weight=6`/`dv_weight=0` — leftover mass is MV/aug/derived, MV obs SNR **−8.9 dB**. Do **not** stack a CV-mask overshoot until P40 EXIT). t_wm **~99 s**. GPU ~17 GB. **No second GPU job.** Next: P1 gate ~iter 75 EXTEND vs CAPPED.
- **HEAD (not in pid):** `img_rollout(..., last_only=True)` for gain-match (committed `b5ed878`). This commit: overshoot `out='obs'` / held `out='h'` skip the unused `(Bm,K,F)` stack (~2 GB at B=128/starts=24/K=55; pointwise decode ≡ batched); last_only materializes `out` once after the K-loop; jsonl `wm_gain_match_loss` alias + isolation/ss keys emit **0** when teacher off. `[resolved-cfg] iso_dcv=off` when teacher off (live pid still prints `iso_dcv=True`, inert).
- **Judge by:** Val MV ss/@H ~×1.0 ±0.1; **DV toward P26 ×0.87 not P39 ×0.68**; det_r toward P26 0.68. If GAIN-READY: first valid actor test of min-of-2 + freeze `return_scale`. If still GAIN_NOT_READY: `[p3-skip]`. If DV stays ×0.68, isolation was not the regression (re-diagnose sample-T / freeze / DOB).
...
- Isolation/ss-match is **off env-free** (P40). P08 auto-enable isolation=1 / ss_match=3 ran on P26 (jsonl iso 1.44; `run_plan` 0.0 was the pre-rewrite dump) and P32–P39. Abs MSE + whole-episode holds drowned DV (×0.87 → ×0.66–0.70). Gain-match is the DC supervisor. Opt in `DREAMER_WM_INPUT_ISOLATION_COEF=1` (len/settle auto). When on: abs MSE only, inv-var **REMOVED**, `wm_isolation_dcv_match=True` with `wm_isolation_dcv_min_scale=1.0`. Structural `|G_max|/|G_min| > 1/op_band` cannot equalize |ΔCV| (P38 shrink FALSIFIED; P39 cube-boost FALSIFIED; val DV ×0.679). Do **not** re-add `wm_isolation_var_norm`. Do not try another isolation reweight. May-2026 per-head/latent diag probes default **OFF**.
...
| P08 auto-enable isolation=1 / ss_match=3 on cont-gain | P26–P39 (P26 jsonl iso 1.44; run_plan 0 was pre-rewrite dump) | SUPERSEDED (P40 env-free off; opt-in) | PARTIAL — P26 DV ×0.87 with weak isolation; P28+ whole-ep + ss_match=3 drowned DV to ×0.66–0.70. Gain-match stays the DC supervisor. |
...
| Gain-match `img_rollout` stacked unused (Bm, K, F) for last-step Huber | P40 GPU-occupied | SUPERSEDED (`last_only=True`) | YES — Huber is ΔCV at step K; GRU recurrence unchanged; smoke last_only ≡ stack[:, -1], batched Huber = seq |

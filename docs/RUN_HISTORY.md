...
| P57 | 2026-08-31 | rest-IC encode L=last `max(K,2τ/sr)` (`gain_match_rest_ic_len=-1`) | **LIVE P1** ~iter 35. Graph **captured** N=6 T=55. `t_wm` median **99 s** (P56 124 s). Teacher FD MV/DV ~×0.99/×1.00. skip 0. last_ok **35** unlocked. `wm_best` iter **30**. | ⏳ LIVE GPU. Encode L PARTIAL until EXIT+val TM. Capture KEEP this pid. Do not launch a second job. |
...
- **Liveness:** **LIVE P1** ~iter **35**. GPU occupied (~12320 MiB). skip **0**. recon **0.080→~0.003**. Teacher FD MV/DV ~**×0.99 / ×1.00**. `t_wm` median **99 s** (P56/P53 ~124 s). last_ok **35** unlocked. `wm_best` iter **30** (gain-blind; do not freeze on that).
- **HEAD (not this pid):** capture hygiene (zero_grad + `cuda.synchronize` + suppress AccumulateGrad warn); skip encoder-var / Huber MV–DV jsonl except last logged inner; P1 H2D `obs`/`act`/`dist` except MTP last; cache rest-IC FD `a_seq`. Identity. CVD="" smoke green. **Do not launch a second GPU job.**
...
| P1 H2D `rew`/`expert` when MTP not in the graph | P57 GPU-occupied (pid **116788** P1) | SUPERSEDED (`_batch_np_to_device(..., keys=('obs','act','dist'))` except last logged inner) | YES — paper `reward_scale_loss_p1=0`. P2 still full batch. Identity values. Not in this pid. |

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P2** iter **130** after **CAPPED GAIN_NOT_READY −0.46@MV / 0.74@DV** last_ok **104**. Orig-P1 @82 **FAIL −1.06@MV**; extra-P1 @94 **FAIL −0.29@MV / 0.80@DV**. Probe100 **H=1 +0.567 / H=55 +0.299 conv=0**; P2@130 H=1 **+0.719** H=55 **+0.475 conv=0** best_h=55 (not freeze health). recon freeze **0.0026**. jsonl @104 **×1.00 / ×1.02** ≠ 5-level. pid **242511**. | ❌ CAPPED — vs P64 **0.91@DV**. Do **not** score actor (`skip_invalid_p3`). |
...
- **LIVE P2 iter 130 (~564 min, nvidia 11.7 GB, P2 `t_wm` ~41 s / wall ~45 s, sps ~137):** P1 **CAPPED** @104. g FROZEN, DOB training (`dobg` 0.0007→**0.072** @130; P64 P2 med **0.121** last **0.210** — P77 early-P2, inverted-g, do not score DOB). recon **0.003–0.004**. skip **0**. last_ok **104 unlocked**. Disk `wm_last_ok.pt` is lock@55 (mtime 00:22), not freeze 104. No detonated-freeze (freeze recon **0.0026** vs best 0.0021 = 1.2×). `wm_best_restore_at_p2=False`. Do **not** score actor. HEAD P78 `prev_state=` is **not** this pid — do **not** launch P78 while GPU occupied. P2 budget left ~**21 min** (~29 iters); expect `[p3-skip]` then val.

...
- Isolation/ss-match trajectory MSE is still absolute (same family as abs Huber). Variance-normalized isolation was waiting for a **healthy GAIN-READY** freeze showing DV ~×0.87. **P33 P1→P2:** full keep-ext still froze at live **0.68@DV** (P32 val ×0.675 was **not** closed-extension starve). Next attributed GPU change after P33 EXIT: variance-normalized isolation (do **not** revive relative Huber).
...
- P32 EXIT confirmed closed-ext RCA (val MV ×1.09 DV ×0.68 det_r 0.11 `[p3-skip]`). P33 (`run_p33_keepext`) **used** keep-ext (iter 75/85 EXTEND, iter 97 CAPPED **0.68@DV**). Extra P1 did not pin DV. Do not pass leftover compile/latent/DOB env-vars. Next GPU after EXIT: variance-normalized isolation.

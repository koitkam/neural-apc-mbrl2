...
### p75 (`run_p75_gmatchfo`, branch `cursor/p28`) — LIVE: FOPDT rise teacher (last step = DC)
...
- **Step 4 CONFIRMED:** `device=cuda` bs=128 `wm_train_T=128` horizon=55 compile=eager. `[cont-latent] GAIN-ONLY` gain_dim=2. `[resolved-cfg] isolation=0 gmatch_len=55 gmatch_step=0.4 gmatch_clip=True gmatch_rest=True held_cv=True gmatch_fo=True p1amp=0.2 p2amp=1 p3amp=1`. Rest-IC graph captured `N=6 T=128`. Huber baseline = rest last-obs CV. FOPDT rise print ON. Iter 1 teacher **×0.561 / ×−0.422** skip **0** persist_rel **0.768** gmatch **1.59** recon **0.100** nvidia **~14.2 GB**. **One GPU job.**
- **LIVE (iter 64, ~1h49, still P1):** skip **0**. last_ok **60** (unlocked@64; wrap lock@61 recon **0.399** after inject, recovered@64 recon **0.032**). Teacher last-step jsonl **×0.990 / ×0.990** (wrap transient DV **−0.32@61 / ×1.38@62**). persist_rel bouncing **0.03–1.66** (P73 P1 same class). Wrap@31–34 recovered earlier. wm_best frozen **iter 30** (gain-blind; fidelity@60 H=1 r=0.603 / H=55 r=**0.261** best_h=27/55 gain_fid=0.668 — H=55 compounding worse as recon improves). nvidia **~14.3 GB**.
- **CPU TM@K=55 n_levels=1 (CVD="", same probe):** P75@20 MV ss **×0.79** mid/FO/ss **0.70**. P75@40 MV ss **×0.63** mid/FO/ss **1.05** (shape by shrinking SS: WM ss −0.197 vs @20 −0.248). P75@60 MV ss **×0.69** mid/FO/ss **1.19** (SS recovered vs @40; rise overshoots FOPDT like P73/P64). DV ss @60 **×0.82** (vs @40 ×0.74). P73@40 MV **×0.76** mid/FO/ss 1.14 DV ×0.80. P64@40 MV **×0.73** mid/FO/ss 1.17 DV ×0.83. FO at K=H with 1.4% rise mass does **not** hold rise: @40 matched FO by shrinking SS, @60 SS came back and rise is fast again. jsonl ×1 ≠ TM. Rise Huber mass **1.4%** of `(k/K)^2` at K=55 (HEAD `rise_wfrac` / `last_wfrac`; not in pid 230450).
- Watch freeze vs P64 **0.91@DV** / P73 **0.83**; val **1step→OL / OL-vs-real** vs P73 ×0.761 / ×0.743 and P64 ×0.85 / ×0.84; `actor_experiment_valid`; det_r must not collapse like P74 **0.074**. If P75 FALSIFIED do **not** FO N+1 at K=H (wrong window vs 4H compounding; rise mass 1.4%). **Do not** start a second GPU job.
...
| jsonl gain-match `*_ratio` last-step only (FO rise weight invisible) | **P75 GPU-occupied** (pid **230450** P1) | SUPERSEDED (`gain_match_{mv,dv}_ratio_mid` / `*_mid_fo` at `k=(K-1)//4`; then `gain_match_{rise,last}_wfrac`) | YES — last-step FO(K)/FO(K)=1 cannot show a contracted rise. `(k/K)^2` rise mass **0.014** at K=55. CPU TM@K=55: P75@20 MV ×0.79 / 0.70; @40 **×0.63 / 1.05** (shape by shrinking SS); @60 **×0.69 / 1.19** (SS recovered, rise overshoots FO). Not in this pid. CVD="" smoke. |

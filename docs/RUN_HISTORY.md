...
- **LIVE (2026-08-28 02:03, iter 26, pid 4176670, `73a5116`):** device=cuda bs=128 det+eager. recon 0.084→**0.0035**, gmatch **0.0004** (n=2). iso/ss **0** (jsonl keys absent). skip **0**. Survived P30 skip-storm window (iter 19 recon 0.0045 gnorm 0.76). Wrap gnorm 22–28 at 6/11/18/23/24 recovered next iter (do **not** lower skip). Iter-10 probe `gain_fid=0.903` then iter-20 **0.636** (gain-blind; restore OFF — WATCH). t_wm **~99 s**. GPU ~17 GB. **No second GPU job.**
- **HEAD (not in pid):** `img_rollout(..., last_only=True)` for gain-match last-step Huber (no unused K-stack). Sequential `img_step` fallback removed. `[resolved-cfg] iso_dcv=off` when teacher off.

...
- **Liveness:** still **LIVE** pid **75966**, tmux `mbrl2_p53`, GPU ~11 GB. ~206 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. jsonl through **P3 iter 167**. last_ok **82 unlocked**. sps ~156. Do not KEEP/FALSIFIED as cascade until EXIT.
  ...
  - **Unfreeze 147 (A/B canary — no cascade verdict yet):** ent **−0.163→−0.101 HELD** through **167** (stop-grad yank-block holding). rscale **2.302 FREEZE** KEEP-class. actor_loss **−0.013** @147 (not P51 **−4.66** / P52 delayed **−0.77**); |aloss|≤0.16 through 167. Unclipped logp_std **0.52@146 → 1.58@147 → 6.71@149** (P52 first-unfreeze class **6.11**, not P51 **38**) then **returned 0.55@167** (not P52 delayed **80.8@165**). `actor_ratio_clip_frac` **0.61@147 / 0.97@149** then **0.32@167**; `actor_ratio_mean` **0.82→0.46@149** then recovered **~1.05**. PPO clip **is firing**. mvv **0@151 / 65@156 / 807@161 / 751@166** (warmup-class, not **813k**). bc spike **0.55@149** then **0.005@167**. ret_w **−48@151 / −46@167** (not P51 **−1822**). ema **−219@137 → −120@167** (not P52 **−2231@209**).
  - **Watch (not a verdict):** rtgt **0.054@147 → 0.0039@167** (below critic healthy **0.015** after ~156; P51 best.pt **161** had **0.0057** then collapsed to **1e-4**). logp_std recovered after the 149 spike — opposite of P52 delayed walk. Next ticks: whether rtgt stays ~0.004 or walks to **1e-4**, whether mvv stays <~2k or jumps to 100k+, whether unclipped logp_std stays ~0.55 or re-rails.
- **Hypothesis:** in-support `(u−μ)/σ²` SGD cannot walk μ onto the tanh rail once `π_new(a)/π_snap(a)` leaves `[0.8, 1.2]`. Unfreeze 147 unclipped logp_std stays ~0.5 and mvv stays warmup-class (~16k), not 813k. **So far:** first-unfreeze std spiked then **came back**; mvv still warmup-class; clip_frac high then easing as ratio_mean ~1. Do not call KEEP/FALSIFIED as cascade until EXIT.
- **Watch:** post-unfreeze delayed rail vs P52 @165–169. Do not call KEEP/FALSIFIED as cascade until EXIT.
...
| P3 in-support μ-walk (\|logp\|<8 SGD) | P52 EXIT; **P53 LIVE** (`run_p53_muratio`) | LIVE (`p3_mu_ratio_clip=0.2` vs frozen unfreeze-μ) | YES — first AC call snapshots policy (warmup does not step actor). Unfreeze 147: ent HELD −0.101; clip_frac 0.61→0.97@149; logp_std spiked 6.71@149 then **returned 0.55@167**; mvv 751@166 not 813k; actor_loss bounded. rtgt 0.054→0.004 **watch**. No cascade verdict until EXIT. Opt out `DREAMER_P3_MU_RATIO_CLIP=0`. Not `actor_kl_coef`. Not lag-copy. |

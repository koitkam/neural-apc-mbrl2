...
| **Critic** | `critic_rew_to_tgt_var` (>0.015) | none healthy | — | P72 val `critic_r` **+0.694** PASS (P68 +0.699; P64 +0.705); P3 rtgt still collapses (P72 med **0.0015**). Freeze rscale KEEP. |
| **Actor** | econ vs baseline + vs expert BC | **P64** (mbrl2) | paired **−4.54 vs −94** | Env-free P2 amp=P3. 9/9 `all_pass`. **P68 −3.90 vs −85 VALID 9/9** (slightly better econ) but mv_viol **1.92 vs 0.33** and TM **×0.69** — not a champ swap. P72 **−22.71 vs −92.72 VALID 9/9** (mv_viol **9.70**, last_ok **15**). P66 **−19.85 vs −104**. P53 stays the μ-ratio-clip RCA. |
...
| P73 | 2026-09-02 | OL gain-c persist at teacher K (`||c_K[:G]−sg(c0[:G])||²` × existing `cont_gain_persist_coef`) | LIVE P2. P1 GAIN-READY **0.83@DV** last_ok **81** skip **0** (P72 froze 15). Wrap @82 restored 81. persist_rel **0.03–1.46** (0.037 @81). | 🔬 LIVE |
...
### p73 (`run_p73_olgpersist`, branch `cursor/p28`) — LIVE: OL gain-c persist at teacher K
...
- **Step 4 CONFIRMED:** `device=cuda` bs=128 `wm_train_T=128` horizon=55 compile=eager. `[cont-latent] GAIN-ONLY` gain_dim=2. `[resolved-cfg] isolation=0 gmatch_len=55 gmatch_ol_tail=0 gmatch_step=0.4 gmatch_clip=True gmatch_rest=True dob_ground=2 dob_reg=0 held_cv=True p1amp=0.2 p2amp=1 p3amp=1`. Rest-IC graph captured `N=6 T=128`. Huber baseline = rest last-obs CV. Process environ: `CUDA_VISIBLE_DEVICES=0` only (no `DREAMER_*`). P1 nvidia **~15.5 GB**; P2 **~4.3 GB** (rest-IC graph released).
- **LIVE (2026-09-02 11:50Z, P2 iter ~104):** P1 **GAIN-READY PASS** @82. skip **0/0** entire P1 (P72 storm **1@6** then **2@16** froze last_ok **15**). last_ok **81 locked**; wrap recon **0.0703** @82 (20× best **0.0023**) → detonated-freeze restored **81** (recon 0.0028, persist_rel **0.037**); wm_best SKIPPED (gain-blind). Freeze TM **DC [0.83, 0.83] @H [0.88, 0.93] worst=0.83@DV** (band [0.8, 1.3]; P64 **0.91@DV** last_ok **82**). Teacher pinned @5 **×0.996 / ×0.980** (P72 @5 still ×0.64/×−1.08). persist_rel **0.03–1.46** (not P70 hold-to-0). jsonl last-ok **×1.002 / ×0.983 ≠ freeze TM 0.83**. P2 `g FROZEN` recon **~0.0047** (P64 ~0.0018 / P72 ~0.011) dob_d **~0.046** dobg rising **0.004→0.08** keep_frac **1** buf **327/327**. Watch P2→P3; val **1step→OL / OL-vs-real** vs P64 ×0.85/×0.84; freeze 0.83 is the floor of the band. Do not score actor until val / `actor_experiment_valid`. Dummy `gmatch_ol_tail=0` banner **REMOVED** on HEAD (not this pid). HEAD P1 banner now prints `persist` from jsonl when the term fires (not this pid). **No second GPU job.**
...
| Restore gain-c in GRU / TSSM token (`recurrence_c_dim=cont_dim`) | **P72 EXIT** (`run_p72_gaingru`, pid **212027**, sha `b009a7e`, 541 iters) | KEEP as P71 REVERT; **FALSIFIED as TM / compounding / champ** | YES as GAIN-READY vs P71 0.76@DV INVALID / **NO as TM pin**. last_ok **15**. Val MV **×0.647 / ×0.626** DV **×0.642 / ×0.641**. 1step **×0.921** but 1step→OL **×0.691** OL **×0.633**. Paired **−22.71 vs −92.72** VALID 9/9, mv_viol **9.70**. Weaker freeze of P68 recipe. **Do not** routing N+1. Next: OL persist ≠ window/hold. |

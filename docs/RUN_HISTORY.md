...
| **Actor** | econ vs baseline + vs expert BC | **P58** (mbrl2) | paired **−6.24 vs −102** | Env-free lookback + freeze-forever μ-ratio. 9/9 pairs. Beats P53 **−13 vs −98**. P53 stays the μ-ratio-clip RCA. Observer still short of P26. |
...
| P61 | 2026-08-31 | env-free cube-clip + realized Δu for gain-match FD | LIVE `run_p61_clipdu` pid **155682** sha `c4ed6f3`. Storm **2/2 @56–57** CAPPED extra P1. Gate **GAIN-READY** last_ok **55** freeze **0.86@DV** (P60 0.86@DV @77). P2 ~iter **88** recon **0.003** dobg **0.006**. Do not FALSIFY clip until val TM. | LIVE |
...
### p61 (`run_p61_clipdu`, branch `cursor/p28`) — LIVE: cube-clip + realized Δu for gain-match FD
- **One attributed GPU change vs P60:** `TrainConfig.gain_match_clip_realized=True` (default). `_cube_step_held` clamp `[-1,1]`; reverse per-start step if clip shrinks `|Δ|`; Huber `/` `_gain_match_realized_du` (diagonal+cat `(n_in, Bm)` — smoke caught `stack` of advanced-index shapes broadcasting `n_mv=1` to `(Bm,Bm)`). Isolation still off. Rest-IC lookback. μ-ratio 0.2 freeze-forever. Serve graph KEEP. Eager. Det latent. DOB on. Env-free: **no leftover `DREAMER_*`**. `DREAMER_GAIN_MATCH_CLIP_REALIZED=0` A/B restores P60 commanded-step. tmux `mbrl2_p61`, pid **155682**, sha `c4ed6f3`.
- **CONFIRMED:** `device=cuda` bs=128. `[gain-match] … step=0.4 clip_realized=True`. `[resolved-cfg] gmatch_clip=True gmatch_step=0.4`. Rest-IC graph captured N=6 T=128. No leftover `env-override`.
- **Not:** leftover `DREAMER_GAIN_MATCH_STEP=1.0`. Critic knobs. μ-refresh. S=H. Isolation/relative Huber. Compounding (overshoot/held) as a second observer A/B. DOB-amp retune.
- **Step 4 (pre-launch):** vs P60 resolved cfg the only TrainConfig delta is `gain_match_clip_realized` False-by-absence → **True**. `gmatch_step` stays 0.4. HEAD speed caches from `0e96f09` ride along (not an observer A/B).
- **Watch:** `[gain-match] … clip_realized=True` / `[resolved-cfg] gmatch_clip=True`; jsonl `wm_gain_match_du_frac` **and** `wm_gain_match_clip_frac` (HEAD; reverse restores `|Δu|` so du_frac≈1 cannot tell whether the cube fired); val MV/DV ss/@H vs P26 and P60; det_r vs P59/P60; paired vs P58.
- **LIVE ~iter 50 (2026-08-31 18:45 CDT, pid 155682, ~88 min):** P1 still healthy; **no second GPU job.** recon last-10 **0.0020–0.0034** (P60 matched **0.0020–0.0034**). Teacher iter 50 **×0.977 / ×0.979**. `du_frac=1.0` every iter (this pid has **no** `clip_frac` key). skip **0/0**. last_ok **50** unlocked. `t_wm` **~98 s**. Fidelity @50 gain_fid **0.794** (P60 0.767).
- **LIVE P1→P2 GATE (2026-08-31 18:57 CDT):** skip-storm **1/2 @56** then **2/2 @57** CAPPED extra P1 (env_steps **569740**; original P1 was 753960). gnorm **5.2e9 / 1.2e9**. last_ok **55** unlocked (never 20×-locked). Teacher through iter 55 **MV ×0.998 / DV ×0.999** then detonated @56 **×0.565 / ×0.634**. Gate probe on last-ok: **MV ×1.01/@H ×1.03, DV ×0.84/@H ×0.91** worst **0.84@DV**. Detonated-freeze restore 55: **MV ×1.00/@H ×1.00, DV ×0.86/@H ×0.92**. **GAIN-READY** (P60 freeze **0.86@DV** last_ok **77**, no skip-storm, wrap-lock @77). `wm_best` restore SKIPPED. Rest-IC graph **released**. P58 stormed 2/2 @61 last_ok 59 0.85@DV; P59 2/2 @78 last_ok 76 0.88@DV; P60 skipped the storm and froze later. Clip did **not** prevent late-P1 detonation; freeze TM is P60-class. **Do not FALSIFY clip from the storm** — val TM / det_r / paired still the test. `du_frac=1.0` every P1 iter.
- **LIVE P2 ~iter 88 (2026-08-31 19:10 CDT, ~1h53):** recon **0.0029–0.0033** (P60 P2 **0.0017–0.0023** — shorter P1). dobg **0.0008→0.006** (P60 mid-P2 **0.005**). d_abs **0.021→0.013**. reward_mtp **1.44→0.91** (P60 1.37→0.91). `t_wm` **21.2 s**. Fidelity @60 gain_fid **0.924**; @70 **0.976** (`wm_best` 70); @80 **0.879**. skip **0** in P2. P2 budget remaining ~24 iters (P3 ~iter **111**, ETA ~19:20). Actor/critic **not yet**. **No second GPU job.**
- **HEAD (not in pid 155682):** jsonl `gain_match_clip_frac` + rest-IC `a_base`/`dv0`/`du` cache (sha `b4cec5e`). Loss identity. CVD="" smoke. Next pid after EXIT sees `clip_frac`.

### Sim-adaptive leftovers (env-free multi-sim; do not promote plants yet)

Already unitless / derived: held-rollout `win` clamped to `(K-1)//4` (identity 8 at K=55; K=15 was dead); isolation TBPTT `max(8, round(K/3.5))` (chunked `img_rollout`, same `h`-only `keep_c` cuts); `gain_match_len` / isolation len = H; **`gain_match_settle_len` default `-1` (P44 storm 2/2 REVERT; `0` still auto-H A/B)**; TM probe settle is `wm_tf_horizon(H)=max(80,4H)`; **rest-IC encode L = lookback** (`gain_match_rest_ic_len=0`; P57 EXIT **REVERT**; `-1` A/B last `max(K, 2τ/sr)`); **`gain_match_step<=0` auto = `wm_tf_step_frac`** (P60 EXIT KEEP as protocol / FALSIFIED as TM pin; test_sim 0.4; explicit `1.0` A/B's the P59 teacher); **`gain_match_clip_realized=True`** (P61: cube-clip + realized Δu; unitless; `op_band+step>1` is the sim-adaptive hole; opt out `=0`); isolation sample window `max(seq_len, K+1)` (test_sim seq_len ≥ H unchanged); P1/P2 main WM sample `max(seq_len, K+1)` (overshoot/gain-match; P3 stays seq_len); gain-match open-loop `K` not truncated to `T-1`; `skip_storm_last_ok_recon_ratio=5` (recon/recon); `skip_storm_p1_cap_after=2` (storm count; **P32 GPU-confirmed** continue 1/2 @iter 53). Storm-1 continue now **keeps** `p1_gate_max_ext_steps` (P32 CAPPED 0.71@DV when it was closed); storm 2 still `_force_p1_cap_at`. Isolation / overshoot / held are **RSSM-interface** (rssm + tssm; SF no-op). HEAD not in P33 process; `p1_gate_wm_ema_min=1.5` (fidelity mix); inject EVERY = f(buffer lap) (test_sim 20/10); inject N = f(n_mv, n_dv) (test_sim 5/2/2/3); isolation settle = 24 **per input** (test_sim 24+24 / cap 48 settle-only; distillation 96+24 / cap 120). Isolation_buf holds only those settle episodes (not MIMO PRBS). Isolation settle is a whole-episode hold at a stratified level (`action_std=0`); DV step is MV-action-isomorphic (`isolated_level × span/2`). Opt-in: `gain_match_huber_beta<=0` → median |tgt| only when `gain_match_huber_per_input` is False (P43 default is per-input `|tgt_ij|`). Gain-ready band `[0.80, 1.30]` and wm-fidelity mix weights are unitless TrainConfig (were os.environ). `rssm_latent_type` default **deterministic** (P26 proven; P29 env-free drop of the leftover env-var). `compile_mode` default **eager**; `DREAMER_COMPILE` / `DREAMER_COMPILE_MODE` are in `ENV_OVERRIDES` (MODE was CLI-only). Intra-op threads and inductor workers scale with `sched_getaffinity` (host-adaptive). `skip_invalid_p3` default ON (unitless validity flag, not a plant unit). Stage-1 skips unused DOB prior-core when `dob_active=False` **and** skips `apply_dob` clone + ground/reg (`d_t≡0`; HEAD, not P33 process). SNR summary/WARN is **measured CV+DV** (`summary_scope`; constant aug channels excluded). Isolation jsonl splits detached `wm_isolation_mv_traj` / `wm_isolation_dv_traj`. jsonl `wm_gain_match_{mv,dv}_ratio` = mean `G_pred/G_tgt` (HEAD; P44 pid Huber-only; teacher FD, not TM). jsonl `wm_gain_match_du_frac` = mean `|Δu|/|step|` (P61). jsonl `wm_gain_match_clip_frac` = mean `+step`-would-clip (HEAD; reverse restores `|Δu|`). TM / postprior / smoke share `wm_tf_horizon` (was inlined `max(80,4H)`). Rest-IC encode `rollout_observed last_only return_feats=False` + Stage-1 `_posterior_step` (P46 pid; P45 pid 16548 still stacked). `gain_match_rest_ic` default **True** (P45 EXIT PROMOTE).
...
| Teacher Huber `/` commanded step; held a/dv unclipped | P60 EXIT (`run_p60_tmstep`) | SUPERSEDED (P61 `_cube_step_held` + `_gain_match_realized_du`; default `gain_match_clip_realized=True`) | YES — TM `compute_transfer_matrix` clips `lev+d` to `[-1,1]`, skips no-ops, divides ΔCV by **realized** ΔMV (p136). Rest `±op_band=0.6` + step 0.4 sits on/over the cube; `op_band+step>1` on other plants silently deflates G. Interior identity. Reverse restores `|Δu|` so jsonl `du_frac≈1` cannot see the cube (HEAD `clip_frac`). `DREAMER_GAIN_MATCH_CLIP_REALIZED=0` restores P60. |
| jsonl `du_frac` as “did the cube fire?” | P61 LIVE (`run_p61_clipdu` pid **155682**) | SUPERSEDED (`gain_match_clip_frac` = mean `+step`-would-clip; HEAD, not this pid) | YES — reverse keeps `|Δu|` so du_frac stayed **1.0** through P1 iter **57** even if outer rest+jitter hits the rail. Matched-iter P1 ≈ P60 until storm @56. Loss identity. CVD="" smoke interior 0 / rail 1. |
| Cube-clip + realized Δu as late-P1 skip-storm preventer | P61 LIVE (`run_p61_clipdu` pid **155682**) | unjudged (storm **2/2 @56–57**; freeze still **GAIN-READY 0.86@DV**) | NO as storm-preventer — P60 (no clip) had **0** skip-storms and wrap-lock @77; P61 detonated gnorm **5e9** @56 after P60-class teacher ×1 through 55. Freeze TM **matches P60 0.86@DV**. Clip as **val-TM closer** still open. Do not FALSIFY from jsonl storm. |

...
- **Judge by:** train-start `latent=deterministic compile=eager skip_invalid_p3=True device=cuda bs=128`. `[resolved-cfg] isolation=0 ss_match=0`. gmatch falling like P26/P37 (not stuck 1.30). Val MV ss/@H ~×1.0 ±0.1; **DV toward P26 ×0.87 not P39 ×0.68**; det_r toward P26 0.68 (pred_std not ~0.16). If GAIN-READY: first valid actor test of min-of-2 + freeze `return_scale` (rscale near warmup, `rtgt>0.015`, econ vs baseline **and** vs P39 BC −44 / P27 BC −59). If still GAIN_NOT_READY: `[p3-skip]`. If DV stays ×0.68, isolation was not the regression (re-diagnose sample-T / freeze / DOB).

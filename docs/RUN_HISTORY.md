...
| P41 | 2026-08-28 | P1 gate recent-floor (not warmup ema_best) | LIVE extra-P1; iter 82 gain-probe 0.76@DV EXTEND | ⏳ GPU |
...
- **Liveness (07:30 CDT):** LIVE extra-P1. tmux `mbrl2_p41`, pid **4185725**, launch `acb8a7b`. GPU ~15.7 GB. **No leftover `DREAMER_*`.** `[resolved-cfg] isolation=0 ss_match=0 iso_dcv=off`. `wm_best` **iter 10 / 6.378**. last_ok **82**. **No second GPU job.**
...
- **P40 last-ok overwrite already happened in original P1:** iter 57 recon **0.0887** / gnorm **1.99** / skip 0 (42× best ~0.0021); last_ok **overwrote 56→64** at recon **0.0098 < 5×**. Iter 66 wrap 0.096; iter 74 silent 0.281 skip 94. last_ok then advanced to **82**.
- **First gate iter 82 (original budget 753960) GPU-confirmed:** recent-floor **KEEP as mechanism**. Printed **`[gate p1->p2] gain-probe`** (P40 printed `not_plateaued` / **no probe** on ema_best 6.541). `recent_ok` (ema 4.391 ≥ 1.5) so the probe ran. **FAIL GAIN_NOT_READY worst=0.76@DV** (MV **1.01** in [0.8,1.3]; DV 0.76). recon **0.0058** skip 94 iso/ss 0. **EXTEND** +75396 to 829356 (cap 929884). Did **not** freeze pre-extra-P1. Original-budget DV 0.76 ≈ P40 CAPPED 0.75 — extra-P1 is not the DV gap. Expect `[p3-skip]` unless a later probe hits 0.80.
- **HEAD (not in pid):** last-ok lock (`skip_storm_last_ok_lock_ratio=20`) recon-only. **Not stacked onto live P41.**
- **Judge remaining:** extra-P1 may still silent-detonate (P40 iter 84). Second gate ~iter 94. If CAPPED GAIN_NOT_READY: `[p3-skip]`. Val MV ~×1.0 ±0.1; DV vs P40 ×0.72 / P26 ×0.87. Actor INVALID if GAIN_NOT_READY.
...
| P1 gate recent-floor (`_p1_fidelity_local_plateau`) | P41 LIVE iter 82 | KEEP as mechanism; did **not** freeze (GAIN_NOT_READY) | YES — gain-probe ran (ema 4.391 ≥ 1.5 vs global 6.378@10). FAIL 0.76@DV (MV 1.01) EXTEND. Original-budget DV already P40 CAPPED 0.75 class. Extra-P1 still on. |

...
- **Liveness (07:15 CDT):** LIVE P1 iter **~70**. tmux `mbrl2_p41`, pid **4185725**, launch `acb8a7b`. GPU ~15.7 GB. **No leftover `DREAMER_*`.** `[resolved-cfg] isolation=0 ss_match=0 iso_dcv=off` gain_match=1 dob_ground=2 restore_p2=False skip_invalid_p3=True storm_cap=2 compile=eager. `wm_best` **iter 10 / 6.378** (global; recent-floor ignores it). **No second GPU job.**
...
- **P40 last-ok overwrite already happened in original P1:** iter 57 recon **0.0887** / gnorm **1.99** / skip 0 (42× best ~0.0021); iter 58 skip 1 recon 0.337. last_ok stayed 56 during the spike then **overwrote 56→64** at recon **0.0098 < 5×**. Iter 66 wrap-like 0.096 recovered. last_ok **70** by iter 70. **Iter 74 another silent spike recon 0.281 skip 94** (this-iter skip 0). Freeze-iter recon in this class would look healthy so detonated-freeze misses 56 (same hole as P40 83→104).
- **HEAD (not in pid):** last-ok lock (`skip_storm_last_ok_lock_ratio=20`) now **recon-only** (not skip-free) so a spike iter with skip 1 still locks. Skip-storm restore still unlocks. **Not stacked onto live P41.**
- **Judge by:** first gate (~iter 82, original P1 753960) prints **gain-probe**, not `not_plateaued` on 6.378. If PASS: freeze current g (`restore_p2=False`); last_ok overwrite only matters if later EXTEND/CAPPED/storm-2. If GAIN_NOT_READY EXTEND: extra-P1 may still fire with last_ok already at recovered-70. Val MV ~×1.0 ±0.1; DV vs P40 ×0.72 / P26 ×0.87. If still GAIN_NOT_READY: `[p3-skip]`.
...
| P1 gate return-to-global `wm_ema_best` (`not_plateaued` EXTEND) | P40 EXIT iter 82 | SUPERSEDED (recent-floor `_p1_fidelity_local_plateau`; P41 GPU) | YES — warmup spike 6.541 blocked the gain probe at healthy iter 82; extra-P1 silent-detonated iter 84 (gnorm 2.51, skip-storm silent). Extra P1 already FALSIFIED as DV lever |

...
| P41 | 2026-08-28 | P1 gate recent-floor (not warmup ema_best) | LIVE extra-P1 through 86 healthy; iter 82 gain-probe 0.76@DV EXTEND | ⏳ GPU |
...
- **Liveness (07:37 CDT):** LIVE extra-P1 through **iter 86**. tmux `mbrl2_p41`, pid **4185725**, launch `acb8a7b`. GPU ~15.7 GB. **No leftover `DREAMER_*`.** `[resolved-cfg] isolation=0 ss_match=0 iso_dcv=off`. `wm_best` **iter 10 / 6.378**. last_ok **86**. **No second GPU job.**
...
- **Extra-P1 through 86 (P40 detonated at extra-P1 iter 84):** **did not detonate** at that window. Iter 84 recon **0.0062** / gnorm **1.04** / this-iter skip 0. Iter 86 recon **0.0038** last_ok **86**. last_ok is still advancing every healthy extra-P1 iter (overwrite hole remains; lock is HEAD not in pid).
...
- **Judge remaining:** second gate ~iter 94; CAPPED ~iter 104. Extra-P1 can still silent-detonate later (P37 iter 88 class). If CAPPED GAIN_NOT_READY: `[p3-skip]`. Val MV ~×1.0 ±0.1; DV vs P40 ×0.72 / P26 ×0.87. Actor INVALID if GAIN_NOT_READY.
...
| P1 gate recent-floor (`_p1_fidelity_local_plateau`) | P41 LIVE iter 82 | KEEP as mechanism; did **not** freeze (GAIN_NOT_READY) | YES — gain-probe ran (ema 4.391 ≥ 1.5 vs global 6.378@10). FAIL 0.76@DV (MV 1.01) EXTEND. Original-budget DV already P40 CAPPED 0.75 class. Extra-P1 through 86 healthy (P40 detonated @84). |

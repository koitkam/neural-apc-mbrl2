...
| P46 | 2026-08-29 | P3 log_std reset (`DREAMER_P3_RESET_LOG_STD=1`) | launching | ⏳ one attributed change vs P45 identity |
...
### p46 (`run_p46_p3sigreset`, branch `cursor/p28`) — LAUNCH: P3 log_std reset
...
- **Step 4 resolved-cfg vs P45:** identical except `p3_sigreset=True` (P45 had no field / False). `gmatch_rest=True` is now the TrainConfig default (P45 passed it as env). Banner: `latent=deterministic restore_p2=False gain_match=1 dob_ground=2 isolation=0 ss_match=0 iso_dcv=off n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 lock=20 huber_per_in=True gmatch_settle=-1 gmatch_rest=True p3_sigreset=True compile=eager`.
- **Watch:** `[p3] reset policy log_std residual`; `[resolved-cfg] gmatch_rest=True p3_sigreset=True`. First P3 entropy **not** −0.363 (toward init ≈−1.5 → H≈−0.08). Warmup rtgt stays >0.015. After unfreeze: no actor_loss −300; no entropy-collapse; econ vs baseline **and** vs P45 −216 / P27 BC −59; MV not railed. Storm 2/2 or GAIN_NOT_READY → observer regression (rest_ic default). If entropy still at floor → **REVERT** reset (residual not in last Linear). If σ opens but still limit-rides → do **not** stack critic knobs; next is DR-timing or DOB amp (P2 too short vs P40 det_r 0.49).
...
| P3 `reset_log_std` (zero last-Linear log_std rows; μ kept) | P46 launch (`run_p46_p3sigreset`) | pending (default False; opt-in env) | P45 RCA: BC pins σ_min → railed warmup → REINFORCE explodes. `DREAMER_P3_RESET_LOG_STD=1`. If entropy still at floor → residual not in last Linear (REVERT). |

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 15**. Probe10 **H=1 r=+0.128 / H=55 r=+0.122 conv=0.00** gain_fid **0.476** (P76 +0.428/+0.310/0.736; P64 +0.584/+0.309 conv=0.50/0.827). Teacher **×1.00 / ×0.96 @15**. recon **0.212@15 best** (P76 @15 **0.007**; ~30×). skip **0**. pid **242511**. | 🔬 LIVE P1 — probe r still ~3–4× worse than RSSM; recon 0.30→0.21; watch freeze vs P64 0.91@DV; later-probe conv>0 |
...
- **LIVE P1 iter 15 (~80 min, nvidia 11.7 GB, `t_wm` ~308 s, sps 19.6):** skip **0**. Teacher jsonl 13 **×0.992 / ×1.072** → 14 **×1.000 / ×1.016** → 15 **×0.995 / ×0.961** (pinned). recon **0.302@13 → 0.300@14 → 0.212@15 new best** vs P76 **0.007@15** / P64 **0.005@15** (~30×, was ~40×). persist_rel **0.43@14 → 0.35@15**. Rest-IC graph not captured. Probe 10 still the last fidelity (`H=1 r=+0.128 conv=0.00 gain_fid=0.476`); next typically iter 20 (~25 min). Do not patch while live. Watch freeze vs P64 **0.91@DV**; later probes conv>0 (P76 uniquely held 0 through P2@100; P73 also 0 @10); val **1step→OL** vs P64 ×0.85 / P76 ×0.770; `actor_experiment_valid`. Do **not** start a second GPU job.

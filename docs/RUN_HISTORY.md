...
- **Liveness:** tmux `mbrl2_p55` LIVE, pid **103504** LIVE (`b321d89`), `device=cuda`. P1 iter **53** recon **0.0019** skip **0** last_ok **53 unlocked**. Teacher FD MV/DV **×1.014 / ×0.999**. `wm_best` iter **30** EMA **5.866** (iter-40 probe H=55 r=+0.385 gain_fid=0.540; iter-50 H=55 r=+0.436 gain_fid=0.724 — gain-blind `wm_best` did not update). `t_wm` **~128 s** (eager T-loop; CUDA-graph capture failed on first WM autocast). P54 `t_wm` ~124 s — same rest-IC class. HEAD after this pid: capture **before** the WM loop + exit parent autocast. **Watch** as above. **Do not launch a second GPU job.**
- **HEAD (GPU occupied, not in pid):** leftover `DREAMER_WM_DIAG_DEVICE` → TrainConfig `wm_diag_device='cuda'` + `ENV_OVERRIDES` (identity with the previous train()-injected env). Unused `pmpo_alpha`/`pmpo_beta` **REMOVED** (`train()` still refuses `actor_loss_type≠reinforce`). Seed counts / `gain_match_max_starts=6` stay sentinels (not GAIN-READY; do not retune). CVD="" smoke green.
...
- Rest-IC encode length is `lookback` (test_sim **128** sequential GRU steps × 100 inner WM steps → `t_wm` ~124 s). A few-τ window would be more sim-adaptive; that is a later A/B (not identity). **HEAD:** CUDA-graph of that T-loop (`gain_match_rest_ic_cuda_graph=True`), warmed **before** the P1 WM autocast loop + parent `enabled=False` then bf16 `cache_enabled=False` (P55 in-loop capture failed). CPU path stays the Python loop (`CVD=""`). `DREAMER_GAIN_MATCH_REST_IC_CUDA_GRAPH=0` keeps eager.
...
| `DREAMER_ACTOR_LOSS=pmpo` / `actor_loss_type` (P3 always inlines REINFORCE + μ-ratio) | P54 GPU-occupied (pid **95956** P3) | **REFUSED at `train()`** (false A/B) | YES — `pmpo_loss` / `reinforce_actor_loss` / policy `kl_to` **REMOVED** (no P3 call sites). Whitelist kept so an explicit override fails loud. `_actor_uses_prior_policy` still gates π_prior snapshot (unreachable in env-free). |
...
| Rest-IC last_only T-loop = 128 sequential kernel launches × 100 WM steps | P54 GPU-occupied (pid **95956** P1); **P55 LIVE** first GPU capture failed | SUPERSEDED (`gain_match_rest_ic_cuda_graph=True`; `make_graphed_callables` + GRU-grad canary; **P55 HEAD** capture **before** WM autocast + parent `enabled=False` then bf16 `cache_enabled=False`) | PARTIAL — P55 pid **103504** captured inside `world_model_loss` autocast (`RuntimeError: make_graphed_callables does not support the autocast caching`) → `_rest_ic_cg_fail` pinned eager (`t_wm` ~127 s, P54 class). Nested `cache_enabled=False` alone was not enough if the parent cache flag is visible. Warmup at train start never re-enters capture. GPU identity still deferred (A10 occupied). `DREAMER_GAIN_MATCH_REST_IC_CUDA_GRAPH=0` keeps the Python loop. |

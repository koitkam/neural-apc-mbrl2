...
| P76 | 2026-09-02 | RSSM GRU update-gate bias `log(H/16)` (~1.23; no new knob) | LIVE P1 **iter 63**. Teacher pinned. recon **0.0024** skip **0**. Wrap@29 recovered. Probe60 H=1 **0.555** (P64 0.671) H=55 **0.291** **conv=0.00** (P64 0.25 / P75 0.50). CPU TM@60 MV **×0.821** DV **×0.842** vs P64 ×0.923/×0.888. z_mean **0.91** vs P64 **0.70**. pid **237453** sha `0ddc372`. | 🔬 LIVE — keep-h stalls conv; TM short of P64@60. No second GPU job. |
...
### p76 (`run_p76_grubias`, branch `cursor/p28`) — LIVE: GRU update-gate bias
...
- **LIVE P1 iter 63 (~1.8 h, nvidia 14.2 GB, `t_wm` ~100 s, sps 61):** skip **0**. last_ok **63 unlocked**. Teacher pinned @3; @63 MV **×0.993** DV **×1.015**. recon **0.0024** (P64@60 **0.0022**). persist_rel still oscillating **0.13–0.79** (P64 monotonically **0.026 @60** / **0.021 @80**). jsonl has **no** FO mid/wfrac keys.
- **Wrap lock@29** recon **0.955** (274× best) teacher DV **×0.488**; unlock@31; last_ok advanced. Blip@55 recon **0.026** recovered (not 20×). skip-storm **0**.
- **Probe iter 10 (WATCH-only; restore OFF):** H=1 r **+0.428** / H=13 **+0.527** / H=27 **+0.472** / H=55 **+0.310** conv **0.00** drift **0.223** best_h **27/55** gain_fid **0.736**. P64@10: H=1 **0.584** H=55 **0.309** conv **0.50** drift **0.185** gain_fid **0.827**. P73@10: H=1 **0.549** H=55 **0.368** drift **0.128** gain_fid **0.772**.
- **Probe iter 20:** H=1 r **+0.589** / H=13 **+0.601** / H=27 **+0.614** / H=55 **+0.543** conv **0.00** drift **0.260** best_h **55/55** gain_fid **0.644**. P64@20: H=1 **0.706** H=55 **0.498** conv **0.50**. H=1 lag recovered vs iter 10; H=55 r **beats** P64. **conv stays 0**.
- **Probe iter 40:** H=1 **+0.561** / H=55 **+0.546** conv **0.00** drift **0.190** gain_fid **0.528**. P64@40 conv **0.25** H=1 **0.819** H=55 **0.456**. P75@40 conv **0.50**.
- **Probe iter 50:** H=1 **+0.502** / H=55 **+0.401** conv **0.00** drift **0.202** gain_fid **0.788**. P64@50 conv **0.25**. P75@50 conv **0.25**.
- **Probe iter 60:** H=1 **+0.555** / H=13 **+0.527** / H=27 **+0.555** / H=55 **+0.291** conv **0.00** drift **0.182** best_h **27/55** gain_fid **0.674**. P64@60: H=1 **0.671** H=55 **0.253** conv **0.25**. P75@60 conv **0.50**. P73@60 conv **0.75** (recovered). H=55 drop is shared late-P1; **conv=0 through 60 is unique**. wm_best still iter-20 gain-blind — ignore for freeze.
- **CPU CVD="" gate probe (ckpt@20/@60, not a training change):** update-slice bias still **1.23** on both GRU slices (P64/P73/P75 ckpt60 **~0.005**). Held `img_step` K=55 z_mean avg/kH: P76 **0.91 / 0.93** vs P64 **0.70 / 0.74** / P73 **0.76 / 0.78**. Keep-h has **not** trained away. Idle logit is **2b** → sigmoid ~0.92. ckpt20 `||h||` climb was rest-encode IC; randn-h IC settles (P76 slower than P64). Mechanism: keep-h keeps the H-probe tail above `eps_std` (conv=0).
- **CPU CVD="" TM@K=55 rest-step 0.4 (not a training change):** P76 ckpt40 MV/DV **×0.837 / ×0.789**; ckpt60 **×0.821 / ×0.842**. P64 ckpt60 **×0.923 / ×0.888** (ckpt40 MV **×0.726**). P75 ckpt60 **×0.692 / ×0.827**. P73 ckpt60 **×0.692 / ×0.751**. jsonl teacher ×1 ≠ TM. P76 MV **stalled** 40→60 while P64 closed **0.73→0.92**.
- Watch freeze vs P64 **0.91@DV** / P75 **0.84@MV**; skip-storm; val **1step→OL** vs P64 ×0.85 / P75 ×0.80; `actor_experiment_valid`. If EXIT FALSIFIES as compounding (TM short + conv=0), **change family** (do not GRU-bias N+1). Do **not** start a second GPU job. Step 5 deferred to EXIT.

...
- **LIVE (2026-08-27 06:05, tmux `mbrl2_p33`, GPU ~19.4 GB, pid 4146603, launch `1cfb530` / keep-ext `8205f4b`):** env-free det+eager CONFIRMED. `[resolved-cfg] latent=deterministic restore_p2=False gain_match=1 dob_ground=2 isolation=1 ss_match=3 n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 compile=eager`. Train-start `device=cuda bs=128`. `[gate-budget]` p1_ext_cap **175924**. Stage 1 @iter0. Through **iter 56**: recon **0.0087**, skip **0**, alive 1017, `t_wm` ~135 s. **Wrap blip 46–48 recovered without skip-storm** (recon 0.238/0.513/0.102; gnorm 21.5@45 applied, then 6.9/5.5/12; skip 0). Iter 50 probe `best_h=0/55` gain_fid=0.956 did **not** overwrite `wm_best` (still iter **40** `best_h=55/55`). **Not a P32-style skip-storm** (P32 @iter 53 recon 0.596 + gnorm 6.28e15 + skip 71). Mean `t_sample` ~0.14 s. **Do not lower skip threshold.** Watch remaining P1 / iter-75 gate. If GAIN_NOT_READY ~iter 75 → **EXTEND**. If GAIN-READY: first valid actor (rscale freeze, rtgt>0.015, econ vs baseline **and** vs P32 BC −23 / P27 BC −59). If still GAIN_NOT_READY: `[p3-skip]`. **No second GPU job.** GPU-occupied HEAD (not in this pid): isolation + P3 frozen-observer encode `store_aux=False`; gain-match tgt cache; Kalman mix budget `clamp(GPU_mem/1500, 4MiB, 64MiB)` (A10 → 16 MiB). Do not retune `wm_overshoot_max_starts` / `gain_match_max_starts`.

### Sim-adaptive leftovers (env-free multi-sim; do not promote plants yet)

Already unitless / derived: isolation TBPTT `max(8, round(K/3.5))` (chunked `img_rollout`, same `h`-only `keep_c` cuts; HEAD not P32 process); `gain_match_len` / isolation len = H; isolation sample window `max(seq_len, K+1)` (test_sim seq_len ≥ H unchanged); P1/P2 main WM sample `max(seq_len, K+1)` (overshoot/gain-match; P3 stays seq_len); gain-match open-loop `K` not truncated to `T-1`; `skip_storm_last_ok_recon_ratio=5` (recon/recon); `skip_storm_p1_cap_after=2` (storm count; **P32 GPU-confirmed** continue 1/2 @iter 53). Storm-1 continue now **keeps** `p1_gate_max_ext_steps` (P32 CAPPED 0.71@DV when it was closed); storm 2 still `_force_p1_cap_at`. HEAD not in P32 process; `p1_gate_wm_ema_min=1.5` (fidelity mix); inject EVERY = f(buffer lap) (test_sim 20/10); inject N = f(n_mv, n_dv) (test_sim 5/2/2/3); isolation settle = 24 **per input** (test_sim 24+24 / cap 48 settle-only; distillation 96+24 / cap 120). Isolation_buf holds only those settle episodes (not MIMO PRBS). Isolation settle is a whole-episode hold at a stratified level (`action_std=0`); DV step is MV-action-isomorphic (`isolated_level × span/2`). Opt-in: `gain_match_huber_beta<=0` → median |tgt| (P28 still uses β=1). Gain-ready band `[0.80, 1.30]` and wm-fidelity mix weights are unitless TrainConfig (were os.environ). `rssm_latent_type` default **deterministic** (P26 proven; P29 env-free drop of the leftover env-var). `compile_mode` default **eager**; `DREAMER_COMPILE` / `DREAMER_COMPILE_MODE` are in `ENV_OVERRIDES` (MODE was CLI-only). Intra-op threads and inductor workers scale with `sched_getaffinity` (host-adaptive). `skip_invalid_p3` default ON (unitless validity flag, not a plant unit). Stage-1 skips unused DOB prior-core when `dob_active=False` (HEAD, not P32 process).
...
- Isolation/ss-match trajectory MSE is still absolute (same family as abs Huber). Variance-normalized isolation waits for a **healthy GAIN-READY** freeze still showing DV ~×0.87 — P32 val DV ×0.68 is the closed-extension starve, not a Huber verdict. Do not revive relative Huber.
- `wm_overshoot_max_starts=24` / `gain_match_max_starts=6` are **per-sequence** caps. With gpu-calib `B=128`, total `Bm=B·S` is ~8× paper `B=16`. Do **not** retune during P33 (would change the observer loss). Host-adaptive thread caps already scale with `sched_getaffinity`.
...
- P32 EXIT confirmed closed-ext RCA (val MV ×1.09 DV ×0.68 det_r 0.11 `[p3-skip]`). P33 (`run_p33_keepext`) is env-free keep-ext continue (HEAD `8205f4b`). Do not pass leftover compile/latent/DOB env-vars. Variance-normalized isolation still waits for a **healthy GAIN-READY** freeze.

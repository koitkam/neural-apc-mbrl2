...
| P42 | 2026-08-28 | last-ok lock after silent recon spike (20×) | LIVE P1 iter 77; lock GPU-confirmed @67 last_ok 66 | ⏳ GPU |
...
- **Liveness (11:15 CDT):** LIVE **P1** iter **81**. tmux `mbrl2_p42`, pid **4192010**, HEAD `72f7b48`. GPU ~15.7 GB. **Lock GPU-confirmed KEEP as mechanism.** `[wm-last-ok] locked iter 66` on spike iter **67** recon **0.3066** (170× best 0.0018) with **skip 1** — recon-only call site (would miss if skip-free-only). last_ok **stays 66** through 81 (P41 overwrote 56→64 on recovery). Iter 77 recon 0.066 still 36×; iter **81 recon 0.891** (second spike, gmatch **1.119 stuck**, skip **1**). `wm_last_ok.pt` **absent on disk** (pid writes only on storm/freeze — HEAD persist-on-lock not in pid). Skip-storm unlock **untested**. Wrap 5–10× **untested**. Probe iter 70 wrecked (H=55 r=+0.121 gain_fid=0.141). **No second GPU job.** First gate ~iter 82 must still **gain-probe**; freeze must restore last_ok 66 (lock and/or detonated recon). HEAD (not in pid): persist last-ok at lock; jsonl `p1_recon_best` + `wm_gain_match_{mv,dv}_loss`.

...
| P76 | 2026-09-02 | RSSM GRU update-gate bias `log(H/16)` (~1.23; no new knob) | LIVE P1 **iter 25**. Teacher pinned @3. recon **0.0040** skip **0**. Probe20 H=1 r **0.589** (P64 0.706) H=55 r **0.543** (P64 0.498) **conv=0.00** (peers 0.50) drift **0.260** gain_fid **0.644**. CPU z-gate **0.91** vs P64 **0.68**. pid **237453** sha `0ddc372`. | 🔬 LIVE — keep-h slows SS (conv=0). No second GPU job. |
...
- **LIVE P1 iter 25 (~44 min, nvidia 14.2 GB, `t_wm` ~100 s, sps 61):** skip **0**, no wrap, last_ok **25 unlocked**. Teacher pinned @3 (MV **×0.988** DV **×1.04**); @25 MV **×0.994** DV **×1.051**. recon **0.3699→0.0040** (P73@22 **0.0087**, P75 **0.0075**). persist_rel **0.423 @25**. jsonl has **no** FO mid/wfrac keys.
...
- **Probe iter 20:** H=1 r **+0.589** / H=13 **+0.601** / H=27 **+0.614** / H=55 **+0.543** conv **0.00** drift **0.260** best_h **55/55** gain_fid **0.644**. P64@20: H=1 **0.706** H=55 **0.498** conv **0.50** drift **0.153** gain_fid **0.734**. P73@20: H=1 **0.614** H=55 **0.440** conv **0.50** drift **0.140** gain_fid **0.692**. P75@20: H=1 **0.668** H=55 **0.539** conv **0.50** drift **0.153** gain_fid **0.641**. H=1 lag **recovered** vs iter 10; H=55 r **beats** P64/P73. **conv stays 0** (peers 0.50) + drift **worse** = keep-h stall, not detonation. wm_best gain-blind — ignore for freeze.
- **CPU CVD="" gate probe (ckpt@20, not a training change):** z-bias still **1.23** on both GRU slices (P64/P73 ~0.00). Held `img_step` K=55: P76 **z_mean 0.91** vs P64 **0.68** / P73 **0.74**. `||h||/||h0||` climbs **1.6→6.7** through k=55 (still rising); P64 jumps **5.5→8.5 by k=5** then sits ~6.9. Idle logit is **2b** (both ih+hh) → sigmoid ~0.92. Mechanism: keep-h slows approach to the held-action SS so the H-probe tail never drops below `eps_std` (conv=0). Do **not** GRU-bias N+1 if EXIT FALSIFIES (family rule).
- Watch freeze vs P64 **0.91@DV** / P75 **0.84@MV**; skip-storm; val 1step→OL vs P64 ×0.85 / P75 ×0.80; `actor_experiment_valid`. If several `h`/teacher/G A/Bs stay FALSIFIED after EXIT, **change family** (do not GRU-bias N+1). Do **not** start a second GPU job. Step 5 deferred to EXIT.

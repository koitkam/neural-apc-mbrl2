...
- **LIVE (2026-08-26 22:00, tmux `mbrl2_p31`, GPU ~19.6 GB, process `4ffe11f`):** env-free confirmed. P1 iter **83** / 819k of extended 829k (cap 930k). `kl=0`. Survived P30's iter-18–25 window **without** a skip-storm. Healthy stretch iter 40–57 recon **0.003–0.005**, `wm_best` **iter 50** `best_h=55/55` raw 5.678. Iter 58–59 buffer-wrap blip recovered (iter 70 probe `best_h=55/55` gain_fid=0.701). Skip still 2 — **not** a skip-storm. **Original P1 budget gate @iter 75:** `gain_not_ready` DC `[0.63, 1.48]` worst=`1.48@MV` spread_x**5.1** (band `[0.8, 1.3]`) — **extending P1** by 75k to 829k (correct; P26 needed ~90 iters). Iter 83 recon **0.0037** alive 1020. t_wm ~150 s, t_collect ~17.4 s. Next gate ~2 iters. **No second GPU job.** HEAD (not in this process): batched gain-match FD + numpy-only random collect + vectorized DOB Kalman scan; relative Huber A/B path **removed**.
...
- Isolation/ss-match trajectory MSE is still absolute (same family as abs Huber). Variance-normalized isolation is the next observer A/B **if** a **deterministic eager** skip-restore run still shows DV ss ~×0.87 after follow-up 10 isomorphic |Δu| + follow-up 13 full-K gain-match + follow-up 14 mean-c0 — do not revive relative Huber. P31 iter-75 live DC `[0.63, 1.48]` (MV over, other input under, spread_x 5.1) is mid-P1 + wrap-recovery — wait for a **completed** det+eager P1 before that A/B.
- Gain-probe log now prints **per-input** `ss_pairs` + `unbiased`/`not_noisy` (P31 GPU-occupied). P31 iter 75 only had min/max+worst; next gate will name MV vs DV.
- P1/P2 random collect is **numpy-only** (no `torch.inference_mode` / SF `a_history`). P3 on-policy still streams. P2 DOB Kalman scan is a closed-form mix (same recurrence; host-adaptive T×T cap 16 MiB). **P31 GPU process includes neither.**
...
- P31 launch: env-free `cursor/p28` (deterministic + eager + `storm_cap=2`). Do not pass leftover P26 env-vars (`DREAMER_COMPILE=0`, `DREAMER_RSSM_LATENT_TYPE`, `DREAMER_DOB_ENABLED`, `DREAMER_ACT_HIST_REQUIRED`). Opt-in compile is `DREAMER_COMPILE=1`. Live iter banner prints `kl`/`jemb`/`iso`/`ss` + `storm_cap=`. jsonl: `wm_ss_match_loss`, `wm_ss_match_wmean`, `wm_isolation_traj_loss`. `skip_invalid_p3` default-on. Isolation K-step decode is batched; overshoot MSE is vectorized; **gain-match FD** stacks baseline+per-input into one `img_rollout`; **P1/P2 random collect is numpy-only**; **P2 DOB Kalman scan is a closed-form mix**. P31 GPU process `4ffe11f` has batched isolation/overshoot + storm_cap=2 only — not the HEAD collect/Kalman/FD-batch opts. Variance-normalized isolation waits until a **completed** det+eager P1 still shows DV ss ~×0.87.

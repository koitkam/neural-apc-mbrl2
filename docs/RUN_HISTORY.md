...
- **ROOT:** extra P1 cannot pin DV when abs isolation/ss-match (and abs Huber gain-match) under-weight the smaller |ΔCV|. Keep-ext stays so GAIN_NOT_READY can extend. Next: inverse-variance reweight of isolation/ss-match (`mean(err/scale)*mean(scale)`). Not relative Huber (P27 full-BPTT).

### p34 (`run_p34_isovarnorm`, branch `cursor/p28`) — FIX: inverse-variance isolation / ss-match
- **One attributed GPU change vs P33:** `wm_isolation_var_norm=True` (TrainConfig default). Per-sequence isolation/ss-match `mean(err/scale)*mean(scale)` with `scale=mean(cv_real²)` (unitless floor 0.05×median). Identical to abs MSE when scale is constant; equalizes relative-gain gradient across isolated MV/DV in a mixed batch. Gain-match stays **absolute Huber** (P26). Env-free. Also picks up HEAD skip unused P1 MTP + last-ok `copy_` (not in P33).
- **Step 4 resolved-cfg vs P33:** TrainConfig + `iso_varnorm=True` on `[resolved-cfg]`. No leftover `DREAMER_*`. Watch `iso_varnorm=True`; jsonl `wm_isolation_var_wmax` / `wm_isolation_var_scale_ratio`; P1 iso magnitude not skip-storm; gain-probe DV not stuck at 0.68.
- **Judge by:** live/val DV ss/@H into `[0.8, 1.3]` (P26 ×0.87; P33 ×0.66) without MV leaving the band (P33 ×1.08). If GAIN-READY: first valid actor (min-of-2 + freeze `return_scale`; econ vs baseline **and** vs P33 BC −21). If still GAIN_NOT_READY: `[p3-skip]`. Revert `DREAMER_WM_ISOLATION_VAR_NORM=0` if skip-storm / recon detonates. Do not revive relative Huber. Do not retune isolation_coef.
...
- Isolation/ss-match default is inverse-variance reweight (`wm_isolation_var_norm=True`, P34): `mean(err/scale)*mean(scale)` with `scale=mean(cv_real²)`, floor 0.05×median (unitless). Abs mean when `DREAMER_WM_ISOLATION_VAR_NORM=0`. **P33 EXIT:** abs isolation left DV ×0.66 after full keep-ext (gain-match |tgt| MV 2.82 vs DV 0.49). Do **not** revive relative Huber.
- `wm_overshoot_max_starts=24` / `gain_match_max_starts=6` are **per-sequence** caps. With gpu-calib `B=128`, total `Bm=B·S` is ~8× paper `B=16`. Do **not** retune during P34 (would stack a second observer change). Host-adaptive thread caps already scale with `sched_getaffinity`.
...
- P33 EXIT (`run_p33_keepext`): keep-ext KEEP as mechanism, FALSIFIED as DV lever (val MV ×1.08 DV ×0.66 det_r 0.40 `[p3-skip]`). Next GPU: P34 inv-var isolation. Do not pass leftover compile/latent/DOB env-vars.
...
| Abs isolation/ss-match MSE on mixed MV/DV | P33 EXIT | SUPERSEDED (`wm_isolation_var_norm=True`) | YES — |tgt| MV 2.82 vs DV 0.49; val DV ×0.66 after full ext |

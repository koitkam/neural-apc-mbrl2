...
| P55 | 2026-08-30 | PPO μ-ratio recopy every P3 iter (`p3_mu_ratio_refresh_iters=1`) | LIVE env-free `run_p55_murefresh` pid **103504** sha `b321d89`. P1 iter **81** recon **0.0024** after skip-storm 1/2 @61–62 + wrap-unlock @71 (heading into gate ~82). Teacher FD MV/DV **×0.998 / ×0.980**. Probe@80 H=55 r=+0.576 gain_fid=0.878; `wm_best` still iter 30. `t_wm` ~127 s (eager; this pid CUDA-graph fail). Watch clip_frac / val vs P53 −13/−98 | LIVE |
...
- **Liveness:** tmux `mbrl2_p55` LIVE, pid **103504** LIVE (`b321d89`), `device=cuda`. P1 iter **81** recon **0.0024** skip **13** last_ok **81 unlocked**. Skip-storm **1/2 @61–62** (lock 60, recon 0.84→1.94, restore). Wrap lock@68 unlock@71. Teacher FD MV/DV **×0.998 / ×0.980**. `wm_best` iter **30** EMA **5.866** (iter-40 gain_fid=0.540; iter-50 0.724; iter-80 H=55 r=+0.576 gain_fid=**0.878** — gain-blind freeze still wins). Heading into typical gate **82**. `t_wm` **~127 s** (eager T-loop; this pid CUDA-graph fail). P54 `t_wm` ~124 s — same rest-IC class. **Watch** as above. **Do not launch a second GPU job.**
- **HEAD (GPU occupied, not in pid):** leftover `DREAMER_WM_DIAG_DEVICE` → TrainConfig `wm_diag_device='cuda'`. Unused `pmpo_alpha`/`pmpo_beta` + PMPO prior-refresh knobs **REMOVED**. Rest-IC in-loop **never captures** under parent autocast (warmup dtype = rest-cache numpy). Wrap `apply_runtime_knobs(cfg)` (identity 0.20 / on). Seed counts / `gain_match_max_starts=6` stay sentinels. CVD="" smoke green.

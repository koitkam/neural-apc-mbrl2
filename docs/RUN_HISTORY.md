...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P3** iter **182** (~218 min). pid **64705**, launch `d910ee2`. Gate **GAIN-READY** @82 last-ok **78** MV **0.95** DV **0.84**. Unfreeze **147**: ent **−0.101 HELD**; first-step logp_std **6.11** / actor **−0.77** (not P51 38 / −4.66). **@165** delayed μ-rail: unclipped logp_std **80.8** actor **−5.80**; peak std **100.7@169**. rtgt **0.0002**. best.pt **176**. rscale **2.83 KEEP**. | 🔬 clip delayed first-unfreeze rail; unclipped μ still walked. Do not call EXIT yet. Do not launch. |
...
### p52 (`run_p52_logpclip`, branch `cursor/p28`) — LIVE P3: clamp P3 REINFORCE logp
...
- **Step 4 CONFIRMED:** `[resolved-cfg] latent=deterministic restore_p2=False gain_match=1 dob_ground=2 dob_reg=0 isolation=0 ss_match=0 iso_dcv=off n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 lock=20 huber_per_in=True gmatch_settle=-1 gmatch_rest=True p3_sigreset=False bc_mean=True p3_sglogstd=True p3_logpclip=8 compile=eager`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. GPU ~11 GB. STAGE 3 @iter136.
- **P1→P2 @82 (~188 min):** lock@79 last_ok **78** (recon 0.331 = 196× best 0.0017@71, skip 0) → iter 80 recon 0.0554 still locked (>20×=0.0337); wm-fid 80 on recovering live H=55 r=0.466 gain_fid=0.820 (no new best). **wrap-unlock@81** recon 0.0226. Gate 82 live recon 0.0306 still >5× → **`[gain-ready-probe] last-ok iter 78`** (P50 RCA **GPU-confirmed in this pid**, not leftover-only). **GAIN-READY** DCgain MV **0.95/@H 0.95** DV **0.84/@H 0.89** worst **0.84@DV** band [0.8,1.3] unbiased. Freeze restored `wm_last_ok` iter 78; wm_best warm-restore SKIPPED. last_ok stayed 78 (wrap recon never <5×). vs P51 0.86@MV / 0.88@DV last_ok 82; vs P50 0.90@DV last_ok 88.
- **P3 unfreeze @147 then delayed μ-rail @165 (iter 182 ~218 min):** STAGE 3 @136; first P3 @137 ent **−0.143** logp_std **0.53** actor **+0.65** bc MSE **0.002**. Critic warmup 137–146 rscale 1.20→**2.83 FREEZE KEEP**. Unfreeze 147: ent **−0.101 HELD** (opened from −0.137, not yanked to σ_min); unclipped logp_std **0.53→6.11** (peak **10.4@152**, then 2.9@159 / 4.7@160 — **not** P51 **38** on the first unfreeze step); actor **−0.77** (peak −1.13@150, then +1.44@153 — **not** P51 **−4.66** at 147); bc 0.002→0.272. **Delayed rail @165–169:** unclipped logp_std **80.8@165 / 98.1@166 / peak 100.7@169** (exceeds P51 38); actor **−5.80@165 / −4.55@166** (P51-class); logp_mean **−83 / −103**; bc **1.08**. Then std eased **26@176 / 46@182**; actor mixed. rtgt 0.045@146 → 0.042@147 → 0.0027@160 → **0.0002@176** (P51-class collapse). mvv 16k@146 → **119k@151 → 290k@156** then 36k@161 / 31k@166 / **124@171 / 2.2k@176 / 256k@181** (not P51’s monotonic 19k→681k). ema −198@147 → −936@161 → **−1095@182**. skip 0. last_ok **78 unlocked**. `best.pt` **176** (`best_det_return` **−634**). Entropy stayed ≈ H(σ_init) **−0.101** (stop-grad yank-block still holding). Actor gnorm jsonl **48–57@177–178** (above clip 10).
- **Watch:** clip **delayed** the first-unfreeze μ-rail (~18 iters) but did **not** keep unclipped logp / actor_loss from later exceeding P51. rtgt already collapsed. Do **not** call KEEP/FALSIFIED as cascade until EXIT + val vs P50 **−56/−104** and P51 **−72/−111**. Do not launch a second GPU job. Do not edit val stack while pid 64705 is live.
- **HEAD (not in pid 64705):** `p3_logp_clip` is nats **per action dim**; summed logp clamp = `8×n_mv` (`_p3_logp_clip_bound`; 1-MV test_sim identity). SNR window and `identify_dynamics` / noise-config no longer invent τ=50 s / θ=5 s when SysID keys are missing (lookback/4 frames; OU θ floor 0.02). APCEnv operator-limit **schedule** knobs (change counts 1–2, ramp 0.10, warmup 0.10, strata 3, inside-margin 0.05) are TrainConfig + `ENV_OVERRIDES`; `auto_derive` jitter aligned to **0.15 / 0.20**. Step-settle `|Δu|` 0.20/0.60 + prefix 0.05/0.20, `shaping_safe_margin_frac=0.25`, PRBS-seg sentinel 0, `wm_ss_match_window_frac=0.34` are now in `ENV_OVERRIDES` (identity; `single_run` used to drop A/B). test_sim identity (τ=53). Do **not** switch APCEnv to `auto_derive`.
...
| P3 REINFORCE unclipped logp (μ-rail with frozen σ) | P51 EXIT; **P52 LIVE** (`run_p52_logpclip`, iter 182) | SUPERSEDED (`p3_logp_clip=8` per action dim); EXIT pending | YES — clamp delayed first-unfreeze (147 unclipped std **6.11** / actor **−0.77** vs P51 38 / −4.66) then unclipped std **80.8@165 / 100.7@169**, actor **−5.80**. Jsonl diag stays unclipped. rtgt 0.0002. best.pt 176. Do not call cascade until EXIT. Opt out `DREAMER_P3_LOGP_CLIP=0`. |

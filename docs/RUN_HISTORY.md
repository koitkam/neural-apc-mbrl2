...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 4**. Teacher jsonl **×0.97 / ×1.07** (pinned @3, tighter @4). recon **0.81→0.52→0.42→0.385** (P76 @4 **0.048**; P64 **0.032**). persist_rel **0.63→0.168**. `t_wm` **~305 s** steady. skip **0**. Rest-IC graph not captured. pid **242511**. | 🔬 LIVE P1 — recon still ~8–12× RSSM; teacher RSSM-class; watch freeze vs P64 0.91@DV; conv>0 |
...
- **LIVE P1 iter 4 (~22 min, nvidia 11.7 GB, `t_wm` ~305 s steady, sps 19.8):** skip **0**. Teacher jsonl 1 **×0.643 / ×−1.07** → 2 **×0.355 / ×−0.14** → 3 **×0.908 / ×1.14** → 4 **×0.970 / ×1.074** (P76 @4 ×0.982/×0.989; P64 @4 ×0.982/×0.957; P73 @4 ×0.959/×1.193). recon **0.806 → 0.524 → 0.416 → 0.385** vs P76 **0.370 → 0.135 → 0.074 → 0.048** / P64 **0.076 → 0.031 → 0.020 → 0.032** / P73 **0.071 → 0.200 → 0.110 → 0.061**. persist_rel **0.45 → 0.52 → 0.63 → 0.168** (P73 also dropped 0.98→0.37 at 3→4; not unique). gmatch **0.0077** (P76 0.0028 / P64 0.0007 / P73 0.0066). No fidelity probe yet (first at 10; ~30 min). `t_wm` is not first-iter warmup — ~5 min/iter (eager rest-IC + TSSM; P76 P1 ~96 s). Do not patch while live. Watch freeze vs P64 **0.91@DV**; conv>0 (P76 uniquely 0); val **1step→OL** vs P64 ×0.85 / P76 ×0.770; `actor_experiment_valid`. Do **not** start a second GPU job.

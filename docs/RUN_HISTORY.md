...
- **Liveness:** **GAIN-READY P1→P2 @82** (no extra-P1). Live probe worst **0.88@DV**. last_ok **82**. Skip-storm **1/2 @73** recovered. STAGE 2 DOB id, g FROZEN. P2 iter **103** recon **0.0017** dobg **0.005** last_ok **82**. wm-fid @**100** H=55 r=+0.357 gain_fid 0.752 (down vs @90 +0.627 / 0.935). Live EMA mixed to **6.037**; best **6.845** @90 held (`wm_best.pt`). Next probe ~110. Still P2 (not P3). sps ~230. GPU ~11 GB. ~187 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Watch first P3 ent ≈ H(σ_init); unfreeze ~147. Do not KEEP/FALSIFIED as cascade until EXIT.

...
| P65 | 2026-09-01 | P1→P2 `TrajectoryBuffer.clear()` (no new knob) | **LIVE P1** tmux `mbrl2_p65` pid **173526** sha `b4ee586`. Iter **69** recon **0.0012** after storm **1/2 @67** (gnorm 1.3e6, 65 skips; restored last_ok **66**; unlock; recon 0.0166→**0.0017**). MV **1.010** DV **0.980** skip **0/65**. Matches P64 storm-1 class (P64 @78). Flush not fired. | ⏳ GPU occupied. Do not score actor. Watch P2 `buf_fill` after banner. |
...
- **LIVE (2026-09-01 ~07:01 CDT, still P1):** iter **69** recon **0.0012** MV **1.010** DV **0.980** skip **0/65** last_ok **69 unlocked**. **Storm 1/2 @67** (recon 0.0021→**0.0166**, gnorm **1.3e6**, 65 inner skips); restored last_ok **66**; wrap-unlock; next iter recon **0.0017** skip_iter **0**. P64 storm 1/2 was @78 — same class, earlier clock. Flush has **not** fired. P1 quality gate still ~iter 82. **No second GPU job.**
...
- **HEAD (GPU occupied, no training-graph change):** leftover P3 jsonl alias **writes** `imag_adv_action_corr` / `pmpo_pos_frac` / `imagined_return_mean` **REMOVED** (parsers still read old logs). Stale `tools/run_nl_then_p09.sh` **DISCARDED**. Prior hour: `force_val_hidden_dist_spread`; dead `_p69` probe.

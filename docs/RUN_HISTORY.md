...
- **One attributed GPU change vs P53 pid:** `early_stop_entropy_collapse_floor_frac=0.25` (HEAD default). Trip at `H(σ_min)+0.25·(H(σ_max)−H(σ_min))` instead of `H(σ_min)+0.20 nats`. With `sigma_min_ratio=1.2`, log(1.2)≈0.182 < 0.20 so the old nats margin put **H(σ_max) below the trip** (P53 −0.101 < −0.083). New thr ≈ **−0.238**; open-σ −0.101 does not trip; σ_min −0.283 still does. Tiny σ-band → skip entropy ES. Opt out `DREAMER_ES_ENT_FLOOR_FRAC=0` (legacy `frac·H(σ=1)` only). Also P53 `p3_mu_ratio_clip=0.2`. Env-free. tmux `mbrl2_p54`. `CUDA_VISIBLE_DEVICES=0`.

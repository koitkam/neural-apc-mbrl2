...
- **P1 LIVE (~3.0 h):** gmatch pinned **0.0002–0.001** (P37-class; not P38 stuck 1.30). Past P38 storm window (iter 45/48) **without** a skip.
  - Storm **1/2 @iter 53** (recon 0.416, gnorm 1.5e10, skip 28) restored last-ok **51**; **extension kept 175924**. Recovered iter 54 recon 0.0049. Continue-after-storm KEEP. Keep-ext KEEP.
  - Iter **75 EXTENDED** (`GAIN_NOT_READY` DC `[0.71,1.04]` worst=**0.71@DV**, MV 1.04 in `[0.8,1.3]`, `unbiased=False not_noisy=True`): +75396 to 829356 (cap 929884). **Same first-gate DV as P32/P33/P37 (0.68–0.71).** Cube-boost has **not** moved the first-gate DV pin. Watch iter 85 DV; extra-P1 silent detonation (P37 iter 88). Storm 2/2 would `_force_p1_cap_at`.
- **HEAD (not in this pid):** SNR measured-CV/DV summary; jsonl isolation mv/dv traj; const-action **and step-test** MIMO per-MV holds + one-MV steps (`_per_mv_hold_rows` / `_step_test_mv_index`; n_mv≤1 identity).
- Do **not** launch a second GPU job. Actor only on a GAIN-READY freeze.
...
- Isolation/ss-match is **abs MSE only**. Inv-var A/B **REMOVED**. Default **`wm_isolation_dcv_match=True`** with **scale floor 1.0** (P38 RCA): isolation settle `Δu_i ∝ 1/|G_i|` (WM-norm |G|, `level*scale` clip ±1, never shrink below op-band; equal-|G| no-op). P38 EXIT (`run_p38_isodcv`, no floor): applied edge |Δu| MV **0.19** / DV **1.0**; storm 2/2 @iter 48 CAPPED GAIN_NOT_READY **0.01@DV**; gmatch stuck 1.30; val MV **×1.25** DV **×0.007** det_r 0.43 amp-dead `[p3-skip]`. P39 env-free expected test_sim edge |Δu| MV **0.60** / DV **1.0**. Audit `run_plan.isolation_dcv_scales.min_scale` + `edge_du_*`. Pre-iso resolve is **scales only**; post-seed always re-resolves Huber. P37 EXIT unscaled abs: val MV ×0.98 DV **×0.69** det_r 0.37 `[p3-skip]`. Do **not** re-add `wm_isolation_var_norm`.
...
| Isolation |ΔCV| excitation with **scale floor 1.0** | P39 LIVE (`run_p39_isodcvfloor`, `92c7662`, pid 4170377) | IN FLIGHT (default ON) | PARTIAL — Floor keeps P37 MV SNR (gmatch 0.0008). Storm 1/2 @53 recovered, ext kept. Iter 75 EXTEND **0.71@DV** (MV 1.04) — same first-gate plateau as P32/P33/P37. Cube-boost has not moved first-gate DV. Watch iter 85 / extra-P1 detonation. |

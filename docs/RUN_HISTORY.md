...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 52**. Probe50 still last **H=1 r=+0.509 / H=55 r=+0.273 conv=0.00** gain_fid **0.389**. recon **0.006@52 best** (P64 @50 **0.0026**; ~2.3×). Teacher **×1.00 / ×0.99 @52**. persist_rel **0.73@51 inject → 0.49@52**. pid **242511**. | 🔬 LIVE P1 — recon closing; persist inject-spike @41/@51 not persist N+1. Probe60 pending. Do not patch live. |
...
- **LIVE P1 iter 52 (~272 min, nvidia 11.7 GB, `t_wm` ~306 s, sps 19.7):** skip **0**. Teacher jsonl 50 **×0.996 / ×0.925** → 51 **×1.005 / ×0.983** → 52 **×0.999 / ×0.985**. recon **0.007@50 → 0.008@51 inject bump → 0.006@52 new best** vs P64 **0.0026@50** (~2.3×; P76 @50 0.0028). last_ok **52 unlocked**. persist_rel **0.51@50 → 0.73@51 inject spike → 0.49@52** (same pattern as **0.82@41 → 0.24@43**; do **not** persist N+1). Rest-IC graph not captured. Next probe typically iter 60 (~00:42 CDT).

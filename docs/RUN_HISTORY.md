...
| P60 | 2026-08-31 | env-free `gain_match_step<=0` auto = `wm_tf_step_frac` (0.4) | LIVE **P1** ~iter 25. Teacher jsonl MV/DV **×0.950 / ×0.975** at Δu=0.4 (iters 1–25 ~×0.93–1.03; pins G(0.4), not P59 G(1)). recon 0.0031 gmatch 0.0018. `wm_best` iter **20** (gain-blind). Rest-IC graph **captured N=6 T=128**. `t_wm` ~97 s. pid **144896**, sha `bdb2e49`. **No second GPU job.** | LIVE |
...
- **LIVE P1 (~iter 25, 2026-08-31 13:58):** jsonl teacher `gain_match_mv_ratio` **×0.950** / `gain_match_dv_ratio` **×0.975** at Δu=0.4 (iter 1–25 both sit ~×0.93–1.03). recon **0.0031**, gmatch **0.0018**, jemb 0.012, iso/ss 0, kl 0. `t_wm` **97 s** / `t_collect` **3.3 s**. Graph captured. `wm_best` iter **20** (gain-blind; expected; probe@20 gain_fid 0.664 vs @10 0.834). **Teacher is pinning G(0.4)** — the P59 jsonl×1.01-at-Δu=1 vs val G(0.4) gap is the hypothesis under test; val TM still pending freeze. **No second GPU job.**
- **HEAD (not in pid 144896):** overshoot/isolation gather idx cache + one `obs_win`; PRBS gain-match start arange cached (`_gmatch_starts`; env-free rest-IC skips that path). Identity speed. CVD="" smoke.

...
| P70 | 2026-09-02 | OL hold of `c[..., :cont_gain_dim]` after first `img_step` (P69 tail REVERT) | LIVE `run_p70_cgainhold`. Mechanism: G is a plant property; 1-step prior still infers G, remaining OL copies it so DC does not need GRU memory across 4H. | 🔬 launch |
...
| OL hold of continuous gain block after first prior step (`hold_gain_c`) | **P70 LIVE** (`run_p70_cgainhold`) | LIVE | — G is a plant property. 1-step prior still infers G; remaining OL copies it. Isolation k=0 still trains `cont_prior`. No new knob. |

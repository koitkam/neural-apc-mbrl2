...
| P65 | 2026-09-01 | P1→P2 `TrajectoryBuffer.clear()` (no new knob) | **LIVE P1** tmux `mbrl2_p65` pid **173526** sha `b4ee586`. Iter ~31 recon **0.0034** MV **1.02** DV **1.03** skip **0** last_ok **31** unlocked. Matches P64@31. Flush not fired yet. | ⏳ GPU occupied. Do not score actor. Watch P2 `buf_fill` after banner. |
...
### p65 (`run_p65_p2flush`, branch `cursor/p28`) — LIVE P1: P1→P2 replay flush
...
- **LIVE (2026-09-01 ~05:50 CDT, still P1):** iter **31** recon **0.0034** gmatch **0.00046** MV ratio **1.019** DV **1.029** skip **0/0** last_ok **31 unlocked** `buf_fill` 99%. P64@31 was recon 0.0039 / MV 1.024 / DV 1.020. No skip-storm. Flush has **not** fired (`[curriculum] P2 replay flush` absent). **No second GPU job.**
- **Not:** raise P1 amp, held-magnitude N+1, `dob_ground` var-skip (second change), critic knobs, μ-refresh, S=H, isolation, relative Huber, revert clip / collect graph / decode-CV space.
- **HEAD (GPU occupied, no training-graph change):** val/diag pin `hidden_dist_spread=True` via `force_val_hidden_dist_spread` (explicit cfg; leftover `DREAMER_HIDDEN_DIST_SPREAD=0` no longer wins). Dead `tools/_p69_prior_rollout_probe.py` removed.

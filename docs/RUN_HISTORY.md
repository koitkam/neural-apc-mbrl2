...
### p44 (`run_p44_gmatchsettle`, branch `cursor/p28`) — LAUNCH: held settle before gain-match FD
...
- Abs Huber gain-match is count-equal per input but **scale-unequal** when `|G_mv|≫|G_dv|`. P43 EXIT: per-input β **equalized jsonl Huber** (mv≈dv ~1e-4) but val DV still **×0.740 / @H ×0.849** (P40/P41/P42 class). Residual was already ~0 — β is not the DV lever. Identified G matches TM real (MV −0.32, DV 0.18). **DV @H ×0.849 vs ss ×0.740 is a 4H-asymptote gap** (MV @H≈ss); P43 DV decomp posterior+1-step **faithful**. Teacher IC (PRBS posterior FD at K=H) ≠ TM rest-then-step (`wm_tf_horizon` S=`H_tf`). CPU probe on P43 freeze: P44 teacher (PRBS+S=H) still FD DV×0.969. P44 GPU storm **2/2 @66** G_pred≈0 CAPPED 0.76@DV → **REVERT env-free settle `-1`**. Do **not** revive relative Huber. Do not try another Huber reweight. Do not set `gain_match_len`=4H as a Huber reweight. Do not retry S=H. Next GPU after P44 EXIT: TM-protocol rest-IC teacher (`DREAMER_GAIN_MATCH_REST_IC=1`; one attributed change; isolation loss stays 0). Collect settle = max(H, lookback), not `wm_tf_horizon`.
...
| Gain-match FD from PRBS + WM held settle S=H | P44 GPU (P2 after storm 2/2) | **REVERTED** env-free default `-1` | YES — storm 2/2 @iter 66 after iter-58 G_pred≈0 (Huber sat \|tgt\|-½β). Lock 57 GPU-confirmed. CAPPED 0.76@DV (MV ×0.94). Extra prior-roll detonated original P1 (P43 had no G_pred=0 lock-up). Do not retry S=H. CPU probe P43 freeze: FD DV×0.969 still ≈×1; TM rest-step remains the gate. Next: rest-IC (`DREAMER_GAIN_MATCH_REST_IC=1`). |
| Gain-match FD from real rest lookback encode (TM protocol IC) | HEAD opt-in (not P44 pid) | pending (default False) | P45. Isolation loss stays 0. Collect = TM `_settle_capture` pairing (obs after step) + `clean_steady_seeds`; settle = max(H, lookback) ≠ `wm_tf_horizon`. Skips P44 WM-held settle (the suspected G_pred=0 path). Promote only after P44 EXIT FAIL. |

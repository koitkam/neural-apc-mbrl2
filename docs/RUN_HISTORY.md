...
- **Judge by:** second gate ~94 / cap ~104. Val MV ss/@H ~×1.0, DV toward P26 ×0.87 not P42 ×0.74. If GAIN-READY: first valid actor. Else `[p3-skip]`. Next GPU (after EXIT, if still ~0.75@DV): not Huber — teacher/probe protocol or EIV/DOB (`gain_match_len`=H≈1τ vs TF 4×H; `unbiased=False`).
...
- Abs Huber gain-match is count-equal per input but **scale-unequal** when `|G_mv|≫|G_dv|`. P43 first gate: per-input β **equalized jsonl Huber** (mv≈dv ~1.5e-4) but probe still **0.75@DV** (P40/P41). Residual was already ~0 — β is not the DV lever. Scalar `gain_match_huber_beta=1` stays the A/B (`DREAMER_GAIN_MATCH_HUBER_PER_INPUT=0`). Do **not** revive relative Huber. Do not try another Huber reweight.
...
| Per-input Huber β = `|tgt_ij|` (L1 sat ±1) | P43 first gate | KEEP as P1 form (not P27); **FALSIFIED as first-gate DV pin** (pending EXIT) | YES — iter 82 MV ×1.04 DV ×0.75 = P40/P41; gmatch already ~1e-4 (mv≈dv). Storm 1/2 @74 recovered. Extra P1 running. |

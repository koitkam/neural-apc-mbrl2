...
- **Step 4 resolved-cfg vs P46:** same env (`p3_sigreset=True` only). Code: Adam log_std-row zero (P46 pid was weights-only). Banner expected: `gmatch_rest=True p3_sigreset=True compile=eager`. Confirm `[p3] reset policy log_std residual` **and** Adam-moment zero line.
- **Watch:** first P3 entropy ~−0.101. Unfreeze must **not** yank −0.101→−0.323. Warmup rtgt >0.015. After unfreeze: no actor_loss −300; no entropy-collapse; econ vs baseline **and** vs P46 −256 / P45 −216. If still yanks → Adam was not the lever. If σ stays open but still limit-rides → **REVERT**; next collect-DV / collect-DOB. **Do not launch a second GPU job.**
...
- Reward clip/cal + `DREAMER_DIAG_*` / `DREAMER_RUN_WM_DIAGNOSTIC` / `DREAMER_WM_DIAG_{N_STARTS,HORIZON}` are TrainConfig + `ENV_OVERRIDES` (identity defaults). `DREAMER_WM_DIAG_DEVICE` stays env-only (device picker).

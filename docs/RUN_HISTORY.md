...
### p36 (`run_p36_isoinpscale`, branch `cursor/p28`) — FIX: isolation scale = per-input `|G|²` not per-seq `|CV|²`
- **One attributed GPU change vs P35:** `_isolation_per_input_scale` (identified `|tgt|²`; no-ID fallback scatter-mean CV² per detected input, 5% loudest floor). Same mean-1 `w·err`. Gain-match stays abs Huber. Env-free. HEAD `596b78e`.
- **Step 4 resolved-cfg vs P35:** TrainConfig knobs **identical** (`iso_varnorm=True`). Diff is the isolation **scale source**, not a `DREAMER_*` override. Watch jsonl `wm_isolation_var_tgt_scale=1`, `wm_isolation_var_scale_ratio` ~33 not 22000, iter-1 iso **~1–3** (P33 class) not 0.0008, gmatch falling from ~1.21 (P33 hit 0.002 by iter 9).
- **Judge by:** if scale_ratio still ~22000 or tgt_scale=0 at iter 1 → LUT miss, kill (do not burn P1). If iso explodes like P34 → kill. If GAIN-READY: first valid actor (rscale freeze, rtgt>0.015, econ vs baseline **and** vs P33 BC −21). If still GAIN_NOT_READY: `[p3-skip]`. Val MV ss/@H ~×1.0 ±0.1 (P26 ×0.97/×0.88; P33 ×1.08), DV not ×0.01 and not stuck at P33 ×0.66. If P36 still drowns DV after a healthy P1: **REVERT var-norm to abs** (P33 ×0.66 ≫ P35 ×0.01) rather than another reweight formula.
- Watch `[resolved-cfg] latent=deterministic compile=eager iso_varnorm=True storm_cap=2`. Do not pass leftover compile/latent/DOB env-vars.
...
- Isolation/ss-match default is inverse-variance reweight (`wm_isolation_var_norm=True`): `mean(w·err)` with `w∝1/scale`, `mean(w)=1`. **Scale is per isolated input** (identified `|tgt|²` from dynamics-ID; no-ID fallback = scatter-mean CV² of that input). P35 per-sequence `|CV|²` **SUPERSEDED** (scale_ratio ~22000, iso 0.0008, gmatch stuck 1.21, storm 2/2 cap GAIN_NOT_READY 0.01@DV). P34 `mean(err/scale)*mean(scale)` **REVERTED**. Abs mean when `DREAMER_WM_ISOLATION_VAR_NORM=0`. Do **not** revive relative Huber.
...
- P35 EXIT (`run_p35_isowmean1`): storm 2/2 CAPPED GAIN_NOT_READY 0.01@DV. Val MV ×0.94/@H ×0.76, DV ×0.013, det_r 0.275, `[p3-skip]`. Per-seq `|CV|²` SUPERSEDED. P36 GPU: per-input `|G|²`, env-free. Do not pass leftover compile/latent/DOB env-vars.

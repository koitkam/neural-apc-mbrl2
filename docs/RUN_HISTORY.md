...
- **P1 iter 46 (LIVE, ~1.8 h, GPU 17736 MiB):** gmatch **0.0007** iso 0.0012 recon **0.0046** skip **0**. Past P38 storm-1 window (iter 45) **without** a skip. P37 iter-19 wrap blip did **not** fire. Approaching P38 storm-2 (iter 48). Watch skip 0 through 53; iter 75 DV ratio toward [0.8, 1.3]. If storm 2/2 early: cube-boost FALSIFIED → REVERT `wm_isolation_dcv_match=False`. Do not launch a second GPU job.
- **HEAD (not in this pid):** SNR summary is measured CV+DV only (P39 log `min=-120dB` was constant `CV0_tgt`/`CV0_tgt_on`; true CV/DV +17.9/+14.8 dB). jsonl `wm_isolation_mv_traj` / `wm_isolation_dv_traj` (detached, no extra forward).
...
- `dv_prbs_seed_episodes=24` / `expert_seed_episodes=24` / `constant_action_seed_episodes=40` are flat episode counts (each DV-PRBS episode already sweeps every DV; episode length already scales with τ). Isolation settle is already per-input. `phase3_onpolicy_buffer_eps=16` is an on-policy episode count (same). **Const-action MIMO OP** is no longer the all-MVs-equal diagonal: `_per_mv_hold_rows` permutes the same linspace per MV (`n_mv<=1` → None, test_sim scalar path + RNG unchanged). Do not raise the 40-episode sentinel without a MIMO run.
...
| Isolation |ΔCV| excitation with **scale floor 1.0** | P39 LIVE (`run_p39_isodcvfloor`, `92c7662`, pid 4170377) | IN FLIGHT (default ON) | PARTIAL — P38 EXIT falsified shrink-the-strong-channel. Floor keeps P37 MV SNR. Iter 46 gmatch **0.0007** skip 0; past P38 storm-1 @45. Watch skip through 53. |

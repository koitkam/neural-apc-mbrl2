...
- **LIVE 2026-09-02T14:50Z P1 iter 57** (pid **221013**, nvidia **~15.3 GB**, skip **0**, last_ok **57** unlocked). Teacher pinned by iter **5** (MV **×0.981** DV **×0.957**; P73 iter 5 ×0.996/×0.980). Iter 57 jsonl MV **×0.991** DV **×1.078** recon **0.0035** gmatch **0.0013** `t_wm` **100 s**. `gain_cv_skip` **still stalled**: rms **0.0006→0.0046 @5, 0.0042 @25, 0.0041 @57** (max **0.0046**). `wm_last_ok.pt` iter **19** W **[+0.0040, −0.0051]**; `ckpt_iter_00040` W **[+0.0031, −0.0049]** — shrinking, not growing. vs |G_mv| **2.62** skip ΔCV ~0.2% of DC if `c~O(1)`. persist_rel still **0.05–0.73** (iter 57 **0.234**; P73 **0.047 @57** / **0.037 @81**). Wrap @20 recon **0.1926** locked 19 / unlocked 21 (hygiene KEEP). Iter 52 recon **0.024** recovered (not 20×). Fidelity iter 30 gain_fid **0.972** / 40 **0.558** / 50 **0.813** (gain-blind mix). jsonl ×1 ≠ val TM. **Do not** score actor. **Do not** start a second GPU job. **Do not** change training while this pid is live. HEAD (not this pid): `gain_cv_skip_rms` jsonl last-logged-inner only (default True; not in `wm_total`).
- Watch freeze vs P64 **0.91@DV** / P73 **0.83**; val **1step→OL / OL-vs-real** vs P73 ×0.761 / ×0.743 and P64 ×0.85 / ×0.84; whether `gain_cv_skip_rms` ever leaves ~0.004; persist_rel at last_ok vs P73 0.037; `actor_experiment_valid`. Teacher Huber already ~0 after iter 5 so the skip has no residual — if it stays dead through freeze, P74 is a teacher-pin no-op (decoder MLP saturates Huber) and compounding is not tested until val TM.

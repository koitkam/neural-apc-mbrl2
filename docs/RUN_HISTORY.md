...
| P50 | 2026-08-29 | P1/P2 BC MSE-on-μ (`bc_mean_only=True`) | **LIVE** P1 extra iter **85**. Wrap-unlock ×2 (56→58, 70→73). Gate **82 FAIL 1.37@MV** (DV ×0.82/@H 0.86) EXTEND; recon 0.0283 at probe. skip **0**. **bc MSE never NLL.** Do not launch. | 🔬 extra-P1; do not score actor until GAIN-READY; watch next gate ~94 |
...
- **LIVE (2026-08-29 19:40 CDT, P1 extra iter 85):** second wrap-unlock **GPU-confirmed**. Iter **71** recon **0.1800** = 123× best, skip 0 → locked last_ok **70**; iter **73** recon 0.0238 → **unlocked wrap-recovery**; last_ok **85**. skip **0** (no storm). **`bc_loss` MSE never NLL** (min 0.000 @65; else +0.23…+0.53).
- **Gate iter 82 FAIL GAIN_NOT_READY** `worst=1.37@MV` (MV ×1.37/@H ×1.33; DV ×0.82/@H ×0.86; band [0.8,1.3]; spread×2.4 unbiased=False). Probe ran with recon **0.0283** (19× best — wrap-adjacent, just under 20× lock). Teacher FD at 82 still ~×1.0. EXTEND P1 +75396 steps to 829356 (cap 929884). P45–P49 first gates were GAIN-READY 0.82–0.90@DV; this fail is **MV over-gain**, not the usual DV under. Extra P1 already FALSIFIED as DV lever (P41). Do **not** score `bc_mean_only` on this observer miss — BC is the actor lever. `wm_best` @80 gain_fid 0.913. GPU ~11 GB.
- **Code follow-up (HEAD, not P50 pid):** leftover expert move-law + CLI extras → TrainConfig + `ENV_OVERRIDES` (`2232112`). Smoke PASSED. **Do not launch.**
- **Watch:** next gate ~iter 94; if CAPPED GAIN_NOT_READY then `[p3-skip]` KEEP (do not train P3). If GAIN-READY: P2 `bc_loss`>0; first P3 ent ≈ **−0.08** not −0.283. Remaining leftover is env-only paths — do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.
...
| Original-P1 wrap last-ok unlock | P49 EXIT (`run_p49_wrapunlock`); **P50 LIVE** (`run_p50_bcmean`) | KEEP as lock hygiene; **FALSIFIED as cascade lever** (P49); wrap-recovery unlock **GPU-confirmed P50** @57–58 | YES — P49 skip-storm 1/2 @64–65 unlocked (wrap path untested). P50 lock@56 recon 0.1412 skip 0; `[wm-last-ok] unlocked after wrap recovery` @58; last_ok **66**. Same P3 cascade still the actor question. |

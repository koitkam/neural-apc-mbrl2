...
- **LIVE P1 (2026-08-30 ~12:02 CDT, pid 95956):** device=cuda bs=128. recon 0.361→**0.0042**, gmatch **0.0002**, skip **0**, jemb 0.036, last_ok **29** unlocked after wrap lock@5 unlock@7. wm_best iter 20 `gain_fid=0.677` best_h=55/55. Teacher FD MV/DV ~×1.02/×1.01 (not TM). `[resolved-cfg] … p3_muratio=0.2 es_ent_floor=0.25 compile=eager`. **Do not launch a second GPU job.**
- **HEAD (not in pid 95956):** p136 `actor_kl_coef` / `DREAMER_ACTOR_KL_COEF` **REMOVED** (FALSIFIED; never wired in `_realsim_actor_critic_step` — false A/B). π_prior snapshot skipped unless `actor_loss_type=pmpo`. CUDA replay H2D reuses pinned host + GPU dest **per slot** (`replay` / `iso` / `critic`) so P3 actor+critic batches cannot alias. Rest-IC numpy→device pins once. CVD="" smoke green.

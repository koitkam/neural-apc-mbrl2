...
| **Critic** | `critic_rew_to_tgt_var` (>0.015) | none healthy | — | P64 val `critic_r` **+0.705** PASS; P3 rtgt 0.068→**0.002** (usual collapse). Freeze rscale KEEP. |
| **Actor** | econ vs baseline + vs expert BC | **P64** (mbrl2) | paired **−4.54 vs −94** | Env-free P2 amp=P3. 9/9 `all_pass`. Beats P58 **−6.24 vs −102**. P53 stays the μ-ratio-clip RCA. Observer still short of P26 MV ss. |
...
| P65 | 2026-09-01 | P1→P2 `TrajectoryBuffer.clear()` (no new knob) | **LIVE P3** tmux `mbrl2_p65` pid **173526** sha `b4ee586`. Flush **ON** (dropped 327). P1 GAIN-READY **0.86@DV** @82. P2 buf **5→276/327**. `dob_d` last **0.085** (P64 **0.063**). Unfreeze **147** ent **−0.101 HELD** rscale **1.88 KEEP**. | ⏳ GPU occupied. Actor VALID to score at EXIT. Watch val pred_std vs 1.93. |
...
### p65 (`run_p65_p2flush`, branch `cursor/p28`) — LIVE P3: P1→P2 replay flush
...
| P1-clean replay at P2 Kalman ID | **P64 EXIT** `dob_d` only 2.4×; `dob_ground` 0.21 not closed; mixed `d≡0` rows | P65 flush `TrajectoryBuffer` at discrete P1→P2 (no new knob) | YES — grounding on zeros ≡ L2-on-d→0 (p19 `dob_reg` hole). Capacity cannot lap in 54 P2 iters. |

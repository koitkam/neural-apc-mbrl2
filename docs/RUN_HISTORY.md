...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P1** iter **47** (~107 min). pid **64705**, launch `d910ee2`. Step 4 **CONFIRMED** `p3_sglogstd=True p3_logpclip=8`. recon **0.0039** (best **0.0027@37**) last_ok **47 unlocked**. lock@38 wrap-unlock@39 last_ok advanced 37→47. skip **0**. Iter 40 probe gain_fid=0.511 (best still 30 EMA 5.798). GPU ~11 GB. Env-free. Watch gate 82; unfreeze ~147 vs P51 μ-rail. | 🔬 wrap-unlock KEEP (P50-class). Do not launch. Do not stack critic knobs. |
...
- **P1 iter 47 (~107 min):** recon **0.0039** (best **0.0027@37**) last_ok **47 unlocked** skip **0**. lock@38 wrap-unlock@39 last_ok advanced 37→47 (P50-class KEEP). Recovering toward pre-spike basin. Iter 40 wm-fid still last probe: H=55 r=0.451 `gain_fid=0.511` (best iter 30 EMA **5.798**). Teacher FD mv **0.976** / dv **0.977**. Gate 82 ~1.3 h away; unfreeze ~147 is the A/B.

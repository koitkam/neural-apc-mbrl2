...
| P55 | 2026-08-30 | PPO μ-ratio recopy every P3 iter (`p3_mu_ratio_refresh_iters=1`) | LIVE env-free `run_p55_murefresh` pid **103504** sha `b321d89`. **GAIN-READY @82** 0.88@DV last_ok **82**. Unfreeze 147: logp_std 0.67→**18.8@175** still **19.4@320**. `clip_frac` 0.02–0.80 (moving ball; P53 0.61→0.14). ema **−170→−2163@260** then ~−1100@320. vs_expert **−2700**. rtgt 0.060→**0.0001**. rscale **1.91 KEEP**. **best.pt iter 166** det **−28.8** (walk detonates @169 logp 10.9). Rest-IC CUDA-graph fail this pid (`t_wm` ~127 s). Not KEEP until EXIT+val. | LIVE |
...
- **Liveness:** tmux `mbrl2_p55` LIVE, pid **103504** LIVE (`b321d89`), `device=cuda`. **P3** (P1 1–82, P2 83–136, P3 from 137). Skip-storm **1/2 @61–62** (lock 60). Wrap lock@68 unlock@71. **Gate @82 PASS GAIN-READY** worst **0.88@DV** (MV **0.89/@H 0.86**, DV **0.88/@H 0.91**); last_ok **82 unlocked**. Teacher FD @82 MV/DV **×1.020 / ×1.016**. `wm_best` iter **30** EMA **5.866** (gain-blind freeze; P1→P2 restore SKIPPED). STAGE 2 g FROZEN. P2 recon **~0.0026**. `t_wm` P1 **~127 s** (this pid CUDA-graph fail) / P2 **~24 s** / P3 `t_ac` ~3.8 s. **P3 (vs P53 freeze-forever):** unfreeze 147 clip 0.47 logp_std 0.67 → **18.8@175** still **19.4@320** (P53 held ~0.47). `clip_frac` stays 0.02–0.80 (P53 0.61→0.14). ema **−170→−2163@260** then ~−1100@320 (P53 ~−120). vs_expert **−2700**. rtgt 0.060→**0.0001**. rscale **1.91 KEEP**. bc 0.002→1.26. **best.pt iter 166** det **−28.8** then walk detonates @169 (logp 10.9). Mechanism: N=1 recopies the ε=0.2 ball onto last-iter μ each collect → 20%/iter compounding walk, not a BC ball. Do **not** stack critic knobs. Do **not** launch a second GPU job. Not Step-5 until EXIT+val — do **not** treat best.pt@166 as a KEEP of refresh=1.
- **HEAD (GPU occupied, not in pid):** leftover `DREAMER_WM_DIAG_DEVICE` → TrainConfig `wm_diag_device='cuda'`. Unused `pmpo_alpha`/`pmpo_beta` + PMPO prior-refresh knobs **REMOVED**. Rest-IC in-loop **never captures** under parent autocast (warmup dtype = rest-cache numpy). Wrap `apply_runtime_knobs(cfg)`. P3 μ-ratio refresh is **in-place** `load_state_dict` (identity vs deepcopy). Rest-IC CUDA graph **released at g freeze** (P2/P3 skip gain-match; host-adaptive VRAM). jsonl `actor_pos_adv_frac` / `adv_action_corr` alias leftover `pmpo_pos_frac` / `imag_adv_action_corr`. `training_diagnostics` 3×3 includes logp_std / clip_frac / rtgt / vs_expert. Seed counts / `gain_match_max_starts=6` stay sentinels. CVD="" smoke.
...
- Rest-IC encode length is `lookback` (test_sim **128** sequential GRU steps × 100 inner WM steps → `t_wm` ~124 s). A few-τ window would be more sim-adaptive; that is a later A/B (not identity). **HEAD:** CUDA-graph of that T-loop (`gain_match_rest_ic_cuda_graph=True`), warmed **before** the P1 WM autocast loop + parent `enabled=False` then bf16 `cache_enabled=False`; **in-loop never captures** (autocast miss is eager that step, not a fail pin). CPU path stays the Python loop (`CVD=""`). `DREAMER_GAIN_MATCH_REST_IC_CUDA_GRAPH=0` keeps eager.
...
| P3 freeze-forever μ-ratio snapshot (P53 KEEP as walk limiter) | P53 EXIT; **P54 EXIT** (`run_p54_esentband`); **P55 LIVE P3** | P55 tests recopy every P3 iter (`p3_mu_ratio_refresh_iters=1`); ε=0.2 KEEP | PARTIAL — P53 KEEP as cascade (val −13/−98). P54 extra P3 FALSIFIED as econ lever: `clip_frac` 0.42→0.11. P55 LIVE: N=1 recopies the ε-ball onto last-iter μ → compounding walk (logp_std 0.67@147 → 18.8@175, still 19.4@320; ema −170→−2163; rtgt 0.0001). best.pt **166** det **−28.8** *before* the walk detonates (@169). P53 held logp_std ~0.47. Not Step-5 until EXIT+val. Do not judge refresh=1 from best.pt@166 alone. 0 = freeze-forever. Opt out `DREAMER_P3_MU_RATIO_REFRESH=0`. |
...
| Rest-IC last_only T-loop = 128 sequential kernel launches × 100 WM steps | P54 GPU-occupied (pid **95956** P1); **P55 LIVE** first GPU capture failed | SUPERSEDED (`gain_match_rest_ic_cuda_graph=True`; `make_graphed_callables` + GRU-grad canary; **P55 HEAD** capture **before** WM autocast + parent `enabled=False` then bf16 `cache_enabled=False`; **in-loop never captures**) | PARTIAL — P55 pid **103504** captured inside `world_model_loss` autocast (`RuntimeError: make_graphed_callables does not support the autocast caching`) → `_rest_ic_cg_fail` pinned eager (`t_wm` ~127 s, P54 class). Nested `cache_enabled=False` alone was not enough if the parent cache flag is visible. Warmup at train start + in-loop autocast skip. GPU identity still deferred (A10 occupied). `DREAMER_GAIN_MATCH_REST_IC_CUDA_GRAPH=0` keeps the Python loop. |

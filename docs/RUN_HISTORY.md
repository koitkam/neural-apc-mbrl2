...
- **Liveness:** **GAIN-READY P1→P2 @82** (no extra-P1). Live probe worst **0.88@DV**. last_ok **82**. Skip-storm **1/2 @73** recovered. STAGE 2 DOB id, g FROZEN. P2 iter **100** recon **0.0018** dobg **0.0027** (0.007 @97 recovered). wm-fid @**100**: H=1 r=+0.692 H=13 r=+0.510 H=27 r=+0.300 **H=55 r=+0.357** conv=1.00 drift=0.008 gain_fid **0.752** best_h=13/55 (not the TM gate; @H down vs @90 +0.627 / gain_fid 0.935 / best_h 55/55). EMA **6.845** held @90 (`wm_best.pt` unchanged). Expert-inject P2 only. Next probe ~110. Still P2 (not P3). sps ~232. GPU ~11 GB. ~186 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Watch first P3 ent ≈ H(σ_init); unfreeze ~147. Do not KEEP/FALSIFIED as cascade until EXIT.

...
| P57 | 2026-08-31 | rest-IC encode L=last `max(K,2τ/sr)` (`gain_match_rest_ic_len=-1`) | **LIVE P3** ~iter **201**. Graph **captured** N=6 T=55 then **released** at g freeze. P1→P2 **GAIN-READY @82** worst **0.89@DV** (MV 0.93/@H 0.94, DV 0.89/@H 0.95). last_ok **82 unlocked**. skip-storm **1/2 @41**. `t_wm` P1 **99 s** / P2 **22.6 s**. P2→P3 @136 mtp **0.905**. Unfreeze **147** ent **−0.101 HELD**; logp_std ~**0.50**. rscale **FREEZE 2.35 KEEP**. | ⏳ LIVE GPU P3. Encode L PARTIAL until EXIT+val TM vs P53 ×0.90 / paired −13/−98. Do not launch a second job. |
...
### p57 (`run_p57_resticlen`, branch `cursor/p28`) — LIVE P3: rest-IC encode L=last max(K, 2τ/sr)
...
- **Liveness:** **LIVE P3** ~iter **201**. GPU occupied (~4404 MiB after g freeze). skip-storm **1/2 @41** (recon **0.2738**, lock last_ok **40**, restored, unlocked, extension kept). Teacher FD MV/DV ~**×1.00**. `t_wm` P1 median **99 s** (P56 eager 124 s) / P2 **22.6 s**. `wm_best` iter **90** (gain-blind; P1→P2 restore SKIPPED after storm).
  ...
  - **Unfreeze 147:** first P3 ent **−0.111** → open-σ **−0.101 HELD**. logp_std **1.46@147** then **~0.50** (P53 ~0.47; not P55 18). `clip_frac` **0.68@147** then ~**0.18–0.22**. rtgt 0.053→collapsing (known; do not stack critic knobs). `critic_pred_target_r` ~**0.93** (not inverted).
- **HEAD (not this pid):** capture hygiene; last-inner enc-diag/Huber; P1 H2D `obs`/`act` (`dist` iff `_wm_need_dist_target`); **`_replay_h2d_keys` skips leftover `cont` every phase and P3 `dist`** (frozen observer re-encodes from `obs`; P2 still copies `dist` for dob-ground); rest-IC FD `a_seq` cache; T/K `_time_unbind`; Stage-1 `cached_zeros_btd`. Identity. CVD="" `_smoke_rssm.py` + `_smoke_tssm.py` green. **Do not launch a second GPU job.**
- **Watch at EXIT:** val TM vs P53 ×0.90; paired vs **−13 vs −98**. Encode L still PARTIAL until val TM. Do not fuse rest-IC encode+FD graphs. Do not stack critic knobs.

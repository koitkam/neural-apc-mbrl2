...
| P48 | 2026-08-29 | P3 collect/val stream measured DV + Kalman | launching `run_p48_collectdv` | ⏳ env-free (no σ-reset); train/serve hole is the attributed change |
...
- **P48-live (GPU occupied, no second job):** leftover `DREAMER_DERIVED_OBSERVABLES` / `_OBS_WINDOW` → TrainConfig + `ENV_OVERRIDES` (identity ON / auto 2τ). Collect/val reuse a persistent GPU obs row and one autocast region; val no longer H2D's the lookback window on the RSSM path.
...
| Leftover `DREAMER_DERIVED_OBSERVABLES` / `_OBS_WINDOW` `os.environ.get` | P48 GPU-occupied | SUPERSEDED (TrainConfig + `ENV_OVERRIDES`; identity ON / auto 2τ) | YES — same leftover-env class as clip/cal. Window 0 = auto. `DREAMER_PROCESS_NOISE` / `HIDDEN_DIST_*` stay env-only. |

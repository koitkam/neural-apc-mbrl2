...
- **Liveness:** P1 STAGE 1. Iter **80** (80 jsonl rows, ~172 min). last_ok **80** unlocked (P52 freeze last_ok 78; P52 GAIN-READY @82 — not frozen yet). recon **0.0022** (best **0.0015** @75; iter-79 bump 0.0036 recovered). gmatch **0.0001** FD **×0.99 / ×1.01**. gnorm **0.16** (was 24 @78). Skip-storm **1/2 @73** holding. Banner skip 42 (cumulative). wm-fidelity probe @**80**: H=1 r=+0.781 H=13 r=+0.658 H=27 r=+0.633 **@H r=+0.606** conv=1.00 drift=0.007 gain_fid **0.914** best_h=55/55 (not the TM gate; @H up vs @70 +0.593 / gain_fid 0.909). **New EMA best 6.578** (raw 6.914) @**80** → saved `wm_best.pt` (was 6.243 @70). Jsonl still 6.243 until mix-in write. Full injects (const/step/dv-prbs/expert). Next probe ~90. Wrap/gate watch ~82. Storm 2/2 would cap. sps ~48. GPU ~11 GB. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Do not KEEP/FALSIFIED as cascade until EXIT.
- **Hypothesis:** in-support `(u−μ)/σ²` SGD cannot walk μ onto the tanh rail once `π_new(a)/π_snap(a)` leaves `[0.8, 1.2]`. Unfreeze 147 unclipped logp_std stays ~0.5 and mvv stays warmup-class (~16k), not 813k.
- **Watch:** Gate GAIN-READY vs P52 last-ok 78. First P3 ent ≈ H(σ_init). Unfreeze 147: ent HELD, `actor_ratio_clip_frac`, actor_loss / mvv. Do not call KEEP/FALSIFIED as cascade until EXIT.
...
| P3 in-support μ-walk (\|logp\|<8 SGD) | P52 EXIT; **P53 LIVE** (`run_p53_muratio`) | LIVE (`p3_mu_ratio_clip=0.2` vs frozen unfreeze-μ) | YES — first AC call snapshots policy (warmup does not step actor). Opt out `DREAMER_P3_MU_RATIO_CLIP=0`. Not `actor_kl_coef`. Not lag-copy. |

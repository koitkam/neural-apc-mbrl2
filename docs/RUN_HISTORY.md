...
- **Liveness:** still **LIVE** pid **75966**, tmux `mbrl2_p53`, GPU ~11 GB. ~210 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. jsonl through **P3 iter 194**. last_ok **82 unlocked**. Do not KEEP/FALSIFIED as cascade until EXIT.
  ...
  - **Unfreeze 147 (A/B canary — no cascade verdict yet):** ent **−0.163→−0.101 HELD** through **194** (stop-grad yank-block holding). rscale **2.302 FREEZE** KEEP-class. actor_loss **−0.013** @147 (not P51 **−4.66** / P52 delayed **−0.77**); |aloss|≲0.20 through 194. Unclipped logp_std **0.52@146 → 1.58@147 → 6.71@149** (P52 first-unfreeze class **6.11**, not P51 **38**) then **held ~0.50–0.56 through 194**. `actor_ratio_clip_frac` **0.61@147 / 0.97@149** then **~0.20@194**; `actor_ratio_mean` recovered **~1.00**. PPO clip **fired then eased**. mvv **5831@171 / 50@186 / 56776@191** (collect spike; still ≪ **813k**, above P51 best **19k@161**). bc **0.003@194**. ret_w **−54@178 → −85@194**. ema **−110@178 → −139@194** (not P52 **−2231@209**).
  - **Passed P52 delayed-rail window @165–169:** P52 unclipped std **80.8@165 / 100.7@169**. P53 same iters **0.61 / 0.55**; still **~0.52@194**. Not a cascade verdict.
  - **Watch (not a verdict):** rtgt **0.0059@178 → 0.0016@194** (walking toward P51 **1e-4**; below critic healthy **0.015**). mvv collect spike **56.8k@191** (was <6k). logp_std **still held ~0.52** (μ-walk limiter still blocking P52-class std rail). Next ticks: rtgt to **1e-4** vs stabilize; mvv stay ~10k–60k vs jump 100k+; ema/ret_w vs P50 **−56/−104**.
- **Hypothesis:** in-support `(u−μ)/σ²` SGD cannot walk μ onto the tanh rail once `π_new(a)/π_snap(a)` leaves `[0.8, 1.2]`. **So far:** unclipped std held through P52's walk window; actor_loss bounded; mvv now spiking (56.8k) while logp_std stays ~0.5 — possible collect/econ walk without μ-rail. Do not call KEEP/FALSIFIED as cascade until EXIT.
- **Watch:** late-P3 rtgt / mvv / ema. Do not call KEEP/FALSIFIED as cascade until EXIT.
...
| P3 in-support μ-walk (\|logp\|<8 SGD) | P52 EXIT; **P53 LIVE** (`run_p53_muratio`) | LIVE (`p3_mu_ratio_clip=0.2` vs frozen unfreeze-μ) | YES — first AC call snapshots policy. Unfreeze 147: ent HELD −0.101; logp_std **held ~0.52 through P52 walk window and @194**; mvv **56.8k@191** (≪813k, >P51 19k); actor_loss bounded. rtgt **0.006→0.0016 watch**. No cascade verdict until EXIT. Opt out `DREAMER_P3_MU_RATIO_CLIP=0`. Not `actor_kl_coef`. Not lag-copy. |

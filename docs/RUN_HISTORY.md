...
- Watch freeze vs P64 **0.91@DV** / P73 **0.83**; val **1step→OL / OL-vs-real** vs P73 ×0.761 / ×0.743 and P64 ×0.85 / ×0.84; jsonl last-step `*_ratio` still DC (not rise-mean); `actor_experiment_valid`; det_r must not collapse like P74 **0.074**. jsonl ×1 ≠ val TM. **Do not** start a second GPU job.

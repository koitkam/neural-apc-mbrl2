...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8`) | **LIVE P1** iter **30** (~68 min). pid **64705**, launch `d910ee2`. Step 4 **CONFIRMED** `p3_sglogstd=True p3_logpclip=8`. recon **0.101→0.0040** (best **0.0030@28**) gmatch **0.172→1e-4** skip **0** last_ok **30 unlocked**. Iter 30 wm-fid H=55 r=0.490 gain_fid=**0.944** (was 0.709@20) `wm_best` EMA 5.798. GPU ~11 GB. Env-free. Watch gate 82 then unfreeze ~147 vs P51 μ-rail. | 🔬 μ-side clamp after P51 cascade FALSIFIED. Do not launch. Do not stack critic knobs. |
...
- **P1 iter 30 (~68 min):** recon **0.101→0.0040** (best **0.0030@28**) gmatch **0.172→1e-4** skip **0**. last_ok **30 unlocked**. Iter 30 wm-fidelity H=55 r=**0.490** `gain_fid=0.944` (iter 20 was 0.709) saved `wm_best.pt` EMA **5.798**. FD ratio mv **1.010** / dv **1.003** (teacher, not TM). ~2.2 min/iter after iter 1. No skip-storm/lock/wrap. Gate 82 ~1.9 h away; unfreeze ~147 is the A/B.
...
| P3 REINFORCE unclipped logp (μ-rail with frozen σ) | P51 EXIT; **P52** (`run_p52_logpclip`) | SUPERSEDED (`p3_logp_clip=8` default) | YES — advantage_clip 8 + grad_clip 10 already saturate (147 gnorm 9.65) but each step still shoves μ via (u−μ)/σ² (logp mean −26 @147 / −227 @315). Clamp logp in the surrogate; jsonl diag stays unclipped. Opt out `DREAMER_P3_LOGP_CLIP=0`. |

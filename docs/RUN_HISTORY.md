...
| P60 | 2026-08-31 | env-free `gain_match_step<=0` auto = `wm_tf_step_frac` (0.4) | LAUNCH. One attributed change vs P59: teacher Δu 1.0→**0.4**. Knobs else = P59/P58. tmux `mbrl2_p60`, out `run_p60_tmstep`. | LAUNCH |
...
### p60 (`run_p60_tmstep`, branch `cursor/p28`) — LAUNCH: teacher Δu auto = val TM `wm_tf_step_frac`
- **One attributed GPU change vs P59:** `_resolve_gain_match_step`: dataclass sentinel **0.0** → `wm_tf_step_frac` (default 0.4). Explicit `DREAMER_GAIN_MATCH_STEP=1.0` keeps the P59 teacher. Isolation still off. Rest-IC lookback. μ-ratio 0.2 freeze-forever. Serve graph KEEP. Eager. Det latent. DOB on.
- **Not:** clip+realized-Δu (p136-class follow-up if 0.4 still leaves a TM gap; rest 0.6+0.4 sits on the cube edge). Overshoot `sample=True→False` (already identity: `latent_noise=0` + `cont_gain_deterministic_roll`). Critic knobs. μ-refresh. S=H.
- **Step 4:** resolved-cfg vs P59 — **only** `gain_match_step` 1.0→**0.4** (auto). Watch `[resolved-cfg] gmatch_step=0.4` and `[gain-match] … step=0.4`. `device=cuda`. No leftover `DREAMER_*`.

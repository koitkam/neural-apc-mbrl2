...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **2nd extra-P1 iter 96** after @94 **FAIL −0.29@MV / 0.80@DV**. Orig-P1 @82 **FAIL −1.06@MV**. 2nd EXTEND to **904752** (cap 929884). Probe90 **H=1 r=+0.592 / H=55 r=+0.147 conv=0**. recon **0.0021@96 new best**. persist **0.028**. Teacher **×0.99 / ×1.01 @96**. pid **242511**. | 🔬 LIVE extra-P1 — vs P64 **0.91@DV**. Do not score actor. |
...
- **LIVE 2nd extra-P1 iter 100 (~522 min, nvidia 11.7 GB, `t_wm` ~309 s, sps 19.5):** skip **0**. recon **0.0021@96 best → 0.0028@99 → 0.0035@100**. Teacher jsonl 99 **×0.992 / ×1.054** → 100 **×0.991 / ×1.097** (still Markovian ×1; DV drifted). last_ok **100 unlocked**. persist_rel **0.022@99 → 0.024@100** still collapsed (do **not** persist N+1). No detonated-freeze. Rest-IC graph not captured. Probe100 **no** new `wm_best` (still iter 90). **2nd EXTEND** to **904752** (cap 929884; next gate ~106, then cap ~110).

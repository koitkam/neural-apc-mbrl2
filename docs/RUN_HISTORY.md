...
- **LIVE (2026-09-01 ~03:55 CDT, P3 warmup+unfreeze):** pid **169231** ALIVE, tmux `mbrl2_p64`, sha `9544451`. GPU **~2.8 GB** (rest-IC graph released; P3 collect ~1.86 s). **No second GPU job.**
  ...
  - **P3:** warmup 137–146 rscale **1.15→2.02 KEEP** rtgt **0.06**. Unfreeze **147** first P3 ent **−0.134** then **−0.101 HELD**. logp_std spiked **6.58@157** then **held ~0.48@183** clip **0.27** (P53-class spike-then-hold, not a walk). rtgt **0.008** (usual post-unfreeze collapse; do not stack critic knobs). `t_collect` **1.86** / `t_ac` **1.79**. Watch val pred_std vs **1.93** (P62 0.394), det_r vs P62 **0.364** / P26 **0.68**, TM vs P26/P62, paired vs P58 **−6.24**. Dummy jsonl `wm_held_ol_ratio` **REMOVED** (HEAD, not this pid). `DREAMER_ACT_HIST_REQUIRED` **REMOVED** (HEAD).
...
| P1/P2 hidden-OU amp cap 0.2 vs P3/val 1.0 | P45–P62 val pred_std/true ~0.2 (P62 0.394/1.93); **P64 P2** `dob_d` 0.063 vs P62 0.026 (~2.4×); val pending | P2 uses P3 cap (no new knob; P1 stays 0.2) | PARTIAL — Kalman SNR-calibrated at 0.2×. P2 amp ON (g frozen). P1 full-scale would be p112 omitted-variable. |

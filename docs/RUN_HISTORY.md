...
- **LIVE (2026-08-27 02:07, tmux `mbrl2_p32`, GPU ~19.5 GB reserved, pid 4139864):** env-free det+eager CONFIRMED (`[resolved-cfg] latent=deterministic compile=eager storm_cap=2`; no leftover `[env-override]`; `[dob-ground] replay n_dist=1`). Process launched at `a138e1f` (isolation TBPTT + P1 prior-core skip are HEAD, **not** in this GPU job).
  - Healthy stretch to iter 51: recon **0.003–0.005**, `kl=0`, `jemb>0`, skip **0**, alive 1024. `wm_best` iter **50** EMA 5.799, `best_h=55/55`, gain_fid=0.898. Wrap/gnorm blips (14/28/32/48) recovered without skip-storm — do **not** lower skip threshold.
  - **Skip-storm 1/2 @ iter 53 (first GPU test of continue-after-storm):** iter 52 recon **0.486** gmatch 1.05 iso 0.99 gnorm **17.7** applied (below `wm_grad_skip_norm=1e4`, skip 0 — same class as P31 iter 95). Iter 53 recon **0.596** gmatch 1.18 iso 2.37 **gnorm 6.28e15**, skip **71** → `[skip-storm] P1 recovered: restored wm_last_ok.pt (wm_last_ok iter 51); reset opt_world; continuing P1 (storm 1/2, extension closed at 753960 steps)`. Iter 54–56 recon **0.0061 / 0.0059 / 0.0029** (last-ok class). Restore **worked**. jsonl `n_grad_skip=71` is **lifetime** (window reset on restore). Next storm is 2/2 → `_force_p1_cap_at`.
  - Still P1 @ iter **56** env **622200 / 753960** (~21 iters / ~50 min of original P1 left, then P2). Continue-after-storm KEEP (P30 capped on first; P31 never stormed). Attributed GPU hypothesis (P1→P2 detonated-freeze last-ok) has **not** fired yet. **No second GPU job.**
  - HEAD (not in process): isolation TBPTT via chunked `img_rollout`; Stage-1 skips unused DOB prior-core; DRY `_lambda_returns`.

### Sim-adaptive leftovers (env-free multi-sim; do not promote plants yet)

Already unitless / derived: isolation TBPTT `max(8, round(K/3.5))` (chunked `img_rollout`, same `h`-only `keep_c` cuts; HEAD not P32 process); `gain_match_len` / isolation len = H; isolation sample window `max(seq_len, K+1)` (test_sim seq_len ≥ H unchanged); P1/P2 main WM sample `max(seq_len, K+1)` (overshoot/gain-match; P3 stays seq_len); gain-match open-loop `K` not truncated to `T-1`; `skip_storm_last_ok_recon_ratio=5` (recon/recon); `skip_storm_p1_cap_after=2` (storm count, not plant units; **P32 GPU-confirmed** continue 1/2 @iter 53, recon 0.596→0.003); `p1_gate_wm_ema_min=1.5` (fidelity mix); inject EVERY = f(buffer lap) (test_sim 20/10); inject N = f(n_mv, n_dv) (test_sim 5/2/2/3); isolation settle = 24 **per input** (test_sim 24+24 / cap 48 settle-only; distillation 96+24 / cap 120). Isolation_buf holds only those settle episodes (not MIMO PRBS). Isolation settle is a whole-episode hold at a stratified level (`action_std=0`); DV step is MV-action-isomorphic (`isolated_level × span/2`). Opt-in: `gain_match_huber_beta<=0` → median |tgt| (P28 still uses β=1). Gain-ready band `[0.80, 1.30]` and wm-fidelity mix weights are unitless TrainConfig (were os.environ). `rssm_latent_type` default **deterministic** (P26 proven; P29 env-free drop of the leftover env-var). `compile_mode` default **eager**; `DREAMER_COMPILE` / `DREAMER_COMPILE_MODE` are in `ENV_OVERRIDES` (MODE was CLI-only). Intra-op threads and inductor workers scale with `sched_getaffinity` (host-adaptive). `skip_invalid_p3` default ON (unitless validity flag, not a plant unit). Stage-1 skips unused DOB prior-core when `dob_active=False` (HEAD, not P32 process).
...
- Isolation/ss-match trajectory MSE is still absolute (same family as abs Huber). Variance-normalized isolation is the next observer A/B **if** a **deterministic eager** skip-restore run still shows DV ss ~×0.87 after a **healthy** (not detonated) P1 freeze — do not revive relative Huber. P31 iter-75 DC `[0.63, 1.48]` was mid-extension; cap-time `[0.08, 1.31]` 0.08@DV is the **exploded** freeze, not a DV-underweight verdict.
- `wm_overshoot_max_starts=24` / `gain_match_max_starts=6` are **per-sequence** caps. With gpu-calib `B=128`, total `Bm=B·S` is ~8× paper `B=16`. Do **not** retune during P32 (would change the observer loss). Host-adaptive thread caps already scale with `sched_getaffinity`.
...
- P1/P2 random collect is **numpy-only** (no `torch.inference_mode` / SF `a_history`). P3 on-policy still streams. P2 DOB Kalman scan is a closed-form mix (same recurrence; host-adaptive T×T cap 16 MiB). **P32 GPU job includes these.**
...
- Validation `wm_gain_pass` is still **MV-only**. HEAD logs `wm_dv_gain_*` + `wm_dv_ss_ratio_worst` (P29 HEALTHY MV rel_err 0.10 hid DV ×0.56; P31 HEALTHY MV rel_err 0.35 hid DV ×0.11). Does not change `wm_gain_pass`.
- P32 launch: env-free `cursor/p28` (deterministic + eager + `storm_cap=2` + detonated-freeze last-ok restore). Do not pass leftover P26 env-vars (`DREAMER_COMPILE=0`, `DREAMER_RSSM_LATENT_TYPE`, `DREAMER_DOB_ENABLED`, `DREAMER_ACT_HIST_REQUIRED`). Opt-in compile is `DREAMER_COMPILE=1`. Live iter banner prints `kl`/`jemb`/`iso`/`ss` + `storm_cap=`. jsonl: `wm_ss_match_loss`, `wm_ss_match_wmean`, `wm_isolation_traj_loss`. `skip_invalid_p3` default-on. Isolation K-step decode is batched; overshoot MSE is vectorized; **gain-match FD** stacks baseline+per-input into one `img_rollout`; **P1/P2 random collect is numpy-only**; **P2 DOB Kalman scan is a closed-form mix**. Isolation TBPTT is chunked `img_rollout` (HEAD, not P32 process). Stage-1 skips unused DOB prior-core (HEAD, not P32 process). Continue-after-storm is **GPU-confirmed** (P32 storm 1/2); second storm still caps. Variance-normalized isolation waits until a **completed** det+eager P1 still shows DV ss ~×0.87 on a **healthy** freeze.

...
- **Liveness:** tmux `mbrl2_p55` LIVE, pid **103504** LIVE (`b321d89`), `device=cuda`. P1 iter **21** recon **0.0026** skip **0** last_ok **21 unlocked**. Teacher FD MV/DV **×0.994 / ×1.016** (not TM). `wm_best` iter **10** then EMA **5.263** @21. `t_wm` **~127 s** (eager T-loop; CUDA-graph capture failed on first WM autocast). P54 iter-18 recon 0.0057 / `t_wm` ~124 s — same rest-IC class. HEAD after this pid: capture **before** the WM loop + exit parent autocast. **Watch** as above. **Do not launch a second GPU job.**

...
| P51 | 2026-08-29 | P3 REINFORCE stop-grad log_std (`p3_stop_grad_log_std=True`) | **LIVE P1** iter **43** (~97 min). pid **54815**, tmux `mbrl2_p51`, launch `d6a4511`. recon **0.0028** skip 0 last_ok **43 unlocked** teacher FD ~×0.99/×1.00 bc **MSE 0.24–0.36**. No wrap/lock/storm. Env-free. Do not launch. | 🔬 watch wrap ~56 (P50 lock@56); unfreeze: ent stays ≈−0.10 |
...
- **Step 4:** `[resolved-cfg] latent=deterministic restore_p2=False gain_match=1 dob_ground=2 dob_reg=0 isolation=0 ss_match=0 iso_dcv=off n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 lock=20 huber_per_in=True gmatch_settle=-1 gmatch_rest=True p3_sigreset=False bc_mean=True p3_sglogstd=True compile=eager`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. `[run] pinned eval modules at launch`. STAGE 1 @iter0. P1 iter **43** recon **0.0028** skip **0** last_ok **43 unlocked** teacher FD MV/DV ~×0.988/×0.995. No wrap/lock/storm (P50 wrap/lock @56). `t_wm` ~129 s. GPU ~11 GB. bc **MSE 0.24–0.36** (never NLL).
- **Watch:** wrap ~56; GAIN-READY at gate 82; P2 bc stays MSE ≥0; first P3 ent ≈ H(σ_init)≈−0.08 not −0.283; **unfreeze must not yank** (P50 169: −0.107→−0.268, logp_std 0.56→54). Remaining leftover is env-only paths — do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.

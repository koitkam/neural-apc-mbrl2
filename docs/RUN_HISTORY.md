...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **2nd extra-P1 after @94 FAIL −0.29@MV**. Orig-P1 @82 **FAIL −1.06@MV**. Extra-P1 @94 **FAIL −0.29@MV / 0.80@DV** (still inverted MV; DV at band floor ss / @H 0.73 short). 2nd EXTEND +75396 to **904752** (cap 929884). Probe90 **H=1 r=+0.592 / H=55 r=+0.147 conv=0**. recon **0.0025@94 new best**. persist **0.026**. Teacher **×1.01 / ×0.96 @94** ≠ 5-level. pid **242511**. | 🔬 LIVE extra-P1 — vs P64 **0.91@DV**; P76 extra-P1@94 was live **0.84@DV** (discarded). Do not score actor. |
...
- **LIVE 2nd extra-P1 after iter 94 (~491 min, nvidia 11.7 GB, `t_wm` ~307 s, sps 19.7):** skip **0**. recon **0.0027@90 → 0.0027@93 → 0.0025@94 new best**. Teacher jsonl 92 **×0.997 / ×0.969** → 93 **×1.003 / ×0.996** → 94 **×1.010 / ×0.956**. last_ok **94 unlocked**. persist_rel **0.029@91 → 0.032@93 → 0.026@94** still collapsed after dv-prbs @90 (do **not** persist N+1). Rest-IC graph not captured. Orig extra-P1 target 829356 hit @94 (829600). **2nd EXTEND** +75396 to **904752** (cap 929884; ~12 more iters → gate ~106, then cap ~110). Next probe typically iter 100.
...
- **RCA (SysID, not a live patch; CPU-confirmed @wm_best iter70, CVD=""):** rest-IC `rollout_observed` **does** keep KV (`pos=128`, 4 layers). Gain-match / overshoot / held start `img_rollout(h,z)` with `kv_cache=None`. Markovian vs `prev_state` held-K CV **mean |Δ|=0.256** (iter50 **0.123**; iter40 **0.142** — **diverging** as recon improves). step1 **0.051** / stepK **0.094**. Markovian ΔCV(K) **0.263** vs cached **0.220**. Fidelity probe / val TM **warm cache** then `img_step` (so probe H=55 r dead is **cached compounding**, not the Markovian hole). Teacher jsonl ×1 pins the **Markovian** DC. CPU posterior-prior @H=55 settle=128: real→post **×0.865** (iter50 **×0.836**) / post→1step **×0.702** (iter50 **×0.607**) / 1step→OL **×0.272** (iter50 **×0.097**; OL-vs-real **×0.165** vs **×0.064**). Compounding improved 2.8× but still **~3× short of P64 ×0.85**. Verdict still `autoencoder` (0.865 < 0.9) with compounding the bigger multiplicative hole. Recon **0.0027@90** beats P64 0.0035 — not an EXIT. Probe90 **conv still 0 @10–90**; H=55 collapsed again 0.423→0.147 / best_h=1 (@80 recovery not sticky); H=1 flat 0.583→0.592. persist after @90 dv-prbs inject **0.029@91** (collapsed, not spike). Next-if EXIT FALSIFIED: one attributed A/B pass `prev_state` into gain-match / overshoot / held. Metric: probe H=55 r / conv / val 1step→OL vs P64 ×0.85. Watch freeze vs P64 **0.91@DV** (orig-P1 @82 FAIL **−1.06@MV** noisy sign-flip; extra-P1 running); persist delayed spike **0.212@85** decayed (do not persist N+1); `actor_experiment_valid`. Do **not** start a second GPU job.

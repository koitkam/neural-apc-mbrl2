...
| P50 | 2026-08-29 | P1/P2 BC MSE-on-μ (`bc_mean_only=True`) | **LIVE** P1 iter **11**. Env-free tmux `mbrl2_p50` pid **49389**. `[resolved-cfg] bc_mean=True`. recon **0.077→0.0042** skip **0** last_ok **11** unlocked. **bc +0.27…+0.35 MSE** (P49 P2 NLL was negative). No leftover `DREAMER_*`. Do not launch. | 🔬 live; watch P2 `bc_loss`>0; first P3 ent ≈ **−0.08** not −0.283 |
...
- **LIVE (2026-08-29 17:00 CDT, P1 iter 11):** recon **0.0772→0.0042** (iters 1→11) kl 0 jemb 0.024 gmatch **0.0001–0.0011** iso 0 skip **0** encvar 0.05 alive 1024. **`bc_loss` +0.27…+0.35 every iter (MSE)** vs P49 P2 NLL **−0.87…−1.13**. Teacher FD ratios ~×1.0. Iter 5 gnorm **20.9** blip then 0.8 (no skip-storm; last_ok advanced 1→**11** unlocked). Fidelity probe iter 10 `best_h=13/55` gain_fid=0.787 → `wm_best`. GPU ~11 GB. Auto-tune `policy_init_log_std=-1.5` / `log_std_min=-1.702` (σ_min_ratio **1.2**). First P3 ent target ≈ H(σ_init) ≈ **−0.08**, not floor **−0.283**.
- **Watch:** P1 recon→~0.003 / skip-storm ~iter 50–65; P2 `bc_loss` stays **>0**; first P3 entropy ≈ **−0.08** not −0.283; freeze rscale ~2; do not train P3 on GAIN_NOT_READY. Remaining leftover is env-only paths/device (`AGENT_*_JSON`, `SIM_NOISE_CONFIG_JSON`, `DREAMER_WM_DIAG_DEVICE`) — do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.

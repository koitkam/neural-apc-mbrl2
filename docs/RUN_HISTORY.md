...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P1** iter **77** (~174 min). pid **64705**, launch `d910ee2`. Step 4 **CONFIRMED** `p3_sglogstd=True p3_logpclip=8`. recon **0.0029** (best **0.0017@71**) last_ok **77 unlocked** skip **0**. lock@38 wrap-unlock@39. Iter 70 wm-fid H=55 r=**0.562** gain_fid=**0.903** `wm_best` EMA **5.863** (beat 5.798@30). GPU ~11 GB. Env-free. Watch gate 82 (~11 min) then wm-fid 80; unfreeze ~147 vs P51 μ-rail. | 🔬 wrap-unlock KEEP. Do not launch. Do not stack critic knobs. |
...
- **P1 iter 77 (~174 min):** recon **0.0029** (best **0.0017@71**; iter 76 near-best 0.00171) last_ok **77 unlocked** skip **0**. lock@38 wrap-unlock@39 last_ok advanced (P50-class KEEP). Iter 70 wm-fid H=55 r=**0.562** `gain_fid=0.903` best_h=55/55 saved `wm_best.pt` EMA **5.863** (beat 5.798@30; H=55 r recovered from 0.216@60). Teacher FD mv **0.999** / dv **0.987**. Gate 82 ~11 min (5 iters × ~132 s); next wm-fid @80 then gate; unfreeze ~147 is the A/B.

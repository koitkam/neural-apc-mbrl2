...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P1** iter **39** (~89 min). pid **64705**, launch `d910ee2`. Step 4 **CONFIRMED** `p3_sglogstd=True p3_logpclip=8`. recon best **0.0027@37**. **lock@38** last_ok **37** (recon **0.412** skip **0**, 151×) **wrap-unlock@39** recon **0.028**. skip **0**. Iter 30 wm-fid H=55 r=0.490 gain_fid=0.944. GPU ~11 GB. Env-free. Watch last_ok 37 vs wrap-advance; gate 82; unfreeze ~147 vs P51 μ-rail. | 🔬 μ-side clamp after P51 cascade FALSIFIED. Do not launch. Do not stack critic knobs. |
...
- **P1 iter 39 (~89 min):** recon best **0.0027@37**. **lock@38** last_ok **37** (recon **0.412** = 151×, skip **0**, gnorm 0.73 — silent spike, not skip-storm). **Wrap-unlock@39** recon **0.028** < 20× (P50-class original-P1 wrap recovery). last_ok stays **37** (0.028 still >5×). Iter 30 wm-fid H=55 r=**0.490** `gain_fid=0.944` EMA **5.798**. Teacher FD still ~×1. No skip-storm. Gate 82 ~1.6 h away; unfreeze ~147 is the A/B. Watch whether last_ok advances after recon <5× vs extra-P1 stay-locked.
...
- **HEAD (not in pid 64705):** `p3_logp_clip` is nats **per action dim**; summed logp clamp = `8×n_mv` (`_p3_logp_clip_bound`; 1-MV test_sim identity). SNR window and `identify_dynamics` / noise-config no longer invent τ=50 s / θ=5 s when SysID keys are missing (lookback/4 frames; OU θ floor 0.02). test_sim identity (τ=53).

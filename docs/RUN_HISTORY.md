...
| **Disturbance (Kalman, dynamic)** | `r_detrended` (→1) | **P26** (mbrl2) | **det_r 0.68** | P64 det_r **0.352** (P62 0.364). Amp closer (pred_std **0.608 vs 1.93**; P62 0.394) but not P26. |
| **Critic** | `critic_rew_to_tgt_var` (>0.015) | none healthy | — | P66 val `critic_r` **+0.698** PASS (P65 +0.719; P64 +0.705); P3 rtgt still collapses. Freeze rscale KEEP. |
| **Actor** | econ vs baseline + vs expert BC | **P64** (mbrl2) | paired **−4.54 vs −94** | Env-free P2 amp=P3. 9/9 `all_pass`. P66 **−19.85 vs −104** VALID but loses to P64; cv/mv viol 4.91/5.70 vs P64 1.01/0.33. P65 **−11.95 vs −139**. P53 stays the μ-ratio-clip RCA. |
...
| P67 | 2026-09-01 | teacher `gain_match_len` auto = `wm_tf_horizon` (K=220; encode T lookback) | **CAPPED GAIN_NOT_READY 0.80@DV** @104. LIVE P2 (g frozen). Gates 82/94/104 **0.80/0.79/0.80@DV** vs P64 PASS 0.91. Extra-P1 MV **1.25/@H 1.39**. `skip_invalid_p3`. Actor INVALID. | ❌ **REVERT** auto K to H (HEAD). FALSIFIED as TM-DC pin |
...
### p67 (`run_p67_gmatch4h`, branch `cursor/p28`) — CAPPED GAIN_NOT_READY: teacher K=4H REVERT
...
- **Storm 1/2 @67 recovered** (same class as P64 @78). Iter 66 silent recon **0.5736** skip 0 locked last_ok **65**; iter 67 skip **16/16** gnorm **1.46e4**. Restore unlocked; last_ok advanced (freeze **104 unlocked**). K=220 did **not** prevent the wrap and did **not** block restore. `t_wm` **134 s** vs P64 **97 s**. P1 nvidia **~14.3 GB** (`gpu_mem_reserved_mb` **13804**; jsonl `gpu_mem_mb` allocated **~325**). Isolation=0.
- **Gate 82 FAIL GAIN_NOT_READY** `DCgain_ratio[0.80,0.98] @H[0.97,1.06]` worst **0.80@DV** (MV **0.98/@H 1.06**, DV **0.80/@H 0.97**). Extended +75396 → 829356. jsonl teacher **×0.999 / ×1.000**. **P64 same iter PASS 0.91@DV** (MV 0.96/@H 0.97, DV 0.91/@H 0.97).
- **Gate 94 FAIL** `DCgain_ratio[0.79,1.11] @H[0.99,1.22]` worst **0.79@DV** (MV **1.11/@H 1.22**, DV **0.79/@H 0.99**). Extra P1 **moved DV the wrong way** and **overshot MV**. jsonl **×1.006 / ×1.021**. Extended +75396 → 904752 (cap **929884**).
- **Gate 104 CAPPED GAIN_NOT_READY** `DCgain_ratio[0.80,1.25] @H[1.01,1.39]` worst **0.80@DV** (MV **1.25/@H 1.39**, DV **0.80/@H 1.01**). jsonl **×1.013 / ×1.022** at K=220. Freeze recon **0.0012**. last_ok **104 unlocked**. Detonated-freeze **not** fired. wm_best restore SKIPPED (skip-storm already restored last_ok 65; wm_best is gain-blind). Phase 1→2 @ env_steps=905240. Rest-IC CUDA graph released (g frozen).
- **LIVE P2** (g frozen; nvidia **~4 GB** after graph release; `gpu_mem_reserved_mb` **3492**). recon ~0.002. `dob_d` **0.030→0.055**, `dob_ground` **0.002→0.28** (`keep_frac=1`; P64-class Kalman; **not** a DOB A/B). `skip_invalid_p3=True` → **actor INVALID**. Do not score P3. Full TM/decomp/det_r/pred_std val deferred to EXIT.
- **RCA (SysID):** last-only Huber at K=220 **does** pin jsonl `G_pred/G_tgt` ×1 (protocol). It does **not** pin live TM DC (DV ss 0.80 vs P64 0.91). **@H DV is healthy** (0.97–1.01) — the miss is DC/ss, not H=55. jsonl ×1 ≠ TM ss (identifier-tgt FD vs plant rest-step). Extra-P1 FALSIFIED as DV lever (0.80→0.79→0.80) and **overshot MV** (0.98→1.11→1.25). Do **not** teacher-K N+1. Do **not** score actor (`skip_invalid_p3`).
- **Step 5 (HEAD):** teacher K auto=4H **REVERT** to control H (`_auto_gain_match_len` sentinel → `horizon`; test_sim **55**). **FALSIFIED as TM-DC pin.** PARTIAL as protocol (jsonl ×1 at K=220). Champion **unchanged: P64 econ / P53 μ-ratio / P26 MV ss / P64 DV ss**. Explicit `DREAMER_GAIN_MATCH_LEN=220` still A/B's 4H. Do not retry S=H. Do not raise P1 amp. Do not flush/skip/`/dvar`. Do not stack critic knobs. Next GPU after EXIT: **new family** (identifier-tgt vs plant TM), not teacher-K N+1, not a P64 replica. **No second GPU job** while pid 187753 holds the A10.
...
| Gain-match FD K = `wm_tf_horizon` (last-only Huber at val TM window) | **P67 CAPPED** (`run_p67_gmatch4h`, pid **187753**, sha `9fbede0`) | **REVERT / FALSIFIED as TM-DC pin**. PARTIAL as protocol (jsonl ×1 at K=220). Gates 82/94/104 **0.80/0.79/0.80@DV** vs P64 PASS **0.91@DV**. Extra-P1 MV **1.25/@H 1.39**. @H DV healthy. jsonl ×1 ≠ TM ss. Storm 1/2 @67 recovered. Actor INVALID (`skip_invalid_p3`). Not S=H. Not a Huber reweight. Do not teacher-K N+1. HEAD auto = control H. |

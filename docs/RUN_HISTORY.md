...
| **Actor** | econ vs baseline + vs expert BC | none valid | — | P45 GAIN-READY P3 **FAIL** econ **−216 vs baseline −92** (limit-ride; σ_min from P1/P2 BC). Do not score GAIN_NOT_READY freezes. |
...
| P50 | 2026-08-29 | P1/P2 BC MSE-on-μ (`bc_mean_only=True`) | **LIVE P3** ~iter 269, pid **49389**, GPU ~11 GB. Freeze last_ok **88** GAIN-READY **0.90@DV**. First P3 ent **−0.107** ≈ H(σ_init) (not −0.283). P2 bc **MSE 0.000–0.015 never NLL**. Unfreeze 169 yank −0.107→−0.268, bc 0.003→0.93, logp_std 0.56→54. rscale **2.33 KEEP**; rtgt 0.05→0.0001. Do not launch. | 🔬 μ-only KEEP as σ-open; **not sufficient** to stop unfreeze cascade (same class as P46/P47). Next after EXIT: P3 stop-grad log_std |
...
### p50 (`run_p50_bcmean`, branch `cursor/p28`) — LIVE P3: P1/P2 BC MSE-on-μ
- **One attributed GPU change vs P49 pid:** `bc_mean_only=True` (HEAD default `c900766`). P1/P2 expert-BC is MSE-on-μ so cloning does not pin last-Linear log_std to σ_min. P3 `expert_bc_p3_loss` was already MSE-on-μ. Opt out `DREAMER_BC_MEAN_ONLY=0`. Launch **`9cbf771`** (pin also imports `training_disturbance`; empty paired records fail `beats_baseline`). Env-free. tmux `mbrl2_p50`, pid **49389**, `CUDA_VISIBLE_DEVICES=0`. Started 16:34:23.
- **Not:** leftover `DREAMER_*`, `p3_reset_log_std`, extra critic knobs, rest-IC settle=`wm_tf_horizon`, isolation reweight. Do **not** launch a second GPU job.
- **Step 4:** `[resolved-cfg] latent=deterministic restore_p2=False gain_match=1 dob_ground=2 dob_reg=0 isolation=0 ss_match=0 iso_dcv=off n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 lock=20 huber_per_in=True gmatch_settle=-1 gmatch_rest=True p3_sigreset=False bc_mean=True compile=eager`. Train-start `device=cuda bs=128 T=128 H=55 lookback=128`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF` (no leftover `DREAMER_*` / `OBJECTIVE_*`). `[run] pinned eval modules at launch`.
- **Observer CLOSED (this pid):** wrap-unlock ×2 GPU-confirmed (56→58, 70→73). **bc MSE never NLL** (P1/P2 always ≥0; P2 **0.000–0.015**). skip **1** (one huge-grad, not a storm). last_ok **88 locked** (extra-P1 recovered basin stays locked).
  - **Gate 82 FAIL** `worst=1.37@MV` (MV ×1.37/@H ×1.33; DV ×0.82/@H ×0.86). Probe recon **0.0283** = 19× best (wrap-adjacent). EXTEND.
  - Extra-P1 **detonated iter 89** recon **0.6286** (430×).
  - **Gate 94 FAIL** `worst=0.69@DV` noisy on detonated g (spread×6.1, 3 signflips).
  - **Gate 104 PASS live 0.85@DV** (MV ×1.06/@H ×0.98; DV ×0.85/@H ×0.86). Detonated-freeze restored `wm_last_ok` iter **88** → freeze probe **GAIN-READY 0.90@DV** (MV ×0.99/@H ×0.94; DV ×0.90/@H ×0.93). Warm-restore SKIPPED. STAGE 2 @104.
  - **P2→P3 PASS @158** reward_mtp median **0.926**. Freeze last_ok **88**. Observer GAIN-READY → actor experiment **valid** (do not score val econ until EXIT).
- **LIVE (2026-08-29 ~21:03 CDT, P3 ~iter 269):** GPU ~11 GB. pid **49389**. Do **not** launch. Do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py`.
  - **μ-only WORKED at P3 entry (the canary):** first P3 ent **−0.107** ≈ H(σ_init)≈−0.08, **not** P45–P49 floor −0.283. P2 `bc_loss` MSE never NLL (P49 P2 was −0.87…−1.13).
  - Critic warmup 159–168: ent stuck **−0.10**, rtgt **HEALTHY 0.04–0.05**, actor_loss ~0, rscale 1.22→**2.33 FREEZE KEEP**, `actor_logp_std` ~0.56.
  - **μ-only NOT sufficient to stop unfreeze cascade** (same class as P46/P47 after opening σ): unfreeze **169** ent **−0.107→−0.268**, actor_loss **−6.4**, bc 0.003→**0.93**, `actor_logp_std` **0.56→53.6**. Recovered ~−0.11; rtgt **0.05→0.0001**; ema **−189→−2448**; imag_ret **−3→−197**. Brief pin **−0.283 @212–217** (iter 215 actor_loss **+603**, logp_std **154**) then back to **−0.10**. Last-30 ent mean **−0.14** (not pinned).
  - **ES watch:** floor-aware thr ≈ **−0.083** so healthy H(σ_init) already sits below thr; last-30 are **30/30 below**. Degeneracy gate (median \|adv_corr\| **~0.07** vs 0.05) is what keeps the run alive. P47 ES was @253; this pid is past that and still LIVE. Watch `entropy_collapse_window` if adv_corr median dips under 0.05.
- **Code follow-up (HEAD, not P50 pid):** leftover expert move-law + CLI extras → TrainConfig + `ENV_OVERRIDES` (`2232112`). **Gain-probe last-ok when live recon >5× best** (`f2c9092`; wrap-adjacent MV ×1.37 RCA); restores live after probe; identity when recon healthy. Next GPU after EXIT (only if GPU free): **P3 stop-grad log_std** on REINFORCE (`log_prob_of` / entropy still train σ — that is the 169 yank). NOT critic knobs, NOT `p3_reset_log_std`. Do **not** pass leftover `DREAMER_*`.
...
- `[resolved-cfg]` prints `iso_dcv=off` when isolation/ss-match are off (P40); when the teacher is on it still prints `iso_dcv=` **and** `min_scale=` **and** `mv=`/`dv=` multipliers **and** `edge_du_mv=`/`edge_du_dv=` **and** `g_ratio=`/`smax=`/`equalize=`. Also prints `lock=` (`skip_storm_last_ok_lock_ratio`; P42), `huber_per_in=` (P43 per-input Huber β), `gmatch_settle=` (P44 held settle; dataclass **`-1` off** after storm 2/2 REVERT; `0` auto-resolves to H), `gmatch_rest=` (P45 EXIT default **True**), `p3_sigreset=` (P46 opt-in; default False), and `bc_mean=` (P50 default **True** MSE-on-μ). `run_plan.isolation_dcv_scales` is the P38/P39 audit trail (inv-var knob removed). Pre-rewrite dump is dataclass defaults (P29 plan still shows `rssm_latent_type=categorical` / `gain_match_coef=0` even though training auto-enabled grounding).
- P3 collect/val streams `stream_serve_step` (measured DV + Kalman when `dob_active`) so served `feat` matches `_realsim_actor_critic_step` re-encode. **P48 EXIT** KEEP as identity; **FALSIFIED as cascade lever**. P47 EXIT Adam-complete **FALSIFIED as yank lever**.
...
| Leftover `DREAMER_HORIZON_SETTLE_NTAU` / `DREAMER_HORIZON_MAX` / `DREAMER_INIT_RANDOMIZATION` / `_FRAC` / `DREAMER_WM_OVERHEAD` | P47 GPU-occupied (pid 29434 P2) | SUPERSEDED (TrainConfig + `ENV_OVERRIDES`; identity 4.0 / 120 / ON / 0.6 / 1.30) | YES — sim-adaptive H formula + IC DR + GPU-calib overhead missing from `run_plan`. Dual-read at `derive_horizon` / sim `reset()` / WM probe (no cfg yet). |
...
| P1/P2 expert-BC Gaussian NLL | P45–P49 valid P3; **P50 LIVE** (`run_p50_bcmean`) | SUPERSEDED (`bc_mean_only=True` default; MSE-on-μ like P3). **KEEP as σ-open; not sufficient as cascade lever** | YES — NLL pins last-Linear log_std to σ_min (P49 first P3 ent −0.283; P2 bc_loss −0.87…−1.13). P50 P2 bc **0.000–0.015 MSE never NLL**; first P3 ent **−0.107**. Unfreeze 169 still yanked **−0.107→−0.268** (logp_std 0.56→54), same class as P46/P47. Opt out `DREAMER_BC_MEAN_ONLY=0`. Next: P3 stop-grad log_std after EXIT. |
...
| Original-P1 wrap last-ok unlock | P49 EXIT (`run_p49_wrapunlock`); **P50 LIVE** (`run_p50_bcmean`) | KEEP as lock hygiene; **FALSIFIED as cascade lever** (P49); wrap-recovery unlock **GPU-confirmed P50** @57–58 and @70–73 | YES — P49 skip-storm 1/2 @64–65 unlocked (wrap path untested). P50 lock@56 recon 0.1412 skip 0; unlock @58; lock@70 unlock@73. Extra-P1 lock@88 stays locked (P40). Same P3 cascade still the actor question. |
| P1 gain-probe on wrap/detonated live g | P50 LIVE gate 82 recon 0.0283 → 1.37@MV EXTEND; gate 94 detonated 0.69@DV | SUPERSEDED (HEAD: probe last-ok when recon >5× best; restore live) | YES — wrap-adjacent FAIL burned extra-P1 which detonated (P41 extra P1 FALSIFIED as DV lever). Identity when recon healthy (P45–P49). Not in P50 pid. |

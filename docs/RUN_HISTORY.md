...
| P45 | 2026-08-28 | TM-protocol rest-IC (`DREAMER_GAIN_MATCH_REST_IC=1`) | launching | ⏳ one attributed change vs P43 identity |
...
### p45 (`run_p45_restic`, branch `cursor/p28`) — LAUNCH: TM-protocol rest-IC teacher
- **One attributed GPU change vs P43 identity (P44 settle REVERT):** `gain_match_rest_ic=True`. Real held-OP lookback encode → FD (TM `_settle_capture` post-step pairing). Skips P44 WM-held settle. Isolation loss stays 0. Collect settle = max(H, lookback), not `wm_tf_horizon`. Cache miss **aborts** (no silent PRBS fallback).
...
- Abs Huber gain-match is count-equal per input but **scale-unequal** when `|G_mv|≫|G_dv|`. P43 EXIT: per-input β **equalized jsonl Huber** (mv≈dv ~1e-4) but val DV still **×0.740 / @H ×0.849** (P40/P41/P42 class). Residual was already ~0 — β is not the DV lever. Identified G matches TM real (MV −0.32, DV 0.18). **DV @H ×0.849 vs ss ×0.740 is a 4H-asymptote gap** (MV @H≈ss); P43 DV decomp posterior+1-step **faithful**. Teacher IC (PRBS posterior FD at K=H) ≠ TM rest-then-step (`wm_tf_horizon` S=`H_tf`). CPU probe on P43 freeze: P44 teacher (PRBS+S=H) still FD DV×0.969. **P44 EXIT** val DV **×0.751 / @H ×0.842** det_r **0.099** — S=H FALSIFIED as DV pin (storm 2/2 G_pred≈0; freeze last_ok 57). **REVERT env-free settle `-1`**. Do **not** revive relative Huber. Do not try another Huber reweight. Do not set `gain_match_len`=4H as a Huber reweight. Do not retry S=H. **P45 GPU:** TM-protocol rest-IC teacher (`DREAMER_GAIN_MATCH_REST_IC=1`; one attributed change; isolation loss stays 0). Collect settle = max(H, lookback), not `wm_tf_horizon`. Cache miss aborts (no PRBS fallback). jsonl `*_ratio` ~×1 is teacher FD (not the TM gate). HEAD rest-IC encode `last_only` (not pid 16548).
...
| Gain-match FD from real rest lookback encode (TM protocol IC) | P45 launch (`run_p45_restic`) | pending (default False; opt-in env) | Isolation loss stays 0. Collect = TM `_settle_capture` pairing (obs after step) + `clean_steady_seeds`; settle = max(H, lookback) ≠ `wm_tf_horizon`. Skips P44 WM-held settle. Cache miss aborts. Promote only if GAIN-READY. |

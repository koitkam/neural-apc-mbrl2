...
| **Critic** | `critic_rew_to_tgt_var` (>0.015) | none healthy | — | P65 val `critic_r` **+0.719** PASS (P64 +0.705); P3 rtgt still collapses. Freeze rscale KEEP. |
| **Actor** | econ vs baseline + vs expert BC | **P64** (mbrl2) | paired **−4.54 vs −94** | Env-free P2 amp=P3. 9/9 `all_pass`. P65 **−11.95 vs −139** VALID but loses to P64; cv/mv viol 1.80/5.67 vs P64 1.01/0.33. P53 stays the μ-ratio-clip RCA. |
...
| P66 | 2026-09-01 | per-seq `dob_ground` var-skip (`var>1e-3`; mixed ring KEEP) | **LIVE P3** tmux `mbrl2_p66` pid **181468** sha `38880f9`. GAIN-READY **0.84@DV** last_ok **76 locked** (detonated freeze; P64 0.91@DV last_ok 82). P2 buf **327/327**. `dob_ground` med **0.272** (P64 **0.121** / P65 **0.266**); `dob_d_absmean` last **0.043** (P64 **0.063** / P65 **0.085**). P2→P3 @136. Unfreeze **147** ent **−0.101 HELD** rscale **2.70 KEEP**. Iter **280** rtgt **0.00085** logp **0.62** clip **0.17** ret **−10**. | ⏳ GPU ~2.8 GB. Actor VALID freeze. Hypothesis **not yet val**. HEAD (not pid): jsonl `dob_ground_keep_frac`. |
...
### p66 (`run_p66_dobvar`, branch `cursor/p28`) — LIVE P3: per-seq `dob_ground` var-skip
...
| P1-clean replay at P2 Kalman ID | **P64 EXIT** `dob_d` only 2.4×; `dob_ground` 0.21 not closed; mixed `d≡0` rows; **P65 EXIT** flush dropped 327, P2 buf 5→276, pred_std **0.518** (P64 0.608), det_r **0.191**, paired **−11.95**; **P66 LIVE P3** mixed ring KEEP, `dob_ground` med **0.272**, `dob_d` last **0.043** (P64 0.063) | P65 flush **REVERT / FALSIFIED**. P66 per-seq `dob_ground` var-skip (1e-3; no new knob). Val pending. | YES as mechanism (zeros ≡ L2-on-d→0) / **NO as flush lever** — emptying the ring starved P2. P66 P2 `|d|` did **not** rise vs P64. Do not flush N+1. Do not `/dvar`. |

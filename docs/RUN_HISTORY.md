...
| P68 | 2026-09-01 | rest-IC Huber baseline = last rest-obs CV (TM `pre`; not held-K) | LIVE P1 iter **40** pid **193014** sha `9c18daa`. `gmatch_len=55`. Iter 1 teacher **×0.655 / ×−1.12** gmatch **1.53** (not jsonl ×1). Wrap lock@32 unlock@33; recon **0.1807→0.0041**. skip 0. Probe40 gain_fid **0.474** (P64 iter40 **0.559**). | 🔬 vs P64 0.91@DV / MV ×0.927; no MV overgain |
...
### p68 (`run_p68_tmpre`, branch `cursor/p28`) — LIVE: rest-pre Huber (TM `pre`)
...
- **LIVE P1 iter 40** (~69 min). Teacher pinned **×0.999 / ×1.022** by iter 8 (Huber 1.53→0.0007); iter 40 **×1.006 / ×0.947** gmatch **0.0005**. Fidelity iter 10/20/30/40: r@H55 **0.259 / 0.493 / 0.580 / 0.555**, gain_fid **0.694 / 0.640 / 0.870 / 0.474**, `wm_best` **30**. P64 probe40 gain_fid was **0.559** then recovered to 0.917 by 70 — not a P68-only drop. Wrap **lock@32** (recon 0.1807 = 59× best 0.0031, gnorm 7.53, skip 0) then **unlock@33**; recon **0.0466→0.0041** @40. skip **0**. `t_wm` **96–103 s**. `kl=0` `jemb>0` alive 1022. First gate still ~82 vs P64 **0.91@DV**.
- **HEAD (not this pid):** rest-pre skips the unused held-K `img_rollout` row (Huber does not read `decode(held_K)`; PRBS fallback keeps it). Identity. CVD="" `_smoke_rssm.py` green. Do not treat as a P68 A/B.
- **Watch:** first gate vs P64 0.91@DV; no MV overgain (P67 val ×1.35); wrap does not lock last-ok through extra-P1. Do not teacher-K N+1. Do not score actor if GAIN_NOT_READY. **No second GPU job.**

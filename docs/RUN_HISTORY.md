...
| Unwhitelisted `DREAMER_ACT_HIST_REQUIRED` + zeros-for-past in `imagine_next_z` | **P64 GPU-occupied** (pid **169231** P1) | **REMOVED** (raise if `action_history` is None) | YES — all three callers already pass history. Env was a silent A/B outside `ENV_OVERRIDES` (direct `os.environ.get`). Not in this pid. CVD="" smoke. |
| P1/P2 hidden-OU amp cap 0.2 vs P3/val 1.0 | P45–P62 val pred_std/true ~0.2 (P62 0.394/1.93); **P64 LAUNCH** | P2 uses P3 cap (no new knob; P1 stays 0.2) | PARTIAL — Kalman SNR-calibrated at 0.2×. P1 full-scale would be p112 omitted-variable (g trains without DOB). |

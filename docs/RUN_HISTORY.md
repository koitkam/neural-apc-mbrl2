...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8`) | **LIVE** env-free `run_p52_logpclip` tmux `mbrl2_p52`. Watch unfreeze ~147: actor_loss / rtgt / mvv vs P51 rail. Step 4 must print `p3_sglogstd=True p3_logpclip=8`. | 🔬 μ-side clamp after stop-grad FALSIFIED as cascade. Do not stack critic knobs. |
...
### p52 (`run_p52_logpclip`, branch `cursor/p28`) — LIVE: clamp P3 REINFORCE logp
- **One attributed GPU change vs P51 pid:** `p3_logp_clip=8.0` (HEAD default). Clamp `logp` in `-(adv * logp)` so frozen-σ `(u−μ)/σ²` cannot explode μ. Jsonl `actor_logp_*` stays **unclipped** (rail still visible). Opt out `DREAMER_P3_LOGP_CLIP=0`. Also carries P51 `p3_stop_grad_log_std=True` (KEEP as yank block) and P50 `bc_mean_only=True`. Env-free. tmux `mbrl2_p52`. `CUDA_VISIBLE_DEVICES=0`.
- **Not:** leftover `DREAMER_*`, `p3_reset_log_std`, extra critic knobs, `actor_kl_coef`.
- **Step 4 must print:** `p3_sglogstd=True p3_logpclip=8`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`.
- **Watch:** unfreeze ~147: ent stays ≈ H(σ_init); unclipped `actor_logp_std` may jump; actor_loss / rtgt / mvv should **not** P51-rail. Do not launch a second GPU job.

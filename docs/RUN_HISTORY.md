...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT; Markovian rest-IC) | EXIT=0, 158 iters, ES `p3_skipped_invalid_observer`. orig-P1 FAIL **−1.06@MV**; extra-P1 @94 **−0.29@MV**; freeze last_ok **104** 5-level **−0.46@MV ready=False**. conv=**0** through P2@150. Val `final.pt` expert-BC: MV **×−0.496 / ×−0.129** DV **×0.736 / ×0.655**. 1step→OL **×0.379**. det_r **0.090** pred_std **3.52 vs 1.93**. Paired **−262.95 vs −87.03** FAIL (do **not** score actor). `actor_experiment_valid=false`. | ❌ **FALSIFIED** Markovian TSSM rest-IC as freeze / TM / GAIN-READY. Keep TSSM default for P78 KV-continue. Do **not** score actor. |
| P78 | 2026-09-03 | TSSM gain-match rest-IC continues KV via `prev_state=` (P77 Markovian FALSIFIED) | LIVE P1 iter **10**. Probe10 **H=1 +0.145 / H=55 +0.144 conv=0** (P77 +0.128/+0.122 conv=0; P64 +0.584/+0.309 conv=0.50). Teacher **×0.953 / ×1.190**. jsonl ×1 ≠ freeze. pid **261143**. | 🔬 LIVE — first probe still dead / conv=0. Do not score actor. |
...
### p77 (`run_p77_tssm`, branch `cursor/p28`) — EXIT: env-free TSSM observer FALSIFIED as freeze (GAIN_NOT_READY CAPPED)
...
- **EXIT=0**, 158 jsonl iters, `early_stop_reason=p3_skipped_invalid_observer`. P2→P3 @158 PASS (reward_mtp median **1.034** ≪ 3.0) then **`[p3-skip]`**. `actor_experiment_valid=false`. `p1_gain_not_ready_capped=true`. `p1_detonated_freeze_restored=false`. Val on `final.pt` = **expert-BC**, not actor. skip **0** entire run. last_ok **104 unlocked**.
- **P2 Kalman:** `dobg` P2 med **0.063** last **0.118** peak **0.244** (P64-class median). recon **0.0023–0.0091**. `[wm-fidelity-ES]` suppressed @150 (P2 remaining 48312 < 75396).
- **Val (`final.pt` expert-BC — do NOT score actor):** MV ss/@H **×−0.496 / ×−0.129** (`wm_ss=+0.159` vs real **−0.320**; @H wm **+0.040** vs real **−0.312**; rel_err **1.50 FAIL**). DV ss/@H **×0.736 / ×0.655** (`wm_dv_ss_ratio_worst=0.736`; printed HEALTHY rel_err 0.26 — **not** GAIN-READY). Freeze was **−0.46@MV** `unbiased=False` — inverted MV **transferred**. Decomp MV: real→post **×1.019**, post→1step **×0.841**, **1step→OL ×0.379**, OL-vs-real **×0.405**, lever printed `free_bits` (kl pinned) but compounding is the bigger multiplicative hole (vs P64 ×0.85 / P76 ×0.770 / CPU@70 ×0.272). DV: real→post **×0.957**, post→1step **×0.836**. det_r **0.090** / R²_det **−2.13**; pred_std **3.52 vs 1.93** (over-amp, not P76 dead 0.638). `critic_r=NaN` (no P3); `wm_next_state_r=0.744` (correlation-only). Fidelity `all_pass=false`. Expert-BC paired **−262.95 vs −87.03** `beats_baseline_pass=false` n=9; mv_viol **60.5**; cv_viol **170**; reversal **0.196**. **Do not attribute econ to actor knobs.**
- **ROOT (SysID):** Markovian rest-IC `img_rollout(h,z)` (`kv_cache=None`) pinned jsonl teacher **×1** while 5-level freeze / val TM are cached. Sign-flipped freeze **−0.46@MV** became val MV **×−0.50**. conv=**0** @10–150 (H=55 @130 +0.475 collapsed @150 +0.152). TSSM-without-KV is **not** a compounding fix vs GRU. Extra-P1 FALSIFIED as freeze lever again (@94 −0.29 worsened to −0.46).
- **Step 5:** env-free TSSM **FALSIFIED as freeze / TM / GAIN-READY**. Keep TSSM default for the already-HEAD P78 A/B (pass `prev_state` into gain-match rest-IC only; overshoot/held stay Markovian). Do **not** Markovian-TSSM N+1. Do **not** revert to RSSM in the same run as `prev_state`. Persist KEEP. Last-step DC Huber KEEP. Champion **unchanged: P64 econ / P53 μ / P26 MV ss / P64 DV ss**.
- **Next GPU (P78):** one attributed change vs P77 identity: gain-match rest-IC OL continues TSSM KV via `prev_state=` (HEAD `b9089c4`; RSSM `prev=None` identity). Overshoot/held stay Markovian this A/B. Metric: freeze GAIN-READY vs P64 **0.91@DV**; jsonl teacher vs 5-level TM (must not pin Markovian ×1 if KV continues); probe H=55 r / conv>0; val 1step→OL vs P64 ×0.85 / P77 ×0.379; `actor_experiment_valid`. Tag `prevkv`. Session `mbrl2_p78`. Out-dir `output/test_sim/run_p78_prevkv`. Watch `[gain-match] TSSM OL continues rest-IC KV-cache`. Smokes CVD="" PASSED (Markovian ≠ cached Δ=7.7e-2). **LAUNCHED** this session.
- **Freeze @104 CAPPED GAIN_NOT_READY −0.46@MV / 0.74@DV** (P64 orig-P1 **0.91@DV**; P76 freeze **0.80@MV**). 5-level `DCgain_ratio[-0.46, 0.74] @H[-0.12, 0.65]` worst=`−0.46@MV CV0<-MV0` DV **0.74/@H 0.65** band `[0.8, 1.3]`. **Noisy:** spread_x1.4 signflips=7 (2 inputs) `unbiased=False not_noisy=False`. vs extra-P1 @94 **−0.29@MV / 0.80@DV**: 2nd extra-P1 budget **worsened** the miss (more inverted MV, DV further below band). jsonl teacher @104 **×1.000 / ×1.023** ≠ 5-level TM (Markovian FD vs cached TM). Freeze = live last_ok **104**, recon **0.0026** healthy. Cap 175924 extra-P1 steps reached (no 3rd EXTEND). Do **not** score actor.
- **LIVE 2nd extra-P1 iter 103 (~539 min, nvidia 11.7 GB, `t_wm` ~308 s):** skip **0**. recon **0.0021@96 best → 0.0025@101 → 0.0043@102 blip → 0.0028@103**. Teacher jsonl 100 **×0.991 / ×1.097** → 101 **×1.037 / ×0.805** (DV dip + gmatch 0.009 after inject) → 102 **×1.002 / ×0.991** → 103 **×0.998 / ×1.019**. last_ok **103 unlocked**. persist_rel **0.024@100 → 0.023@101 → 0.041@102 → 0.026@103** after full inject @100 — **no** 0.21-class spike (unlike delayed 0.212@85 after @80). Do **not** persist N+1. Steps **899140**; iter 104 hit 905240 ≥ 904752.
- **Probe 150 (P2, frozen g):** `H=1:r=+0.585 H=13:r=+0.254 H=27:r=+0.244 H=55:r=+0.152 conv=0.00 drift=0.617 floor=0.40 best_h=1/55 gain_fid=0.851`. vs probe140 **+0.669 / +0.392 conv=0 best_h=1**: H=55 **collapsed** 0.392→0.152 (same P1 pattern @80 0.423 → @90 0.147); H=13/27 dropped below floor; **conv still 0 @10–150**. `[wm-fidelity-ES]` suppressed (P2 remaining 48312 < 75396). **Do not treat as freeze health.** Cached-probe ≠ 5-level TM freeze.
- **Probe 140 (P2, frozen g):** `H=1:r=+0.669 H=13:r=+0.359 H=27:r=+0.395 H=55:r=+0.392 conv=0.00 drift=0.600 floor=0.40 best_h=1/55 gain_fid=0.634`. vs probe130 **+0.719 / +0.475 conv=0 best_h=55**: H=55 slipped back below floor 0.475→0.392; **best_h 55→1**; **conv still 0**. Expert inject @140. Not freeze health.
- **Probe 130 (P2, frozen g):** `H=1:r=+0.719 H=13:r=+0.344 H=27:r=+0.416 H=55:r=+0.475 conv=0.00 drift=0.650 floor=0.40 best_h=55/55 gain_fid=0.357`. vs probe120 **+0.796 / +0.355 conv=0 best_h=27**: H=55 **crossed floor** 0.355→0.475 and **best_h 27→55**; H=1 slipped 0.796→0.719; H=13 dropped 0.545→0.344 below floor; **conv still 0 @10–130**. **Do not treat as freeze health** (g frozen at −0.46@MV; probe mix can move from DOB/reward). First P2 H=55-above-floor + best_h=55 (P1 @80 was 0.423 then collapsed @90). Cached-probe ≠ 5-level TM freeze.
- **Probe 120 (P2, frozen g):** `H=1:r=+0.796 H=13:r=+0.545 H=27:r=+0.420 H=55:r=+0.355 conv=0.00 drift=0.722 floor=0.40 best_h=27/55 gain_fid=0.537`. vs probe110 **+0.810 / +0.262 conv=0 best_h=13**: H=13/27 held floor; H=55 recovered 0.262→0.355 still short of 0.40; **conv still 0**. Not freeze health.
- **Probe 110 (P2, frozen g):** `H=1:r=+0.810 H=13:r=+0.619 H=27:r=+0.379 H=55:r=+0.262 conv=0.00 drift=0.701 floor=0.40 best_h=13/55 gain_fid=0.800`. `wm_best` saved EMA **4.077** (raw 4.582) — **do not treat as freeze health** (g already frozen at −0.46@MV). vs probe100 **+0.567 / +0.299 conv=0 best_h=1**: H=1 jumped above P64@100 0.720; H=13 crossed floor; H=55 slipped 0.299→0.262; **conv still 0 @10–110**. Cached-probe ≠ 5-level TM freeze.
- **Probe 100:** `H=1:r=+0.567 H=13:r=+0.305 H=27:r=+0.366 H=55:r=+0.299 conv=0.00 drift=0.622 floor=0.40 best_h=1/55 gain_fid=0.636`. `wm_best` **not** updated (EMA **3.701** @90). Peers probe100: P76 **H=1 +0.534 H=55 +0.279 conv=0.00 gain_fid=0.602 best_h=1**; P64 **+0.720 / +0.376 conv=0.00 0.700 best_h=13**; P73 **+0.771 / +0.349 conv=0.75 0.572 best_h=27**; P75 **+0.611 / +0.431 conv=0.25 0.762 best_h=55**. vs P77 probe90 **+0.592 / +0.147 conv=0 0.859 best_h=1**: H=1 slipped 0.592→0.567 (still short of P64 0.720); H=13 recovered 0.246→0.305 (still dead vs P64 0.507); H=27 0.283→0.366 near floor; H=55 **recovered from collapse** 0.147→0.299 still below floor (P76 0.279 / P64 0.376 also below); **best_h stuck at 1**; **conv still 0 @10–100**; gain_fid 0.859→0.636 (less H=1-dominated — not health). Cached compounding still not a sticky H=55 floor.
- **Probe 90:** `H=1:r=+0.592 H=13:r=+0.246 H=27:r=+0.283 H=55:r=+0.147 conv=0.00 drift=0.604 floor=0.40 best_h=1/55 gain_fid=0.859`. `wm_best` saved EMA **3.701** (raw 3.848) — **do not treat as health** (H=1-driven mix). Peers probe90: P76 **H=1 +0.681 H=55 +0.573 conv=0.00 gain_fid=0.988 best_h=55**; P64 **+0.827 / +0.544 conv=0.25 0.745 best_h=55**; P73 **+0.536 / +0.438 conv=0.25 0.940 best_h=55**; P75 **+0.642 / +0.492 conv=0.50 0.692 best_h=55**. vs P77 probe80 **+0.583 / +0.423 conv=0 0.520 best_h=55**: H=1 flat 0.583→0.592 (still short of P64 0.827); H=13 still dead 0.246 vs peers 0.59–0.62; H=27 dropped 0.446→0.283 below floor; H=55 **collapsed again** 0.423→0.147 (same as @70 0.160; @80 recovery was not sticky); **best_h 55→1**; **conv still 0 @10–90**; gain_fid jumped 0.520→0.859 because H=1 dominates — not health. P76 also conv=0@90 but H=55 0.573 / mid-H healthy.
- **Extra-P1 gate @94 FAIL GAIN_NOT_READY −0.29@MV** (P64 orig-P1 **0.91@DV**; P76 extra-P1 live **0.84@DV** discarded / freeze **0.80@MV**). 5-level `DCgain_ratio[-0.29, 0.80] @H[-0.00, 0.73]` worst=`−0.29@MV CV0<-MV0` DV **0.80/@H 0.73** band `[0.8, 1.3]`. **Noisy:** spread_x1.1 signflips=5 (2 inputs) `unbiased=False not_noisy=False` (less wild than @82 spread×5.2 / 6 flips, still inverted). MV improved −1.06→−0.29 but **still sign-flipped**; DV at ss band floor, @H short. jsonl teacher @94 **×1.010 / ×0.956** ≠ 5-level TM (same Markovian vs TM hole). **Not** a P76-style live pass then detonated-freeze — recon **0.0025** healthy, last_ok **94 unlocked**, no restore. 2nd EXTEND keep-ext KEEP as mechanism. Freeze @104 **CAPPED −0.46@MV / 0.74@DV** — 2nd extra-P1 **worsened**. Do **not** score actor.
- **Original-P1 gate @82 FAIL GAIN_NOT_READY −1.06@MV** (P64 @82 **PASS 0.91@DV**; P76 **FAIL 0.79@DV**; P75 **0.84@MV**; P73 **0.83@DV**). 5-level `DCgain_ratio[-1.06, 0.75] @H[-0.20, 0.70]` worst=`−1.06@MV CV0<-MV0` DV **0.75/@H 0.70** band `[0.8, 1.3]`. **Noisy:** spread_x5.2 signflips=6 (2 inputs) `unbiased=False not_noisy=False`. Sign-flipped MV + noisy is worse than P76's in-band-short DV miss. jsonl teacher @82 **×1.005 / ×0.974** ≠ 5-level TM (jsonl ×1 is Markovian FD; 5-level is the freeze). EXTEND keep-ext KEEP as mechanism. Do **not** score actor. Extra-P1 @94 did **not** pass (unlike P76 live 0.84@DV).
- **Probe 80:** `H=1:r=+0.583 H=13:r=+0.210 H=27:r=+0.446 H=55:r=+0.423 conv=0.00 drift=0.459 floor=0.40 best_h=55/55 gain_fid=0.520`. `wm_best` saved EMA **3.554** (raw 3.707) — **do not treat as health**. Peers probe80: P76 **H=1 +0.679 H=55 +0.532 conv=0.00 gain_fid=0.957 best_h=55**; P64 **+0.763 / +0.594 conv=0.25 0.936 best_h=55**; P73 **+0.799 / +0.578 conv=0.50 0.988 best_h=55**; P75 **+0.608 / +0.557 conv=0.50 0.938 best_h=55**. vs P77 probe70 **+0.746 / +0.160 conv=0 0.537 best_h=27**: H=55 **recovered above floor** 0.160→0.423 and **best_h moved 27→55**; H=27 0.404→0.446 (floor); H=1 **slipped** 0.746→0.583 (no longer beats P64 0.763); H=13 uniquely dead 0.210 vs peers 0.62–0.66; **conv still 0 @10–80**; gain_fid flat 0.537→0.520. H=55 recovery is cached compounding (probe warms KV); still short of P64 0.594 / P76 0.532. P76 also conv=0@80 but mid-H healthy (H=13 0.647).
- **Probe 70:** `H=1:r=+0.746 H=13:r=+0.350 H=27:r=+0.404 H=55:r=+0.160 conv=0.00 drift=0.389 floor=0.40 best_h=27/55 gain_fid=0.537`. `wm_best` saved EMA **3.401** (raw 3.500, down from 4.288@60) — **do not treat as health**. Peers probe70: P76 **H=1 +0.539 H=55 +0.571 conv=0.00 gain_fid=0.886 best_h=55**; P64 **+0.734 / +0.622 conv=0.50 0.917 best_h=55**; P73 **+0.641 / +0.498 conv=0.25 0.839 best_h=55**; P75 **+0.550 / +0.517 conv=0.50 0.806 best_h=55**. vs P77 probe60 **+0.682 / +0.293 conv=0 0.882 best_h=1**: H=1 still beat P64 (0.746 vs 0.734); H=27 reached floor (0.404) and **best_h moved to 27**; H=13 flat 0.35 vs P64 0.64; H=55 **collapsed** 0.293→0.160 (now uniquely dead vs peers 0.50–0.62); **conv still 0 @10–70**; gain_fid **worse** 0.882→0.537. Compounding: 1-step keeps improving while cached H=55 got worse. P76 also conv=0@70 but H=55 0.571 / best_h=55 / mid-H healthy.
- **Probe 60:** `H=1:r=+0.682 H=13:r=+0.348 H=27:r=+0.341 H=55:r=+0.293 conv=0.00 drift=0.341 floor=0.40 best_h=1/55 gain_fid=0.882`. `wm_best` saved EMA **3.303** (raw 4.288) — **do not treat as health** (best_h stuck at 1; H=55 below floor). Peers probe60: P76 **H=1 +0.555 H=55 +0.291 conv=0.00 gain_fid=0.674 best_h=27**; P64 **+0.671 / +0.253 conv=0.25 0.649 best_h=27**; P73 **+0.461 / +0.210 conv=0.75 0.685 best_h=1**; P75 **+0.603 / +0.261 conv=0.50 0.668 best_h=27**. vs P77 probe50 **+0.509 / +0.273 conv=0 0.389**: H=1 **beat P64** (0.682 vs 0.671) and all peers; mid-H recovered from @50 dip but still short of P64 (H=13 0.348 vs 0.500); H=55 **flat dead** (0.293 vs floor 0.40; ~P76 0.291 / slightly above P64 0.253); **conv still 0 @10/20/30/40/50/60**; gain_fid jumped 0.389→0.882 because H=1 dominates the mix. Compounding signature is conv=0 + best_h=1 + mid-H lag, not uniquely-dead H=55 vs P64 at this snapshot. P76 also conv=0@60 but best_h=27 / mid-H healthy (~0.53).
- **Probe 50:** `H=1:r=+0.509 H=13:r=+0.253 H=27:r=+0.241 H=55:r=+0.273 conv=0.00 drift=0.321 floor=0.40 best_h=1/55 gain_fid=0.389`. `wm_best` saved EMA **2.317** (raw 2.433) — H=1 still above floor but dipped; H=13/27 dropped vs @40; H=55 flat below floor; do not treat EMA as health. Peers probe50: P76 **H=1 +0.502 H=55 +0.401 conv=0.00 gain_fid=0.788 best_h=55**; P64 **+0.651 / +0.395 conv=0.25 0.769 best_h=27**; P73 **+0.741 / +0.380 conv=0.00 0.783 best_h=27**; P75 **+0.706 / +0.455 conv=0.25 0.735 best_h=55**. vs P77 probe40 **+0.554 / +0.267 conv=0 0.524**: H=1 slipped, mid-horizon r fell, H=55 still dead, **conv still 0 @10/20/30/40/50**, gain_fid **worse** 0.524→0.389. Recon improved while cached OL stalled — compounding. P76 also conv=0@50 but H=55 at floor / best_h=55.
- **Probe 40:** `H=1:r=+0.554 H=13:r=+0.364 H=27:r=+0.334 H=55:r=+0.267 conv=0.00 drift=0.404 floor=0.40 best_h=1/55 gain_fid=0.524`. `wm_best` saved EMA **2.202** (raw 3.060) — H=1 now above floor; H=55 still below; do not treat EMA as health. Peers probe40: P76 **H=1 +0.561 H=55 +0.546 conv=0.00 gain_fid=0.528 best_h=55**; P64 **+0.819 / +0.456 conv=0.25 0.559 best_h=55**; P73 **+0.591 / +0.448 conv=0.00 0.592 best_h=55**; P75 **+0.553 / +0.462 conv=0.50 0.495 best_h=55**. vs P77 probe30 **+0.403 / +0.172 conv=0 0.387**: H=1 matched P75/P76 (~0.55) still short of P64 0.819; H=55 still uniquely dead (0.267 vs peers 0.45–0.55); **conv still 0 @10/20/30/40**; gain_fid ≈ P76/P64 mix but best_h stuck at 1. drift 0.404 is the H=1 vs H=55 gap (peers 0.15–0.22).
- **Probe 30:** `H=1:r=+0.403 H=13:r=+0.253 H=27:r=+0.255 H=55:r=+0.172 conv=0.00 drift=0.215 floor=0.40 best_h=1/55 gain_fid=0.387`. `wm_best` saved EMA **1.345** (raw 2.154) — H=1 just at floor; H=55 still below; do not treat EMA as health. Peers probe30: P76 **H=1 +0.157 H=55 +0.271 conv=1.00 gain_fid=0.873 best_h=0/55**; P64 **+0.751 / +0.448 conv=0.50 0.944 best_h=55**; P73 **+0.476 / +0.416 conv=0.00 0.952 best_h=55**; P75 **+0.559 / +0.561 conv=0.50 0.953 best_h=55**. vs P77 probe20 **+0.325 / +0.145 conv=0 0.261**: H=1 reached floor, H=55 still dead, **conv still 0 @10/20/30**, gain_fid recovered from @20 dip but still ~2× worse than P64. P73 also conv=0@30 but H=55 above floor.
- **Probe 20:** `H=1:r=+0.325 H=13:r=+0.162 H=27:r=+0.201 H=55:r=+0.145 conv=0.00 drift=0.154 floor=0.40 best_h=0/55 gain_fid=0.261`. `wm_best` saved EMA **0.536** (raw 1.185) — r still below floor 0.40; do not treat as health. Peers probe20: P76 **H=1 +0.589 H=55 +0.543 conv=0.00 gain_fid=0.644 best_h=55**; P64 **+0.706 / +0.498 conv=0.50 0.734**; P73 **+0.614 / +0.440 conv=0.50 0.692**; P75 **+0.668 / +0.539 conv=0.50 0.641**. vs P77 probe10 **+0.128 / +0.122 conv=0 0.476**: H=1 r improved, H=55 still dead, **conv still 0**, gain_fid **worse**. P73 flipped 0@10 → 0.50@20; P77 did not.
- **RCA (SysID, not a live patch; CPU-confirmed @wm_best iter70, CVD=""):** rest-IC `rollout_observed` **does** keep KV (`pos=128`, 4 layers). Live pid gain-match / overshoot / held start `img_rollout(h,z)` with `kv_cache=None`. Markovian vs `prev_state` held-K CV **mean |Δ|=0.256** (iter50 **0.123**; iter40 **0.142** — **diverging** as recon improves). step1 **0.051** / stepK **0.094**. Markovian ΔCV(K) **0.263** vs cached **0.220**. Fidelity probe / val TM **warm cache** then `img_step` (so probe H=55 r dead is **cached compounding**, not the Markovian hole). Teacher jsonl ×1 pins the **Markovian** DC. CPU posterior-prior @H=55 settle=128: real→post **×0.865** (iter50 **×0.836**) / post→1step **×0.702** (iter50 **×0.607**) / 1step→OL **×0.272** (iter50 **×0.097**; OL-vs-real **×0.165** vs **×0.064**). Compounding improved 2.8× but still **~3× short of P64 ×0.85**. Verdict still `autoencoder` (0.865 < 0.9) with compounding the bigger multiplicative hole. Recon **0.0021@96** beats P64 0.0035 — not an EXIT. Probe100 **conv still 0 @10–100**; H=55 recovered 0.147→0.299 still below floor / best_h=1; recon **0.0035@100**. Extra-P1 @94 **FAIL −0.29@MV / 0.80@DV**; freeze @104 **CAPPED −0.46@MV / 0.74@DV** last_ok **104** (2nd extra-P1 worsened; not a P76 detonated-freeze). persist **0.026@103**. **P78 launched** after this EXIT: gain-match rest-IC OL continues KV via `prev_state=` (`run_p78_prevkv`). Overshoot/held stay Markovian this A/B. Do not persist N+1; do **not** Markovian-TSSM N+1; `actor_experiment_valid=false` unless GAIN-READY.
- **Probe 10 (first fidelity):** `H=1:r=+0.128 H=13:r=+0.161 H=27:r=+0.072 H=55:r=+0.122 conv=0.00 drift=0.147 floor=0.40 best_h=0/55 gain_fid=0.476`. `wm_best` saved EMA **−0.113**. Peers probe10: P76 **H=1 +0.428 H=55 +0.310 conv=0.00 gain_fid=0.736**; P64 **+0.584 / +0.309 conv=0.50 gain_fid=0.827**; P73 **+0.549 / +0.368 conv=0.00 gain_fid=0.772**; P75 **+0.336 / +0.210 conv=0.25 gain_fid=0.726**. TSSM 1-step r is ~3–4× worse than RSSM identity; conv=0 at first probe is **not** yet P76-unique (P73 also 0 @10; P76 held 0 through P2@100).

### p78 (`run_p78_prevkv`, branch `cursor/p28`) — LIVE: TSSM rest-IC KV continue (`prev_state=`)
- **One attributed GPU change vs P77 identity:** gain-match rest-IC `img_rollout(..., prev_state=)` continues TSSM KV (`_rest_ic_last_state` + `_expand_dyn_state`). Overshoot/held stay Markovian this A/B. RSSM `prev=None` identity. Persist KEEP. Last-step DC Huber KEEP. Env-free: **no leftover `DREAMER_*`**. tmux `mbrl2_p78`, pid **261143**, sha `9c8727a` (code `b9089c4`), out-dir `output/test_sim/run_p78_prevkv`. Process environ: `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF` only.
- **Step 4 CONFIRMED:** `device=cuda` bs=**16** (gpu-calib TSSM `per_sample=647.7 MB`) `wm_train_T=128` horizon=55 compile=eager. `[cont-latent] GAIN-ONLY` gain_dim=2. `[resolved-cfg] wm=tssm latent=deterministic isolation=0 gmatch_len=55 gmatch_step=0.4 gmatch_clip=True gmatch_rest=True gmatch_rest_L=128 gmatch_rest_cg=True held_cv=True gru_zbias=0 p1amp=0.2 p2amp=1 p3amp=1`. Rest-IC CUDA graph **not captured** (TSSM kv grows; warmup retry after `empty_cache`). Huber baseline = rest last-obs CV. **`[gain-match] TSSM OL continues rest-IC KV-cache (pos=128+K=55; not Markovian h/z)`**. Iter 1 teacher **×0.146 / ×−0.028** (P77 Markovian iter 1 was **×0.643 / ×−1.07** — FD is not a no-op) skip **0** persist_rel **0.837** recon **0.553** `t_wm` **295 s** nvidia **~12.2 GB**. **One GPU job.** Do **not** launch a second.
- **LIVE P1 iter 10 (~52 min, nvidia 12.3 GB, `t_wm` 295 s):** skip **0**. last_ok **10 unlocked**. recon **0.280@9 best → 0.284@10** (beats P77@10 **0.384** / P77 best-then **0.305**). Teacher jsonl **×0.953 / ×1.190** (iter 9 **×1.000 / ×1.035**; DV overgain recurred, P77@10 **×1.008 / ×0.951**). persist_rel **0.830@9 → 0.479@10** (rebound collapsed again; P77@10 **0.553**). gmatch **0.0073** gnorm **5.54**. jsonl still last-step ~×1 — KV-continue did **not** unpin teacher DC. **Probe10:** `H=1:r=+0.145 H=13:r=+0.181 H=27:r=+0.122 H=55:r=+0.144 conv=0.00 drift=0.181 floor=0.40 best_h=0/55 gain_fid=0.835`. vs P77 probe10 **H=1 +0.128 H=55 +0.122 conv=0 gain_fid=0.476 best_h=0**; P64 **+0.584 / +0.309 conv=0.50 0.827**; P76 **+0.428 / +0.310 conv=0 0.736**. H=55 still uniquely dead vs RSSM peers; **conv still 0**; tiny H=1/H=55 lift vs P77 is not a compounding win. gain_fid 0.835 is **equal-dead horizons** (drift 0.181), not health. `[wm-best]` EMA **1.060** — **do not treat as health**. Next probe @20 (~50 min). Gate ~82 (~6 h). Do **not** score actor. Do **not** persist N+1. Do **not** launch a second GPU job.
- **LIVE P1 iter 6 (~32 min, nvidia 12.3 GB, `t_wm` 295 s):** skip **0**. last_ok **6 unlocked**. recon **0.375@4→0.364@5→0.423@6** (~P77@6 **0.423** wrap blip). Teacher jsonl **×1.000 / ×0.980** (iter 5 **×0.904 / ×1.445**; iter 4 **×1.014 / ×0.787**). persist_rel **0.315@4→0.256@5→0.783@6** (rebounded; P77@6 **0.407**). gmatch **0.018@4→0.035@5→0.0021@6** (tiny, same class as P77@6 0.0022). gnorm **26.2** vs P77@6 **0.37**. **@5 DV overgain ×1.445** (worse than P77@5 ×1.067) then both channels pinned ×1 at iter 6. KV-continue did **not** stop last-step teacher pin or DV overshoot. persist rebound with Huber≈0 means G still drifts over K while DC sat — freeze/5-level TM / probe@10 remain the test (P77 jsonl ×1 ≠ 5-level **−0.46@MV**). First probe @10 (~20 min). Do **not** score actor. Do **not** persist N+1. Do **not** launch a second GPU job.
- **Metric:** freeze GAIN-READY vs P64 **0.91@DV** (P77 CAPPED **−0.46@MV**); jsonl teacher vs 5-level TM (P77 jsonl ×1 ≠ freeze); probe H=55 r / conv>0 vs P77 conv=0; val 1step→OL vs P64 ×0.85 / P77 ×0.379. Do **not** score actor if GAIN_NOT_READY. Champion **P64**.
...
| env-free `world_model_type='tssm'` (in-context SysID; no GRU) | **P77 EXIT** (`run_p77_tssm`, pid **242511**, sha `8869d66`, 158 iters) | **FALSIFIED** as freeze / TM / GAIN-READY. KEEP as default for P78 `prev_state=` A/B | YES as Markovian rest-IC OL (jsonl ×1 ≠ 5-level **−0.46@MV**; val MV **×−0.496 / ×−0.129** inverted). conv=0 @10–150. 1step→OL **×0.379** vs P64 ×0.85 / P76 ×0.770. det_r **0.090** pred_std **3.52 vs 1.93**. Actor INVALID (`[p3-skip]`). Extra-P1 worsened freeze. Do **not** Markovian-TSSM N+1. Next P78: rest-IC KV `prev_state=`. |

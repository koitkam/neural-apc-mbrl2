...
| P52 | 2026-08-30 | P3 REINFORCE logp clip (`p3_logp_clip=8` per action dim) | **LIVE P1** iter **44** (~100 min). pid **64705**, launch `d910ee2`. Step 4 **CONFIRMED** `p3_sglogstd=True p3_logpclip=8`. recon best **0.0027@37**. lock@38 last_ok 37 (recon 0.412 skip 0) wrap-unlock@39; **last_ok advanced 37→44** (0.010@40 <5×). Iter 40 probe H=55 r=0.451 gain_fid=0.511 (best still 30 EMA 5.798). skip **0**. GPU ~11 GB. Env-free. Watch gate 82; unfreeze ~147 vs P51 μ-rail. | 🔬 wrap-unlock KEEP (P50-class). Do not launch. Do not stack critic knobs. |
...
- **P1 iter 44 (~100 min):** recon best **0.0027@37**. lock@38 last_ok 37 (recon 0.412 skip 0). wrap-unlock@39 recon 0.028. **last_ok advanced 37→40@0.010 (<5×) →44** (P50-class original-P1 wrap recovery KEEP; extra-P1 stay-locked not in play yet). Iter 44 recon **0.0053** skip **0**. Iter 40 wm-fid H=55 r=0.451 `gain_fid=0.511` (did not beat iter 30 EMA **5.798**). Teacher FD mv **0.979** / dv **0.954**. Gate 82 ~1.4 h away; unfreeze ~147 is the A/B.

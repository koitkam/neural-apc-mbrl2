...
| **Critic** | `critic_rew_to_tgt_var` (>0.015) | none healthy | — | P68 val `critic_r` **+0.699** PASS (P66 +0.698; P64 +0.705); P3 rtgt still collapses. Freeze rscale KEEP. |
| **Actor** | econ vs baseline + vs expert BC | **P64** (mbrl2) | paired **−4.54 vs −94** | Env-free P2 amp=P3. 9/9 `all_pass`. **P68 −3.90 vs −85 VALID 9/9** (slightly better econ) but mv_viol **1.92 vs 0.33** and TM **×0.69** — not a champ swap. P66 **−19.85 vs −104**. P53 stays the μ-ratio-clip RCA. |
...
| P72 | 2026-09-02 | restore gain-c in GRU / TSSM token (`recurrence_c_dim=cont_dim`; P71 REVERT) | LIVE P2. Storm 2/2 @16 CAPPED extra-P1. Freeze **GAIN-READY 0.87@DV** last_ok **15**. Teacher last-ok ×1.001/×1.055. P2 iter ~60 recon ~0.011 dobg ~0.10 buf 327/327. jsonl ×1 ≠ val TM. | 🔬 LIVE |
...
### p72 (`run_p72_gaingru`, branch `cursor/p28`) — LIVE: G back in GRU
- **One attributed GPU change vs P71:** `recurrence_c_dim=cont_dim` so env-free gain-only GRU sees `[z, c_gain, a, dv]` again. **No new TrainConfig field.** Env-free: **no leftover `DREAMER_*`**. tmux `mbrl2_p72`, pid **212027**, sha `b009a7e`, out-dir `output/test_sim/run_p72_gaingru`.
- **Step 4 CONFIRMED:** `device=cuda` bs=128 `wm_train_T=128` horizon=55 compile=eager. `[cont-latent] GAIN-ONLY` gain_dim=2. `[resolved-cfg] isolation=0 gmatch_len=55 gmatch_ol_tail=0 gmatch_step=0.4 gmatch_clip=True gmatch_rest=True`. Rest-IC graph captured `N=6 T=128`. Process environ: `CUDA_VISIBLE_DEVICES=0` only (no `DREAMER_*`). nvidia **~14 GB** one job.
- **LIVE (2026-09-02 02:52, P2 iter 60):** Storm **1/2 @6** restored last_ok **5**. Teacher **pinned @8–15** MV ×0.98–1.01 DV ×0.96–1.14 (jsonl). Storm **2/2 @16** (recon 0.109, skip 68, MV ×0.684 DV ×1.88) CAPPED extra-P1. Gate last-ok **15** **GAIN-READY** DC **[0.89, 1.20]** @H **[0.87, 1.18] worst=0.89@DV**. Detonated-freeze restored 15, re-probe **0.87@DV** (MV 1.17/@H 1.15). `wm_best` restore SKIPPED (gain-blind iter 10/20/30). Rest-IC graph **released**. P2 recon **0.009–0.011** (vs P64 ~0.002; freeze recon_best **0.010**). `dobg` rising **0.02→0.10** (P64 P2 first35 med 0.063; P68 0.055). `dob_d` **~0.07–0.09** (P64 first35 0.036). `buf_fill` **327/327**. Probe60 gain_fid 0.646 (correlation). last_ok **15 vs P64 82 / P68 73** — early freeze is the val-TM confound; do not score actor until val. jsonl ×1 ≠ val TM. Dummy `gain_match_ol_tail_*` jsonl **REMOVED** (HEAD; not this pid). **No second GPU job.**
- **Watch at EXIT:** freeze vs P64 **0.91@DV** (this pid **0.87**, band PASS). Val 1step→OL / OL-vs-real vs P64 **×0.85 / ×0.84**; ss toward ×0.93. `actor_experiment_valid` should be true (GAIN-READY). Do not treat last_ok=15 as a P64-class observer until val TM. After P72 GAIN-READY, next **new** family is compounding ≠ window/hold/routing, or DOB-reg vs ground (not skip/flush).
...
| Dummy jsonl `gain_match_ol_tail_{len,loss,mv_ratio,dv_ratio}` (always 0 after P69 REVERT) | **P72 GPU-occupied** (pid **212027** P2) | **REMOVED** (`_gain_match_ol_tail_len` + dummy diag tensors; banner still `gmatch_ol_tail=0`) | YES — same class as `wm_held_ol_ratio`. Training graph unchanged. Not in this pid. CVD="" smoke. |
| Gain-c decoder/feat only (not in GRU / TSSM token; `recurrence_c_dim=cont_dist_dim`) | **P71 EXIT** (`run_p71_nogainc`, pid **208659**, sha `5a16c67`, 93 iters) | **REVERT / FALSIFIED** | YES — G=f(h) made G a slave of contracting h. CAPPED **0.76@DV** last_ok **38**. Val MV **×1.089 / ×0.899** DV **×0.701 / ×0.759**. 1step→OL **×0.785** OL-vs-real **×0.748** (P64 ×0.85/×0.84). DV prior **×0.889** (P64 faithful ×0.987). jsonl ×1 ≠ val TM. Same family as P69–P70. **Do not** routing N+1. P72 restores G in GRU. |

...
- **LIVE (2026-08-26 18:01, tmux `mbrl2_p31`, GPU ~18.6 GB):** env-free. Train-start `latent=deterministic compile=eager skip_invalid_p3=True storm_cap=2 device=cuda`. `[resolved-cfg]` same + `gain_match=1 isolation=1 ss_match=3 n_critics=2 rs_freeze=True restore_p2=False`. No leftover `[env-override]`, no `torch.compile` banner. STAGE 1 @iter0. **No second GPU job.**
...
- P31 launch: env-free `cursor/p28` (deterministic + eager + `storm_cap=2`). Do not pass leftover P26 env-vars (`DREAMER_COMPILE=0`, `DREAMER_RSSM_LATENT_TYPE`, `DREAMER_DOB_ENABLED`, `DREAMER_ACT_HIST_REQUIRED`). Opt-in compile is `DREAMER_COMPILE=1`. Live iter banner prints `kl`/`jemb`/`iso`/`ss` + `storm_cap=`. jsonl: `wm_ss_match_loss`, `wm_ss_match_wmean`, `wm_isolation_traj_loss`. `skip_invalid_p3` default-on. Isolation K-step decode is batched; overshoot MSE is vectorized (same mean). Variance-normalized isolation waits until a **completed** det+eager P1 (not a cap-at-iter-18 freeze) still shows DV ss ~×0.87.

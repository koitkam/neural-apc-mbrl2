...
| P70 | 2026-09-02 | OL hold of `c[..., :cont_gain_dim]` after first `img_step` (P69 tail REVERT) | LIVE `run_p70_cgainhold` pid **204329** sha `35af8cf`. Step 4: `device=cuda` `gmatch_len=55` `gmatch_ol_tail=0` rest-pre KEEP isolation=0. Iter 1 teacher ×0.640/×−1.06 (P68-class, not held-K tautology) skip 0 `t_wm` 99 s ~14 GB. | 🔬 LIVE |
...
| OL hold of continuous gain block after first prior step (`hold_gain_c`) | **P70 LIVE** (`run_p70_cgainhold`, pid **204329**, sha `35af8cf`) | LIVE | — G is a plant property. 1-step prior still infers G; remaining OL copies it. Isolation k=0 still trains `cont_prior`. No new knob. Step 4 CONFIRMED `gmatch_ol_tail=0`. |

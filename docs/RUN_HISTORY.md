...
| P71 | 2026-09-02 | gain-c decoder/feat only — not in GRU / TSSM token (P70 hold REVERT) | Planned `run_p71_nogainc`. Dist-c still enters the core (`recurrence_c_dim=cont_dist_dim`; env-free 0 ⇒ `[z, a, dv]`). No new knob. | 🔬 next |
...
| Gain-c decoder/feat only (not in GRU / TSSM token; `recurrence_c_dim=cont_dist_dim`) | **P71** (`run_p71_nogainc`) | next | — Compounding was G fading through `h`; P70 hold kept G in recurrence and detonated. Dist-c still a state. Env-free `cont_dist_dim=0` ⇒ GRU sees `[z, a, dv]`. No new knob. Watch freeze vs P64 0.91@DV. |

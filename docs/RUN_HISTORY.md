...
- Abs Huber gain-match is count-equal per input but **scale-unequal** when `|G_mv|≫|G_dv|`. P43 EXIT: per-input β **equalized jsonl Huber** (mv≈dv ~1e-4) but val DV still **×0.740 / @H ×0.849** (P40/P41/P42 class). Residual was already ~0 — β is not the DV lever. Identified G matches TM real (MV −0.32, DV 0.18); val @H ≈ ss so `gain_match_len`=H vs TF 4×H is **not** the miss. Teacher IC (PRBS posterior FD) ≠ TM rest-then-step. HEAD `gain_match_settle_len` auto=H (P44). Scalar `gain_match_huber_beta=1` stays the A/B (`DREAMER_GAIN_MATCH_HUBER_PER_INPUT=0`). Do **not** revive relative Huber. Do not try another Huber reweight.
...
| Gain-match FD from PRBS posterior (no held settle) | P26–P43 | SUPERSEDED (HEAD `gain_match_settle_len` auto=H; P44 GPU) | YES — Huber~0 at excited IC; TM rest-step 0.75@DV at ss and @H. Identified G = TM real. Horizon H vs 4×H falsified (@H≈ss). |

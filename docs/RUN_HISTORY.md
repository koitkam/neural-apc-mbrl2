...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 45**. Probe40 **H=1 r=+0.554 / H=55 r=+0.267 conv=0.00** gain_fid **0.524**. recon **0.009@44 best** (P64 @45 **0.0021**; ~4×). Teacher **×1.01 / ×0.91 @45**. persist_rel **0.24@43 → 0.27@45** (plateau after inject rebound). pid **242511**. | 🔬 LIVE P1 — persist plateaued ~0.26. H=55/conv still dead. Do not patch live. |
...
- **LIVE P1 iter 45 (~236 min, nvidia 11.7 GB, `t_wm` ~307 s, sps 19.7):** skip **0**. Teacher jsonl 43 **×0.991 / ×1.016** → 44 **×0.995 / ×1.030** → 45 **×1.008 / ×0.911** (DV undergain). recon **0.011@43 → 0.009@44 new best → 0.011@45** vs P64 **0.0021@45** (~4×). last_ok **45 unlocked**. persist_rel **0.239@43 → 0.258@44 → 0.273@45** (plateau after 0.82@41 inject rebound; not a re-spike). Rest-IC graph not captured. Probe 40 still last fidelity. Next typically iter 50.

...
| P48 | 2026-08-29 | P3 collect/val stream measured DV + Kalman | LIVE P1 iter **75** recon 0.002 skip 0 last_ok **locked 24** (iter-25 recon 0.145 / 43× wrap) teacher FD ~×0.99/×1.00 `wm_best`@70 | ⏳ env-free; GPU occupied pid **34706**; freeze restores 24 (P41 lock class). No second job |
...
### p48 (`run_p48_collectdv`, branch `cursor/p28`) — LAUNCH: P3 collect/val = training observer feat
- **One attributed GPU change vs P47 identity:** `stream_serve_step` — P3 `collect_episode` and val closed-loop stream measured DV into the GRU and Kalman `d_t` when DOB is live, matching `rollout_observed`. Smoke: serve feat ≡ re-encode (max|Δ|=0). Not a knob.
- **Not:** `DREAMER_P3_RESET_LOG_STD` (REVERT the A/B; default False). Extra critic knobs. Isolation reweight. Rest-IC settle=`wm_tf_horizon`.
- **Step 4 resolved-cfg vs P47:** env-free. `p3_sigreset=False` (P47 True). `sigma_min_ratio=1.2` now actually resolves (P47 pid still floored 1.3 — leftover landing, not the A/B). Banner otherwise identity: `latent=deterministic restore_p2=False gain_match=1 dob_ground=2 isolation=0 ss_match=0 iso_dcv=off n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 lock=20 huber_per_in=True gmatch_settle=-1 gmatch_rest=True p3_sigreset=False compile=eager`. Watch `[p3] on-policy collect streams measured DV + Kalman`.
- **Watch:** first P3 ent (expect σ_min ≈−0.34 from ratio 1.2, not −0.101); unfreeze actor_loss not −300; mvv/reversal; rtgt>0.015; val econ vs baseline **and** vs P45 −216 / P47 −221; TM MV+DV ss/@H + det_r. If still bang-bang / worse than OL → collect was not the lever (re-diagnose; do not restack σ-reset).
- **P48-live (GPU occupied, no second job):** leftover plant-SNR `SIM_OU_*` / `SIM_MEAS_*` / `SIM_NOISE_ADAPTIVE` **and** wrapper `SIM_NOISE_SEED` / `SIM_NOISE_JITTER_PCT` / `SIM_DOMAIN_RANDOMIZATION` / `SIM_NOISE_ENABLED` → TrainConfig + `ENV_OVERRIDES` (identity ON / 0.008 / 0.15 / 0.60 / 0.005 / 0.010 / jitter 0.20 / DR ON). Dual-read at bake + wrap. Dead `SIM_NOISE_AMPLITUDE_JITTER_PCT` now zeros SysID jitter (`clean_mode`). `SIM_NOISE_CONFIG_JSON` stays env-only (path). P1 iter **75**: recon 0.002 skip 0 teacher FD MV/DV ~×0.99/×1.00 (not val TM). `wm_best` iter 70 (gain_fid=0.907, H=55 r=+0.546). **last_ok locked iter 24** (iter-25 wrap 43×; P41 class). Freeze restores 24. Storm 0. Do not retune lock. Do not launch. Do not promote `p3_reset_log_std`.
...
- P3 collect/val streams `stream_serve_step` (measured DV + Kalman when `dob_active`) so served `feat` matches `_realsim_actor_critic_step` re-encode. P47 EXIT Adam-complete **FALSIFIED as yank lever**.
...
| P3 collect `_posterior_step`/`obs_step` with `dv=None` `obs=None` | P45–P47; P48 LAUNCH | SUPERSEDED (`stream_serve_step`) | YES — served feat ≠ `rollout_observed` re-encode (GRU zero-fill DV; `d_t` decay from 0). P47 bang-bang 0.53 / econ −221 vs −121. |
...
| Leftover `SIM_NOISE_SEED` / `SIM_NOISE_JITTER_PCT` / `SIM_NOISE_ENABLED` / `SIM_DOMAIN_RANDOMIZATION` `os.environ.get` | P48 GPU-occupied | SUPERSEDED (TrainConfig `sim_noise_enabled`/`sim_noise_seed`/`sim_noise_jitter_pct`/`sim_domain_randomization` + `ENV_OVERRIDES`; identity ON / '' / 0.20 / ON; dual-read at wrap) | YES — wrapper jitter/DR missing from `run_plan`. Dead `SIM_NOISE_AMPLITUDE_JITTER_PCT` (SysID `clean_mode`) now actually zeros jitter. Wrapper now honours `SIM_NOISE_ENABLED=0` (was softsensor-only). Leftover `DREAMER_DOMAIN_RANDOMIZATION` alias now read. |

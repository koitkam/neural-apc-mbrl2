...
| P51 | 2026-08-29 | P3 REINFORCE stop-grad log_std (`p3_stop_grad_log_std=True`) | **LIVE P1** iter **59** (~2h10). pid **54815**, launch `d6a4511`. Lock@**47** recon 0.4725; skip-storm **1/2** restore 47 @52; last_ok **59 unlocked**. recon **0.0018** skip banner 98. Iter 56 healthy (no P50-class wrap). bc **MSE**. Env-free. Do not launch. | 🔬 storm 1/2 GPU-confirmed; watch 2nd wrap ~70, gate 82, unfreeze yank |
...
- **Step 4:** `[resolved-cfg] … bc_mean=True p3_sglogstd=True compile=eager`. Process environ: only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. STAGE 1. P1 iter **59** (~2h10) recon **0.0018** (min so far) bc **0.284 MSE** (0.24–0.38, never NLL) last_ok **59 unlocked** teacher FD ~×1.02/×0.99. GPU ~11 GB.
- **Wrap / skip-storm (LIVE):** lock@**47** (recon **0.4725** > 20× best 0.0020; snapshot is healthy iter 47 recon 0.0027). Iter 48–52 detonated (recon 0.47→**1.01**→0.31; FD collapsed ×0.02 @48). **Skip-storm 1/2** @52 restored `wm_last_ok` iter 47, unlocked, extension kept. Iter 53 recon 0.0033; last_ok advancing. **Iter 56 healthy recon 0.0019** (P50 wrap/lock was @56 skip 0 — this pid used skip-storm, not wrap-unlock). Storm 1/2 used (2/2 still caps). Banner skip 98 is the detonation window.
- **Watch:** 2nd wrap ~70 (P50 lock@70 unlock@73); GAIN-READY gate 82; P2 bc MSE ≥0; first P3 ent ≈ H(σ_init)≈−0.08 not −0.283; **unfreeze must not yank** (P50 169: −0.107→−0.268, logp_std 0.56→54). Remaining leftover is env-only paths — do not edit `evaluation/validate.py` / `wm_transfer_matrix.py` / `dreamer_v4_rssm.py` while this pid is live.

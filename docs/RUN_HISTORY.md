...
- **Liveness:** **GAIN-READY P1→P2 @82** (no extra-P1). Live probe worst **0.88@DV**. last_ok **82**. Skip-storm **1/2 @73** recovered. STAGE 2 DOB id, g FROZEN. P2 iter **121** recon **0.0017** dobg **0.009** last_ok **82**. wm-fid @**120**: H=1 r=+0.756 H=13 r=+0.665 H=27 r=+0.712 **H=55 r=+0.596** conv=1.00 drift=0.009 gain_fid **0.634** best_h=55/55 (not the TM gate; @H up vs @110 +0.564 / gain_fid 0.579, still below @90 +0.627 / 0.935). Live EMA **6.073**; best **6.845** @90 held. Expert-inject P2. Next probe ~130. Still P2 (not P3). sps ~288 post-inject. GPU ~11 GB. ~195 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. Watch first P3 ent ≈ H(σ_init); unfreeze ~147. Do not KEEP/FALSIFIED as cascade until EXIT.

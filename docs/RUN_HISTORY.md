...
| P50 | 2026-08-29 | P1/P2 BC MSE-on-μ (`bc_mean_only=True`) | **LIVE** P1 iter **48**. Env-free pid **49389**. recon best **0.0015** (now 0.0031) skip **0** last_ok **48** unlocked. **bc +0.27…+0.41 MSE 48/48 pos.** Entering skip-storm window. Do not launch. | 🔬 live; watch storm ~50–65; P2 `bc_loss`>0; first P3 ent ≈ **−0.08** not −0.283 |
...
- **LIVE (2026-08-29 18:20 CDT, P1 iter 48):** recon best **0.0015** @42 (iter 48 **0.0031** = 2.1×, not wrap). skip **0** last_ok **48** unlocked. **`bc_loss` +0.27…+0.41 MSE 48/48 pos** (never NLL). Teacher FD ~×1.0. gnorm blips @5/@23/@33 recovered. Fidelity wm_best still iter **30** (gain_fid 0.928); @40 gain_fid 0.546 no new best. **Entering skip-storm window (~50–65)** with no storm yet. GPU ~11 GB. First P3 ent target ≈ **−0.08**, not floor **−0.283**.

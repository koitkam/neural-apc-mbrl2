...
| P32 | 2026-08-27 | last-ok at detonated freeze; continue-after-storm | storm 1/2 @53 KEEP; **P1→P2 CAPPED GAIN_NOT_READY 0.71@DV** (healthy recon, extension closed) | 🔬 LIVE P2; P3 will `[p3-skip]`; next: keep ext on storm 1 |
...
- Isolation/ss-match trajectory MSE is still absolute (same family as abs Huber). Variance-normalized isolation waits for a **healthy** freeze still showing DV ~×0.87 — P32 freeze is GAIN_NOT_READY 0.71@DV because extension was closed, not a DV-Huber verdict. Do not revive relative Huber.
...
- P32 launch was env-free det+eager `storm_cap=2` + detonated-freeze last-ok. **P32 P1→P2:** healthy recon CAPPED GAIN_NOT_READY 0.71@DV because storm-1 continue closed extension. HEAD (not in P32): keep extension on storm 1; isolation TBPTT chunked `img_rollout`; Stage-1 prior-core skip; TSSM `img_rollout`; DV-PRBS DRY. Next GPU job after EXIT: that keep-ext continue. Do not pass leftover compile/latent/DOB env-vars. Variance-normalized isolation still waits for a **healthy GAIN-READY** freeze.

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 33**. Probe30 **H=1 r=+0.403 / H=55 r=+0.172 conv=0.00** gain_fid **0.387**. recon **0.027@33 best** (P64 @33 **0.0024**; ~11×). Teacher **×1.00 / ×1.06 @33**. persist_rel **0.40@31 → 0.055@33**. pid **242511**. | 🔬 LIVE P1 — H=1 at floor; H=55 dead; **conv=0 @10/20/30**. Persist collapsed. Do not patch live. |
...
- **LIVE P1 iter 33 (~173 min, nvidia 11.7 GB, `t_wm` ~309 s, sps 19.6):** skip **0**. Teacher jsonl 31 **×0.979 / ×1.027** → 32 **×1.016 / ×0.896** → 33 **×1.004 / ×1.060** (DV still wobbling). recon **0.035@31 → 0.028@32 → 0.027@33 new best** vs P64 **0.0024@33** (~11×; P76 @33 0.012). last_ok **33 unlocked**. persist_rel **0.40@31 → 0.082@32 → 0.055@33** (P73 EXIT was 0.037@81). Rest-IC graph not captured. Probe 30 still last fidelity. Next typically iter 40.

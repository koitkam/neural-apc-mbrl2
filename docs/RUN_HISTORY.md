...
- **Liveness:** still **LIVE** pid **75966**, tmux `mbrl2_p53`, GPU ~11 GB. ~214 min wall. Env-free still only `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF`. jsonl through **P3 iter 223**. last_ok **82 unlocked**. **Passed P52 ES iter 209** still LIVE (P52 `entropy_collapse_window` @209 ema **−2231**; P53 ema **−116@209 / −123@223**). `best.pt` iter **166** det **−46.2** (P51 best 161 det −87; P52 176 det −634). Do not KEEP/FALSIFIED as cascade until EXIT.
  ...
  - **Unfreeze 147 (A/B canary — no cascade verdict yet):** ent **−0.101 HELD** through **223**. rscale **2.302 FREEZE**. actor_loss bounded. Unclipped logp_std **held ~0.50–0.55**. clip_frac **~0.17–0.23**. mvv **56.8k@191** then **0@206 / 211@216 / 0@221** (transient). ret_w **~−85 to −91**. ema **−119@207 → −116@209 → −123@223**.
  - **Passed P52 delayed-rail @165–169 and ES @209:** P52 std **80.8/100.7** then ES @209. P53 std **~0.51@209**, still LIVE. `adv_corr` flickers 0.004–0.145 (not a sticky 30-iter <0.05 window yet).
  - **Watch (not a verdict):** rtgt plateau **~0.0015**. `best.pt` **166 / −46.2** is early-unfreeze (19 iters after 147) — same “early best during P3” class as P50/P51; val will judge. entropy_collapse can still fire if adv_corr stays <0.05 while H≈−0.101 < thr −0.083.
- **Hypothesis:** in-support `(u−μ)/σ²` SGD cannot walk μ onto the tanh rail once `π_new(a)/π_snap(a)` leaves `[0.8, 1.2]`. **So far:** unclipped std held past P52 rail+ES windows; mvv transient; rtgt low but plateaued. Do not call KEEP/FALSIFIED as cascade until EXIT.
- **Watch:** entropy_collapse window / late ema vs P50 **−56**. Do not call KEEP/FALSIFIED as cascade until EXIT.
...
| P3 in-support μ-walk (\|logp\|<8 SGD) | P52 EXIT; **P53 LIVE** (`run_p53_muratio`) | LIVE (`p3_mu_ratio_clip=0.2` vs frozen unfreeze-μ) | YES — unfreeze 147: ent HELD −0.101; logp_std **held ~0.54 @207** (not P52 80+); mvv **56.8k@191 then 0@206** (transient); rtgt plateau **~0.002** not 1e-4; ema **−119@207** (P52 −2231@209). No cascade verdict until EXIT. Opt out `DREAMER_P3_MU_RATIO_CLIP=0`. |

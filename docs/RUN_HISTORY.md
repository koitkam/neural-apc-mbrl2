...
| P46 | 2026-08-29 | P3 log_std reset (`DREAMER_P3_RESET_LOG_STD=1`) | P1 live ~iter 44 recon 0.004 skip 0; GPU occupied | ⏳ one attributed change vs P45 identity |
...
- **P1 live (2026-08-29 ~01:55, iter ~44, pid 24426, ~1.6 h):** recon **0.002–0.004**, kl=0, jemb>0, gmatch ~1e-4, skip 0, last_ok advancing, teacher FD MV/DV ~×1.00 (not TM). Probe iter 20 gain_fid 0.676; iter 30 **0.945** (wm_best); iter 40 **0.534** (gain-blind spike, same class as P45 iter 40 0.566). mean `t_wm` **125 s** vs P45 **135 s** (this pid has last_only + `_posterior_step` rest-IC). Matches P45 early-P1. **Do not launch a second GPU job.**
...
- **GPU-occupied (no second job):** held-rollout `win` clamped to `(K-1)//4` (identity 8 at test_sim K=55; K=15 was a **dead** term). P3 collect still `dv=None` (reviewed, not patched). P3 `set_dob_active(False)` + `obs=None` still leaves `d_t≡0` in the actor belief. P1/P2 BC is still NLL. CUDA-graph rest-IC encode deferred (needs a GPU smoke; A10 occupied).

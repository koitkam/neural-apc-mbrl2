...
| P46 | 2026-08-29 | P3 log_std reset (`DREAMER_P3_RESET_LOG_STD=1`) | P1 live iter **75**; last_ok **locked 58** skip 0 (P45 unlocked); recon 0.0026; **Do not launch** | ⏳ σ-reset not yet at P3
...
### p46 (`run_p46_p3sigreset`, branch `cursor/p28`) — P1 LIVE: P3 log_std reset
- **One attributed GPU change vs P45 identity:** `DREAMER_P3_RESET_LOG_STD=1`. At P3 entry, zero the log_std half of the last Linear so σ = auto-tuned `policy_init_log_std`; leave μ (BC launchpad) intact. Rest-IC is now the **default** (P45 EXIT PROMOTE) — not a second A/B.
- **Not:** rest-IC settle=`wm_tf_horizon`, extra critic knobs, leftover `DREAMER_GAIN_MATCH_REST_IC`, compile/latent/DOB.
- **Step 4 resolved-cfg vs P45:** identical except `p3_sigreset=True`. Banner: `latent=deterministic restore_p2=False gain_match=1 dob_ground=2 isolation=0 ss_match=0 iso_dcv=off n_critics=2 rs_freeze=True skip_invalid_p3=True storm_cap=2 lock=20 huber_per_in=True gmatch_settle=-1 gmatch_rest=True p3_sigreset=True compile=eager`. Confirmed `[env-override] p3_reset_log_std=True` only; device=cuda eager; rest-ic N=6 L=128 settle=128.
- **P1 live (2026-08-29 ~03:05, iter **75**, pid 24426, ~2.8 h):** recon **0.0026**, kl=0, jemb 0.006, gmatch ~1e-4, **skip 0**, teacher FD MV/DV ~×1.00/×0.99 (not TM). Probe 20/30/40/50/60/70 gain_fid 0.676 / **0.945** / 0.534 / 0.791 / 0.660 / **0.867** (iter 60 best_h=0/55 after the spike; iter 70 recovered 55/55). `wm_best` still iter **30** (gain-blind). After lock, zrank 865→730 and alive 1024→1014 (watch at gate 82). mean `t_wm` **125 s** vs P45 **135 s**. **Do not launch a second GPU job.**
- **last-ok lock vs P45 (Control/ML checkpoint):** recon spike **iter 59** 0.1799 (P45 0.2864@58 then skip-storm). P46 **skip stays 0** so lock **holds** at last_ok **58** (jsonl locked True through 75). P45 skip-storm **unlocked** @59 and last_ok advanced to **78** before gate 82 PASS. Gate 82 still probes **LIVE** weights (P45 did). Freeze restores last_ok only if detonated. Iter 65 recon 0.060 recovered (skip 0). **Do not unlock-on-recovery** (that is the P42 overwrite hole).
- **Watch:** `[p3] reset policy log_std residual`; first P3 entropy **not** −0.363 (toward init clamp ≈−1.52 → H≈−0.10). Warmup rtgt stays >0.015. After unfreeze: no actor_loss −300; no entropy-collapse; econ vs baseline **and** vs P45 −216 / P27 BC −59; MV not railed. Storm 2/2 or GAIN_NOT_READY → observer regression (rest_ic default). If entropy still at floor → **REVERT** reset (residual not in last Linear). If σ opens at P3 entry then re-collapses at unfreeze → P46 pid is **weights-only**; HEAD zeros Adam log_std-row moments (same lever). If σ opens but still limit-rides → do **not** stack critic knobs; next is collect-DV / collect-DOB (train/serve) or DR-timing. **Do not** also change collect-DV / P3 `dob_active` until this A/B verdicts.
- **GPU-occupied (no second job):** held-rollout `win` clamped to `(K-1)//4` (identity 8 at test_sim K=55). Seed op-bands promoted to TrainConfig + `ENV_OVERRIDES` (env-free identity). P3 collect still omits `dv` (GRU zero-fills) and skips Kalman. P1/P2 BC is still NLL. CUDA-graph rest-IC encode deferred (needs a GPU smoke; A10 occupied). Rest-IC encode L=`lookback` (test_sim 128); shortening to few-τ is a later A/B.
...
| min-of-2 critics + freeze `return_scale` | P27/P28; **P45 first valid P3** | freeze **KEEP**; min-of-2 **FALSIFIED as sufficient** | YES — rscale 1.15→**2.18** (not 49.5). Warmup rtgt 0.08 then **0.0004** after unfreeze. Entropy pinned σ_min **−0.363** from P3 iter 137 (P1/P2 BC). Econ −216 vs −92. Do not stack more critic knobs. |
| P3 `reset_log_std` (zero last-Linear log_std rows; μ kept) | P46 launch (`run_p46_p3sigreset`, pid 24426) | pending (default False; opt-in env) | P45 RCA: BC pins σ_min → railed warmup → REINFORCE explodes. `DREAMER_P3_RESET_LOG_STD=1`. If entropy still at floor → residual not in last Linear (REVERT). |
| P3 reset_log_std weight-only (Adam P2 NLL moments live) | P46 GPU-occupied (pid 24426 weights-only) | SUPERSEDED (zero log_std-row `exp_avg`/`exp_avg_sq`; μ rows kept) | YES — first `opt_actor.step` after warmup would re-collapse σ. Same `p3_reset_log_std` lever, not a new knob. |
| P3 on-policy collect `obs_step` (unused prior heads) | P46 GPU-occupied | SUPERSEDED (`_posterior_step`; `obs=None` Kalman already off) | YES — next GRU input is this posterior; rest-IC smoke identity. Skip `prior_net`/`cont_prior_net` every P3 step. |
...
| Extra-P1 last-ok overwrite after silent recon spike | P40 EXIT iter 98 (83→104); P41 EXIT original-P1 iter 64 (56→64 then 104); **P42 EXIT freeze 66**; **P45 storm 1/2 unlock** | KEEP lock as 20× fire; **FALSIFIED as DV pin / det_r-collapse fix**; <5× overwrite-prevention **untested**; skip-storm unlock **GPU-confirmed P45** | YES — P42 locked last_ok **66** @iter 67 (170×) and freeze restored 66. Extra-P1 recon stayed ≥14× so overwrite never opened. Val MV **×1.179/×1.161** DV **×0.737/×0.804** det_r **0.124**. P45 locked 57 @iter 58 (recon 0.2864), skip-storm @59 restored and **unlocked**; last_ok advancing (77). Wrap 5–10× untested. |

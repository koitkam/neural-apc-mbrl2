...
| P47 | 2026-08-29 | P3 σ-reset + Adam log_std-row zero | launching `run_p47_p3sigadam` | ⏳ same env as P46; attributed code is Adam-complete |
...
### p47 (`run_p47_p3sigadam`, branch `cursor/p28`) — LIVE P2: Adam-complete σ-reset; GAIN-READY 0.90@DV
- **One attributed GPU change vs P46 pid:** HEAD `reset_log_std(opt)` zeros Adam `exp_avg`/`exp_avg_sq` on log_std rows (μ rows kept). Same `DREAMER_P3_RESET_LOG_STD=1`. Not a new knob. Collect `_posterior_step` / held-rollout win clamp / seed-band whitelist / clip-cal-DIAG whitelist are identity.
- **Not:** collect-DV, P3 Kalman, extra critic knobs, leftover compile/latent/DOB, promote `p3_reset_log_std`.
- **Liveness:** **P2** (iter **~109**, recon **0.0026**, `dobg` **0.003–0.010** live, skip **167** frozen not restorming). GPU python pid **29434**. tmux `mbrl2_p47`. Launch `0acde1d`. Storm **1/2** then **2/2** @iter 77–80 (`_force_p1_cap_at` **727120**). Lock last_ok **77** (recon 0.2226 > 20× best 0.0014); skip-storm restore **unlocked** then recapped. Gate **PASS iter 80** GAIN-READY worst **0.90@DV** (MV ×0.95/@H ×0.94, DV ×0.90/@H ×0.97). Detonated-freeze restored last_ok **77**; freeze probe **0.91@DV**. Warm-restore SKIPPED. `[curriculum] STAGE 2` g frozen. Probe iter 90: H=55 r=+0.413 gain_fid=0.560 (P2 DOB; freeze was 0.876@70). Better first-gate DV than P45 0.86 / P46 0.88. Watch P2→P3 ~iter 134; first P3 ent not −0.363; unfreeze must not yank. **Do not launch a second GPU job.** GPU-occupied: leftover eval TM + horizon formula + IC DR + `wm_overhead` → TrainConfig + `ENV_OVERRIDES` (identity). CPU smoke. Do not patch collect-DV / P3 Kalman. Do not promote `p3_reset_log_std`.
...
| P3 `reset_log_std` (zero last-Linear log_std rows; μ kept) | P46 EXIT (`run_p46_p3sigreset`, `62cf1f5`, 220 iters) | **KEEP-AS-OVERRIDE** (default False) | PARTIAL — residual **is** last Linear (ent −0.101). Opening σ **not** sufficient: warmup mvv 14104; unfreeze actor_loss −357; econ **−256 vs −129**. Weights-only Adam yank 147 −0.101→−0.323. Do not promote. Next: P47 Adam-complete. |
...
| Leftover eval `DREAMER_WM_TF_*` / `DREAMER_VAL_WM_*` `os.environ.get` | P47 GPU-occupied (pid 29434 P2) | SUPERSEDED (TrainConfig + `ENV_OVERRIDES`; identity) | YES — TM protocol + val-suite gates missing from `run_plan`. Horizon/settle 0 = auto `max(80,4H)` (already sim-adaptive). Levels/span/step_frac unitless. `DREAMER_WM_DIAG_DEVICE` stays env-only. `DREAMER_PROCESS_NOISE_AMP_RAMP` / `DREAMER_HIDDEN_DIST_*` stay env-only (noise curriculum). |

...
- **Liveness (08:09 CDT):** LIVE **P2** after CAPPED. tmux `mbrl2_p41`, pid **4185725**, launch `acb8a7b`. GPU ~15.7 GB. **No leftover `DREAMER_*`.** last_ok **104**. **No second GPU job.** Expect `[p3-skip]`.
...
- **CAPPED iter 104 (cap 175924) GPU-confirmed:** cap-time **gain-probe** (P40 cap-time still said `not_plateaued` + 0.75@DV). **GAIN_NOT_READY worst=0.74@DV** (MV **1.02**; *slightly worse* than 0.76). Freeze recon **0.0028**. Warm-restore **SKIPPED** (storm used last-ok iter 38; wm_best gain-blind). STAGE 2 g FROZEN. P2 iter **105** recon 0.0036 **gmatch 0** **dobg 0.0038 live**. Extra P1 FALSIFIED as DV lever (0.76→0.76→**0.74**). Expect `[p3-skip]`. Actor INVALID.

...
| P62 | 2026-08-31 | held-rollout decode-CV stationarity (`out='obs'` + CV late−early; no new knob) | **LIVE** tmux `mbrl2_p62` pid **160203** sha `6841f5c`. CONFIRMED `device=cuda` `held_cv=True` `gmatch_clip=True` `gmatch_step=0.4`. Iter 1 held **0.00166** (P61 later ~6e-4 on `h`); `clip_frac` **0.083**. | 🔬 Watch jsonl held vs P61; val 1step→openloop vs ×0.772; paired vs P58 |
...
- **Watch:** jsonl `wm_held_rollout_loss` staying above P61's silent ~6e-4 once recon is healthy; live gate TM vs P60/P61; val 1step→openloop vs **×0.772**; MV/DV ss/@H vs P26 ×0.97 / P61 ×0.91/×0.77; det_r vs P61 0.153 / P59 0.577; paired vs P58 **−6.24**. Do not FALSIFY from iter-1. **One GPU job at a time.**
...
| Held-rollout late−early on GRU `h` (`out='h'`) | **P61 EXIT** (`run_p61_clipdu`; P1 median ~6.4e-4 vs overshoot ~0.041) | SUPERSEDED (P62 `out='obs'` + `cv_index_t` late−early; still gain-neutral; no new knob) | YES — p139: z/c wander while `h` sits; compounding is decoded-CV contraction (1step-faithful / open-loop ×0.77). Isolation TBPTT stays `out='h'`. |

...
| P61 | 2026-08-31 | env-free cube-clip + realized Δu for gain-match FD | LIVE `run_p61_clipdu` pid **155682** sha `c4ed6f3`. CONFIRMED `device=cuda` `gmatch_clip=True` `gmatch_step=0.4` `clip_realized=True`. Isolation off. μ-ratio 0.2. Serve graph KEEP. | LIVE |
...
- **LIVE ~iter 24 (2026-08-31 18:00 CDT, pid 155682, ~43 min):** P1 healthy. recon **0.0025–0.005**, jemb **0.011**, gmatch **~4e-4**, teacher MV/DV **×1.005 / ×0.996**, `du_frac=1.0` (expected: reverse keeps `|Δu|`; this pid has no `clip_frac` key). `t_wm` **~98 s** (P58/P59 class). skip **0/0**. last_ok **24**. Rest-IC graph captured. Fidelity probe @20 `best_h=55/55` gain_fid=0.692. **No second GPU job.**
- **HEAD this session (GPU occupied):** jsonl `gain_match_clip_frac` = mean `+step`-would-clip (identity loss); rest-IC last-frame `a_base`/`dv0` + FD `du` cached with `a_seq` (100×/iter identity). CVD="" smoke green. Not in pid 155682.
...
| jsonl `du_frac` as “did the cube fire?” | P61 LIVE (`run_p61_clipdu` pid **155682**) | SUPERSEDED (`gain_match_clip_frac` = mean `+step`-would-clip; HEAD, not this pid) | YES — reverse keeps `|Δu|` so du_frac stayed **1.0** through iter 22 even if outer rest+jitter hits the rail. Loss identity. CVD="" smoke interior 0 / rail 1. |

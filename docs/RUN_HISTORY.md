...
| P55 | 2026-08-30 | PPO μ-ratio recopy every P3 iter (`p3_mu_ratio_refresh_iters=1`) | LIVE env-free `run_p55_murefresh` pid **103504** sha `b321d89`. **P2** iter **~90** after **GAIN-READY @82** worst **0.88@DV** (MV 0.89/@H 0.86, DV 0.88/@H 0.91); last_ok **82 unlocked**. Skip-storm 1/2 @61–62; wrap-unlock @71. `wm_best` restore skipped (storm; freeze last-ok not iter-30). `t_wm` P1 ~127 s / P2 ~24 s (g frozen). Watch P3 clip_frac / val vs P53 −13/−98 | LIVE |
...
- **Liveness:** tmux `mbrl2_p55` LIVE, pid **103504** LIVE (`b321d89`), `device=cuda`. **P2** iter **~90**. Skip-storm **1/2 @61–62** (lock 60). Wrap lock@68 unlock@71. **Gate @82 PASS GAIN-READY** worst **0.88@DV** (MV **0.89/@H 0.86**, DV **0.88/@H 0.91**); last_ok **82 unlocked**. Teacher FD @82 MV/DV **×1.020 / ×1.016**. `wm_best` iter **30** EMA **5.866** (probe@80 gain_fid=0.878; probe@90 gain_fid=0.910 — gain-blind freeze). P1→P2 wm_best restore **SKIPPED** (storm already restored last-ok; must not overwrite with iter-30). STAGE 2 g FROZEN. P2 recon **~0.0026** dob_ground **~0.004**. `t_wm` P1 **~127 s** (eager; this pid CUDA-graph fail) / P2 **~24 s** (gain-match skipped). **Watch** P3 clip_frac / val vs P53 −13/−98. **Do not launch a second GPU job.**
- **HEAD (GPU occupied, not in pid):** leftover `DREAMER_WM_DIAG_DEVICE` → TrainConfig `wm_diag_device='cuda'`. Unused `pmpo_alpha`/`pmpo_beta` + PMPO prior-refresh knobs **REMOVED**. Rest-IC in-loop **never captures** under parent autocast (warmup dtype = rest-cache numpy). Wrap `apply_runtime_knobs(cfg)` (identity 0.20 / on; enable restore). Seed counts / `gain_match_max_starts=6` stay sentinels. CVD="" smoke green (`8953aa9`).

...
| P68 | 2026-09-01 | rest-IC Huber baseline = last rest-obs CV (TM `pre`; not held-K) | LIVE **P2** iter **82** pid **193014**. Storm **2/2 @76–77** CAPPED extra P1. Freeze last_ok **73** **GAIN-READY 0.89@DV** (MV **0.90/@H 0.96**). No MV overgain. P2 recon **0.003** `dob_d` **0.05** buf 327. | 🔬 vs P64 0.91@DV last_ok 82; actor not yet |
...
### p68 (`run_p68_tmpre`, branch `cursor/p28`) — LIVE P2: rest-pre Huber (TM `pre`)
...
- **Step 4 CONFIRMED:** `device=cuda` bs=128 `wm_train_T=128` horizon=55 compile=eager. `[gain-match] len=55 settle=-1 step=0.4`. Huber baseline print **ON**. `[resolved-cfg] … isolation=0 gmatch_len=55 gmatch_step=0.4 gmatch_clip=True gmatch_rest=True gmatch_rest_L=128 held_cv=True p1amp=0.2 p2amp=1 p3amp=1 compile=eager`. Rest-IC graph captured `N=6 T=128`. Iter 1 teacher **×0.655 / ×−1.12** gmatch **1.53** skip **0** (P64/P67 iter 1 already ×1.02/×1.03 — protocol is live, not tautological held-K). nvidia **~14 GB**. **One GPU job.**
- **P1 COMPLETE (GAIN-READY).** Teacher pinned **×0.993 / ×1.036** by iter 8 (iter 1 **×0.655 / ×−1.12** gmatch **1.53** — not tautological held-K). Wrap lock@32 unlock@33; lock@57 unlock@59. Probe40 gain_fid **0.474** then probe70 **0.871** (P64 0.559→0.917). Wrap3@73 (recon 0.2504, zrank 462) did **not** wrap-unlock (recon 0.1486 still >20×); storm **1/2 @76** skip 83 restored last_ok **73**; storm **2/2 @77** skip 97 **CAPPED** extra P1 at 708820 steps. Gate probe on last-ok 73: live **worst 0.87@MV** DV **0.88/@H 0.95**. **Detonated-freeze** restored last_ok 73: **GAIN-READY worst 0.89@DV** (MV **0.90/@H 0.96**, DV **0.89/@H 0.96**). wm_best restore SKIPPED. Rest-IC graph **released**. nvidia **~4 GB**.
- **LIVE P2 iter 82.** recon **0.0031** (restore held). `dob_d` **0.050** `dob_ground` **0.006** `keep_frac=1`. buf **327/327** (no flush). `t_wm` **20.4 s**. skip **0** this iter (banner leftover 180). vs P64 freeze **0.91@DV** last_ok **82** storm 1/2 @78 — P68 freeze **0.89@DV** last_ok **73** storm 2/2 earlier cap. **No P67 MV overgain.** Do **not** score actor until val. P2→P3 still ~iter 136.
- **HEAD (not this pid):** rest-pre skips unused held-K `img_rollout` (`include_held=False`; PRBS keeps it). Identity. Stale `neural-apc-mbrl-env` paths in README/smokes → `neural-APC-mbrl2-env`. **No second GPU job.**
- **Watch:** P2 `dob_d` vs P64 0.023→0.063; val TM vs P64 ×0.927/×0.893; det_r/pred_std; paired vs **−4.54**. `actor_experiment_valid`. Do not teacher-K N+1. Do not score actor if freeze TM misses.
...
| Rest-pre Huber still rolled unused held-K prior (`decode(held_K)` unused as `cv_base`) | **P68 GPU-occupied** (pid **193014** P1) | SUPERSEDED (`include_held=False`; PRBS fallback keeps held-K) | YES — Huber reads rest last-obs CV, not `decode(held_K)`. Extra `Bm×K` roll (test_sim 3→2). Identity. Not in this pid. CVD="" smoke. |

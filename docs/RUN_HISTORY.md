...
| Gain-match FD from PRBS posterior (no held settle) | P26–P43 | SUPERSEDED (HEAD `gain_match_settle_len` auto=control H; P44 GPU) | YES — Huber~0 at excited IC; TM rest-step 0.75@DV. Identified G = TM real. DV @H ×0.849 vs ss ×0.740 (4H compounding); MV @H≈ss. |
| Gain-match FD from PRBS + WM held settle S=H | P44 GPU (pending EXIT) | pending | CPU probe P43 freeze: FD DV×0.969 still ≈×1; TM rest-step remains the gate. Do not treat Huber~0 as GAIN-READY. Next if EXIT FAIL: rest-IC TM-protocol teacher, not 4H FD-on-PRBS. |

...
| P77 | 2026-09-03 | env-free `world_model_type='tssm'` (P76 z-bias REVERT) | LIVE **P1 iter 72**. Probe70 still last **H=1 r=+0.746 / H=55 r=+0.160 conv=0.00** gain_fid **0.537** best_h=27. recon **0.0041@72 best** (P64 @70 **0.0035** ~1.2×). Teacher **×1.00 / ×0.97 @72**. persist_rel **0.57@70 → 0.08@72** after dv-prbs inject. pid **242511**. | 🔬 LIVE P1 — recon closing; persist collapsed after inject (not spike). H=55/conv still dead **@10–70**. Do not patch live. |
...
- **LIVE P1 iter 72 (~376 min, nvidia 11.7 GB, `t_wm` ~305 s, sps 19.8):** skip **0**. recon **0.0055@70 → 0.0060@71 inject bump → 0.0041@72 new best** vs P64 **0.0035@70** (~1.2×). Teacher jsonl 70 **×1.011 / ×0.868** → 71 **×1.011 / ×0.967** → 72 **×1.002 / ×0.969**. last_ok **72 unlocked**. persist_rel **0.57@70 → 0.11@71 → 0.077@72** (collapsed after **dv-prbs-only** inject, unlike full-inject spikes @41/@51/@61; do **not** persist N+1). Rest-IC graph not captured. Next probe typically iter 80 (~02:26 CDT).
...
- **RCA (SysID, not a live patch; CPU-confirmed @wm_best iter50, CVD=""):** rest-IC `rollout_observed` **does** keep KV (`pos=128`, 4 layers). Gain-match / overshoot / held start `img_rollout(h,z)` with `kv_cache=None`. Markovian vs `prev_state` held-K CV **mean |Δ|=0.123** (iter40 **0.142**; iter20 **0.184**). step1 **0.092** / stepK **0.091** (iter40 0.195/0.056 — now uniform). Markovian ΔCV(K) **0.330** ≈ cached **0.337** (iter40 **0.407 vs 0.161** — amplitude gap closed; trajectories still differ). Fidelity probe **warms cache** (so probe H=55 r dead is **not** the Markovian hole — it is cached compounding + remaining 1-step underfit). CPU posterior-prior @H=55 settle=128: real→post **×0.836** (iter40 **×0.810**; iter20 **×0.547**) / post→1step **×0.607** (iter40 **×0.538**) / 1step→OL **×0.097** (iter40 **×0.077**; OL-vs-real **×0.064**). Verdict still `autoencoder` (0.84 < 0.9) but the bigger multiplicative hole is compounding (P64 1step→OL **×0.85**). Do not patch while live. Recon **0.007@50** (~2.5× P64) — not an EXIT. Probe50 **conv still 0 @10–50**; H=1 dipped, H=55 dead (cached compounding). persist_rel held **~0.51**. Next-if EXIT FALSIFIED: one attributed A/B pass `prev_state` into gain-match / overshoot / held. Metric: probe H=55 r / conv / val 1step→OL vs P64 ×0.85. Watch freeze vs P64 **0.91@DV**; probe80 conv>0 / H=55 vs floor 0.40 (probe70 still conv=0 / H=55 **collapsed 0.160**); `actor_experiment_valid`. Do **not** start a second GPU job.

...
- **Step 4:** env-free `CUDA_VISIBLE_DEVICES=0` + `PYTORCH_CUDA_ALLOC_CONF` only. Watch `[held-rollout] decode-CV 1step→K magnitude (P63; FO τ-scale, not late−early)`; `[resolved-cfg] held_mag=True gmatch_clip=True gmatch_step=0.4`; device=cuda; jsonl `wm_held_ol_ratio` not stuck ~0 after recon gate; val OL-vs-real vs P62 ×0.785 / P61 ×0.809; paired vs P58.

...
Plant this cycle: `nonlinear_sim` HeatExchangerTower (P118 EXIT INVALID CAPPED **0.68@MV**; **P119 locgfix LIVE P1 @81** — wrap@77 skip-storm **1** GAIN_NOT_READY **0.56@MV** cap-deferred / recovered; probe@80 conv **0.75** / **not** GAIN-READY; then return-to-`test_sim`).
...
**LIVE P1 @81 (2026-09-09 16:59:23, ~169 min):** skip_iter **0** last_ok **81 unlocked**. recon **0.0069** (best **0.0028@72/@79**). jsonl MV/DV **×0.998 / ×1.016**. gnorm **0.57 finite**. persist_rel **0.11**. `wm_op_scale_dev` **0.79**. jsonl NaN/Inf **0**. STAGE 1. storm **1/0** `cap_deferred=True`. Inject@80 dv-prbs: recon **0.0048@80 → 0.0069@81** — **no 20×-lock**.
...
**Probe@80:** `H=1:r=+0.586 H=14:r=+0.613 H=28:r=+0.563 H=56:r=+0.636 conv=0.75 drift=0.154 floor=0.40 best_h=56/56 gain_fid=0.991`. vs @70 H=56 **+0.444** (near floor) / H=28 **+0.434** — long-H recovered, still above floor. **Not** GAIN-READY. `wm_best` EMA **6.299@80** — gain-blind. Next: orig-P1@~87.

Predicted remaining: orig-P1@~87 (~12 min / ~6 iters); then GAIN-READY vs P118 CAPPED **0.68@MV** / P117 **0.71@DV**. Score actor only if freeze GAIN-READY **and** P3.
...
- Signal: P119 locgfix teacher CONFIRMED (jsonl @81 MV **1.00** skip_iter **0** recon **0.0069**). **20× lock@60 ABSENT**. Wrap@77 182× + skip-storm **1** GAIN_NOT_READY **0.56@MV** cap-deferred / recovered @78–@81. Probe@80 all-H above floor (H=1 **+0.586** H=56 **+0.636**) / conv **0.75** — **not** GAIN-READY. Inject@80 no lock. GPU P119 P1 **~14.8 GB**. Disk `/home` 64% / 61G. Keep P119+P118+P117+P116+P64+P53.

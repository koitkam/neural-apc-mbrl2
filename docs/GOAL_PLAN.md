...
Plant this cycle: `nonlinear_sim` HeatExchangerTower (P118 EXIT INVALID CAPPED **0.68@MV**; **P119 locgfix LIVE P1 @91** — orig-P1@**91 FAIL** GAIN_NOT_READY **0.63@MV**; extra-P1 lottery **closed**; then return-to-`test_sim`).
...
**Trusted:** val TM ss/@H/curve_iae (MV and DV); gain decomp real→post / post→1step / 1step→OL; `det_r` + pred_std vs true; paired econ vs baseline **and** champion when freeze GAIN-READY and P3 ran; CV d2/reversal (`cv_d2_rms_normed` + `cv_reversal_rate` — **not** the suite's current `smooth_pass`); `cv_opt_headroom` / `cv_viol_frac`; DR `cv_return_headroom` / `cv_return_time_frac`. Skip-storm 5-level TM (`DCgain_ratio` / `@H` / `compound_ok`) is a freeze/RCA diagnostic, not a champ score. Launch `|du_mv|` vs teacher `step` (P119 print): O(step) = WM-norm; ≫5×step = engineering-unit bug. jsonl `wm_op_scale_dev` is observability that LPV moved, not a TM score. Post-restore recon + skip **this-iter** (P119 @81 recon **0.0069** skip **0**) is the skip-storm liveness check, not a TM score.
...
**LIVE P1 @91 (2026-09-09 17:19:55, ~189 min):** skip_iter **0** last_ok **91 unlocked**. recon **0.0133** (best **0.0028@72**; **4.75×** — below 5× last-ok swap). jsonl MV/DV **×0.991 / ×1.017**. gnorm **0.80 finite**. persist_rel **1.12**. `wm_op_scale_dev` **0.84**. jsonl NaN/Inf **0**. STAGE 1. storm **1** `ready_n=0` cap-deferred. nvidia **~14751 MiB**.

**Orig-P1 @91 FAIL (fair locgfix teacher):** `[gate p1->p2] FAIL (gain_not_ready)` live g (no last-ok swap). median 3/3 worsts **[0.63, 0.63, 0.65]** `DCgain_ratio[0.63,1.03] @H[0.61,0.98]` worst **0.63@MV** `@H 0.61` DV **1.03/@H 0.98** `unbiased=False` `not_noisy=False` spread_x1.2 signflips=2. **Extending P1** +82500 → **907500** (cap **1017602**). extra-P1 lottery **closed** (P41 / P117 / P118) — do **not** promote extra-P1 as freeze. vs P118 orig-P1 **0.34@MV** (wrong-unit teacher) / P117 orig-P1 **0.74@MV**. Locgfix orig-P1 **0.63@MV** is a fair miss (jsonl teacher ×0.99 ≠ TM 0.63). DV in ss band; MV short.
...
Predicted remaining: extra-P1 ~iter 104 (~26 min, **not** a freeze score); then cap / P2. Freeze vs P118 CAPPED **0.68@MV** / P117 **0.71@DV**. If freeze still CAPPED with this teacher: **P120 `gop`**. Score actor only if freeze GAIN-READY **and** P3.
...
- Signal: P119 locgfix teacher CONFIRMED. **Orig-P1@91 FAIL 0.63@MV** (live g; jsonl MV **×0.991** ≠ TM). DV **1.03/@H 0.98** in band. extra-P1 lottery closed. Wrap@77 skip-storm **1** **0.56@MV** recovered. Probe@90 conv **0.75** / H=56 **+0.479** — **not** GAIN-READY. GPU P119 P1 **~14.8 GB**. Disk `/home` 64% / 62G. Keep P119+P118+P117+P116+P64+P53.

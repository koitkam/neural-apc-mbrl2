...
Plant this cycle: `nonlinear_sim` HeatExchangerTower (P118 = job 2 of 2, **LIVE P1**; then return-to-`test_sim`).
...
tmux `mbrl2_p118` pid **551549** sha **`ff5f84c`** `device=cuda` bs=128 compile=eager nvidia **~14751 MiB**. `[resolved-cfg] opscale=True` no ol1. STAGE 1 `g=84 dob=8`. Rest-IC graph captured N=6 T=128. sps **~52**. jsonl **28** P1. Heartbeat ~2 min/iter.

**Teacher print (launch):** local G mean MV **0.0153** / DV **−0.345** vs identified **2.627 / −0.426**. jsonl MV ratio still sign-flips (**−74…+16**, @28 **+2.61**); DV ratio **~0.7–1.1**. `wm_op_scale_dev` **0.52→0.99** then **0.88**. recon **0.44→0.033@22**, spike **0.366@23** recovered **0.042@28**. skip **0** except storm.

**Skip-storm 1 @iter 13 (RCA, not kill):** recon **0.676** `wm_grad_norm=inf` skip **56/56**. Probe median 3/3 worsts **[0.60, 0.33, 0.54]** DC **[0.55, 0.86]** `@H[0.40, 0.95]` worst **0.55@DV**. Restored `wm_last_ok` iter **12**. **cap-deferred** (`ready_n=0`; last_ok GAIN_NOT_READY). last_ok then walked to **28**. Inf is only `wm_grad_norm@13`. **Do not score actor. Do not treat opscale as FALSIFIED** — MV teacher was not WM-norm. Chasing G_MV≈0.015 detonates grads; LPV saturating is the same story.

RCA: `_plant_fd_rest_local_g` in this pid used `env._prev_control` (valve %) minus rest action ∈[-1,1] as Δu. |du|~50 ⇒ G_MV≈dcv/50≈0.015. DV path already used obs-norm Δu.

HEAD (not this pid): `_wm_norm_realized_du` + snapshot/restore `_prev_cmd_norm` including **None**. CVD="" locgfix smoke **PASSED**.

## Next job (after P118 EXIT or hard-fail — not while LIVE)

**P119 `locgfix`** — same mechanism as P118 (LPV + local rest-IC G), **one bugfix**: MV plant-FD Δu = `_prev_cmd_norm` (WM-norm; P61 realized/rate-limit), never engineering `_prev_control`. Snapshot/restore `_prev_cmd_norm` **including None**. Print WARNING if |G_MV| << 5% of identified. Env-free. No new TrainConfig / no `DREAMER_*`. Tag `locgfix`. Session `mbrl2_p119`. Out-dir `output/nonlinear_sim/run_p119_locgfix`.

Predicted signature: rest-ic local G MV same order as SysID (~2, OP span ~2× not 0.015); **no** WARNING; jsonl `gain_match_mv_ratio` ~O(1) not ±70; `wm_op_scale_dev` not pinned at tanh sat; skip-storm Inf@13-class absent or rare; then GAIN-READY vs P117 CAPPED **0.71@DV**.

Falsifier: still CAPPED ~0.71@DV **after** local MV G is WM-norm (WARNING absent; jsonl MV ratio O(1)).
...
3. If P119 still CAPPED ~0.71@DV **with** WM-norm local G: input-scale LPV is too thin for equal-% DC. Next mechanism (still neural): OP-conditioned gain-c / affine `G(op)` inside the observer, not a gray-box equal-% formula, not decoder FF (P74 closed).
...
- Signal: P118 jsonl NaN/Inf = **one** skip-storm `wm_grad_norm=inf@13`; recon recovered; skip 0 after restore; GPU 14.7 GB; sps ~52.
- Control: do not score actor until freeze GAIN-READY and P3.
- ML: LPV `wm_op_scale_dev` left identity then sat (~0.99) chasing a ~0 MV Huber target. Inf grad @13 is teacher-unit, not RSSM collapse.

...
Plant this cycle: `nonlinear_sim` HeatExchangerTower (P118 EXIT INVALID CAPPED **0.68@MV**; **P119 locgfix LIVE extra-P1 @92** — orig-P1@**91 FAIL** GAIN_NOT_READY **0.63@MV**; extra-P1 lottery **closed**; then return-to-`test_sim`).
...
**LIVE extra-P1 @92 (2026-09-09 17:22:35, ~192 min):** skip_iter **0** last_ok **92 unlocked**. recon **0.0045** (best **0.0028@72**; recovered from gate **0.0133@91**). jsonl MV/DV **×0.990 / ×1.033**. gnorm **0.82 finite**. persist_rel **1.03**. `wm_op_scale_dev` **0.84**. jsonl NaN/Inf **0**. STAGE 1. storm **1** `ready_n=0` cap-deferred. nvidia **~14751 MiB**.

**Orig-P1 @91 FAIL (fair locgfix teacher):** `[gate p1->p2] FAIL (gain_not_ready)` live g (no last-ok swap; recon **4.75×** < 5×). median 3/3 worsts **[0.63, 0.63, 0.65]** `DCgain_ratio[0.63,1.03] @H[0.61,0.98]` worst **0.63@MV** `@H 0.61` DV **1.03/@H 0.98** `unbiased=False` `not_noisy=False` spread_x1.2 signflips=2. **Extending P1** +82500 → **907500** (cap **1017602**). extra-P1 lottery **closed** (P41 / P117 / P118) — do **not** promote extra-P1 as freeze. vs P118 orig-P1 **0.34@MV** (wrong-unit teacher) / P117 orig-P1 **0.74@MV**. Locgfix orig-P1 **0.63@MV** is a fair miss (jsonl teacher ×0.99 ≠ TM 0.63). DV in ss band; MV short.

**Probe@90:** `H=1:r=+0.532 H=14:r=+0.479 H=28:r=+0.497 H=56:r=+0.479 conv=0.75 drift=0.123 floor=0.40 best_h=56/56 gain_fid=0.960`. **Not** GAIN-READY. `wm_best` EMA **6.299@80** — gain-blind.

**20× lock@60 ABSENT.** Wrap@77 skip-storm **1** **0.56@MV** cap-deferred / recovered. Wrap@84 26.8× skip **0** unlocked @85 (no storm). Inject@80/@90 no lock.

Predicted remaining: extra-P1 ~iter 104 (~24 min, **not** a freeze score); then cap / P2. Freeze vs P118 CAPPED **0.68@MV** / P117 **0.71@DV**. If freeze still CAPPED with this teacher: **P120 `gop`**. Score actor only if freeze GAIN-READY **and** P3.
...
- Signal: P119 locgfix teacher CONFIRMED. **Orig-P1@91 FAIL 0.63@MV** (live g; jsonl MV **×0.991** ≠ TM). @92 recon **0.0045** skip **0** recovered. DV **1.03/@H 0.98** in band. extra-P1 lottery closed. Wrap@77 skip-storm **1** **0.56@MV** recovered. Probe@90 conv **0.75** / H=56 **+0.479** — **not** GAIN-READY. GPU P119 P1 **~14.8 GB**. Disk `/home` 64% / 62G. Keep P119+P118+P117+P116+P64+P53.
...
- ML: Orig-P1@91 **FAIL 0.63@MV** after WM-norm teacher (fair vs P118 0.34@MV unit bug). LPV `wm_op_scale_dev` **0.84** (not sat). **20× lock@60 ABSENT**. Wrap@77 inject-storm recovered; wrap@84 recovered skip 0. Probe@90 conv **0.75** is gain-blind. `wm_best` @80 EMA **6.299** is gain-blind. extra-P1 lottery closed. If freeze still CAPPED ~0.68@MV: **P120 `gop`**. P73 persist 0.1 forbids in-place `c*=gop`.

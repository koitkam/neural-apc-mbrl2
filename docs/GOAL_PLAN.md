...
Plant this cycle: `nonlinear_sim` HeatExchangerTower (P118 EXIT INVALID CAPPED **0.68@MV**; **P119 locgfix LIVE P2 @124** — orig-P1@**91 FAIL 0.63@MV**; extra-P1@**101 PASS** last_ok **100** GAIN-READY **0.83@MV**; extra-P1 lottery **closed**; then return-to-`test_sim`).
...
**Trusted:** val TM ss/@H/curve_iae (MV and DV); gain decomp real→post / post→1step / 1step→OL; `det_r` + pred_std vs true; paired econ vs baseline **and** champion when freeze GAIN-READY and P3 ran; CV d2/reversal (`cv_d2_rms_normed` + `cv_reversal_rate` — **not** the suite's current `smooth_pass`); `cv_opt_headroom` / `cv_viol_frac`; DR `cv_return_headroom` / `cv_return_time_frac`. Skip-storm 5-level TM (`DCgain_ratio` / `@H` / `compound_ok`) is a freeze/RCA diagnostic, not a champ score. Launch `|du_mv|` vs teacher `step` (P119 print): O(step) = WM-norm; ≫5×step = engineering-unit bug. jsonl `wm_op_scale_dev` is observability that LPV moved, not a TM score. Post-restore recon + skip **this-iter** (P119 P2 @124 recon **0.0098** skip **0**) is the skip-storm liveness check, not a TM score.
...
tmux `mbrl2_p119` pid **557257** sha **`ea7def9`** `device=cuda` bs=128 compile=eager nvidia **~4141 MiB** (rest-IC released). Env-free (`CUDA_VISIBLE_DEVICES=0`, no `DREAMER_*`). `[resolved-cfg] opscale=True` no ol1. STAGE 2 `g` FROZEN. Train start **2026-09-09 14:10:33**. **One GPU job.** jsonl NaN/Inf **0**.
...
**LIVE P2 @124 (2026-09-09 17:53:47, ~223 min):** skip_iter **0** last_ok **100 locked**. recon **0.0098**. first dobg **0.0034@102** skip **0** KEEP vs P95. `dob_A` **0.95257** held. `dob_K` **0.119→0.017@102** min **0.006@104** then **0.067@124** SS **≈1.41**. leftover `|d_slow|/|d|` **0.85@102→1.27@105→0.43@124**. `std_ratio` **38822@102 → 0.59@124**. `wm_op_scale_dev` **0.86**. α **0.00391**. sps **~218**. jsonl NaN/Inf **0**. nvidia **~4141 MiB**.

**Orig-P1 @91 FAIL:** live g **0.63@MV** `@H 0.61` DV **1.03/@H 0.98**. Fair locgfix miss (jsonl ×0.99 ≠ TM). extra-P1 lottery **closed**.

**Extra-P1 @101 PASS (log, not a freeze KEEP):** wrap@101 after inject@100 recon **0.0872 = 31×** best **0.0028** skip **0** gnorm **5.23** → last_ok **100 locked**. Gate measured last_ok **100** (live >5×). `[gain-ready-probe] median 3/3 worsts=[0.82,0.84,0.83]` `DCgain_ratio[0.83,0.92] @H[0.95,0.98]` worst **0.83@MV** `@H 0.98` DV **0.92/@H 0.95** `unbiased=True` `not_noisy=True` `compound_ok=True` 1step→OL **0.86**. `[gate p1->p2] PASS` recent_max=**6.299**. Detonated-freeze restore last_ok **100**; re-probe **0.85@MV** 1step→OL **0.90**. Warm-restore SKIPPED (`wm_best` gain-blind). `[phase] 1→2` env_steps **908750**. extra-P1 **not** a KEEP of extra-P1 as freeze (P41/P117/P118). Freeze g **is** last_ok **100** GAIN-READY so P3 should run (`skip_invalid_p3` will not skip). Score actor only after P3 if `actor_experiment_valid`. Val TM vs orig-P1 **0.63@MV** is the locgfix score.

**Probe@120:** `H=1:r=+0.640 H=14:r=+0.524 H=28:r=+0.508 H=56:r=+0.536 conv=0.50 drift=0.140 floor=0.40 best_h=56/56 gain_fid=0.857`. vs @110 H=56 **+0.461** / conv **0.50**. `wm_best` EMA **6.299@80** — gain-blind.

Predicted remaining: P2→P3 ~iter 160 (~16 min). Score actor only if freeze GAIN-READY **and** P3. If val TM still short: encode-L then **P120 `gop`**. Do **not** extra-P1 N+1.
...
1. **P119 locgfix** (LIVE P2 above) — freeze last_ok **100** GAIN-READY **0.83@MV** (extra-P1; lottery closed). Orig-P1 **0.63@MV** is the fair miss. Do not rewrite the live recipe. Do **not** extra-P1 N+1.
...
- Signal: P119 locgfix teacher CONFIRMED. **Orig-P1@91 FAIL 0.63@MV**. **Extra-P1@101 PASS** last_ok **100** GAIN-READY **0.83@MV** 1step→OL **0.86** (lottery closed — not a KEEP). P2 @124 first dobg **0.0034** skip **0** K **0.067** leftover **0.43** std **0.59**. Probe@120 H=56 **+0.536** / conv **0.50**. GPU **~4.1 GB**. Disk `/home` 64% / 62G. Keep P119+P118+P117+P116+P64+P53.
- Control: freeze last_ok **100** GAIN-READY → P3 should run (`skip_invalid_p3` will not skip). Score P119 only after P3 if `actor_experiment_valid`. Do not kill P119 / second GPU / extra-P1 N+1.
- ML: Orig-P1 **0.63@MV** is the fair locgfix miss. extra-P1 last_ok **100** **0.83@MV** is P100-class confound. LPV `wm_op_scale_dev` **0.86**. Detonated-freeze restored 100 (wrap@101 31×). Warm-restore SKIPPED. P2 K **0.119→0.006@104→0.067@124** leftover **1.27→0.43** (recovering vs P118 crush). extra-P1 lottery closed. If val TM still short: encode-L then **P120 `gop`**. P73 persist 0.1 forbids in-place `c*=gop`.

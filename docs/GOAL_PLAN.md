...
Plant this cycle: **return-to-`test_sim`**. **P121 `srmed` P1-gate CAPPED → P2 LIVE** pid **588400** sha `4422be5` @iter **118**. last_ok **91** not READY (re-probe **0.61@MV**). `skip_invalid_p3=True` → likely `[p3-skip]`. P120 EXIT `[p3-skip]` INVALID. **P120 `gop` PARKED.** Do **not** score actor / opscale. Extra-P1 lottery **closed**. Snapshot P2 until P2→P3 / EXIT. Do **not** wait for jsonl.
...
## P121 `srmed` — P1-gate CAPPED (P2 LIVE; do not score actor / do not FALSIFY opscale)

tmux `mbrl2_p121` **live**. pid **588400**. sha **`4422be5`**. Env-free `CUDA_VISIBLE_DEVICES=0` (no `DREAMER_*`). Train start **2026-09-10 01:06:11**. `run_plan` **sr=4 H=55** `sample_rate_source=auto:tau_fast/10_or_dead_dom/2` `dead_fast=7` `tau_fast=45` `dead_dom=8`. `[resolved-cfg] opscale=True` `skip_invalid_p3=True` gmatch_len=55. locgfix `|du_mv|=0.4000=step`. jsonl NaN/Inf **0**. skip_cum **1** (iter 68). Graph released at freeze (VRAM ~15 GB → ~4 GB). Do **not** kill / second GPU / rewrite recipe / wait for P2→P3.
...
**P2 snapshot (do not score Kalman / actor):** first dobg **0.0084@112** skip **0** KEEP vs P95. `dob_A` **0.95257** held. `dob_K` **0.017@112→0.0057@118** leftover **0.74→1.25** `std_ratio` noisy-first then **0.65@114**. `opdev` **~0.83**. ~28 s/iter. `skip_invalid_p3` on last_ok **91** **0.61@MV** → expect `[p3-skip]` at P2→P3.

**Judge:** srmed **sampling KEEP** (`sr=4 H=55` vs P120 `sr=3 H=74`). srmed **FALSIFIED as orig-P1 PASS restorer** (P116 @87 **0.84@DV** last_ok **78**). LIVE extra-P1 also missed the 0.8 band (**0.66 / 0.68**) — stay-lock did not cause that miss, but it **did** freeze wrap-91 (**0.61**) instead of a later snapshot. **Not** an opscale KEEP/FALSIFY. Extra-P1 lottery **closed**. Next GPU after this pid EXIT: **#1 stay-lock-only-if-READY**. Champion **P64**. Dist champ **P26**.
...
**#1 Extra-P1 stay-lock only if last_ok is GAIN-READY (next GPU after this pid EXIT; expected `[p3-skip]`).** Align extra-P1 wrap stay-lock with skip-storm P92. **P120 + P121 GPU-confirmed.** P121: wrap@95 last_ok **91** stayed `locked=True` `grl=False` through extra-P1@101 LIVE **0.66@MV** and cap@111 LIVE **0.68@MV** while recon **0.0146→0.0063 <20×**; freeze re-probe **0.61@MV**. Orig-P1 wrap@86 **unlocked@87** KEEP (`extra_p1=False`); that miss was 5× snapshot of last_ok **85**, not stay-lock. LIVE extra-P1 also missed the 0.8 band — #1 lets last_ok walk (P119-class) but does **not** invent READY. If the next job's LIVE extra-P1 is still <0.8, CAPPED again → opscale still unscored; then piece/metric, **not** extra-P1 N+1. **Do not land on the P121 pid.**
- **Mechanism:** `_should_lock_last_ok` `if already_locked: if extra_p1 or gain_ready_locked: return True`. Extra-P1 recovered basin stay-locks **even when** `gain_ready_locked=False` (P120 last_ok **85**; P121 last_ok **91**). Change: `if already_locked and extra_p1: return bool(gain_ready_locked)` then fall through to recon-vs-`lock_ratio` (same as orig-P1). Orig-P1 wrap-recovery unlock KEEP when not READY. P40 overwrite of a **READY** last_ok still stay-locks via `gain_ready_locked`.
- **Predicted signature:** wrap@gate FAIL then extra-P1 last_ok walks (P119-class). Next env-free HEAD job (srmed already on HEAD; do **not** identity-relaunch P121).
- **Falsifier:** P40 overwrite of a **READY** last_ok — must still stay-lock when the locked snapshot **was** GAIN-READY. Smoke (add next to P40 extra-P1 lock tests in `tools/_smoke_rssm.py` L470–474): `extra_p1 + already_locked + gain_ready_locked=False + recon<20×` **unlocks**; same with `gain_ready_locked=True` **stays** locked. Existing P40 test (`extra_p1=True`, no `gain_ready_locked`) currently asserts lock — rewrite it to pass `gain_ready_locked=True` (READY) vs False (P120/P121 not-READY).
...
**#2 P121 `srmed` sampling — KEEP; freeze-restorer FALSIFIED.** `run_plan` sr=**4** H=**55** source `dead_dom` CONFIRMED. Orig-P1@88 FAIL **0.65@MV** last_ok **85** (wrap-adjacent 5×). Do **not** N+1 the Nyquist formula. Do **not** pin `DREAMER_SAMPLE_RATE`. Lookback `derive_sample_rate_for_lookback` KEEP.

**#3 After a VALID observer on this plant:** score opscale vs P116 R1 TM (autoencoder ×0.706). If TM << P116 with sr matched → gate `op_scale_net` when rest-IC `|G|` span ≲ 1.3× (linear identity; SysID-derived). If TM ≥ P116 → KEEP opscale as default even on linear. **Do not** identity-regularizer λ lottery. P121 CAPPED is **not** that verdict.
...
- **Files:** `utils/auto_weights.py`, `utils/objective_runtime.py` (need prev CV + sticky sign in the step cache), `tools/_test_cv_reversal_surrogate.py`, `tools/_test_mv_reversal.py` (MV penalty → 0). Env-free. **Do not** land on the P121 pid.
...
**Do not:** extra-P1 N+1, ol1 N+1, identity-relaunch P117/P118/P119/P120/P121, launch `gop`, stack critic knobs, second GPU, sr pin as engineering default, stay-lock on this pid, score P2 `dob_K` / actor before EXIT val.
...
- Signal: P1-gate CAPPED @111 last_ok **91** re-probe **0.61@MV**. skip_iter **0** skip_cum **1**. jsonl NaN/Inf **0**. P2 ~28 s/iter (graph released). nvidia **~4 GB** after freeze (was ~15 GB in P1). Disk `/home` 64% / 61G. Keep P121+P120+P119+P118+P117+P116+P64+P53. Wrap@22/@65/@68 orig-P1 unlocked; wrap@95 extra-P1 stay-locked.
- Control: do not score actor. Freeze not READY. R2 still open on P116 (CV rev 0.421, d2 PASS). DCS `mv_rate_limit` is an actuator constraint (KEEP); train MV-relu is the wrong residual.
...
- Metric: do not GPU-optimize jsonl ×1 / `opdev` / MV reversal gate / amplitude-weighted CV relu / `|s|≥0.5` as val-deadband / LIVE extra-P1 TM as a freeze score. Trusted R2 is `_sign_reversal_rate` + d2 + headroom on `econ_side=sign(G)`. Sticky thresh **`tanh(1)`**. Freeze score is last_ok re-probe **0.61@MV**, not LIVE cap **0.68**.
...
Config audit Step 4 (P121 P1-gate): env-free HEAD CONFIRMED. `run_plan` sr=**4** H=**55** source `dead_dom`. `[resolved-cfg] opscale=True` `skip_invalid_p3=True`. locgfix `|du_mv|=step`. last_ok **91** not READY. #❌ **0**. #🆕 **0**. Do not score TM/actor until EXIT. Step 5 (P121 P1-gate): srmed sampling **KEEP**; srmed as orig-P1 PASS **FALSIFIED**; extra-P1 stay-lock **GPU-confirmed**; opscale **unscored**; extra-P1 lottery **closed**. Step 5 (P120): `[p3-skip]` KEEP as validity gate.

P117 process died mid-P2 (SIGKILL). P118 completed P2 then `[p3-skip]`. P119 completed P2 then **P3 then EXIT VALID**. P120 completed P2 then **`[p3-skip]`**. P121 P1-gate CAPPED, P2 live. Do not identity-relaunch P117–P121.

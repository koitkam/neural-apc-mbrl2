...
Plant this cycle: `nonlinear_sim` HeatExchangerTower (P118 EXIT INVALID CAPPED **0.68@MV**; **P119 locgfix LIVE P2 @107** — orig-P1@**91 FAIL 0.63@MV**; extra-P1@**101 PASS** last_ok **100** GAIN-READY **0.83@MV** 1step→OL **0.86**; extra-P1 lottery **closed**; then return-to-`test_sim`).
...
**Trusted:** val TM ss/@H/curve_iae (MV and DV); gain decomp real→post / post→1step / 1step→OL; `det_r` + pred_std vs true; paired econ vs baseline **and** champion when freeze GAIN-READY and P3 ran; CV d2/reversal (`cv_d2_rms_normed` + `cv_reversal_rate` — **not** the suite's current `smooth_pass`); `cv_opt_headroom` / `cv_viol_frac`; DR `cv_return_headroom` / `cv_return_time_frac`. Skip-storm 5-level TM (`DCgain_ratio` / `@H` / `compound_ok`) is a freeze/RCA diagnostic, not a champ score. Launch `|du_mv|` vs teacher `step` (P119 print): O(step) = WM-norm; ≫5×step = engineering-unit bug. jsonl `wm_op_scale_dev` is observability that LPV moved, not a TM score. Post-restore recon + skip **this-iter** (P119 P2 @107 recon **0.0035** skip **0**) is the skip-storm liveness check, not a TM score.
...
**LIVE P2 @107 (2026-09-09 17:45:32, ~215 min):** skip_iter **0** last_ok **100 locked**. recon **0.0035**. first dobg **0.0034@102** skip **0** KEEP vs P95. `dob_A` **0.95257** held. `dob_K` **0.119→0.017@102→0.016@107** SS **≈0.33**. leftover `|d_slow|/|d|` **0.85@102→1.13@107**. `std_ratio` **38822@102 → 0.36@107**. `wm_op_scale_dev` **0.86**. sps **~217**. jsonl NaN/Inf **0**.
...
**Probe@100:** `H=1:r=+0.479 H=14:r=+0.520 H=28:r=+0.517 H=56:r=+0.590 conv=0.25 drift=0.128 floor=0.40 best_h=56/56 gain_fid=0.980`. `wm_best` EMA **6.299@80** — gain-blind.

Predicted remaining: P2→P3 ~iter 160 (~25 min). Score actor only if freeze GAIN-READY **and** P3. If val TM still short: encode-L then **P120 `gop`**. Do **not** extra-P1 N+1.
...
- Signal: P119 locgfix teacher CONFIRMED. **Orig-P1@91 FAIL 0.63@MV**. **Extra-P1@101 PASS** last_ok **100** GAIN-READY **0.83@MV** 1step→OL **0.86** (lottery closed — not a KEEP). P2 @107 first dobg **0.0034** skip **0** K **0.017** leftover **1.13**. GPU **~4.1 GB**. Disk `/home` 64% / 62G. Keep P119+P118+P117+P116+P64+P53.
...
- ML: Orig-P1 **0.63@MV** is the fair locgfix miss. extra-P1 last_ok **100** **0.83@MV** is P100-class confound. LPV `wm_op_scale_dev` **0.86**. Detonated-freeze restored 100 (wrap@101 31×). Warm-restore SKIPPED. P2 K crush **0.119→0.017** same class as P118 entry. extra-P1 lottery closed. If val TM still short: encode-L then **P120 `gop`**. P73 persist 0.1 forbids in-place `c*=gop`.

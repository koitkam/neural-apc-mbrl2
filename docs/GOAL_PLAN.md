...
Plant this cycle: `nonlinear_sim` HeatExchangerTower (P118 EXIT INVALID CAPPED **0.68@MV**; **P119 locgfix LIVE P1 @15** — Inf@13 **ABSENT**; inject@10 no lock; then return-to-`test_sim`).
...
**LIVE P1 @15 (2026-09-09 14:43:00):** skip **0** last_ok **15 unlocked**. recon **0.7686@1 → 0.0302@14** (best) **→ 0.0313@15** (held). jsonl MV/DV **×0.983 / ×1.079** (O(1) pin since @5 **×0.961 / ×1.033**; @1 was **0.574 / −2.37**). gnorm **2.38 finite** (peak **27.3@8**). persist_rel **1.02** (peak **4.43@9**, not 31). `wm_op_scale_dev` **0.80**. jsonl NaN/Inf **0**. **Inf@13 ABSENT** vs P118 @13 recon **0.676** skip **56/56** `wm_grad_norm=inf` MV **−23.5**. dv-prbs inject **@10** (buf 204/320) did **not** 20×-lock or recon-spike (0.064@10 → **0.030@14→0.031@15**) — P118 inject@60 locked last_ok **60** recon **0.935**.

**Probe@10:** `H=1:r=+0.458 H=14:r=+0.219 H=28:r=+0.202 H=56:r=+0.184 conv=1.00 drift=0.026 floor=0.40 best_h=1/56 gain_fid=0.445`. H=1 above floor; H=56 below — 1-step only, **not** GAIN-READY. `wm_best` EMA **3.214@10** — gain-blind (same class as P118 `wm_best` @100 SKIPPED). Do not treat conv=1 / jsonl ×1 as freeze. Next probe @20.

Predicted remaining: 20× lock@60-class / orig-P1@~87 (~2.1 h); then GAIN-READY vs P118 CAPPED **0.68@MV** / P117 **0.71@DV**. Score actor only if freeze GAIN-READY **and** P3.
...
- Signal: P119 locgfix teacher CONFIRMED (MV G **2.89** vs ident **2.71**; `|du_mv|=0.4=step`; jsonl @15 MV **0.98** skip **0**). **Inf@13 ABSENT**. Inject@10 no lock (recon kept falling). Probe@10 H=56 **+0.184** below floor / best_h=1 — not freeze. GPU P119 P1 **~14.8 GB**. Disk `/home` 64% / 62G. Keep P119+P118+P117+P116+P64+P53.

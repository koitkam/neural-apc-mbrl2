...
- **GAIN block** (`cont_gain_dim = n_cv·(n_mv+n_dv)`): inferred in-context from the
  ...
  **P74 EXIT REVERT:** decoder `gain_cv_skip` was a teacher-pin no-op
  ...
  stays last-only). Not P75 rise-mass. P1@61 wrap-2 recovering
  skip 0 last_ok 51 locked; persist/teacher re-pin while recon
  still ~130× best — last_ok is recon-keyed; do not retune mid-run. **P76 EXIT REVERT:** RSSM GRU

...
> and champ (paired **−23.62**). Joseph SS capped at `1/A`. **P100:** Luenberger
> plant-residual `d_t = A d + K (CV−g)` (SS `K/(1−A)`). `dob_reg_coef → 0` when
...
- **Grounding (KalmanNet, P19)**: the recon innovation alone under-drives `d_t`
  ...
  **P98 EXIT:** Wiener HP MSE **KEEP as P2-entry** / **FALSIFIED as amp recovery**
  ...
  KEEP as A-hold / FALSIFIED as champ; Joseph SS capped at `1/A`); **P100:**
  Luenberger plant residual so this

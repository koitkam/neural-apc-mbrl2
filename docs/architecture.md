...
- **Stage 2** — `g` is frozen, so the recon innovation can only be reduced by
  ...
  **P91 EXIT:** feat tail is raw `d.detach()` (P89 identity; FALSIFIED as P89 freeze — last_ok **44** **0.76@DV**; serve-HP family closed). Kalman state
  stays raw. jsonl `dob_ground_keep_frac` is 1.0 when grounding fires. **P92:** skip-storm continue stay-locks last_ok iff 5-level GAIN-READY.

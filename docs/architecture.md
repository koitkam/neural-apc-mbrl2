...
purpose). Backbone-agnostic: the **RSSM** (P64/P79 env-free default; GRU)
and **TSSM** (opt-in `DREAMER_WORLD_MODEL_TYPE=tssm`; transformer over the
lookback) are duck-compatible — `TSSMState`
...
> (P77+P78 TSSM family **FALSIFIED**; P78 EXIT last_ok **91** **4.08@MV / 0.72@DV**; val MV **×1.721 / ×1.710** DV **×0.729 / ×0.624**; KV-continue KEEP as OL lift / FALSIFIED as freeze; opt-in `DREAMER_WORLD_MODEL_TYPE=tssm`; **P79 EXIT** KEEP rssm / **FALSIFIED as P64-identity** (`run_p79_rssmrevert`, pid **277462**, 204 iters): GAIN-READY **0.81@DV** last_ok **81**; val MV **×0.923 / ×0.953** DV **×0.781 / ×0.838**; 1step→OL **×0.840**; paired **−10.57 vs −82.79** VALID 9/9 loses to P64 **−4.54**; **P80 EXIT** persist 0.1→0.0 **REVERT** (`run_p80_persist0`, pid **293761**, 515 iters budget): freeze GAIN-READY **0.86@MV / 0.87@DV** last_ok **82**; val MV **×0.723 / ×0.755** DV **×0.789 / ×0.857**; 1step→OL **×0.735** OL **×0.743**; paired **−35.33 vs −107.43** VALID 8/9 loses to P64 **−4.54** / P79 **−10.57**; persist **0.1 KEEP**; **P85 EXIT** always-call KEEP as P1 / FALSIFIED as val TM (MV **×0.671** 1step→OL **×0.697** paired **−24.84**); **P86 EXIT** CV-only overshoot **REVERT** (GAIN_NOT_READY 3.36@MV; actor INVALID); **P87 EXIT** overshoot stop-grad start KEEP as freeze / **FALSIFIED as compounding** (`run_p87_ovsgstart`, 515 iters): GAIN-READY **0.86@DV** last_ok **82**; val MV **×0.681 / ×0.732** 1step→OL **×0.733**; det_r **0.157**; paired **−21.19 vs −100.44** VALID 9/9; overshoot family **closed**; **P88 EXIT** residual GRU mix **REVERT** (`run_p88_grures`, pid **343419**, 158 iters): last_ok **59** **GAIN_NOT_READY 0.40@MV**; extra-P1 live **0.83@DV** discarded P40; actor INVALID; P76+P88 keep-h **closed**; **P89 EXIT** high-pass `dob_ground` KEEP as protocol / PARTIAL amp / FALSIFIED champ (`run_p89_dobhp`, 216 iters): GAIN-READY **0.83@DV** last_ok **67**; val MV **×0.879 / ×0.880** 1step→OL **×0.935**; det_r **0.392**; paired **−9.91 vs −132** VALID 9/9 loses to P64 **−4.54** mv_viol **2.81**; **P90 EXIT** serve HP d-tail **REVERT** (`run_p90_dobhpfeat`, pid **352555**, sha `91a695e`, 65 iters): storm **2/2 @11** last_ok **7** GAIN_NOT_READY **−0.53@MV**; actor INVALID; P1 `d≡0` so feat HP was a no-op; val MV **×−18.49**; **P91 EXIT** identity replay of P89 raw-`d` feat tail (`run_p91_nofeathp`, pid **355470**, sha `9d41fb8`, 101 iters): storm **1/2 @32** unlocked last_ok **31** then walked; freeze GAIN_NOT_READY **0.76@DV** last_ok **44**; actor INVALID; serve-HP family **closed**; **P92 EXIT** skip-storm GAIN-READY stay-lock (`run_p92_stormlock`, pid **378277**, sha `fdd97a5`, 446 iters): stay-lock **KEEP as hygiene / unfired / FALSIFIED as freeze-saver and champ**; freeze last_ok **74** GAIN-READY **0.84@DV**; val MV **×0.853 / ×0.909** DV **×0.766 / ×0.858** 1step→OL **×0.638**; det_r **−0.061**; paired **−31.67 vs −113** VALID 9/9 mv_viol **16.29**; **P93 EXIT** GAIN-READY also requires 1step→OL in the same [0.80, 1.30] band (KEEP as protocol / FALSIFIED as TM/champ; val 1step→OL ×0.832); **P94 CRASH** disk-full @340 (observer VALID unscored amp); **P95 EXIT FALSIFIED** linear `(ratio−1)²` amp as P2-entry (`run_p95_dobamp`, pid **391822** sha `36c82b6`, 83 iters ES `grad_skip_storm`; freeze GAIN-READY **0.85@DV** 1step→OL **0.90**; first P2 `dobg` **11259** `std_ratio` **107** skip **46**; actor INVALID; z-score shape KEEP); **P96 EXIT** log-std amp KEEP as P2-entry / FALSIFIED as recovery (`run_p96_doblogamp`, pid **396412** sha `58a95c0`, 100 iters; first P2 skip **0** then `std_ratio` **0.39**; freeze GAIN_NOT_READY **0.77@DV**; actor INVALID); **P97 EXIT** recon-sg KEEP as P2-entry / FALSIFIED as recovery; **P98 EXIT** Wiener HP MSE KEEP as P2-entry / FALSIFIED as amp recovery (`run_p98_dobwien`, pid **402823**, sha `9f22da4`, 386 iters: freeze GAIN-READY **0.82@DV** last_ok **27**; val pred_std **0.215 vs 1.93**; paired **−15.25** VALID 9/9); **P99 EXIT** KEEP as A-hold / FALSIFIED as champ (`run_p99_dobafreeze`; pred_std **0.628 vs 1.93**; paired **−23.62**); **P100 EXIT** Luenberger KEEP as Kalman form / FALSIFIED as champ (`run_p100_dobluen`, pid **409971**, 471 iters: freeze extra-P1@**94** GAIN-READY **0.84@DV**; val MV **×0.770 / ×0.803** DV **×0.744 / ×0.833**; 1step→OL **×0.979**; det_r **0.469** pred_std **0.737 vs 1.93**; paired **−8.72**; `dob_K` **0.119→0.049**); **P101 EXIT** KEEP as K-hold / FALSIFIED as det_r·TM·champ (`run_p101_dobraw`, pid **414648**, 515 iters: freeze orig-P1@**82** GAIN-READY **0.82@DV**; val MV **×0.752 / ×0.770** DV **×0.754 / ×0.816**; 1step→OL **×0.789**; det_r **0.197** pred_std **1.086 vs 1.93**; paired **−19.10** 8/9; `dob_K` **0.097** SS **≈2.04**); **P102 EXIT** increment MSE FALSIFIED as K-hold (`run_p102_dobinc`, pid **418208**, 158 iters: freeze GAIN_NOT_READY **0.79@DV** last_ok **41**; `dob_K` **0.116→0.042** SS **≈0.89**; val pred_std **0.603 vs 1.93** det_r **0.335**; actor INVALID); **P103 EXIT** two-timescale DOB KEEP as filter / FALSIFIED as champ (`run_p103_dob2ts`, pid **426632**, 406 iters: freeze extra-P1@**94** GAIN-READY **0.81@DV**; val MV **×0.834 / ×0.854** DV **×0.773 / ×0.863**; 1step→OL **×0.893**; det_r **0.405** pred_std **0.941 vs 1.93**; paired **−21.64** VALID 8/9; `|d_slow|/|d|~0.72`); **P104 EXIT INVALID** feat tail `d_fast` (`run_p104_dobfeatfast`, sha `adbe4f5`, 101 iters): CAPPED@47 **GAIN_NOT_READY 0.79@DV** last_ok **45**; first P2 dobg **0.230** skip **0** KEEP as P2-entry vs P95; **FALSIFIED as K-hold** vs P103; **cannot attribute** — **REVERT** feat to served `d`; **P105 EXIT INVALID** identity = P103 feat (`run_p105_featd`, pid **439130**, sha `492c0fd`, 59 iters): CAPPED@5 **GAIN_NOT_READY 0.02@DV** last_ok **3**; storm **2/2 @4–5**; val MV **×−0.807** DV **×0.020**; **FALSIFIED as P103 freeze identity**; feat family **closed**; do **not** score actor; **P106 EXIT VALID** READY-only storm-cap KEEP as P1-survival / FALSIFIED as champ (`run_p106_stormnr`, pid **442202**): orig-P1@**82 PASS** **0.86@DV**; paired **−9.28 vs −93.44** 9/9; **P107 EXIT INVALID** α=`1/H` FALSIFIED as leftover/K (`run_p107_2tsh`, pid **449196`): leftover **1.03** K **0.039/0.83**; freeze CAPPED **−1.40@MV**; actor INVALID; **P108 EXIT VALID** α=`1/T` KEEP leftover/K / FALSIFIED amp/champ/TM (`run_p108_2tst`, pid **453524**): leftover **0.592** K **0.057/1.20**; freeze orig-P1@**82** GAIN-READY last_ok **74** **0.86@MV / 0.90@DV**; val MV **×0.694** DV **×0.772** 1step→OL **×0.761** det_r **0.605** pred_std **0.453 vs 1.93** paired **−13.73 vs −74.36** VALID 9/9; **P109 EXIT=0 VALID** (`run_p109_p2g`, pid **463855**, sha `70cee52`, 428 iters): extra-P1 freeze last_ok **85** **0.82@DV / 0.95@MV** 1step→OL **0.98**; P2→P3@**158 PASS** MTP **0.938** `[p3-skip]` did not fire; End-P2 leftover **0.367** K **0.109/2.30** **FALSIFIED leftover-split vs P108 0.592** (KEEP vs crush; PARTIAL K); last-P2 recon **0.072** skip **0**; rscale **2.68 KEEP**; val MV **×0.170 / @H ×0.068** **FALSIFIED TM** vs freeze 0.95@MV; DV **×0.469**; 1step→OL **×0.626** autoencoder; det_r **0.741** KEEP vs P108 crush (do **not** swap P26); pred_std **2.02 vs 1.93 KEEP** amp; paired **−35.43 vs −100.98** VALID 7/9 loses to P64 **−4.54**; **REVERT p2g**. **P110 EXIT=0 VALID** α=`1/(2T)` frozen-g (`run_p110_2ts2t`, pid **493057 DEAD**, 515 iters): leftover **0.229** K **0.096/2.03** KEEP vs P108; val MV **×0.718** DV **×0.771** 1step→OL **×0.851** det_r **0.494** pred_std **0.778 vs 1.93** paired **−24.00 vs −113.25** VALID **8/9**; KEEP leftover/K/amp/compounding/TM vs P108; **FALSIFIED leftover matching smoke / champ / TM-to-P64 / det_r vs P108**; do **not** blindly N+1 α; Champion **P64**. Rogue `run_p110_p2gaux` pid **478033 DEAD** — do **not** score as P110 / do **not** KEEP p2gaux). **P111 LIVE P2** rest-IC last-step DC Huber KEEP + uniform FOPDT traj Huber (`run_p111_gmatchtraj`, pid **503163**, sha `513eaa2`; banner `gmatch_traj=FO`; `img_rollout last_only=not stack_k`; no new field; not P75 rise-mass; extra-P1@**94 LIVE PASS** **0.82@DV / 0.92@MV** 1step→OL **1.15** discarded P40/P41; freeze last_ok **81** **GAIN_NOT_READY 1.35@MV** — expect `[p3-skip]`; do not FALSIFY until val). Continuous latent, compile **eager**, **DOB on** (GAIN-ONLY cont; `d_t` is the
...
> conv=0; actor INVALID. **P78 EXIT FALSIFIED** KV-continue as GAIN-READY (`run_p78_prevkv`, pid **261143**, 158 iters): last_ok **91** **4.08@MV / 0.72@DV**; val MV **×1.721 / ×1.710** DV **×0.729 / ×0.624**; 1step→OL **×0.857** OL-vs-real **×0.567**; det_r **0.010** pred_std **2.45 vs 1.93**; actor INVALID. TSSM family closed; env-free default **rssm**. **P79 EXIT** KEEP rssm / **FALSIFIED as P64-identity** (`run_p79_rssmrevert`, pid **277462**, 204 iters): GAIN-READY **0.81@DV** last_ok **81**; val MV **×0.923 / ×0.953** DV **×0.781 / ×0.838**; 1step→OL **×0.840**; paired **−10.57 vs −82.79** VALID 9/9 loses to P64 **−4.54**. **P80 EXIT** persist 0.1→0.0 **REVERT** (`run_p80_persist0`, pid **293761**, 515 iters budget): freeze GAIN-READY **0.86@MV / 0.87@DV** last_ok **82**; val MV **×0.723 / ×0.755** DV **×0.789 / ×0.857**; 1step→OL **×0.735** OL **×0.743**; paired **−35.33 vs −107.43** VALID 8/9 loses to P64 **−4.54** / P79 **−10.57**; persist **0.1 KEEP**. **P85 EXIT** always-call KEEP as P1 / **FALSIFIED as val TM** (`run_p85_p87call`, pid **331081**, 515 iters budget): GAIN-READY **0.81@DV** last_ok **82**; val MV **×0.671 / ×0.737** DV **×0.730 / ×0.823**; 1step→OL **×0.697**; paired **−24.84 vs −102.33** VALID 9/9 loses to P64 **−4.54** / P79 **−10.57**. **P86 EXIT** CV-only overshoot **REVERT** (`run_p86_ovcv`, pid **335153**, 158 iters): freeze GAIN_NOT_READY **3.36@MV** last_ok **99**; val MV **×3.696 / ×3.951** DV **×0.875 / ×0.965**; 1step→OL **×0.863**; actor INVALID. Overshoot-channel family **closed**. **P87** stop-grad overshoot start (all-obs MSE restored; no new knob). Champion **P64**. Hourly cron must **not** start a second GPU job.
...
> `dob_ground_coef · ‖d−load‖²` on served `d=d_slow+d_fast` (**P103:** two-timescale DOB; **P107:** α=`1/H` not `(1−A)` so `d_slow` is slower than settling; P106 `|d_slow|/|d|` **0.93** stole in-band residual;
> **P102 EXIT:** increment MSE KEEP as P2-entry / FALSIFIED as K-hold — `|Δd|∝K`
> crushed K **0.116→0.042** pred_std **0.603 vs 1.93**;
> **P101 EXIT:** raw level MSE KEEP as K-hold / FALSIFIED as det_r **0.197**
> pred_std **1.086 vs 1.93** paired **−19.10** 8/9 — DC fitted into `d`;
> P89 EXIT KEEP high-pass MA `min(4H,T)` as **val** detrend / SUPERSEDED as
> training ground — test_sim T=128 crop-demean crushed P100 K **0.119→0.049**;
> **P94/P95:** Wiener HP MSE → z-score shape
...
> FALSIFIED as amp recovery; **P98 EXIT:** Wiener HP MSE **KEEP as P2-entry** /
> **FALSIFIED as amp recovery** (pred_std **0.215 vs 1.93**; HP ratio **23→0.29**);
> **P99 EXIT:** P2 pins `A` at `dob_decay_init` (sigmoid≈0.95) and trains `K` only —
> unit-matched via the running CV obs-norm std; P90 serve-HP feat tail
> **REVERT** — actor/critic feat uses served `d.detach()` (P104 feat-fast **REVERT**; P90–P91 serve-HP closed); this term is the P2
> ground only) that tunes the Kalman `K` (A pinned) to TRACK the load — the structural fix
> for the manual `dob_gain_init` amplitude tuning. **P99 EXIT** KEEP as A-hold /
> KEEP as val-amp vs P98 (pred_std **0.628 vs 0.215**) / FALSIFIED as det_r **0.273**
> and champ (paired **−23.62**). Joseph SS capped at `1/A`. **P100 EXIT:** Luenberger
...
> Gain-match `img_rollout(..., last_only=not stack_k, out='obs', prev_state=)`
> (P111 rest-IC FOPDT traj K-stack when τ>0; last-step DC Huber KEEP; P75
> rise-mass **REVERT**). Continues TSSM rest-IC KV (P78 EXIT FALSIFIED as
> GAIN-READY / KEEP as OL lift; RSSM `prev_state=None` identity). jsonl emits
...
flowchart LR
  ...
  CVOBS["CV_obs"] --> INNOV{"nu = CV_obs - g(feat)"}
  G --> INNOV
  INNOV -->|"learned gain K"| DT["d_t = A*d_(t-1) + K*nu"]
  DPREV["d_(t-1)"] -->|"decay A"| DT
...
- **Output**: decoder `CV = g(h,z) + d_t`. `g` now learns the *true* gain because
  ...
  posterior recon compares `g(feat)+sg(d_t)` vs `obs` (**P97:** stop-grad `d` so
  P2 Kalman is not trained by the residual). P1 no longer pins
...
- **Grounding (KalmanNet, P19)**: the recon innovation alone under-drives `d_t`
  ...
  `‖d−load‖²` on served `d=d_slow+d_fast` (**P103:** two-timescale DOB; **P107:** α=`1/H` not `(1−A)`; **P102 EXIT:**
  increment MSE FALSIFIED as K-hold; **P101 EXIT:** raw
  level KEEP as K-hold / FALSIFIED as det_r; **P89:** high-pass MA
  `min(round(4H),T)` KEEP as **val** detrend with
  existing `disturbance_detrend_settle_mult`; test_sim crop was per-window DC
  and crushed P100 K; `mult<=0` was already raw; **P94/P95:** Wiener `‖HP(d)−HP(load)‖²` → z-score shape
  ...
  **P98 EXIT:** Wiener HP MSE **KEEP as P2-entry** / **FALSIFIED as amp recovery**
  (val pred_std **0.215 vs 1.93**); **P99 EXIT:** pin `A` at init, train `K` only
  (A held; val pred_std **0.628 vs 1.93**; det_r **0.273**; paired **−23.62**;
  KEEP as A-hold / FALSIFIED as champ; Joseph SS capped at `1/A`); **P100 EXIT:**
  Luenberger plant residual KEEP as Kalman form (pred_std **0.737**; det_r **0.469**;
  paired **−8.72**; FALSIFIED as champ / TM; HP crushed K **0.119→0.049**); **P101 EXIT:**
  raw MSE KEEP as K-hold / FALSIFIED as det_r (pred_std **1.086**; det_r **0.197**; paired **−19.10** 8/9; K **0.097** SS **≈2.04**); **P102 EXIT:** increment MSE FALSIFIED as K-hold (`dob_K` **0.042** pred_std **0.603**); **P103:** two-timescale DOB; so this
  term is the K trainer of the sim's known hidden load, unit-matched to `d_t`'s
  normalized space via the running CV obs-norm std, threaded as `cfg._cv_obs_std`)
  tunes the Kalman `K` (A pinned) to TRACK the load — the structural replacement for the
...
- **Feedforward (Scope 2, shipped 2026-06-11)**: `d_t` is appended to `feat`
  (`feat = [h, z_flat, d.detach()]`) so the actor / critic / reward heads
  condition on the **served** Kalman load and pre-empt it (P104 feat-fast
  **REVERT**; P105 identity **FALSIFIED** as P103 freeze — CAPPED@5
  **0.02@DV** last_ok **3**; feat family closed). `d` is **detached** into `feat`
  (the DOB is
  ...
  preserved (`apply_dob` still adds served `d`). Scope 1 de-confounds the gain; Scope 2 adds the explicit feedforward.
...
- **GAIN block** (`cont_gain_dim = n_cv·(n_mv+n_dv)`): inferred in-context from the
  ...
  **P74 EXIT REVERT:** decoder `gain_cv_skip` was a teacher-pin no-op
  ...
  Last-step DC Huber restored. **P111 LIVE P2:** rest-IC last DC KEEP +
  ...
  stays last-only). Not P75 rise-mass. extra-P1@94 LIVE PASS discarded
  P40; freeze last_ok 81 GAIN_NOT_READY 1.35@MV; do not retune last_ok
  mid-run. **P76 EXIT REVERT:** RSSM GRU
  ...
  **P77 EXIT FALSIFIED** Markovian rest-IC; **P78 EXIT FALSIFIED**
  KV-continue as GAIN-READY (KEEP as OL lift; last_ok **91** **4.08@MV**; val MV **×1.721 / ×1.710** DV **×0.729 / ×0.624**; 1step→OL **×0.857**; det_r **0.010**). TSSM family closed. Env-free default **rssm** (opt-in `DREAMER_WORLD_MODEL_TYPE=tssm`). **P79 EXIT** KEEP rssm / **FALSIFIED as P64-identity** (GAIN-READY **0.81@DV** last_ok **81**; val MV **×0.923 / ×0.953** DV **×0.781 / ×0.838**; paired **−10.57 vs −82.79** VALID 9/9). **P80 EXIT** persist 0.1→0.0 **REVERT** (`run_p80_persist0`, pid **293761**, 515 iters budget): freeze GAIN-READY **0.86@MV / 0.87@DV** last_ok **82**; val MV **×0.723 / ×0.755** DV **×0.789 / ×0.857**; 1step→OL **×0.735** OL **×0.743**; paired **−35.33 vs −107.43** VALID 8/9 loses to P64 **−4.54** / P79 **−10.57**; persist **0.1 KEEP**. Champion **P64**.
  Explicit
...
- **Stage 2** — `g` is frozen, so the recon innovation can only be reduced by
  ...
  Mean MSE over all sequences. Do not `/dvar`. **P89 EXIT:** grounding was
  high-pass (`x−MA(x,w)`, `w=min(4H,T)`) so DC drift that val `det_r`
  already ignores does not dominate P2 (KEEP as **val** protocol; PARTIAL amp;
  FALSIFIED champ; **P101 SUPERSEDED as training ground** — crop-demean crushed
  P100 K). **P94 CRASH** freeze GAIN-READY **0.84@DV** 1step→OL **0.85** skip **0** last_ok **82**; P2 HP `std_ratio` **0.64→0.33**; P2→P3 PASS @136; aborted @340 disk-full. **P95 EXIT FALSIFIED** `(ratio−1)²` amp as P2-entry (`run_p95_dobamp`; freeze GAIN-READY **0.85@DV** 1step→OL **0.90**; first P2 `std_ratio` **107** skip-storm; actor INVALID). Z-score shape KEEP. **P96 EXIT** log-std amp KEEP as P2-entry / **FALSIFIED as amp recovery** (`run_p96_doblogamp`, pid **396412**, 100 iters: first P2 skip **0** then `std_ratio` **1.08→0.39**; freeze GAIN_NOT_READY **0.77@DV**; actor INVALID). **P97 EXIT** recon-sg KEEP as P2-entry / FALSIFIED as recovery. **P98 EXIT** Wiener HP MSE KEEP as P2-entry / FALSIFIED as amp recovery (`run_p98_dobwien`; pred_std **0.215 vs 1.93**; paired **−15.25**). **P99 EXIT** pin Kalman A, train K only (`dob_afreeze=True`; `dob_A` **0.95257 held** all P2; `dob_K` **0.119→0.062**; HP ratio **4.18→0.24**; val pred_std **0.628 vs 1.93**; det_r **0.273**; paired **−23.62** VALID 9/9; KEEP as A-hold / KEEP as val-amp vs P98 / **FALSIFIED as det_r / champ**; pin-A/pin-K family **closed**). **P100 EXIT** Luenberger plant-residual Kalman KEEP as Kalman form (`dob_luen=True`; SS `K/(1−A)`; pin-A KEEP; freeze extra-P1@**94** GAIN-READY **0.84@DV**; val MV **×0.770 / ×0.803** DV **×0.744 / ×0.833**; 1step→OL **×0.979**; det_r **0.469** pred_std **0.737 vs 1.93**; paired **−8.72** VALID 9/9; `dob_K` **0.119→0.049**; **FALSIFIED as champ / TM**). **P101 EXIT** raw level MSE KEEP as K-hold / FALSIFIED as det_r·champ (`dob_hp=0`; val det_r **0.197** pred_std **1.086 vs 1.93** paired **−19.10** 8/9; K SS **≈2.04**). **P102 EXIT** increment MSE FALSIFIED as K-hold (`dob_inc=True`; `dob_K` **0.042**; pred_std **0.603 vs 1.93**; actor INVALID). **P103 EXIT** two-timescale DOB KEEP as filter / FALSIFIED as champ (`dob_2ts=True`; val det_r **0.405** pred_std **0.941 vs 1.93** paired **−21.64**). **P106 EXIT** READY-only storm-cap KEEP as P1-survival / FALSIFIED as champ (paired **−9.28**). **P107 EXIT:** α=`1/H` **FALSIFIED as leftover/K** (leftover **1.03** K **0.039/0.83**; CAPPED **−1.40@MV**). **P108 EXIT:** α=`1/T` KEEP leftover/K **0.592 / 0.057/1.20** / FALSIFIED amp/champ/TM (paired **−13.73**; val MV **×0.694**). **P109 EXIT=0 VALID:** extra-P1@**104 PASS** last_ok **85** **0.82@DV / 0.95@MV** 1step→OL **0.98** (`p2g=True`); P2→P3 MTP **0.938**; leftover **0.367** K **0.109/2.30** FALSIFIED leftover-split vs P108; val MV **×0.170** det_r **0.741** pred_std **2.02 vs 1.93** paired **−35.43** 7/9; **REVERT p2g**. **P110 EXIT=0 VALID** α=`1/(2T)` (`run_p110_2ts2t`, pid **493057 DEAD**, 515 iters): leftover **0.229** K **0.096/2.03** KEEP vs P108; val MV **×0.718** DV **×0.771** 1step→OL **×0.851** det_r **0.494** pred_std **0.778 vs 1.93** paired **−24.00 vs −113.25** VALID **8/9**; KEEP leftover/K/amp vs P108; **FALSIFIED leftover matching smoke / champ / TM-to-P64 / det_r vs P108**; do **not** blindly N+1 α. Rogue `run_p110_p2gaux` **DEAD** — do **not** score as P110 / do **not** KEEP p2gaux. **P104 EXIT:** feat-fast **REVERT** (CAPPED@47 **GAIN_NOT_READY**; cannot attribute). **P90 EXIT REVERT:** served feat tail was causal EMA HP
  (`d−EMA(d,4H)`); storm 2/2 last_ok **7** GAIN_NOT_READY — cannot attribute.
  **P91 EXIT:** feat tail was raw `d.detach()` through P103 (P89 identity; FALSIFIED as P89 freeze — last_ok **44** **0.76@DV**; serve-HP family closed). Kalman state
  stays served `d`. jsonl `dob_ground_keep_frac` is 1.0 when grounding fires. **P92 EXIT:** skip-storm continue stay-locks last_ok iff GAIN-READY (KEEP as hygiene / unfired). **P93 EXIT:** that READY flag also requires 1step→OL in the same [0.80, 1.30] band (KEEP as protocol / FALSIFIED as TM / champ; missing decomp identity); stay-lock **FIRED** last_ok **56**; freeze GAIN-READY **0.85@DV** 1step→OL **0.83**; val 1step→OL **×0.832**.
...
| WM loss (recon + KL / overshoot all-obs + **stop-grad start** (P87; P86 CV-only **REVERT**) / held-rollout; dist-head always called, zeros when None) | `training/train.py` (`world_model_loss`, `_rssm_world_model_loss`, `_wm_latent_overshoot_loss`, `_disturbance_head_loss`; `_wm_need_dist_head_loss` is H2D only) |

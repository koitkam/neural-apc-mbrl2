...
> **REVERT** — actor/critic feat uses `d_fast` (P104; P90–P91 serve-HP closed); this term is the P2
...
> FALSIFIED as det_r. **P103 EXIT:** two-timescale DOB KEEP as filter / FALSIFIED as champ. **P104:** feat tail `d_fast`. **P102 EXIT:** increment MSE
...
- **Feedforward (Scope 2, shipped 2026-06-11)**: `d_t` is appended to `feat`
  (`feat = [h, z_flat, d_fast.detach()]`) so the actor / critic / reward heads
  condition on the **fast** disturbance residual and pre-empt the load
  (P104; P103 served `d` was DC-dominated). `d_fast` is **detached** into `feat`
  (the DOB is

...
| WM loss (recon + KL / overshoot **CV-only** (P86) / held-rollout; P87 head always called, zeros when None) | `training/train.py` (`world_model_loss`, `_rssm_world_model_loss`, `_wm_latent_overshoot_loss`, `_disturbance_head_loss`; `_wm_need_dist_head_loss` is H2D only) |

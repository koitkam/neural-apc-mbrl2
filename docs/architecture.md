...
> **P99:** P2 pins `A` at `dob_decay_init` (sigmoid≈0.95) and trains `K` only —
...
- **Grounding (KalmanNet, P19)**: the recon innovation alone under-drives `d_t`
  ...
  **P98 EXIT:** Wiener HP MSE **KEEP as P2-entry** / **FALSIFIED as amp recovery**
  (val pred_std **0.215 vs 1.93**); **P99:** pin `A` at init, train `K` only so this

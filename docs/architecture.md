...
> (P77 EXIT Markovian rest-IC **FALSIFIED**; P78 LIVE P1 iter 22 rest-IC `prev_state=` probe20 H=55 **+0.225 conv=0** still short of P64 +0.498; RSSM via `DREAMER_WORLD_MODEL_TYPE=rssm`) continuous latent, compile **eager**, **DOB on** (GAIN-ONLY cont; `d_t` is the
...
> conv=0; actor INVALID. **P78 LIVE** rest-IC `prev_state=` (`run_p78_prevkv`, pid **261143** P1 iter **22**; probe20 H=55 **+0.225 conv=0**).

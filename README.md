...
| `DREAMER_SAMPLE_RATE` | force sample rate (else auto `min(τ_fast/10, θ_dom/2)`; channel/dominant θ, not min-θ). Leftover `SIM_SAMPLE_RATE` is ignored at derive; `single_run` still writes `SIM_SAMPLE_RATE` after derivation as IPC. |
...
| `sample_rate` | 5 | `min(τ_fast / 10, θ_dom / 2)` (channel/dominant θ, not min-θ) | `DREAMER_SAMPLE_RATE` (leftover `SIM_SAMPLE_RATE` ignored) |

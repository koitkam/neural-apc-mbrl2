...
| `DREAMER_LAMBDA_EFF_DEPTH` | upper bound on the effective λ-return bootstrap depth `1/(1-λ)` used by the adaptive `gae_lambda` auto-tune (default `15` ≈ DreamerV3 paper's `H=15`, giving `λ=0.933`). Lower values (e.g. `10` → `λ=0.90`) tighten the λ-tail against compounding WM error on long auto-derived horizons; higher values (e.g. `20` → `λ=0.95`) restore paper-strict behaviour. Final override: `DREAMER_GAE_LAMBDA` (sets λ directly and wins over auto-tune). |

...
def test_noise_curriculum():
    ...
    # TrainConfig default ≡ unset env; empty leftover env is ignored (P92).
    class _Cfg:
    ...
    try:
        assert noise_curriculum_scale(0.0, phase=1) == 0.0
        assert noise_curriculum_scale(0.0, phase=1, cfg=_Cfg()) == 0.0
    finally:
        os.environ.pop('DREAMER_PROCESS_NOISE_AMP_RAMP', None)
    print('[smoke] OK noise curriculum leftover env ignored')

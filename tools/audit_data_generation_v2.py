...
Sim-agnostic usage:
    python tools/audit_data_generation_v2.py --sim test_sim \
        --source-run output/test_sim/run_p64_p2amp
    python tools/audit_data_generation_v2.py --sim distillation \
        --tau 40 --dead 6 --episode-len 900
    # τ/θ/sr/ep_len/lookback come from run_plan.json / plant_id.json.
    # Do not invent test_sim 55 / 8 / 4 / 1220 / 120.
"""
...
def _pos(v, typ):
    """Positive number or None. Never treat 0 / missing as a plant default."""
    if v is None or v == '':
        return None
    try:
        x = typ(v)
...
def _resolve_audit_plant_knobs(args, source_run: Optional[Path]):
    ...
    if source_run is not None:
        ...
        if plan_path.is_file():
            ...
            maybe_cfg = plan.get('config') or {}
            cfg = maybe_cfg if isinstance(maybe_cfg, dict) else {}
        pid = rd / 'plant_id.json'
        if pid.is_file():
            plant = json.loads(pid.read_text())

    tau = _pos(getattr(args, 'tau', None), float)
    if tau is None:
    ...
    if sr is None:
        sr = _pos(plan.get('sample_rate'), int)
    if sr is None:
    ...
    if ep is None:
        ep = _pos(plan.get('episode_length'), int)
    if ep is None:
    ...
    if lb is None:
        lb = _pos(plan.get('lookback'), int)
    if lb is None:
        lb = _pos(cfg.get('lookback'), int)
    if lb is None:
        lb = _pos(plant.get('lookback') or plant.get('identified_lookback'),
                  int)

    missing = [n for n, v in (('tau', tau), ('dead', dead),
                              ('sample_rate', sr), ('episode_len', ep),
                              ('lookback', lb)) if v is None]
    if missing:
        raise SystemExit(
            '[v2] refuse to invent test_sim knobs '
...
_p.add_argument('--dead', type=float, default=_opt_env_num('AUDIT_DEAD', float),
                help='Identified dead time (sample steps). Unset = SysID.')
_p.add_argument('--sample-rate', type=int,
                default=_opt_env_num('AUDIT_SAMPLE_RATE', int),
                help='Sample rate (Hz / steps). Unset = SysID.')
_p.add_argument('--episode-len', type=int,
                default=_opt_env_num('AUDIT_EPISODE_LEN', int),
                help='Episode length (steps). Unset = SysID.')
_p.add_argument('--lookback', type=int,
                default=_opt_env_num('AUDIT_LOOKBACK', int),
                help='Lookback (steps). Unset = SysID.')
...
def main(argv=None):
    ...
    TAU, DEAD, SAMPLE_RATE, EPISODE_LEN, LOOKBACK, _params_src = (
        _resolve_audit_plant_knobs(_args, SOURCE_RUN))

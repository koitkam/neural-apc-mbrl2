...
def _test_img_rollout_last_only() -> None:
    ...
    cont = m.img_rollout(
        ...
        c0=st.c, prev_state=st.detach(), return_state=False)
    long = m.img_rollout(
    ...
    _, sa = m.img_rollout(
        h0.detach(), z0, acts, sample=False, c0=c_a,
    ...
    sa.c = c_pert
    d2 = m.decode(sa.feat).detach()
    dec_err = float((d1 - d2).abs().max())
    assert dec_err > 1e-4, 'decoder dropped gain-c (feat/decode path dead)'
    ...
    _, sg1 = md.img_rollout(
        h0.detach(), z0, acts, sample=False, c0=c_g,
        last_only=True, out='obs', return_state=True)
    _, sg2 = md.img_rollout(
        h0.detach(), z0, acts, sample=False, c0=c_g2,
        last_only=True, out='obs', return_state=True)
    _, sd = md.img_rollout(
...
def _test_dob_ground_hp_mse() -> None:
    """Level-MSE helper identity (not the P102 training ground)."""
    ...
    print('[smoke] OK  dob_ground level MSE helper (not P102 train path)')


def _test_dob_ground_inc_mse() -> None:
    """P102: Δd vs Δload. DC offset is free; std_ratio stays level amp."""
    from training.train import _dob_ground_inc_mse, _dob_ground_hp_mse
    torch.manual_seed(0)
    dt = torch.randn(4, 32, 1)
    loss, ratio = _dob_ground_inc_mse(dt, dt)
    assert abs(float(ratio) - 1.0) < 1e-5
    assert float(loss) < 1e-6
    # Persistent DC must not train K (P101 RCA). Increment loss ~0;
    # level MSE is huge. std_ratio is level amp (offset does not change std).
    ds_dc = dt + 5.0
    loss_inc, ratio_dc = _dob_ground_inc_mse(ds_dc, dt)
    loss_lvl, _ = _dob_ground_hp_mse(ds_dc, dt)
    assert float(loss_inc) < 1e-6, loss_inc
    assert float(loss_lvl) > 10.0, loss_lvl
    assert abs(float(ratio_dc) - 1.0) < 0.05
    # Matching increments of a scaled trajectory: loss is on diffs.
    ds = 0.5 * dt
    loss, ratio = _dob_ground_inc_mse(ds, dt)
    assert abs(float(ratio) - 0.5) < 1e-4
    dds = ds.float()[:, 1:] - ds.float()[:, :-1]
    ddt = dt.float()[:, 1:] - dt.float()[:, :-1]
    expected = (dds - ddt).pow(2).mean()
    assert abs(float(loss) - float(expected)) < 1e-5
    ds_loud = 100.0 * dt
...
def _test_isolation_dcv_scales() -> None:
    ...
    assert "os.environ.get('SIM_IDENTIFIED_TAU_DOMINANT'" not in _sn
...
def _test_envfree_observer_recipe() -> None:
    ...
    assert abs(float(c.disturbance_detrend_settle_mult) - 4.0) < 1e-12
    assert not hasattr(c, 'dob_feat_hp_w')
    assert not hasattr(c, 'dob_feat_hp_alpha')
...
def _test_identified_tau_cfg() -> None:
    ...
    keys = (
        ...
        'SIM_IDENTIFIED_TAU_DOMINANT', 'SIM_IDENTIFIED_DEAD_TIME',
    )
    prev = {k: os.environ.get(k) for k in keys}
    try:
        os.environ['IDENTIFIED_TAU_DOMINANT'] = '99'
        os.environ['IDENTIFIED_DEAD_TIME'] = '1'
        os.environ['SIM_IDENTIFIED_TAU_DOMINANT'] = '50'
        os.environ['SIM_IDENTIFIED_DEAD_TIME'] = '9'
        ...
        env_sim.cfg = TrainConfig()
        env_sim.sim = None
        tau_s, dead_s = env_sim._resolve_plant_timing()
        assert abs(tau_s - 0.0) < 1e-12, tau_s  # leftover SIM_* ignored
        assert abs(dead_s - 0.0) < 1e-12, dead_s
        tau_p, dead_p = _plant_timing_for_integral(TrainConfig())
        assert abs(tau_p - 0.0) < 1e-12, tau_p
        assert abs(dead_p - 0.0) < 1e-12, dead_p
        ...
        rd.sample_episode(n_dvs=0)
        assert abs(rd.actuator_tau_steps - 0.0) < 1e-12, rd.actuator_tau_steps
        os.environ['IDENTIFIED_TAU_DOMINANT'] = '50'
        rd2 = DomainRandomizer(
            domain_randomization=True, param_randomization_pct=0.10,
            randomization_seed=0)

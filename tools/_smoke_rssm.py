...
def _test_dob_two_timescale_scan() -> None:
    ...
    alpha_a = 1.0 - A
    assert float(alpha) < float(alpha_h) * 0.6
...
def _test_isolation_dcv_scales() -> None:
    ...
    assert 'def _dob_ground_inc_mse' not in _src
    ...
    assert 'dob_feat_tail(' not in _rssm_src
...
def _test_gain_match_rest_ic() -> None:
    ...
    assert 'gain_match_ol1_ratio' not in d3
...
def _test_stream_serve_matches_rollout() -> None:
    ...
    # P104 REVERT: last n_cv of feat is served d, not d_fast.
    n_cv = int(rssm.n_cv)
    feat_tail = streamed[:, -1, -n_cv:]
    assert torch.allclose(feat_tail, state.d.detach(), atol=1e-5, rtol=1e-4)

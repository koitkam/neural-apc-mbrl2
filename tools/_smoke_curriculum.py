...
the properties that make the staged Kalman/DOB identification correct:
  ...
  Stage 2 API freeze-g: g FROZEN, DOB trainable + ACTIVE remains a valid
    partition (P108 identity). P110 curriculum P2 recon-finetunes g while
    K trains (A pinned) AND runs P1 g-aux (overshoot/held/gain-match).
...
def _check(wm_type):
    ...
    # ---- P110: curriculum P2 recon-finetunes g AND runs P1 g-aux ----
    cfg, model, batch = _mk(wm_type)
    rssm = model.dynamics
    g_ref = _g_param(rssm)
    ...
    k_grad = _grad_sum(rssm.dob_log_gain)
    a_grad = _grad_sum(rssm.dob_log_decay)
    assert g_grad > 0.0, 'P110 Stage2: g recon-finetune must get recon gradient'
    assert k_grad > 0.0, 'P110 Stage2: K must get Kalman-ID gradient'
    assert a_grad == 0.0, 'P110 Stage2 P99: A is pinned -> must get NO gradient'
    if wm_type == 'rssm':
        assert float(losses.get('wm_overshoot_loss', 0.0)) > 0.0, \
            'P110: overshoot must run when g trains (tiny model may have gain_match=0)'
    print(f'[smoke] OK  P110 Stage2: g recon-finetune (|g_grad|={g_grad:.4f}) '
          f'+ K (|k_grad|={k_grad:.4f}, A pinned |a_grad|={a_grad:.1f}) '
          f'and g-aux ON [{wm_type}]')

    # Non-curriculum DOB + g must still run g-aux.
    ...
    print(f'[smoke] OK  P110 non-curriculum DOB+g keeps g-aux [{wm_type}]')
